        ...
        record(type, detail) {
          this._events.push(
            new LoaderEvent(
              type,
              detail,
              AMDLoader2.Utilities.getHighPerformanceTimestamp()
            )
          );
        }
        getEvents() {
          ...
          for (let i = 0, len = this.sortedPathsRules.length; i < len; i++) {
            ...
            if (AMDLoader2.Utilities.startsWith(moduleId, pathRule.from)) {
              ...
              for (let j = 0, lenJ = pathRule.to.length; j < lenJ; j++) {
                result.push(
                  pathRule.to[j] + moduleId.substr(pathRule.from.length)
                );
              }
              return result;
            }
        ...
        load(moduleManager, scriptSrc, callback, errorback) {
          if (/^node\|/.test(scriptSrc)) {
            ...
            const nodeRequire = ensureRecordedNodeRequire(
              ...
              opts.nodeRequire || AMDLoader2.global.nodeRequire
            );
          ...
          if (/^node\|/.test(scriptSrc)) {
            ...
            if (!isCrossOrigin && this._canUseEval(moduleManager)) {
              fetch(scriptSrc).then((response) => {
                ...
                text = `${text}
//# sourceURL=${scriptSrc}`;
                const func = trustedTypesPolicy ? self.eval(
      ...
      class NodeScriptLoader {
        ...
        _initNodeRequire(nodeRequire, moduleManager) {
          ...
          function makeRequireFunction(mod) {
            ...
            const require2 = /* @__PURE__ */ __name(function require3(path) {
              try {
            ...
            require2.resolve = /* @__PURE__ */ __name(function resolve(request, options) {
              return Module2._resolveFilename(
                request,
                mod,
                false,
                options
        ...
        load(moduleManager, scriptSrc, callback, errorback) {
          ...
          if (/^node\|/.test(scriptSrc)) {
            ...
            scriptSrc = AMDLoader2.Utilities.fileUriToFilePath(
              this._env.isWindows,
              scriptSrc
            );
            const normalizedScriptSrc = this._path.normalize(scriptSrc);
            const vmScriptPathOrUri = this._getElectronRendererScriptPathOrUri(
              normalizedScriptSrc
            );
        ...
        _createAndEvalScript(moduleManager, contents, options, callback, errorback) {
          ...
          const localDefineFunc = /* @__PURE__ */ __name(() => {
        ...
        _handleCachedData(script, scriptSource, cachedDataPath, createCachedData, moduleManager) {
          if (script.cachedDataRejected) {
            this._fs.unlink(cachedDataPath, (err) => {
              moduleManager.getRecorder().record(
                62,
                cachedDataPath
              );
        ...
        _createAndWriteCachedData(script, scriptSource, cachedDataPath, moduleManager) {
          const timeout = Math.ceil(
            moduleManager.getConfig().getOptionsLiteral().nodeCachedData.writeDelay * (1 + Math.random())
          );
          let lastSize = -1;
          let iteration = 0;
          let hashData;
          ...
          setTimeout(
            () => {
              ...
              if (!hashData.equals(hashDataNow)) {
                moduleManager.getConfig().onError(
                  new Error(
    ...
    ((AMDLoader2) => {
      ...
      class Module {
        ...
        complete(recorder, config, dependenciesValues, inversedependenciesProvider) {
          ...
          if (producedError) {
            const err = AMDLoader2.ensureError(producedError);
            err.phase = "factory";
      ...
      class ModuleManager {
        ...
        static _findRelevantLocationInStack(needle, stack) {
          const normalize = /* @__PURE__ */ __name((str) => str.replace(/\\/g, "/"), "normalize");
        ...
        getBuildInfo() {
          ...
          for (let i = 0, len = this._modules2.length; i < len; i++) {
            ...
            result[resultLen++] = {
              id: m.strId,
              path: location,
              defineLocation: location && defineStack ? ModuleManager._findRelevantLocationInStack(
                location,
                defineStack
              ) : null,
              dependencies,
        ...
        getRecorder() {
          if (!this._recorder) {
            if (this._config.shouldRecordStats()) {
              this._recorder = new AMDLoader2.LoaderEventRecorder(
                this._loaderAvailableTimestamp
              );
            } else {
        ...
        _relativeRequire(moduleIdResolver, dependencies, callback, errorback) {
          ...
          this.defineModule(
            AMDLoader2.Utilities.generateAnonymousModule(),
            dependencies,
            callback,
            errorback,
            null,
        ...
        _createLoadError(moduleId, _err) {
          ...
          const neededBy = (this._inverseDependencies2[moduleId] || []).map(
            (intModuleId) => this._moduleIdProvider.getStrModuleId(intModuleId)
          );
        ...
        _createRequire(moduleIdResolver) {
          const result = /* @__PURE__ */ __name((dependencies, callback, errorback) => {
        ...
        _loadModule(moduleId) {
          ...
          let lastPathIndex = -1;
          const loadNextPath = /* @__PURE__ */ __name((err) => {
            lastPathIndex++;
        ...
        _loadPluginDependency(plugin, pluginDependency) {
          ...
          const load = /* @__PURE__ */ __name((value) => {
            this.defineModule(
         ...
         */
        _resolve(module2) {
          const dependencies = module2.dependencies;
          if (dependencies) {
            for (let i = 0, len = dependencies.length; i < len; i++) {
              const dependency = dependencies[i];
              ...
              if (this._hasDependencyPath(dependency.id, module2.id)) {
                ...
                const cyclePath = this._findCyclePath(dependency.id, module2.id, 0) || [];
                cyclePath.reverse();
                cyclePath.push(dependency.id);
                console.warn(
                  cyclePath.map(
                    (id) => this._moduleIdProvider.getStrModuleId(id)
                  ).join(" => \n")
                );
                module2.unresolvedDependenciesCount--;
                continue;
              ...
              if (dependency instanceof PluginDependency) {
                ...
                let inversePluginDeps = this._inversePluginDependencies2.get(
                  dependency.pluginId
                );
        ...
        _onModuleComplete(module2) {
          ...
          const inverseDeps = this._inverseDependencies2[module2.id];
    ...
    ((AMDLoader2) => {
      ...
      const DefineFunc = /* @__PURE__ */ __name((id, dependencies, callback) => {
        if (typeof id !== "string") {
      ...
      const _requireFunc_config = /* @__PURE__ */ __name((params, shouldOverwrite = false) => {
        moduleManager.configure(params, shouldOverwrite);
      }, "_requireFunc_config");
      const RequireFunc = /* @__PURE__ */ __name(() => {
      ...
      function init() {
        if (typeof AMDLoader2.global.require !== "undefined" || typeof require !== "undefined") {
          ...
          if (typeof _nodeRequire === "function" && typeof _nodeRequire.resolve === "function") {
            const nodeRequire = AMDLoader2.ensureRecordedNodeRequire(
              moduleManager.getRecorder(),
              _nodeRequire
            );
      ...
      if (typeof AMDLoader2.global.define !== "function" || !AMDLoader2.global.define.amd) {
        moduleManager = new AMDLoader2.ModuleManager(
          env,
          AMDLoader2.createScriptLoader(env),
          DefineFunc,
          RequireFunc,
          AMDLoader2.Utilities.getHighPerformanceTimestamp()
        );
        ...
        define = /* @__PURE__ */ __name(() => DefineFunc.apply(null, arguments), "define");
