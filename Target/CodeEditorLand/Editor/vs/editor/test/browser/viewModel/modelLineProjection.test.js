...
} from "../../../common/viewModel/modelLineProjection.js";
...
suite("Editor ViewModel - SplitLinesCollection", () => {
  ...
  test("SplitLine", () => {
    ...
    assert.strictEqual(
      line1.getViewLineContent(model1, 1, 0),
      "My First Line"
    );
    assert.strictEqual(
      line1.getViewLineContent(model1, 1, 1),
      "My Second Line"
    );
    assert.strictEqual(
      line1.getViewLineContent(model1, 1, 2),
      "And another one"
    );
    ...
    for (let col = 1; col <= 14; col++) {
      assert.strictEqual(
        line1.getModelColumnOfViewPosition(0, col),
        col,
        "getInputColumnOfOutputPosition(0, " + col + ")"
      );
    }
    for (let col = 1; col <= 15; col++) {
      assert.strictEqual(
        line1.getModelColumnOfViewPosition(1, col),
        13 + col,
        "getInputColumnOfOutputPosition(1, " + col + ")"
      );
    }
    for (let col = 1; col <= 16; col++) {
      assert.strictEqual(
        line1.getModelColumnOfViewPosition(2, col),
        13 + 14 + col,
        "getInputColumnOfOutputPosition(2, " + col + ")"
      );
    }
    for (let col = 1; col <= 13; col++) {
      assert.deepStrictEqual(
        line1.getViewPositionOfModelPosition(0, col),
        pos(0, col),
        "getOutputPositionOfInputPosition(" + col + ")"
      );
    }
    ...
    for (let lineIndex = 0; lineIndex < line1.getViewLineCount(); lineIndex++) {
      ...
      for (let col = 1; col <= line1.getViewLineMaxColumn(model1, 1, lineIndex); col++) {
        actualLineViewColumnMapping.push(
          line1.getModelColumnOfViewPosition(lineIndex, col)
        );
      }
    ...
    for (let col = 1; col <= 13; col++) {
      assert.deepStrictEqual(
        line1.getViewPositionOfModelPosition(0, col),
        pos(0, col),
        "6.getOutputPositionOfInputPosition(" + col + ")"
      );
    }
    for (let col = 1 + 13; col <= 14 + 13; col++) {
      assert.deepStrictEqual(
        line1.getViewPositionOfModelPosition(0, col),
        pos(1, 4 + col - 13),
        "7.getOutputPositionOfInputPosition(" + col + ")"
      );
    }
    for (let col = 1 + 13 + 14; col <= 15 + 14 + 13; col++) {
      assert.deepStrictEqual(
        line1.getViewPositionOfModelPosition(0, col),
        pos(2, 4 + col - 13 - 14),
        "8.getOutputPositionOfInputPosition(" + col + ")"
      );
    }
  });
  function withSplitLinesCollection(text, callback) {
    ...
    const wordWrapBreakAfterCharacters = config.options.get(
      EditorOption.wordWrapBreakAfterCharacters
    );
    const wordWrapBreakBeforeCharacters = config.options.get(
  ...
  test("Invalid line numbers", () => {
    ...
    withSplitLinesCollection(text, (model, linesCollection) => {
      ...
      assert.deepStrictEqual(
        linesCollection.getViewLinesIndentGuides(0, 0),
        [0]
      );
      assert.deepStrictEqual(
        linesCollection.getViewLinesIndentGuides(1, 1),
      ...
      assert.deepStrictEqual(
        linesCollection.getViewLinesIndentGuides(2, 2),
        [1]
      );
      assert.deepStrictEqual(
        linesCollection.getViewLinesIndentGuides(3, 3),
        [0]
      );
      assert.deepStrictEqual(
        linesCollection.getViewLinesIndentGuides(4, 4),
        [0]
      );
      assert.deepStrictEqual(
        linesCollection.getViewLinesIndentGuides(5, 5),
        [1]
      );
      assert.deepStrictEqual(
        linesCollection.getViewLinesIndentGuides(6, 6),
        [0]
      );
      assert.deepStrictEqual(
        linesCollection.getViewLinesIndentGuides(7, 7),
        [0]
      );
      assert.deepStrictEqual(
        linesCollection.getViewLinesIndentGuides(0, 7),
        [0, 1, 0, 0, 1, 0]
      );
      assert.strictEqual(
        linesCollection.getViewLineContent(-1),
        "int main() {"
      );
      assert.strictEqual(
        linesCollection.getViewLineContent(0),
        "int main() {"
      );
      assert.strictEqual(
        linesCollection.getViewLineContent(1),
      ...
      assert.strictEqual(
        linesCollection.getViewLineContent(2),
        '	printf("Hello world!");'
      );
      assert.strictEqual(linesCollection.getViewLineContent(3), "}");
      assert.strictEqual(
        linesCollection.getViewLineContent(4),
        "int main() {"
      );
      assert.strictEqual(
        linesCollection.getViewLineContent(5),
        '	printf("Hello world!");'
      );
      ...
      assert.deepStrictEqual(
        linesCollection.convertViewPositionToModelPosition(-1, 1),
        new Position(1, 1)
      );
      assert.deepStrictEqual(
        linesCollection.convertViewPositionToModelPosition(0, 1),
        new Position(1, 1)
      );
      assert.deepStrictEqual(
        linesCollection.convertViewPositionToModelPosition(1, 1),
      ...
      assert.deepStrictEqual(
        linesCollection.convertViewPositionToModelPosition(2, 1),
        new Position(2, 1)
      );
      assert.deepStrictEqual(
        linesCollection.convertViewPositionToModelPosition(3, 1),
        new Position(3, 1)
      );
      assert.deepStrictEqual(
        linesCollection.convertViewPositionToModelPosition(4, 1),
        new Position(4, 1)
      );
      assert.deepStrictEqual(
        linesCollection.convertViewPositionToModelPosition(5, 1),
        new Position(5, 1)
      );
      assert.deepStrictEqual(
        linesCollection.convertViewPositionToModelPosition(6, 1),
        new Position(6, 1)
      );
      assert.deepStrictEqual(
        linesCollection.convertViewPositionToModelPosition(7, 1),
        new Position(6, 1)
      );
      assert.deepStrictEqual(
        linesCollection.convertViewPositionToModelPosition(8, 1),
        new Position(6, 1)
      );
    });
  });
    ...
    withSplitLinesCollection(text, (model, linesCollection) => {
      ...
      for (let lineNumber = 0; lineNumber <= modelLineCount + 1; lineNumber++) {
        ...
        for (let column = lineMinColumn - 1; column <= lineMaxColumn + 1; column++) {
          const viewPosition = linesCollection.convertModelPositionToViewPosition(
            lineNumber,
            column
          );
          ...
          const validViewPosition = new Position(
            viewLineNumber,
            viewColumn
          );
          assert.strictEqual(
            viewPosition.toString(),
            validViewPosition.toString(),
            "model->view for " + lineNumber + ", " + column
          );
        }
      }
      for (let lineNumber = 0; lineNumber <= viewLineCount + 1; lineNumber++) {
        ...
        for (let column = lineMinColumn - 1; column <= lineMaxColumn + 1; column++) {
          const modelPosition = linesCollection.convertViewPositionToModelPosition(
            lineNumber,
            column
          );
          ...
          assert.strictEqual(
            ...
            validModelPosition.toString(),
            "view->model for " + lineNumber + ", " + column
          );
        }
      }
    });
...
suite("SplitLinesCollection", () => {
  ...
  function assertAllMinimapLinesRenderingData(splitLinesCollection, all) {
    ...
    for (let line = 1; line <= lineCount; line++) {
      assert.strictEqual(
        splitLinesCollection.getViewLineData(line).content,
        splitLinesCollection.getViewLineContent(line)
      );
    }
    for (let start = 1; start <= lineCount; start++) {
      for (let end = start; end <= lineCount; end++) {
        ...
        for (let desired = Math.pow(2, count) - 1; desired >= 0; desired--) {
          ...
          const actual = splitLinesCollection.getViewLinesData(
            start,
            end,
            needed
          );
  ...
  test("getViewLinesData - no wrapping", () => {
    withSplitLinesCollection(model, "off", 0, (splitLinesCollection) => {
      ...
      assert.strictEqual(
        splitLinesCollection.modelPositionIsVisible(1, 1),
        true
      );
      assert.strictEqual(
        splitLinesCollection.modelPositionIsVisible(2, 1),
        true
      );
      assert.strictEqual(
        splitLinesCollection.modelPositionIsVisible(3, 1),
        true
      );
      assert.strictEqual(
        splitLinesCollection.modelPositionIsVisible(4, 1),
        true
      );
      assert.strictEqual(
        splitLinesCollection.modelPositionIsVisible(5, 1),
        true
      );
      assert.strictEqual(
        splitLinesCollection.modelPositionIsVisible(6, 1),
        true
      );
      assert.strictEqual(
        splitLinesCollection.modelPositionIsVisible(7, 1),
        true
      );
      assert.strictEqual(
        splitLinesCollection.modelPositionIsVisible(8, 1),
        true
      );
      const _expected = [
        {
  ...
  test("getViewLinesData - with wrapping and injected text", () => {
    ...
    withSplitLinesCollection(
      ...
      (splitLinesCollection) => {
        ...
        assert.strictEqual(
          splitLinesCollection.getViewLineMaxColumn(1),
          24
        );
        const _expected = [
          {
        ...
        const data = splitLinesCollection.getViewLinesData(
          1,
          14,
        ...
        assert.deepStrictEqual(
          data.map((d) => ({
            inlineDecorations: d.inlineDecorations?.map((d2) => ({
              startOffset: d2.startOffset,
              endOffset: d2.endOffset
            }))
          })),
          [
            {
              inlineDecorations: [
                { startOffset: 8, endOffset: 23 }
              ]
            },
            {
              inlineDecorations: [
                { startOffset: 4, endOffset: 30 }
              ]
            },
            {
              inlineDecorations: [
  ...
  function withSplitLinesCollection(model2, wordWrap, wordWrapColumn, callback) {
    const configuration = new TestConfiguration({
      wordWrap,
      wordWrapColumn,
      wrappingIndent: "indent"
    });
    const wrappingInfo = configuration.options.get(
      EditorOption.wrappingInfo
    );
    const fontInfo = configuration.options.get(EditorOption.fontInfo);
    const wordWrapBreakAfterCharacters = configuration.options.get(
      EditorOption.wordWrapBreakAfterCharacters
    );
    const wordWrapBreakBeforeCharacters = configuration.options.get(
      EditorOption.wordWrapBreakBeforeCharacters
    );
    const wrappingIndent = configuration.options.get(
