...
function moveLeft(editor, viewModel, inSelectionMode = false) {
  if (inSelectionMode) {
    CoreNavigationCommands.CursorLeftSelect.runCoreEditorCommand(
      viewModel,
      {}
    );
  } else {
...
function moveRight(editor, viewModel, inSelectionMode = false) {
  if (inSelectionMode) {
    CoreNavigationCommands.CursorRightSelect.runCoreEditorCommand(
      viewModel,
      {}
    );
  } else {
...
function moveDown(editor, viewModel, inSelectionMode = false) {
  if (inSelectionMode) {
    CoreNavigationCommands.CursorDownSelect.runCoreEditorCommand(
      viewModel,
      {}
    );
  } else {
...
function moveUp(editor, viewModel, inSelectionMode = false) {
  if (inSelectionMode) {
    CoreNavigationCommands.CursorUpSelect.runCoreEditorCommand(
      viewModel,
      {}
    );
  } else {
...
function moveToBeginningOfLine(editor, viewModel, inSelectionMode = false) {
  if (inSelectionMode) {
    CoreNavigationCommands.CursorHomeSelect.runCoreEditorCommand(
      viewModel,
      {}
    );
  } else {
...
function moveToEndOfLine(editor, viewModel, inSelectionMode = false) {
  if (inSelectionMode) {
    CoreNavigationCommands.CursorEndSelect.runCoreEditorCommand(
      viewModel,
      {}
    );
  } else {
...
function moveToBeginningOfBuffer(editor, viewModel, inSelectionMode = false) {
  if (inSelectionMode) {
    CoreNavigationCommands.CursorTopSelect.runCoreEditorCommand(
      viewModel,
      {}
    );
  } else {
...
function moveToEndOfBuffer(editor, viewModel, inSelectionMode = false) {
  if (inSelectionMode) {
    CoreNavigationCommands.CursorBottomSelect.runCoreEditorCommand(
      viewModel,
      {}
    );
  } else {
...
suite("Editor Controller - Cursor", () => {
  ...
  test("issue #144041: Cursor up/down works", () => {
    const model = createTextModel(
      ["Word1 Word2 Word3 Word4", "Word5 Word6 Word7 Word8"].join("\n")
    );
    withTestCodeEditor(
      model,
      {
      ...
      (editor, viewModel) => {
        ...
        function reportCursorPosition() {
          cursorPositions.push(
            viewModel.getCursorStates()[0].viewState.position.toString()
          );
        }
        reportCursorPosition();
        CoreNavigationCommands.CursorDown.runEditorCommand(
          null,
          editor,
          null
        );
        ...
        reportCursorPosition();
        CoreNavigationCommands.CursorUp.runEditorCommand(
          null,
          editor,
          null
        );
  ...
  test("select all", () => {
    runTest((editor, viewModel) => {
      CoreNavigationCommands.SelectAll.runCoreEditorCommand(
        viewModel,
        {}
      );
    ...
    runTest((editor, viewModel) => {
      ...
      const disposable = viewModel.onEvent((e) => {
        if (e.kind === OutgoingViewModelEventKind.CursorStateChanged) {
          events++;
          assert.deepStrictEqual(e.selections, [
  ...
  test("grapheme breaking", () => {
    withTestCodeEditor(
      ["abcabc", "a\u0303a\u0303a\u0303a\u0303a\u0303a\u0303", "\u8FBB\u{E0100}\u8FBB\u{E0100}\u8FBB\u{E0100}", "\u0BAA\u0BC1"],
      {},
      (editor, viewModel) => {
  ...
  test("issue #20087: column select with keyboard", () => {
    withTestCodeEditor(
      ...
      (editor, viewModel) => {
        moveTo(editor, viewModel, 10, 10, false);
        assertCursor(viewModel, new Position(10, 10));
        CoreNavigationCommands.CursorColumnSelectLeft.runCoreEditorCommand(
          viewModel,
          {}
        );
        ...
        CoreNavigationCommands.CursorColumnSelectRight.runCoreEditorCommand(
          viewModel,
          {}
        );
        assertCursor(viewModel, [new Selection(10, 10, 10, 9)]);
        CoreNavigationCommands.CursorColumnSelectUp.runCoreEditorCommand(
          viewModel,
          {}
        );
        assertCursor(viewModel, [
        ...
        CoreNavigationCommands.CursorColumnSelectDown.runCoreEditorCommand(
          viewModel,
          {}
        );
        assertCursor(viewModel, [new Selection(10, 10, 10, 9)]);
  ...
  test("setSelection / setPosition with source", () => {
    ...
    const languageRegistration = TokenizationRegistry.register(
      LANGUAGE_ID,
      tokenizationSupport
    );
...
suite("Editor Controller", () => {
  ...
  setup(() => {
    ...
    disposables.add(
      languageService.registerLanguage({ id: surroundingLanguageId })
    );
    disposables.add(
      languageConfigurationService.register(surroundingLanguageId, {
    ...
    disposables.add(
      languageService.registerLanguage({ id: electricCharLanguageId })
    );
    disposables.add(
      languageConfigurationService.register(electricCharLanguageId, {
  ...
  function setupOnEnterLanguage(indentAction) {
    ...
    disposables.add(
      languageService.registerLanguage({ id: onEnterLanguageId })
    );
    disposables.add(
      languageConfigurationService.register(onEnterLanguageId, {
        onEnterRules: [
          {
            beforeText: /.*/,
            action: {
  ...
  function setupAutoClosingLanguage() {
    disposables.add(
      languageService.registerLanguage({ id: autoClosingLanguageId })
    );
    disposables.add(
      languageConfigurationService.register(autoClosingLanguageId, {
        comments: {
          blockComment: ["/*", "*/"]
        },
        autoClosingPairs: [
  ...
  function setupAutoClosingLanguageTokenization() {
    ...
    disposables.add(
      TokenizationRegistry.register(autoClosingLanguageId, {
        getInitialState: () => new BaseState(),
        tokenize: void 0,
        tokenizeEncoded: (line, hasEOL, _state) => {
          let state = _state;
          const tokens = [];
            ...
            if (state instanceof BaseState) {
              ...
              if (/^['"`]/.test(line)) {
                return generateToken(
                  ...
                  new StringState(line.charAt(0), state)
                );
              }
              if (/^{/.test(line)) {
                return generateToken(
  ...
  test("Bug 9121: Auto indent + undo + redo is funky", () => {
    const model = createTextModel2([""].join("\n"), void 0, {
      insertSpaces: false,
    ...
    withTestCodeEditor2(model, {}, (editor, viewModel) => {
      ...
      CoreNavigationCommands.CursorLeft.runCoreEditorCommand(
        viewModel,
        {}
      );
      assert.strictEqual(
  ...
  test("issue #23539: Setting model EOL isn't undoable", () => {
    withTestCodeEditor2(["Hello", "world"], {}, (editor, viewModel) => {
  ...
  test("issue #46208: Allow empty selections in the undo/redo stack", () => {
    ...
    withTestCodeEditor2(model, {}, (editor, viewModel) => {
      ...
      model.pushEditOperations(
        [],
        [EditOperation.replaceMove(new Range(1, 12, 1, 13), "")],
        () => []
      );
      assert.strictEqual(model.getLineContent(1), "Hello world");
  ...
  test("issue #95591: Unindenting moves cursor to beginning of line", () => {
    ...
    withTestCodeEditor2(
      ...
      (editor, viewModel) => {
        ...
        CoreEditingCommands.Outdent.runEditorCommand(
          null,
          editor,
          null
        );
  ...
  test("bug #16740: [editor] Cut line doesn't quite cut the last line", () => {
    withTestCodeEditor2(["asdasd", "qwerty"], {}, (editor, viewModel) => {
      const model = editor.getModel();
    ...
    withTestCodeEditor2(["asdasd", ""], {}, (editor, viewModel) => {
      const model = editor.getModel();
      moveTo(editor, viewModel, 2, 1, false);
      assertCursor(viewModel, new Selection(2, 1, 2, 1));
      viewModel.cut("keyboard");
  ...
  test("issue #128602: When cutting multiple lines (ctrl x), the last line will not be erased", () => {
    withTestCodeEditor2(["a1", "a2", "a3"], {}, (editor, viewModel) => {
      const model = editor.getModel();
  ...
  test("issue #3071: Investigate why undo stack gets corrupted", () => {
    const model = createTextModel2(
      ["some lines", "and more lines", "just some text"].join("\n")
    );
    withTestCodeEditor2(model, {}, (editor, viewModel) => {
      moveTo(editor, viewModel, 1, 1, false);
  ...
  test("issue #12950: Cannot Double Click To Insert Emoji Using OSX Emoji Panel", () => {
    usingCursor(
      ...
      (editor, model, viewModel) => {
        ...
        assert.strictEqual(
          model.getValue(),
          ["some lines", "and more lines", "\u{1F60D}just some text"].join(
            "\n"
          )
        );
      }
    ...
    usingCursor(
      ...
      (editor, model, viewModel) => {
        ...
        function assertWordRight(col, expectedCol) {
          ...
          assert.strictEqual(
            viewModel.getSelection().startColumn,
            1,
            "TEST FOR " + col
          );
          assert.strictEqual(
            viewModel.getSelection().endColumn,
            expectedCol,
            "TEST FOR " + col
          );
        }
        ...
        assertWordRight(
          31,
          "   /* Just some   more   text ".length + 1
        );
        assertWordRight(
          32,
          "   /* Just some   more   text a".length + 1
        );
        assertWordRight(
          33,
          "   /* Just some   more   text a+".length + 1
        ...
        assertWordRight(
          36,
          "   /* Just some   more   text a+= 3".length + 1
        );
        assertWordRight(
          37,
          "   /* Just some   more   text a+= 3 ".length + 1
        ...
        assertWordRight(
          39,
          "   /* Just some   more   text a+= 3 +5".length + 1
        );
        assertWordRight(
          40,
          "   /* Just some   more   text a+= 3 +5-".length + 1
        ...
        assertWordRight(
          41,
          "   /* Just some   more   text a+= 3 +5-3".length + 1
        );
        assertWordRight(
          42,
          "   /* Just some   more   text a+= 3 +5-3 ".length + 1
        ...
        assertWordRight(
          45,
          "   /* Just some   more   text a+= 3 +5-3 + 7".length + 1
        );
        assertWordRight(
          46,
          "   /* Just some   more   text a+= 3 +5-3 + 7 ".length + 1
  ...
  test("issue #33788: Wrong cursor position when double click to select a word", () => {
    const model = createTextModel2(["Just some text"].join("\n"));
    withTestCodeEditor2(model, {}, (editor, viewModel) => {
  ...
  test("issue #12887: Double-click highlighting separating white space", () => {
    const model = createTextModel2(["abc def"].join("\n"));
    withTestCodeEditor2(model, {}, (editor, viewModel) => {
      CoreNavigationCommands.WordSelect.runCoreEditorCommand(viewModel, {
  ...
  test("issue #36740: wordwrap creates an extra step / character at the wrapping point", () => {
    withTestCodeEditor2(
      ...
      { wordWrap: "wordWrapColumn", wordWrapColumn: 16 },
      (editor, viewModel) => {
  ...
  test("issue #110376: multiple selections with wordwrap behave differently", () => {
    withTestCodeEditor2(
      ...
      { wordWrap: "wordWrapColumn", wordWrapColumn: 25 },
      (editor, viewModel) => {
        viewModel.setSelections("test", [
  ...
  test("issue #41573 - delete across multiple lines does not shrink the selection when word wraps", () => {
    withTestCodeEditor2(
      ...
      { wordWrap: "wordWrapColumn", wordWrapColumn: 100 },
      (editor, viewModel) => {
  ...
  test("issue #37967: problem replacing consecutive characters", () => {
    ...
    withTestCodeEditor2(
      ...
      (editor, viewModel) => {
        ...
        CoreEditingCommands.DeleteLeft.runEditorCommand(
          null,
          editor,
          null
        );
  ...
  test("issue #15761: Cursor doesn't move in a redo operation", () => {
    const model = createTextModel2(["hello"].join("\n"));
    withTestCodeEditor2(model, {}, (editor, viewModel) => {
  ...
  test("issue #42783: API Calls with Undo Leave Cursor in Wrong Position", () => {
    const model = createTextModel2(["ab"].join("\n"));
    withTestCodeEditor2(model, {}, (editor, viewModel) => {
  ...
  test("issue #99629: Emoji modifiers in text treated separately when using backspace", () => {
    const model = createTextModel2(["\u{1F476}\u{1F3FE}"].join("\n"));
    withTestCodeEditor2(
      model,
      { useTabStops: false },
      (editor, viewModel) => {
  ...
  test("issue #99629: Emoji modifiers in text treated separately when using backspace (ZWJ sequence)", () => {
    const model = createTextModel2(["\u{1F468}\u200D\u{1F469}\u{1F3FD}\u200D\u{1F467}\u200D\u{1F466}"].join("\n"));
    withTestCodeEditor2(
      model,
      { useTabStops: false },
      (editor, viewModel) => {
        ...
        assert.strictEqual(
          model.getValue(EndOfLinePreference.LF),
          "\u{1F468}\u200D\u{1F469}\u{1F3FD}\u200D\u{1F467}"
        );
        CoreEditingCommands.DeleteLeft.runEditorCommand(
          null,
          editor,
  ...
  test("issue #123178: sticky tab in consecutive wrapped lines", () => {
    const model = createTextModel2("    aaaa        aaaa", void 0, {
      tabSize: 4
    });
    withTestCodeEditor2(
      model,
  ...
  test("Enter auto-indents with insertSpaces setting 1", () => {
    ...
    usingCursor(
      ...
      (editor, model, viewModel) => {
        ...
        viewModel.type("\n", "keyboard");
        assert.strictEqual(
          model.getValue(EndOfLinePreference.CRLF),
          "	hello\r\n        "
        );
      }
    );
    ...
    usingCursor(
      ...
      (editor, model, viewModel) => {
        moveTo(
          ...
          viewModel,
          1,
          model.getLineContent(1).length + 1
        );
        viewModel.type("\n", "keyboard");
        assert.strictEqual(
          model.getLineContent(1),
  ...
  test("removeAutoWhitespace on: test 1", () => {
    const model = createTextModel2(["    some  line abc  "].join("\n"));
    withTestCodeEditor2(model, {}, (editor, viewModel) => {
    ...
    withTestCodeEditor2(
      ...
      (editor, viewModel) => {
        moveTo(
          ...
          viewModel,
          2,
          model.getLineContent(2).length + 1
        );
        CoreEditingCommands.DeleteLeft.runEditorCommand(
          null,
          editor,
        ...
        moveTo(
          ...
          viewModel,
          3,
          model.getLineContent(3).length + 1
        );
        CoreEditingCommands.DeleteLeft.runEditorCommand(
          null,
          editor,
    ...
    usingCursor(
      ...
      (editor, model, viewModel) => {
        ...
        assert.strictEqual(
          model.getLineContent(5),
          "	}e",
          "This line should not decrease indent"
        );
      }
    );
  });
  ...
  test("issue #111128: Multicursor `Enter` issue with indentation", () => {
    const model = createTextModel2(
      "    let a, b, c;",
      indentRulesLanguageId,
      { detectIndentation: false, insertSpaces: false, tabSize: 4 }
    );
    withTestCodeEditor2(model, {}, (editor, viewModel) => {
      editor.setSelections([
        new Selection(1, 11, 1, 11),
  ...
  test("issue #122714: tabSize=1 prevent typing a string matching decreaseIndentPattern in an empty file", () => {
    ...
    const model = createTextModel2("\\end", latextLanguageId, {
      tabSize: 1
    });
    withTestCodeEditor2(
      model,
    ...
    usingCursor(
      ...
      (editor, model, viewModel) => {
        ...
        for (let i = 0, len = autoClosePositions.length; i < len; i++) {
          ...
          const autoCloseColumns = extractAutoClosingSpecialColumns(
            model.getLineMaxColumn(lineNumber),
            autoClosePositions[i]
          );
          for (let column = 1; column < autoCloseColumns.length; column++) {
            ...
            if (autoCloseColumns[column] === 1 /* Special1 */) {
              ...
              assertType(
                ...
                "(",
                "(",
                `does not auto close @ (${lineNumber}, ${column})`
              );
            }
          }
        }
    ...
    usingCursor(
      ...
      (editor, model, viewModel) => {
        ...
        assert.strictEqual(
          model.getLineContent(1),
          "(**)",
          `doesn't add entire close when already closed substring is there`
        );
        model.setValue("(");
        ...
        assert.strictEqual(
          model.getLineContent(1),
          "(**)",
          `does add entire close if not already there`
        );
        model.setValue("");
        viewModel.type("<@", "keyboard");
        assert.strictEqual(model.getLineContent(1), "<@@>");
        ...
        assert.strictEqual(
          model.getLineContent(1),
          "<@@@@>",
          `autocloses when before multi-character closing brace`
        );
        viewModel.type("(", "keyboard");
        assert.strictEqual(
          model.getLineContent(1),
    ...
    usingCursor(
      ...
      (editor, model, viewModel) => {
        ...
        assertType(
          ...
          '"',
          '"',
          `does not double quote when ending with open`
        );
        model.tokenization.forceTokenization(model.getLineCount());
        assertType(
          editor,
    ...
    usingCursor(
      ...
      (editor, model, viewModel) => {
        assertType(
          ...
          '"',
          '"',
          `does not over type and will not auto close`
        );
      }
    );
  });
  ...
  test("issue #78975 - Parentheses swallowing does not work when parentheses are inserted by autocomplete", () => {
    usingCursor(
      ...
      (editor, model, viewModel) => {
        ...
        viewModel.executeEdits(
          "snippet",
          [{ range: new Range(1, 6, 1, 8), text: 'id=""' }],
          () => [new Selection(1, 10, 1, 10)]
        );
  ...
  test("issue #7100: Mouse word selection is strange when non-word character is at the end of line", () => {
    ...
    withTestCodeEditor2(model, {}, (editor, viewModel) => {
      ...
      CoreNavigationCommands.WordSelectDrag.runEditorCommand(
        null,
        editor,
        {
  ...
  test("issue #112039: shift-continuing a double/triple-click and drag selection does not remember its starting mode", () => {
    const model = createTextModel2(
      ["just some text", "and another line", "and another one"].join(
        "\n"
      )
    );
    withTestCodeEditor2(model, {}, (editor, viewModel) => {
  ...
  test("issue #158236: Shift click selection does not work on line number indicator", () => {
    ...
    withTestCodeEditor2(model, {}, (editor, viewModel) => {
      ...
      CoreNavigationCommands.LineSelectDrag.runEditorCommand(
        null,
        editor,
        {
          position: new Position(2, 1)
