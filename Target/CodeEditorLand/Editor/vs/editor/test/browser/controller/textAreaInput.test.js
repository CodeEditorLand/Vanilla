...
suite("TextAreaInput", () => {
  ...
  async function simulateInteraction(recorded) {
    ...
    const host = {
      ...
      getScreenReaderContent: () => new TextAreaState("", 0, 0, null, void 0),
      deduceModelPosition: (viewAnchorPosition, deltaOffset, lineFeedCnt) => {
        throw new Error("Function not implemented.");
      }
    };
    const wrapper = disposables.add(
      new class extends Disposable {
        _onKeyDown = this._register(
          new Emitter()
        );
        onKeyDown = this._onKeyDown.event;
        _onKeyPress = this._register(
          new Emitter()
        );
        ...
        _onCompositionEnd = this._register(
          new Emitter()
        );
        onCompositionEnd = this._onCompositionEnd.event;
        _onBeforeInput = this._register(
          new Emitter()
        );
        ...
        setValue(reason, value) {
          if (this._currDispatchingEvent?.type === "compositionstart") {
            assert.fail(
              "should not change the state of the textarea in a compositionstart"
            );
          }
  ...
  function interpretTypeEvents(OS, browser, initialState, events) {
    ...
    for (const event of events) {
      if (event.type === "type") {
        text = text.substring(
          0,
          selectionStart - event.replacePrevCharCnt
        ) + event.text + text.substring(selectionEnd + event.replaceNextCharCnt);
  ...
  test("macOS - Chrome - Korean using 2-Set Korean (1)", async () => {
    ...
    const actualResultingState = interpretTypeEvents(
      recorded.env.OS,
      recorded.env.browser,
      recorded.initial,
      actualOutgoingEvents
    );
