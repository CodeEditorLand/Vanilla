  ...
  paint(output) {
    ...
    for (let i = 1; i <= this._model.getLineCount(); i++) {
      const textNode = document.createTextNode(
        this._model.getModelLineContent(i)
      );
...
function doCreateTest(description, inputStr, expectedStr) {
  ...
  handler.onType((e) => {
    console.log(
      "type text: " + e.text + ", replaceCharCnt: " + e.replacePrevCharCnt
    );
...
const TESTS = [
  ...
  { description: "Mac hold key 1", in: "e long press and 1", out: "\xE9" }
];
TESTS.forEach((t) => {
  mainWindow.document.body.appendChild(
    doCreateTest(t.description, t.in, t.out)
  );
});
