...
class SingleLineTestModel {
  static {
    __name(this, "SingleLineTestModel");
  }
  _line;
  constructor(line) {
    this._line = line;
...
function doCreateTest(description, inputStr, expectedStr) {
  ...
  const updatePosition = /* @__PURE__ */ __name((off, len) => {
    cursorOffset = off;
    cursorLength = len;
    handler.writeNativeTextAreaContent("selection changed");
    handler.focusTextArea();
  }, "updatePosition");
  const updateModelAndPosition = /* @__PURE__ */ __name((text, off, len) => {
    model._setText(text);
...
__name(doCreateTest, "doCreateTest");
const TESTS = [
