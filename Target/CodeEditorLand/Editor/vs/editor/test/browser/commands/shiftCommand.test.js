...
function prepareDocBlockCommentLanguage(accessor, disposables) {
  ...
  disposables.add(
    new DocBlockCommentMode(languageService, languageConfigurationService)
  );
}
  ...
  test("bug #16815:Shift+Tab doesn't go back to tabstop", () => {
    ...
    function _assertUnshiftCommand(tabSize, indentSize, insertSpaces, text, expected) {
      return withEditorModel(text, (model) => {
        ...
        const op = new ShiftCommand(
          new Selection(1, 1, text.length + 1, 1),
          {
            isUnshift: true,
            tabSize,
            indentSize,
