...
suite("EditorSimpleWorker", () => {
  ...
  test("ICommonModel#validatePosition, issue #15882", () => {
  ...
  test("MoreMinimal", () => {
    return worker.$computeMoreMinimalEdits(
      model.uri.toString(),
      [{ text: "This is line One", range: new Range(1, 1, 1, 17) }],
      false
    ).then((edits) => {
  ...
  test("MoreMinimal, merge adjacent edits", async () => {
    const model2 = worker.addModel(
      ["one", "two", "three", "four", "five"],
      "\n"
    );
    const newEdits = await worker.$computeMoreMinimalEdits(
      model2.uri.toString(),
      [
        {
  ...
  test("MoreMinimal, issue #15385 newline changes only", () => {
    const model2 = worker.addModel(["{", '	"a":1', "}"], "\n");
    return worker.$computeMoreMinimalEdits(
      model2.uri.toString(),
  ...
  test("MoreMinimal, issue #15385 newline changes and other", () => {
    const model2 = worker.addModel(["{", '	"a":1', "}"], "\n");
    return worker.$computeMoreMinimalEdits(
      model2.uri.toString(),
      [{ text: '{\r\n	"b":1\r\n}', range: new Range(1, 1, 3, 2) }],
  ...
  test("MoreMinimal, issue #15385 newline changes and other 2/2", () => {
    const model2 = worker.addModel([
      "package main",
      // 1
      "func foo() {",
  ...
  async function testEdits(lines, edits) {
    ...
    return smallerEdits.map((e) => ({
      range: Range.lift(e.range).toString(),
      text: e.text
    }));
  }
  ...
  test('[Bug] Getting Message "Overlapping ranges are not allowed" and nothing happens with Inline-Chat ', async () => {
    await testEdits(
...
`
  ...
  test("ICommonModel#getValueInRange, issue #17424", () => {
    const model2 = worker.addModel([
      "package main",
      // 1
      "func foo() {",
    ...
    const value = model2.getValueInRange({
      startLineNumber: 3,
      startColumn: 1,
      endLineNumber: 4,
      endColumn: 1
  ...
  test("textualSuggest, issue #17785", () => {
    const model2 = worker.addModel([
      "foobar",
      // 1
      "f f"
  ...
  test("get words via iterator, issue #46930", () => {
    const model2 = worker.addModel([
      "one line",
      // 1
      "two line",
    ...
    const words = [...model2.words(/[a-z]+/gim)];
...
class PositionOffsetTransformer {
  ...
  getOffset(position) {
    ...
    return Math.min(
      this.lineStartOffsetByLineIdx[position.lineNumber - 1] + position.column - 1,
      maxLineOffset
    );
  }
}
