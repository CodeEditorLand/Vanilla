...
suite("ModelService", () => {
  ...
  test("EOL setting respected depending on root", () => {
    ...
    const model2 = modelService.createModel(
      ...
      null,
      URI.file(
        platform.isWindows ? "c:\\myroot\\myfile.txt" : "/myroot/myfile.txt"
      )
    );
    const model3 = modelService.createModel(
      ...
      null,
      URI.file(
        platform.isWindows ? "c:\\other\\myfile.txt" : "/other/myfile.txt"
      )
    );
    assert.strictEqual(model1.getOptions().defaultEOL, DefaultEndOfLine.LF);
    assert.strictEqual(
      model2.getOptions().defaultEOL,
  ...
  test("_computeEdits first line changed", () => {
    const model = disposables.add(
      createTextModel(
        [
          "This is line one",
  ...
  test("_computeEdits EOL changed", () => {
    const model = disposables.add(
      createTextModel(
        [
          "This is line one",
  ...
  test("_computeEdits EOL and other change 1", () => {
    const model = disposables.add(
      createTextModel(
        [
          "This is line one",
  ...
  test("_computeEdits EOL and other change 2", () => {
    const model = disposables.add(
      createTextModel(
        [
          "package main",
  ...
  test("maintains undo for same resource and same content", () => {
    ...
    model1.pushEditOperations(
      null,
      [{ range: new Range(1, 5, 1, 5), text: "1" }],
  ...
  test("maintains version id and alternative version id for same resource and same content", () => {
    ...
    assert.strictEqual(
      model2.getAlternativeVersionId(),
...
function assertComputeEdits(lines1, lines2) {
  ...
  const { disposable, textBuffer } = createTextBuffer(
    lines2.join("\n"),
    DefaultEndOfLine.LF
  );
