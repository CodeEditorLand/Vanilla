...
} from "../../../common/services/languagesAssociations.js";
suite("LanguagesAssociations", () => {
  ...
  test("Dynamically Register Text Mime", () => {
    ...
    guess = getMimeTypes(
      URI.file("Randomfile.noregistration"),
      "RegexesAreNice"
    );
  ...
  test("Specificity priority 1", () => {
    ...
    assert.deepStrictEqual(getMimeTypes(URI.file("foo.monaco2")), [
      "text/monaco2",
      "text/plain"
    ]);
  });
  test("Specificity priority 2", () => {
    ...
    assert.deepStrictEqual(getMimeTypes(URI.file("foo.monaco3")), [
      "text/monaco3",
      "text/plain"
    ]);
  });
  ...
  test("Data URIs", () => {
    ...
    assert.deepStrictEqual(
      getMimeTypes(
        URI.parse(`data:;label:something.data;description:data,`)
      ),
