  ...
  test("issue #94521: unusual backing array buffer", () => {
    function wrapAndSliceUint8Arry(buff, prefixLength, suffixLength) {
      const wrapped = new Uint8Array(
        prefixLength + buff.byteLength + suffixLength
      );
      wrapped.set(buff, prefixLength);
      return wrapped.subarray(
        prefixLength,
        prefixLength + buff.byteLength
      );
    }
    function wrapAndSlice(buff, prefixLength, suffixLength) {
      return VSBuffer.wrap(
        wrapAndSliceUint8Arry(buff.buffer, prefixLength, suffixLength)
      );
    }
    const dto = {
      id: 5,
    ...
    assertEqualFull(
      decodeSemanticTokensDto(wrapAndSlice(encoded, 1, 1)),
      dto
    );
    assertEqualFull(
      decodeSemanticTokensDto(wrapAndSlice(encoded, 1, 4)),
      dto
    );
    assertEqualFull(
      decodeSemanticTokensDto(wrapAndSlice(encoded, 4, 1)),
      dto
    );
    assertEqualFull(
      decodeSemanticTokensDto(wrapAndSlice(encoded, 4, 4)),
      dto
    );
  });
});
