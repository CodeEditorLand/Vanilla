...
import {
  _tokenizeToString,
  tokenizeLineToHTML
} from "../../../common/languages/textToHtmlTokenizer.js";
...
suite("Editor Modes - textToHtmlTokenizer", () => {
  ...
  function toStr(pieces) {
    const resultArr = pieces.map(
      (t) => `<span class="${t.className}">${t.text}</span>`
    );
  ...
  test("TextToHtmlTokenizer 1", () => {
    ...
    const actual = _tokenizeToString(
      ".abc..def...gh",
      new LanguageIdCodec(),
      support
    );
  ...
  test("tokenizeLineToHTML", () => {
    ...
    const lineTokens = new TestLineTokens([
      ...
      new TestLineToken(5, 1 << MetadataConsts.FOREGROUND_OFFSET >>> 0),
      new TestLineToken(
        10,
  ...
  test("tokenizeLineToHTML handle spaces #35954", () => {
    const text = "  Ciao   hello world!";
    const lineTokens = new TestLineTokens([
      new TestLineToken(2, 1 << MetadataConsts.FOREGROUND_OFFSET >>> 0),
      new TestLineToken(
        6,
        (3 << MetadataConsts.FOREGROUND_OFFSET | (FontStyle.Bold | FontStyle.Italic) << MetadataConsts.FONT_STYLE_OFFSET) >>> 0
      ),
      new TestLineToken(9, 1 << MetadataConsts.FOREGROUND_OFFSET >>> 0),
      new TestLineToken(
        14,
        4 << MetadataConsts.FOREGROUND_OFFSET >>> 0
      ),
