...
suite("Token theme matching", () => {
  ...
  test("gives higher priority to deeper matches", () => {
    ...
    assert.deepStrictEqual(
      actual,
      new ThemeTrieElementRule(FontStyle.None, _D, _B)
    );
  });
  test("can match", () => {
    const theme = TokenTheme.createFromRawTokenTheme(
    ...
    function assertMatch(scopeName, expected) {
      ...
      assert.deepStrictEqual(
        actual,
        expected,
        "when matching <<" + scopeName + ">>"
      );
    }
    function assertSimpleMatch(scopeName, fontStyle, foreground, background) {
      assertMatch(
        scopeName,
        new ThemeTrieElementRule(fontStyle, foreground, background)
      );
    }
    function assertNoMatch(scopeName) {
      assertMatch(
        scopeName,
        new ThemeTrieElementRule(FontStyle.None, _A, _B)
      );
    }
    assertNoMatch("");
    assertNoMatch("bazz");
...
suite("Token theme resolving", () => {
  ...
  test("respects incoming defaults 3", () => {
    ...
    assert.deepStrictEqual(
      actual.getThemeTrieElement(),
      new ExternalThemeTrieElement(
        new ThemeTrieElementRule(FontStyle.Bold, _A, _B)
      )
    );
  });
  ...
  test("defaults are inherited", () => {
    ...
    assert.deepStrictEqual(actual.getColorMap(), colorMap.getColorMap());
    const root = new ExternalThemeTrieElement(
      new ThemeTrieElementRule(FontStyle.None, _A, _B),
      {
        var: new ExternalThemeTrieElement(
          new ThemeTrieElementRule(FontStyle.None, _C, _B)
        )
      }
    );
  ...
  test("same rules get merged", () => {
    ...
    const root = new ExternalThemeTrieElement(
      ...
      {
        var: new ExternalThemeTrieElement(
          new ThemeTrieElementRule(FontStyle.Bold, _C, _B)
        )
      }
    );
    assert.deepStrictEqual(actual.getThemeTrieElement(), root);
    ...
    const root = new ExternalThemeTrieElement(
      new ThemeTrieElementRule(FontStyle.None, _A, _B),
      {
        var: new ExternalThemeTrieElement(
          new ThemeTrieElementRule(FontStyle.Bold, _C, _B),
          {
            identifier: new ExternalThemeTrieElement(
              new ThemeTrieElementRule(FontStyle.Bold, _D, _B)
            )
          }
        )
      }
  ...
  test("rules are inherited 2", () => {
    ...
    const root = new ExternalThemeTrieElement(
      ...
      {
        var: new ExternalThemeTrieElement(
          new ThemeTrieElementRule(FontStyle.Bold, _F, _B),
          {
            identifier: new ExternalThemeTrieElement(
        ...
        constant: new ExternalThemeTrieElement(
          new ThemeTrieElementRule(FontStyle.Italic, _C, _B),
          {
            numeric: new ExternalThemeTrieElement(
              new ThemeTrieElementRule(FontStyle.Italic, _D, _B),
              {
                hex: new ExternalThemeTrieElement(
                ...
                dec: new ExternalThemeTrieElement(
                  new ThemeTrieElementRule(
                    FontStyle.None,
                    _E,
                    _B
