...
} from "./editableTextModelTestUtils.js";
suite(
...
suite(
  ...
  () => {
    ...
    test("start with ASCII, insert NON-ASCII", () => {
      testApplyEdits(
        ["Hello,\nworld!"],
        [editOp(1, 1, 1, 1, ["Z\xFCrich", "Z\xFCrich"])],
        false,
        true
      );
    });
...
suite("EditorModel - EditableTextModel.applyEdits", () => {
  ...
  test("high-low surrogates 1", () => {
    testApplyEditsWithSyncedModels(
      ["\u{1F4DA}some", "very nice", "text"],
      [editOp(1, 2, 1, 2, ["a"])],
      ["a\u{1F4DA}some", "very nice", "text"],
  ...
  test("issue #47733: Undo mangles unicode characters", () => {
    ...
    assert.deepStrictEqual(
      model.validateRange(new Range(1, 3, 1, 4)),
      new Range(1, 3, 1, 4)
    );
    model.applyEdits([
