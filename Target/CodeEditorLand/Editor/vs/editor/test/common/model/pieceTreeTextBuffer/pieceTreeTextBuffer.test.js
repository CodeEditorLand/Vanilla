...
function randomStr(len) {
  ...
  return (() => {
    let j, ref;
    const results = [];
    for (j = 1, ref = len; ref >= 1 ? j < ref : j > ref; ref >= 1 ? j++ : j--) {
...
suite("prefix sum for line feed", () => {
  ...
  test("insert", () => {
    ...
    assert.deepStrictEqual(pieceTable.getPositionAt(9), new Position(4, 4));
    assert.deepStrictEqual(
      pieceTable.getPositionAt(12),
      new Position(6, 1)
    );
    assert.deepStrictEqual(
      pieceTable.getPositionAt(13),
      new Position(6, 2)
    );
    assert.deepStrictEqual(
      pieceTable.getPositionAt(14),
      new Position(6, 3)
    );
    assert.strictEqual(pieceTable.getOffsetAt(4, 1), 6);
...
suite("get text in range", () => {
  ...
  test("getContentInRange", () => {
    ...
    pieceTable.delete(7, 2);
    assert.strictEqual(
      pieceTable.getValueInRange(new Range(1, 1, 1, 3)),
      "a\n"
    );
    assert.strictEqual(
      pieceTable.getValueInRange(new Range(2, 1, 2, 3)),
      "b\n"
    );
    assert.strictEqual(
      pieceTable.getValueInRange(new Range(3, 1, 3, 3)),
      "c\n"
    );
    assert.strictEqual(
      pieceTable.getValueInRange(new Range(4, 1, 4, 4)),
      "dh\n"
    );
    assert.strictEqual(
      pieceTable.getValueInRange(new Range(5, 1, 5, 3)),
      "i\n"
    );
    assert.strictEqual(
      pieceTable.getValueInRange(new Range(6, 1, 6, 3)),
      "jk"
    );
    assertTreeInvariants(pieceTable);
  });
...
suite("random is unsupervised", () => {
  const ds = ensureNoDisposablesAreLeakedInTestSuite();
  test("splitting large change buffer", () => {
    const pieceTree = createTextBuffer([""], false);
    ds.add(pieceTree);
    const pieceTable = pieceTree.getPieceTree();
    let str = "";
...
suite("buffer api", () => {
  ...
  test("getLineCharCode - issue #45735", () => {
    ...
    const pieceTable = pieceTree.getPieceTree();
    assert.strictEqual(
      pieceTable.getLineCharCode(1, 0),
      "L".charCodeAt(0),
      "L"
    );
    assert.strictEqual(
      pieceTable.getLineCharCode(1, 1),
      "I".charCodeAt(0),
      "I"
    );
    assert.strictEqual(
      pieceTable.getLineCharCode(1, 2),
      "N".charCodeAt(0),
      "N"
    );
    assert.strictEqual(
      pieceTable.getLineCharCode(1, 3),
      "E".charCodeAt(0),
      "E"
    );
    assert.strictEqual(
      pieceTable.getLineCharCode(1, 4),
      "1".charCodeAt(0),
      "1"
    );
    assert.strictEqual(
    ...
    assert.strictEqual(
      pieceTable.getLineCharCode(2, 0),
      "l".charCodeAt(0),
      "l"
    );
    assert.strictEqual(
      pieceTable.getLineCharCode(2, 1),
      "i".charCodeAt(0),
      "i"
    );
    assert.strictEqual(
      pieceTable.getLineCharCode(2, 2),
      "n".charCodeAt(0),
      "n"
    );
    assert.strictEqual(
      pieceTable.getLineCharCode(2, 3),
      "e".charCodeAt(0),
      "e"
    );
    assert.strictEqual(
      pieceTable.getLineCharCode(2, 4),
      "2".charCodeAt(0),
      "2"
    );
  });
