...
import {
  ...
  nodeAcceptEdit,
  setNodeStickiness
} from "../../../common/model/intervalTree.js";
...
suite("IntervalTree 1", () => {
  ...
  class TestState {
    ...
    acceptOp(op) {
      if (op.type === "insert") {
        if (PRINT_TREE) {
          console.log(
            `insert: {${JSON.stringify(new Interval(op.begin, op.end))}}`
          );
        }
        const nodeId = ++this._lastNodeId;
        this._treeNodes[nodeId] = new IntervalNode(
          null,
          op.begin,
          op.end
        );
        this._tree.insert(this._treeNodes[nodeId]);
        this._oracleNodes[nodeId] = this._oracle.insert(
          new Interval(op.begin, op.end)
        );
      } else if (op.type === "delete") {
        if (PRINT_TREE) {
          console.log(
            `delete: {${JSON.stringify(this._oracleNodes[op.id])}}`
          );
        }
        ...
        const actualNodes = this._tree.intervalSearch(
          op.begin,
          op.end,
          0,
          false,
        ...
        const actual2 = actualNodes.map(
          (n) => new Interval(
            n.cachedAbsoluteStart,
            n.cachedAbsoluteEnd
          )
        );
        const expected2 = this._oracle.search(
          new Interval(op.begin, op.end)
        );
      ...
      const actual = this._tree.getAllInOrder().map(
        (n) => new Interval(
          n.cachedAbsoluteStart,
          n.cachedAbsoluteEnd
        )
  ...
  class AutoTest {
    ...
    run() {
      while (this._insertCnt > 0 || this._deleteCnt > 0 || this._changeCnt > 0) {
        ...
        const searchRange = getRandomRange(
          MIN_INTERVAL_START,
          MAX_INTERVAL_END
        );
        this._run({
  ...
  suite("generated", () => {
    ...
    test("force delta overflow", () => {
      testIntervalTree([
        {
          type: "insert",
          begin: 686081138593427,
          end: 733009856502260
        },
        {
          type: "insert",
          begin: 591031326181669,
          end: 591031326181672
        },
        {
          type: "insert",
          begin: 940037682731896,
          end: 940037682731903
        },
        {
          type: "insert",
          begin: 598413641151120,
          end: 598413641151128
        },
        {
          type: "insert",
          begin: 800564156553344,
          end: 800564156553351
        },
        {
          type: "insert",
          begin: 894198957565481,
          end: 894198957565491
        }
      ]);
    });
  ...
  suite("searching", () => {
    ...
    function assertIntervalSearch(start, end, expected) {
      const actualNodes = T.intervalSearch(
        start,
        end,
        0,
        false,
...
suite("IntervalTree 2", () => {
  ...
  function assertNodeAcceptEdit(msg, nodeStart, nodeEnd, nodeStickiness, start, end, textLength, forceMoveMarkers, expectedNodeStart, expectedNodeEnd) {
    ...
    assert.deepStrictEqual(
      [node.start, node.end],
      [expectedNodeStart, expectedNodeEnd],
      msg
    );
  }
  test("nodeAcceptEdit", () => {
    {
      assertNodeAcceptEdit(
        "A.000",
        0,
        0,
...
function _printTree(T, n, indent, delta, out) {
  out.push(
    `${indent}[${getNodeColor(n) === NodeColor.Red ? "R" : "B"},${n.delta}, ${n.start}->${n.end}, ${n.maxEnd}] : {${delta + n.start}->${delta + n.end}}, maxEnd: ${n.maxEnd + delta}
`
  );
  if (n.left !== SENTINEL) {
