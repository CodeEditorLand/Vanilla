...
function createTextModelWithBrackets(disposables, text, brackets) {
  ...
  disposables.add(
    languageConfigurationService.register(languageId, { brackets })
  );
      ...
      for (let lineNumber = contents.length; lineNumber >= 1; lineNumber--) {
        ...
        for (let column = lineText.length + 1; column >= 1; column--) {
          ...
          assert.deepStrictEqual(
            toRelaxedFoundBracket(actual),
            toRelaxedFoundBracket(currentExpectedBracket),
            "findPrevBracket of " + lineNumber + ", " + column
          );
        }
      }
    }
      ...
      for (let lineNumber = 1; lineNumber <= contents.length; lineNumber++) {
        ...
        for (let column = 1; column <= lineText.length + 1; column++) {
          ...
          assert.deepStrictEqual(
            toRelaxedFoundBracket(actual),
            toRelaxedFoundBracket(currentExpectedBracket),
            "findNextBracket of " + lineNumber + ", " + column
          );
        }
      }
    }
...
function assertIsNotBracket(model, lineNumber, column) {
  const match = model.bracketPairs.matchBracket(
    new Position(lineNumber, column)
  );
  assert.strictEqual(
    match,
    null,
    "is not matching brackets at " + lineNumber + ", " + column
  );
}
function assertIsBracket(model, testPosition, expected) {
  ...
  assert.deepStrictEqual(
    actual,
    expected,
    "matches brackets at " + testPosition
  );
}
suite("TextModelWithTokens - bracket matching", () => {
  ...
  test("bracket matching 1", () => {
    ...
    assertIsBracket(model, new Position(1, 4), [
      new Range(1, 4, 1, 5),
      new Range(2, 3, 2, 4)
    ]);
    assertIsBracket(model, new Position(1, 5), [
      new Range(1, 5, 1, 6),
      new Range(2, 2, 2, 3)
    ]);
    assertIsBracket(model, new Position(1, 6), [
      new Range(1, 6, 1, 7),
      new Range(2, 1, 2, 2)
    ]);
    assertIsBracket(model, new Position(1, 7), [
      new Range(1, 6, 1, 7),
      new Range(2, 1, 2, 2)
    ]);
    assertIsBracket(model, new Position(2, 1), [
      new Range(2, 1, 2, 2),
      new Range(1, 6, 1, 7)
    ]);
    assertIsBracket(model, new Position(2, 2), [
      new Range(2, 2, 2, 3),
      new Range(1, 5, 1, 6)
    ]);
    assertIsBracket(model, new Position(2, 3), [
      new Range(2, 3, 2, 4),
      new Range(1, 4, 1, 5)
    ]);
    assertIsBracket(model, new Position(2, 4), [
      new Range(2, 3, 2, 4),
      new Range(1, 4, 1, 5)
    ]);
...
suite("TextModelWithTokens 2", () => {
  ...
  test("bracket matching 3", () => {
    ...
    assertIsBracket(model, new Position(3, 9), [
      new Range(3, 9, 3, 11),
      new Range(4, 9, 4, 15)
    ]);
    assertIsBracket(model, new Position(4, 9), [
      new Range(4, 9, 4, 15),
      new Range(3, 9, 3, 11)
    ]);
    assertIsBracket(model, new Position(2, 5), [
      new Range(2, 5, 2, 9),
      new Range(5, 5, 5, 13)
    ]);
    assertIsBracket(model, new Position(5, 5), [
      new Range(5, 5, 5, 13),
      new Range(2, 5, 2, 9)
    ]);
    assertIsBracket(model, new Position(1, 1), [
      new Range(1, 1, 1, 6),
      new Range(6, 1, 6, 4)
    ]);
    assertIsBracket(model, new Position(6, 1), [
      new Range(6, 1, 6, 4),
      new Range(1, 1, 1, 6)
    ]);
    disposables.dispose();
  ...
  test("bracket matching 4", () => {
    ...
    assertIsBracket(model, new Position(4, 1), [
      new Range(4, 1, 4, 10),
      new Range(1, 1, 1, 12)
    ]);
    assertIsBracket(model, new Position(2, 3), [
    ...
    assertIsBracket(model, new Position(3, 3), [
      new Range(3, 3, 3, 12),
      new Range(2, 3, 2, 20)
    ]);
    disposables.dispose();
  ...
  test("issue #95843: Highlighting of closing braces is indicating wrong brace when cursor is behind opening brace", () => {
    ...
    disposables.add(
      TokenizationRegistry.register(mode1, tokenizationSupport)
    );
    disposables.add(
    ...
    disposables.add(
      languageConfigurationService.register(mode2, {
        brackets: [
          ["{", "}"],
          ["[", "]"],
          ["(", ")"]
    ...
    const model = disposables.add(
      instantiateTextModel(
        instantiationService,
        ["function f() {", "  return <p>{true}</p>;", "}"].join("\n"),
        mode1
      )
    );
  ...
  test("issue #88075: TypeScript brace matching is incorrect in `${}` strings", () => {
    ...
    disposables.add(
      TokenizationRegistry.register(mode, tokenizationSupport)
    );
    disposables.add(
      languageConfigurationService.register(mode, {
        brackets: [
          ["{", "}"],
          ["[", "]"],
          ["(", ")"]
    ...
    const model = disposables.add(
      instantiateTextModel(
        instantiationService,
        ["function hello() {", "    console.log(`${100}`);", "}"].join(
          "\n"
        ),
        mode
      )
    ...
    model.tokenization.forceTokenization(3);
    assert.deepStrictEqual(
      model.bracketPairs.matchBracket(new Position(2, 23)),
      null
    );
    assert.deepStrictEqual(
      model.bracketPairs.matchBracket(new Position(2, 20)),
      null
    );
    disposables.dispose();
  });
...
suite("TextModelWithTokens regression tests", () => {
  ...
  test("microsoft/monaco-editor#122: Unhandled Exception: TypeError: Unable to get property 'replace' of undefined or null reference", () => {
    ...
    const registration1 = TokenizationRegistry.register(
      LANG_ID1,
      tokenizationSupport
    );
    const registration2 = TokenizationRegistry.register(
      LANG_ID2,
      tokenizationSupport
    );
  ...
  test("issue #63822: Wrong embedded language detected for empty lines", () => {
    ...
    disposables.add(
      TokenizationRegistry.register(outerMode, tokenizationSupport)
    );
    const model = disposables.add(
      instantiateTextModel(
        instantiationService,
        "A model with one line",
        outerMode
      )
    );
    model.tokenization.forceTokenization(1);
...
suite("TextModel.getLineIndentGuide", () => {
  ...
  function assertIndentGuides(lines, indentSize) {
    ...
    for (let line = 1; line <= model.getLineCount(); line++) {
      const activeIndentGuide = model.guides.getActiveIndentGuide(
        line,
        1,
        model.getLineCount()
      );
