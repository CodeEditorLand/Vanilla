...
suite("Editor Model - Injected Text Events", () => {
  ...
  test("Basic", () => {
    ...
    thisModel.applyEdits([
      EditOperation.replace(new Range(2, 2, 2, 2), "Hello")
    ]);
    assert.deepStrictEqual(recordedChanges.splice(0), [
      {
        kind: "lineChanged",
    ...
    assert.deepStrictEqual(
      thisModel.getAllDecorations(void 0).map((d) => ({
...
function mapChange(change) {
  if (change.changeType === RawContentChangedType.LineChanged) {
    ...
    return {
      kind: "linesInserted",
      lines: change.detail.map(
        (e, idx) => getDetail(e, change.injectedTexts[idx])
      ),
