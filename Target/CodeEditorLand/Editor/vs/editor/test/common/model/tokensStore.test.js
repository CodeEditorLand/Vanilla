...
suite("TokensStore", () => {
  ...
  function parseTokensState(state) {
    ...
    for (let i = 0; i < state.length; i++) {
      ...
      while (true) {
        ...
        tokens.push(
          i + 1 - baseLine,
          tokenStartCharacter,
          tokenStartCharacter + tokenLength,
          metadata
        );
    ...
    return {
      text: text.join("\n"),
      tokens: SparseMultilineTokens.create(
        baseLine,
  ...
  test("deleting a newline", () => {
    testTokensAdjustment(
      ...
      [
        `import { |URI| } from '../../../../../vs/base/common/uri.js';const foo = |URI|.parse('hey');`
      ]
    );
  });
  ...
  test("deleting a newline 2", () => {
    testTokensAdjustment(
      [
        `import { `,
        `    |URI| } from '../../../../../vs/base/common/uri.js';const foo = |URI|.parse('hey');`
      ],
...
` },
  ...
  test('issue #147944: Language id "vs.editor.nullLanguage" is not configured nor known', () => {
    ...
    const model = disposables.add(
      instantiateTextModel(instantiationService, "--[[\n\n]]")
    );
    model.tokenization.setSemanticTokens(
      [
        SparseMultilineTokens.create(
  ...
  test("partial tokens 1", () => {
    ...
    const lineTokens = store.addSparseTokens(
      10,
      new LineTokens(new Uint32Array([12, 1]), `enum Enum1 {`, codec)
    );
  ...
  test("partial tokens 3", () => {
    ...
    store.setPartial(new Range(11, 1, 16, 2), [
      SparseMultilineTokens.create(
        10,
        new Uint32Array([0, 5, 10, 3, 5, 5, 10, 4])
      )
    ]);
    const lineTokens = store.addSparseTokens(
      5,
  ...
  test("bug", () => {
    function createTokens(str) {
      ...
      for (const strToken of strTokens) {
        ...
        const lineNumber = Number.parseInt(pieces[0], 10);
        const startChar = Number.parseInt(chars[0], 10);
        const endChar = Number.parseInt(chars[1], 10);
        ...
        result.push(
          ...
          startChar,
          endChar,
          (lineNumber + startChar) % 13
        );
      }
      return SparseMultilineTokens.create(
        firstLineNumber,
