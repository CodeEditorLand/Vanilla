...
} from "../../../common/model/textModelSearch.js";
...
suite("TextModelSearch", () => {
  ...
  function assertFindMatch(actual, expectedRange, expectedMatches = null) {
    assert.deepStrictEqual(
      actual,
      new FindMatch(expectedRange, expectedMatches)
    );
  }
  function _assertFindMatches(model, searchParams, expectedMatches) {
    const actual = TextModelSearch.findMatches(
      model,
      searchParams,
    ...
    let match = TextModelSearch.findNextMatch(
      model,
      searchParams,
      startPos,
      false
    ...
    for (const expectedMatch of expectedMatches) {
      ...
      match = TextModelSearch.findNextMatch(
        model,
        searchParams,
        startPos,
        false
    ...
    match = TextModelSearch.findPreviousMatch(
      model,
      searchParams,
      startPos,
      false
  ...
  function assertFindMatches(text, searchString, isRegex, matchCase, wordSeparators, _expected) {
    const expectedRanges = _expected.map(
      (entry) => new Range(entry[0], entry[1], entry[2], entry[3])
    );
    const expectedMatches = expectedRanges.map(
      (entry) => new FindMatch(entry, null)
    );
  ...
  test("issue #3623: Match whole word does not work for not latin characters", () => {
    assertFindMatches(
      ["\u044F", "\u043A\u043E\u043C\u043F\u0438\u043B\u044F\u0442\u043E\u0440", "\u043E\u0431\u0444\u0443\u0441\u043A\u0430\u0446\u0438\u044F", ":\u044F-\u044F"].join("\n"),
      "\u044F",
      false,
      false,
      USUAL_WORD_SEPARATORS,
  ...
  test("findNextMatch without regex", () => {
    ...
    let actual = TextModelSearch.findNextMatch(
      model,
      searchParams,
      new Position(1, 1),
      false
    ...
    actual = TextModelSearch.findNextMatch(
      model,
      searchParams,
      actual.range.getEndPosition(),
      false
    );
  ...
  test("findNextMatch with capturing matches", () => {
    const model = createTextModel("one line line\ntwo line\nthree");
    const searchParams = new SearchParams("(l(in)e)", true, false, null);
    const actual = TextModelSearch.findNextMatch(
      model,
      searchParams,
      new Position(1, 1),
      true
  ...
  test("findNextMatch multiline with capturing matches", () => {
    ...
    assertFindMatch(actual, new Range(1, 10, 2, 1), [
      "line\n",
      "line",
      "in"
    ]);
  ...
  test("findPreviousMatch with capturing matches", () => {
    const model = createTextModel("one line line\ntwo line\nthree");
    const searchParams = new SearchParams("(l(in)e)", true, false, null);
    const actual = TextModelSearch.findPreviousMatch(
      model,
      searchParams,
      new Position(1, 1),
      true
  ...
  test("findPreviousMatch multiline with capturing matches", () => {
    ...
    assertFindMatch(actual, new Range(2, 5, 3, 1), [
      "line\n",
      "line",
      "in"
    ]);
  ...
  test("\\n matches \\r\\n", () => {
    ...
    actual = TextModelSearch.findMatches(
      model,
      searchParams,
      model.getFullModelRange(),
      true,
  ...
  test("parseSearchRequest non regex", () => {
    assertParseSearchResult(
      ...
      false,
      null,
      new SearchData(/foo/giu, null, null)
    );
    assertParseSearchResult(
      "foo",
      false,
      false,
      USUAL_WORD_SEPARATORS,
      new SearchData(/foo/giu, usualWordSeparators, null)
    );
    assertParseSearchResult(
      "foo",
      false,
      true,
      null,
      new SearchData(/foo/gu, null, "foo")
    );
    assertParseSearchResult(
      "foo",
      false,
      true,
      USUAL_WORD_SEPARATORS,
      new SearchData(/foo/gu, usualWordSeparators, "foo")
    );
    assertParseSearchResult(
      "foo\\n",
      false,
    ...
    assertParseSearchResult(
      ...
      false,
      null,
      new SearchData(/foo\\r/giu, null, null)
    );
    assertParseSearchResult(
      "foo\\\\r",
      false,
  ...
  test("parseSearchRequest regex", () => {
    ...
    assertParseSearchResult(
      ...
      true,
      null,
      new SearchData(/foo/gu, null, null)
    );
    assertParseSearchResult(
      "foo",
      true,
      true,
      USUAL_WORD_SEPARATORS,
      new SearchData(/foo/gu, usualWordSeparators, null)
    );
    assertParseSearchResult(
      "foo\\n",
      true,
    ...
    assertParseSearchResult(
      ...
      false,
      null,
      new SearchData(/foo\r/gimu, null, null)
    );
    assertParseSearchResult(
      "foo\\\\r",
      true,
  ...
  test("issue #53415. W should match line break.", () => {
    assertFindMatches(
      ["text", "180702-", "180703-180704"].join("\n"),
      "\\d{6}-\\W",
      true,
      false,
      null,
  ...
  test("issue #100134. Zero-length matches should properly step over surrogate pairs", () => {
    assertFindMatches("1\u{1F4BB}1", "()", true, false, null, [
      [1, 1, 1, 1],
      [1, 2, 1, 2],
      [1, 4, 1, 4],
      [1, 5, 1, 5]
