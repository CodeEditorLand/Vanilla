...
suite("Editor Model - Model Decorations", () => {
  ...
  test("decorations are updated when inserting one line text before it 2", () => {
    ...
    thisModel.applyEdits([
      EditOperation.replace(new Range(1, 1, 1, 1), "Hallo ")
    ]);
    modelHasDecoration(thisModel, 1, 1, 3, 2, "myType");
  });
  ...
  test("decorations change when inserting text after them", () => {
    ...
    modelHasDecoration(thisModel, 1, 2, 3, 2, "myType");
    thisModel.applyEdits([
      EditOperation.insert(new Position(3, 2), "Hallo")
    ]);
  ...
  test("decorations are updated when inserting text inside 2", () => {
    ...
    modelHasDecoration(thisModel, 1, 2, 3, 2, "myType");
    thisModel.applyEdits([
      EditOperation.insert(new Position(3, 1), "Hallo ")
    ]);
  ...
  test("an apparently simple edit", () => {
    addDecoration(thisModel, 1, 2, 4, 1, "myType1");
    thisModel.applyEdits([
      EditOperation.replace(new Range(1, 14, 2, 1), "x")
    ]);
    modelHasDecorations(thisModel, [
...
suite("Decorations and editing", () => {
  ...
  function runTest(decRange, editRange, editText, expectedDecRange) {
    _runTest(
      ...
      editText,
      false,
      expectedDecRange[0][0],
      "no-0-AlwaysGrowsWhenTypingAtEdges"
    );
    _runTest(
      decRange,
      1,
      ...
      editText,
      false,
      expectedDecRange[0][1],
      "no-1-NeverGrowsWhenTypingAtEdges"
    );
    _runTest(
      decRange,
      2,
      ...
      editText,
      false,
      expectedDecRange[0][2],
      "no-2-GrowsOnlyWhenTypingBefore"
    );
    _runTest(
      decRange,
      3,
      ...
      editText,
      false,
      expectedDecRange[0][3],
      "no-3-GrowsOnlyWhenTypingAfter"
    );
    _runTest(
      decRange,
      0,
      ...
      editText,
      true,
      expectedDecRange[1][0],
      "force-0-AlwaysGrowsWhenTypingAtEdges"
    );
    _runTest(
      decRange,
      1,
      ...
      editText,
      true,
      expectedDecRange[1][1],
      "force-1-NeverGrowsWhenTypingAtEdges"
    );
    _runTest(
      decRange,
      2,
      ...
      editText,
      true,
      expectedDecRange[1][2],
      "force-2-GrowsOnlyWhenTypingBefore"
    );
    _runTest(
      decRange,
      3,
      ...
      editText,
      true,
      expectedDecRange[1][3],
      "force-3-GrowsOnlyWhenTypingAfter"
    );
  }
  suite("insert", () => {
    suite("collapsed dec", () => {
...
suite("deltaDecorations", () => {
  ...
  function testDeltaDecorations(text, decorations, newDecorations) {
    ...
    const initialIds = model.deltaDecorations(
      [],
      decorations.map(toModelDeltaDecoration)
    );
    const actualDecorations = readModelDecorations(model, initialIds);
    assert.strictEqual(
      initialIds.length,
      decorations.length,
      "returns expected cnt of ids"
    );
    assert.strictEqual(
      initialIds.length,
      model.getAllDecorations().length,
    ...
    const newIds = model.deltaDecorations(
      initialIds,
      newDecorations.map(toModelDeltaDecoration)
    );
  ...
  test("result respects input", () => {
    ...
    assert.deepStrictEqual(
      model.getDecorationRange(ids[0]),
      range(1, 1, 1, 12)
    );
    assert.deepStrictEqual(
      model.getDecorationRange(ids[1]),
      range(2, 1, 2, 13)
    );
    model.dispose();
  });
  ...
  test("issue #4317: editor.setDecorations doesn't update the hover message", () => {
    ...
    assert.deepStrictEqual(actualDecoration.hoverMessage, {
      value: "hello2"
    });
    model.dispose();
  });
    ...
    model.deltaDecorations(
      ...
      [
        ...
        },
        {
          ...
          options: { description: "test", className: "3" }
        },
        {
          range: new Range(2, 1, 2, 4),
          options: { description: "test", className: "4" }
        },
        {
          range: new Range(2, 8, 2, 13),
          options: { description: "test", className: "5" }
        },
        {
          range: new Range(3, 1, 4, 6),
          options: { description: "test", className: "6" }
        },
        {
          range: new Range(1, 1, 3, 6),
          options: { description: "test", className: "x1" }
        },
        {
          range: new Range(2, 5, 2, 8),
          options: { description: "test", className: "x2" }
        },
        {
          range: new Range(1, 1, 2, 8),
          options: { description: "test", className: "x3" }
        },
        {
          range: new Range(2, 5, 3, 1),
          options: { description: "test", className: "x4" }
        }
      ]
    );
