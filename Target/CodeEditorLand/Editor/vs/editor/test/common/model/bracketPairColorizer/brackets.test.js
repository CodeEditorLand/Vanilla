...
import {
  DenseKeyProvider,
  SmallImmutableSet
} from "../../../../common/model/bracketPairsTextModelPart/bracketPairsTree/smallImmutableSet.js";
import {
  TokenKind
...
suite("Bracket Pair Colorizer - Brackets", () => {
  ...
  test("Basic", () => {
    ...
    const languageConfigService = disposableStore.add(
      new TestLanguageConfigurationService()
    );
    disposableStore.add(
      languageConfigService.register(languageId, {
        brackets: [
          ["{", "}"],
          ["[", "]"],
          ["(", ")"],
    ...
    const brackets = new LanguageAgnosticBracketTokens(
      denseKeyProvider,
      (l) => languageConfigService.getLanguageConfiguration(l)
    );
    ...
    const bracketsActual = bracketsExpected.map(
      (x) => tokenToObject(brackets.getToken(x.text, languageId), x.text)
    );
