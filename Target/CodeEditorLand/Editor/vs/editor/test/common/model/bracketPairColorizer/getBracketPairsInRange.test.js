...
suite("Bracket Pair Colorizer - getBracketPairsInRange", () => {
  ...
  test("Basic 1", () => {
    disposeOnReturn((store) => {
      ...
      const model = createTextModelWithColorizedBracketPairs(
        store,
        doc.text
      );
  ...
  test("getBracketsInRange", () => {
    disposeOnReturn((store) => {
      ...
      assert.deepStrictEqual(
        model.bracketPairs.getBracketsInRange(doc.range(1, 2)).map((b) => ({
  ...
  test("colorizedBracketsVSBrackets", () => {
    disposeOnReturn((store) => {
      ...
      assert.deepStrictEqual(
        model.bracketPairs.getBracketsInRange(doc.range(1, 2), true).map((b) => ({
          level: b.nestingLevel,
          levelEqualBracketType: b.nestingLevelOfEqualBracketType,
          range: b.range.toString()
        })).toArray(),
      ...
      assert.deepStrictEqual(
        model.bracketPairs.getBracketsInRange(doc.range(1, 2), false).map((b) => ({
          level: b.nestingLevel,
          levelEqualBracketType: b.nestingLevelOfEqualBracketType,
          range: b.range.toString()
        })).toArray(),
