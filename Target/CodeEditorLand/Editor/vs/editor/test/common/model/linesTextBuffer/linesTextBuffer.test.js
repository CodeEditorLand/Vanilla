...
import {
  PieceTreeTextBuffer
} from "../../../../common/model/pieceTreeTextBuffer/pieceTreeTextBuffer.js";
...
suite("PieceTreeTextBuffer._getInverseEdits", () => {
  ...
  test("two unrelated multiline deletes", () => {
    assertInverseEdits(
      [editOp(1, 1, 2, 6, null), editOp(3, 1, 5, 5, null)],
      [inverseEditOp(1, 1, 1, 1), inverseEditOp(2, 1, 2, 1)]
    );
  });
  test("two multiline deletes 1", () => {
    assertInverseEdits(
      [editOp(1, 1, 2, 6, null), editOp(2, 7, 4, 5, null)],
      [inverseEditOp(1, 1, 1, 1), inverseEditOp(1, 2, 1, 2)]
    );
  });
  test("single replace", () => {
...
suite("PieceTreeTextBuffer._toSingleEditOperation", () => {
  ...
  function testToSingleEditOperation(original, edits, expected) {
    const { disposable, textBuffer } = createTextBufferFactory(
      original.join("\n")
    ).create(DefaultEndOfLine.LF);
    const actual = textBuffer._toSingleEditOperation(
      edits
    );
    assert.deepStrictEqual(actual, expected);
    disposable.dispose();
