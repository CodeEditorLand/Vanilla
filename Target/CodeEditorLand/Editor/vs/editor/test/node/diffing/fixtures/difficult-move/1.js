...
BUILD_TARGETS.forEach(({ platform, arch }) => {
	gulp.task(
		task.define(`node-${platform}-${arch}`, () => {
...
function nodejs(platform, arch) {
	...
	switch (platform) {
		case "win32":
			return (
				product.nodejsRepository !== "https://nodejs.org"
					? fetchGithub(product.nodejsRepository, {
							version: `${nodeVersion}-${internalNodeVersion}`,
							name: `win-${arch}-node.exe`,
							checksumSha256,
						})
					: fetchUrls(`/dist/v${nodeVersion}/win-${arch}/node.exe`, {
		...
		case "linux":
			return (
				product.nodejsRepository !== "https://nodejs.org"
					? fetchGithub(product.nodejsRepository, {
							version: `${nodeVersion}-${internalNodeVersion}`,
							name: `node-v${nodeVersion}-${platform}-${arch}.tar.gz`,
							checksumSha256,
						})
					: fetchUrls(
		...
		case "alpine": {
			...
			if (checksumSha256) {
				...
				if (actualSHA256Checksum !== checksumSha256) {
					throw new Error(
						`Checksum mismatch for node.js from docker image (expected ${options.checksumSha256}, actual ${actualSHA256Checksum}))`,
					);
				}
			}
			return es.readArray([
				new File({
					path: "node",
					contents,
					stat: { mode: Number.parseInt("755", 8) },
				}),
			]);
		}
	}
...
function packageTask(
	...
	return () => {
				...
				default: {
					...
					if (
						...
						Object.keys(manifest.contributes).some(
							(key) =>
								workspaceExtensionPoints.indexOf(key) !== -1,
						)
					) {
						return false;
					}
		...
		const localWorkspaceExtensions = glob
			.sync("extensions/*/package.json")
			.filter((extensionPath) => {
				...
				const manifest = JSON.parse(
					fs
						.readFileSync(path.join(REPO_ROOT, extensionPath))
						.toString(),
				);
		...
		const marketplaceExtensions = JSON.parse(
			...
			.builtInExtensions.filter(
				(entry) =>
					!entry.platforms || new Set(entry.platforms).has(platform),
			)
			.filter((entry) => !entry.clientOnly)
			.map((entry) => entry.name);
		const extensionPaths = [
			...localWorkspaceExtensions,
			...marketplaceExtensions,
		].map((name) => `.build/extensions/${name}/**`);

		const extensions = gulp.src(extensionPaths, {
			base: ".build",
			dot: true,
		});
		...
		const packageJsonStream = gulp
			.src(["remote/package.json"], { base: "remote" })
			.pipe(
				json({
					name,
					version,
		...
		const productJsonStream = gulp
			.src(["product.json"], { base: "." })
		...
		if (platform === "win32") {
			result = es.merge(
				result,
				gulp
					.src("resources/server/bin/remote-cli/code.cmd", {
						base: ".",
					})
				...
				gulp
					.src("resources/server/bin/helpers/browser.cmd", {
						base: ".",
					})
					.pipe(replace("@@VERSION@@", version))
					.pipe(replace("@@COMMIT@@", commit))
			...
			result = es.merge(
				...
				gulp
					.src(
						`resources/server/bin/remote-cli/${platform === "darwin" ? "code-darwin.sh" : "code-linux.sh"}`,
						{ base: "." },
					)
					.pipe(replace("@@VERSION@@", version))
					.pipe(replace("@@COMMIT@@", commit))
				...
				gulp
					.src(
						`resources/server/bin/helpers/${platform === "darwin" ? "browser-darwin.sh" : "browser-linux.sh"}`,
						{ base: "." },
					)
					.pipe(replace("@@VERSION@@", version))
					.pipe(replace("@@COMMIT@@", commit))
				...
				gulp
					.src(
						`resources/server/bin/${platform === "darwin" ? "code-server-darwin.sh" : "code-server-linux.sh"}`,
						{ base: "." },
					)
...
["reh", "reh-web"].forEach((type) => {
	const optimizeTask = task.define(
		`optimize-vscode-${type}`,
		task.series(
			...
			optimize.optimizeTask({
				...
				amd: {
					...
					fileContentMapper: createVSCodeWebFileContentMapper(
						".build/extensions",
						type === "reh-web"
							? tweakProductForServerWeb(product)
							: product,
					),
				},
				commonJS: {
	...
	BUILD_TARGETS.forEach((buildTarget) => {
		...
		["", "min"].forEach((minified) => {
			...
			const serverTaskCI = task.define(
				`vscode-${type}${dashed(platform)}${dashed(arch)}${dashed(minified)}-ci`,
				task.series(
			...
			const serverTask = task.define(
				`vscode-${type}${dashed(platform)}${dashed(arch)}${dashed(minified)}`,
				task.series(
					compileBuildTask,
					compileExtensionsBuildTask,
					compileExtensionMediaBuildTask,
