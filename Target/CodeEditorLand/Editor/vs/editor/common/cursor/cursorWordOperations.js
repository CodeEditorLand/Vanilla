...
class WordOperations {
  ...
  static _doFindPreviousWordOnLine(lineContent, wordSeparators, position) {
    let wordType = 0 /* None */;
    const previousIntlWord = wordSeparators.findPrevIntlWordBeforeOrAtOffset(
      lineContent,
      position.column - 2
    );
  ...
  static moveWordLeft(wordSeparators, model, position, wordNavigationType, hasMulticursor) {
    ...
    if (wordNavigationType === 1 /* WordStartFast */) {
      ...
      prevWordOnLine && prevWordOnLine.wordType === 2 /* Separator */ && prevWordOnLine.end - prevWordOnLine.start === 1 && prevWordOnLine.nextCharClass === WordCharacterClass.Regular) {
  ...
  static _moveWordPartLeft(model, position) {
    ...
    if (position.column === 1) {
      return lineNumber > 1 ? new Position(
        lineNumber - 1,
