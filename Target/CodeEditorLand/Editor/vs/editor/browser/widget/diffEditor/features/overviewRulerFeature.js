...
let OverviewRulerFeature = class extends Disposable {
  constructor(_editors, _rootElement, _diffModel, _rootWidth, _rootHeight, _modifiedEditorLayoutInfo, _themeService) {
    ...
    const currentColors = derived((reader) => {
      ...
      const removeColor = theme.getColor(diffOverviewRulerRemoved) || (theme.getColor(diffRemoved) || defaultRemoveColor).transparent(
        2
      );
    ...
    this._register(
      autorunWithStore((reader, store) => {
        ...
        const originalOverviewRuler = this._editors.original.createOverviewRuler(
        ...
        const modifiedOverviewRuler = this._editors.modified.createOverviewRuler(
