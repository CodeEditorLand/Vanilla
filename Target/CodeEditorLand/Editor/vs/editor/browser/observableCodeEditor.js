...
class ObservableCodeEditor extends Disposable {
  ...
  model = this._model;
  isReadonly = observableFromEvent(
    this,
    this.editor.onDidChangeConfiguration,
    () => this.editor.getOption(EditorOption.readOnly)
  );
  _versionId = observableValueOpts(
  ...
  layoutInfo = observableFromEvent(
    this.editor.onDidLayoutChange,
    () => this.editor.getLayoutInfo()
  );
  layoutInfoContentLeft = this.layoutInfo.map(
    (l) => l.contentLeft
  );
  layoutInfoDecorationsLeft = this.layoutInfo.map(
    (l) => l.decorationsLeft
  );
  contentWidth = observableFromEvent(
  ...
  getOption(id) {
    return observableFromEvent(
      this,
      (cb) => this.editor.onDidChangeConfiguration((e) => {
        if (e.hasChanged(id)) {
          cb(void 0);
        }
      }),
