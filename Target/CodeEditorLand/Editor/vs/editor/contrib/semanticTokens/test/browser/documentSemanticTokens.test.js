...
suite("ModelSemanticColoring", () => {
  ...
  setup(() => {
    ...
    const semanticTokensStylingService = disposables.add(
      new SemanticTokensStylingService(
        themeService,
        logService,
        languageService
      )
    ...
    disposables.add(
      new DocumentSemanticTokensFeature(
        semanticTokensStylingService,
        modelService,
        themeService,
        configService,
        new LanguageFeatureDebounceService(logService, envService),
  ...
  test("DocumentSemanticTokens should be fetched when the result is empty if there are pending changes", async () => {
    await runWithFakedTimers({}, async () => {
      disposables.add(
        languageService.registerLanguage({ id: "testMode" })
      );
  ...
  test("issue #149412: VS Code hangs when bad semantic token data is received", async () => {
    await runWithFakedTimers({}, async () => {
      ...
      textModel.applyEdits([
        { range: new Range(1, 1, 1, 1), text: "foo" }
      ]);
      await Event.toPromise(textModel.onDidChangeTokens);
  ...
  test("DocumentSemanticTokens should be pick the token provider with actual items", async () => {
    await runWithFakedTimers({}, async () => {
      let callCount = 0;
      disposables.add(
        languageService.registerLanguage({ id: "testMode2" })
      );
      disposables.add(
        languageFeaturesService.documentSemanticTokensProvider.register(
          "testMode2",
      ...
      const textModel = modelService.createModel(
        "Hello world 2",
        languageService.createById("testMode2")
      );
      try {
        let result = await getDocumentSemanticTokens(
          languageFeaturesService.documentSemanticTokensProvider,
          textModel,
          null,
          null,
        ...
        assert.ok(
          result.tokens,
          `Tokens are found from multiple providers (1)`
        );
        assert.ok(
          isSemanticTokens(result.tokens),
          `Tokens are full (1)`
        );
        assert.ok(
          result.tokens.resultId,
          `Token result id found from multiple providers (1)`
        );
        assert.deepStrictEqual(
          toArr(result.tokens.data),
          [0, 1, 1, 1, 1, 0, 2, 1, 1, 1],
          `Token data returned for multiple providers (1)`
        );
        assert.deepStrictEqual(
          callCount,
          2,
          `Called both token providers (1)`
        );
        assert.deepStrictEqual(
          result.provider.getLegend(),
          { tokenTypes: ["class1"], tokenModifiers: [] },
          `Legend matches the tokens (1)`
        );
        result = await getDocumentSemanticTokens(
          languageFeaturesService.documentSemanticTokensProvider,
          textModel,
          result.provider,
          result.tokens.resultId,
        ...
        assert.ok(
          !result.tokens.resultId,
          `Token result id found from multiple providers (2)`
        );
        assert.deepStrictEqual(
          toArr(result.tokens.data),
          [2, 1, 1, 1, 1, 0, 2, 1, 1, 1],
          `Token data returned for multiple providers (2)`
        );
        assert.deepStrictEqual(
          callCount,
          4,
          `Called both token providers (2)`
        );
        assert.deepStrictEqual(
          result.provider.getLegend(),
          { tokenTypes: ["class1"], tokenModifiers: [] },
          `Legend matches the tokens (2)`
        );
      } finally {
        disposables.clear();
        await timeout(0);
