...
suite("ParameterHintsModel", () => {
  ...
  function createMockEditor(fileContents) {
    const textModel = disposables.add(
      createTextModel(fileContents, void 0, void 0, mockFile)
    );
    const editor = disposables.add(
      createTestCodeEditor(textModel, {
  ...
  function getNextHint(model) {
    return new Promise(
      (resolve) => {
        const sub = disposables.add(
          model.onChangedHints((e) => {
            sub.dispose();
            return resolve(
              e ? { value: e, dispose: () => {
              } } : void 0
  ...
  test("Provider should not be retriggered if previous help is canceled first", async () => {
    ...
    const triggerChar = "(";
    const editor = createMockEditor("");
    const hintModel = disposables.add(
      new ParameterHintsModel(editor, registry)
    );
    let invokeCount = 0;
    disposables.add(
      registry.register(
  ...
  test("Should cancel existing request when new request comes in", async () => {
    ...
    const hintsModel = disposables.add(
      new ParameterHintsModel(editor, registry)
    );
    ...
    disposables.add(
      registry.register(mockFileSelector, longRunningProvider)
    );
    await runWithFakedTimers({ useFakeTimers: true }, async () => {
      hintsModel.trigger(
        { triggerKind: languages.SignatureHelpTriggerKind.Invoke },
  ...
  test("should use first result from multiple providers", async () => {
    ...
    const model = disposables.add(
      new ParameterHintsModel(editor, registry, 5)
    );
    disposables.add(
      registry.register(
        mockFileSelector,
    ...
    await runWithFakedTimers({ useFakeTimers: true }, async () => {
      ...
      assert.strictEqual(
        firstHint.signatures[0].parameters[0].label,
        paramterLabel
      );
      const secondHint = (await getNextHint(model)).value;
      assert.strictEqual(
        secondHint.signatures[0].label,
        secondProviderId
      );
      assert.strictEqual(secondHint.activeSignature, 1);
      assert.strictEqual(
        secondHint.signatures[0].parameters[0].label,
        paramterLabel
      );
    });
  });
  test("Quick typing should use the first trigger character", async () => {
    const editor = createMockEditor("");
    const model = disposables.add(
      new ParameterHintsModel(editor, registry, 50)
    );
