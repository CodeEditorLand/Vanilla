    ...
    switch (choice.id) {
      ...
      case "saveTextureAtlas":
        instantiationService.invokeFunction(async (accessor2) => {
          const workspaceContextService = accessor2.get(
              ...
              promises.push(
                ...[
                  fileService.writeFile(
                    ...
                    VSBuffer.wrap(
                      new Uint8Array(
                        await (await page.source.convertToBlob()).arrayBuffer()
                      )
                    )
                  ),
                  fileService.writeFile(
                    ...
                    VSBuffer.wrap(
                      new Uint8Array(
                        await (await page.getUsagePreview()).arrayBuffer()
                      )
                    )
                  )
                ]
      ...
      case "drawGlyph":
        instantiationService.invokeFunction(async (accessor2) => {
          const configurationService = accessor2.get(
          ...
          const atlas = ViewGpuContext.atlas;
          const fontFamily = configurationService.getValue(
            "editor.fontFamily"
          );
          const fontSize = configurationService.getValue(
            "editor.fontSize"
          );
