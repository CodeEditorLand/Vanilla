  ...
  test("issue #26393: Multiple cursors + Word wrap", () => {
    withTestCodeEditor(
      ...
      (editor, viewModel) => {
        ...
        assert.strictEqual(
          viewModel.getCursorStates()[0].viewState.position.lineNumber,
          1
        );
        assert.strictEqual(
          viewModel.getCursorStates()[1].viewState.position.lineNumber,
          3
        );
...
suite("Multicursor selection", () => {
  ...
  test("issue #8817: Cursor position changes when you cancel multicursor", () => {
    withTestCodeEditor(
      ...
      (editor) => {
        ...
        const multiCursorSelectController = editor.registerAndInstantiateContribution(
  ...
  test('issue #5400: "Select All Occurrences of Find Match" does not select all if find uses regex', () => {
    withTestCodeEditor(
      ...
      (editor) => {
        ...
        assert.strictEqual(
          findController.getState().searchString,
          "some+thing"
        );
        multiCursorSelectController.dispose();
        findController.dispose();
  ...
  test("AddSelectionToNextFindMatchAction starting with single collapsed selection", () => {
    ...
    testAddSelectionToNextFindMatchAction(
      text,
      (editor, action, findController) => {
