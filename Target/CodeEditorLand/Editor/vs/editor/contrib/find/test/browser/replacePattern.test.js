...
import {
  ...
  ReplacePiece,
  parseReplaceString
} from "../../browser/replacePattern.js";
  ...
  test("replace has JavaScript semantics", () => {
    const testJSReplaceSemantics = (target, search, replaceString, expected) => {
      ...
      assert.deepStrictEqual(
        actual,
        expected,
        `${target}.replace(${search}, ${replaceString})`
      );
    };
    ...
    testJSReplaceSemantics(
      ...
      /(hi)/,
      "hello$100",
      "hi".replace(/(hi)/, "hello$100")
    );
    testJSReplaceSemantics(
      "hi",
      /(hi)/,
      "hello$20",
      "hi".replace(/(hi)/, "hello$20")
    );
  });
