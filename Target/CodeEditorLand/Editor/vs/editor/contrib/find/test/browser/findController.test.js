    ...
    await withAsyncTestCodeEditor(
      ...
      async (editor, _, instantiationService) => {
        ...
        assert.deepStrictEqual(
          fromSelection(editor.getSelection()),
          [1, 1, 1, 4]
        );
        ...
        editor.executeEdits("test", [
          EditOperation.delete(new Range(1, 1, 1, 4))
        ]);
        editor.executeEdits("test", [
          EditOperation.insert(new Position(1, 1), "XYZ")
        ]);
        editor.pushUndoStop();
  ...
  test("issue #18111: Regex replace with single space replaces with no space", async () => {
    await withAsyncTestCodeEditor(
      ...
      async (editor, _, instantiationService) => {
        ...
        assert.deepStrictEqual(
          editor.getSelections().map(fromSelection),
          [[1, 39, 1, 42]]
        );
  ...
  test("issue #58604: Do not update searchScope if it is empty", async () => {
    await withAsyncTestCodeEditor(
      ...
      async (editor) => {
        ...
        assert.deepStrictEqual(
          findController.getState().searchScope,
          null
        );
      }
    );
