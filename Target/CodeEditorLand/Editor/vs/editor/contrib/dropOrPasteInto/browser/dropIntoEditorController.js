...
const dropWidgetVisibleCtx = new RawContextKey(
  "dropWidgetVisible",
  false,
...
let DropIntoEditorController = class extends Disposable {
  constructor(editor, instantiationService, _configService, _languageFeaturesService, _treeViewsDragAndDropService) {
    ...
    this._register(
      editor.onDropIntoEditor(
        (e) => this.onDropIntoEditor(editor, e.position, e.event)
      )
    );
  }
  static {
  ...
  async onDropIntoEditor(editor, position, dragEvent) {
    ...
    const p = createCancelablePromise(async (token) => {
      ...
      try {
        ...
        const providers = this._languageFeaturesService.documentDropEditProvider.ordered(model).filter((provider) => {
          ...
          return provider.dropMimeTypes.some(
            (mime) => ourDataTransfer.matches(mime)
          );
        });
    ...
    this._dropProgressManager.showWhile(
      ...
      p,
      { cancel: /* @__PURE__ */ __name(() => p.cancel(), "cancel") }
    );
    ...
    const results = await raceCancellation(
      Promise.all(
        providers.map(async (provider) => {
          try {
            const edits2 = await provider.provideDocumentDropEdits(
              model,
              position,
              dataTransfer,
              tokenSource.token
    ...
    if (this.treeItemsTransfer.hasData(DraggedTreeItemsIdentifier.prototype)) {
      ...
      if (Array.isArray(data)) {
        for (const id of data) {
          const treeDataTransfer = await this._treeViewsDragAndDropService.removeDragOperationTransfer(
            id.identifier
          );
          if (treeDataTransfer) {
