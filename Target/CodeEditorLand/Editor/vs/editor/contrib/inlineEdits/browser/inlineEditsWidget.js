...
let InlineEditsWidget = class extends Disposable {
  constructor(_editor, _edit, _userPrompt, _instantiationService) {
    ...
    const visible = derived(
      this,
      (reader) => this._edit.read(reader) !== void 0 || this._userPrompt.read(reader) !== void 0
    );
    this._register(
      applyStyle(this._elements.root, {
        display: derived(
          this,
          (reader) => visible.read(reader) ? "block" : "none"
        )
      })
    );
    this._register(
  ...
  _promptTextModel = this._register(
    this._instantiationService.createInstance(
      TextModel,
      "",
      PLAINTEXT_LANGUAGE_ID,
