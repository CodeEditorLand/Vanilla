...
let InlineEditsModel = class extends Disposable {
  ...
  static _createUniqueUri() {
    return URI.from({ scheme: "inline-edits", path: (/* @__PURE__ */ new Date()).toString() + String(InlineEditsModel._modelId++) });
  }
  ...
  _originalModel = derivedDisposable(() => this._modelService.createModel("", null, InlineEditsModel._createUniqueUri())).keepObserved(this._store);
  _modifiedModel = derivedDisposable(() => this._modelService.createModel("", null, InlineEditsModel._createUniqueUri())).keepObserved(this._store);
  _pinnedRange = new TrackedRange(this.textModel, this._textModelVersionId);
  isPinned = this._pinnedRange.range.map((range) => !!range);
  userPrompt = observableValue(this, void 0);
  inlineEdit = derived(this, (reader) => {
  ...
  _inlineEdits = derivedOpts({ owner: this, equalsFn: structuralEquals }, (reader) => {
  ...
  _filteredInlineEditItems = derivedOpts({ owner: this, equalsFn: itemsEquals() }, (reader) => {
  ...
  selectedInlineEdit = derived(this, (reader) => {
    const filteredCompletions = this._filteredInlineEditItems.read(reader);
