...
let InlineEditsController = class extends Disposable {
  ...
  static get(editor) {
    return editor.getContribution(InlineEditsController.ID);
  }
  _enabled = observableConfigValue("editor.inlineEdits.enabled", false, this._configurationService);
  _editorObs = observableCodeEditor(this.editor);
  _selection = derived(this, (reader) => this._editorObs.cursorSelection.read(reader) ?? new Selection(1, 1, 1, 1));
  ...
  _hadInlineEdit = derivedObservableWithCache(this, (reader, lastValue) => lastValue || this.model.read(reader)?.inlineEdit.read(reader) !== void 0);
  _widget = derivedDisposable(this, (reader) => {
    ...
    return this._instantiationService.createInstance(
      ...
      flattenSettableObservable((reader2) => this.model.read(reader2)?.userPrompt ?? observableValue("empty", ""))
    );
  });
};
