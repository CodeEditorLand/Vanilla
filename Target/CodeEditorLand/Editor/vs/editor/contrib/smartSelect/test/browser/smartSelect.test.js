...
suite("SmartSelect", () => {
  ...
  setup(() => {
    ...
    const languagConfigurationService = instantiationService.get(
      ILanguageConfigurationService
    );
    const languageService = instantiationService.get(ILanguageService);
    disposables.add(languageService.registerLanguage({ id: languageId }));
    disposables.add(
      languagConfigurationService.register(languageId, {
        brackets: [
          ["(", ")"],
          ["{", "}"],
          ["[", "]"]
  ...
  async function assertGetRangesToPosition(text, lineNumber, column, ranges, selectLeadingAndTrailingWhitespace = true) {
    ...
    const model = modelService.createModel(
      text.join("\n"),
      new StaticLanguageSelector(languageId),
      uri
    );
    const [actual] = await provideSelectionRanges(
      providers,
      model,
      [new Position(lineNumber, column)],
      { selectLeadingAndTrailingWhitespace, selectSubwords: true },
      CancellationToken.None
    );
    const actualStr = actual.map(
      (r) => new Range(
        r.startLineNumber,
        r.startColumn,
        r.endLineNumber,
...
E: ${desiredStr}`
  ...
  async function assertRanges(provider, value, ...expected) {
    ...
    const all = await provider.provideSelectionRanges(
      model,
      [pos],
      CancellationToken.None
    );
    ...
    for (const range of ranges) {
      ...
      assert.ok(
        Range.equalsRange(range.range, exp),
        `A=${range.range} <> E=${exp}`
      );
    }
  }
  ...
  test("Default selection should select current word/hump first in camelCase #67493", async () => {
    await assertRanges(
      new WordSelectionRangeProvider(),
  ...
  test("Smart select: only add line ranges if they're contained by the next range #73850", async () => {
  ...
  test("Expand selection in words with underscores is inconsistent #90589", async () => {
    await assertRanges(
      new WordSelectionRangeProvider(),
      "Hel|lo_World",
      new Range(1, 1, 1, 6),
