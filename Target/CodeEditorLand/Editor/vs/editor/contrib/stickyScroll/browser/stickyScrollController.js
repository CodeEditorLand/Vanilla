...
let StickyScrollController = class extends Disposable {
  ...
  _registerMouseListeners() {
        ...
        (mouseEvent) => {
          ...
          if (mouseEvent.shiftKey) {
            ...
            const position2 = new Position(
              this._endLineNumbers[lineIndex],
              1
            );
    ...
    this._register(
      gesture.onMouseMoveOrRelevantKeyDown(
        ([mouseEvent, _keyboardEvent]) => {
          ...
          sessionStore.add(
            toDisposable(() => cancellationToken.dispose(true))
          );
  ...
  _toggleFoldingRegionForLine(line) {
    ...
    const scrollTop = (foldingIcon.isCollapsed ? this._editor.getTopForLineNumber(foldingIcon.foldingEndLine) : this._editor.getTopForLineNumber(
      foldingIcon.foldingStartLine
  ...
  _readConfiguration() {
    ...
    if (options.enabled === false) {
      ...
      this._sessionStore.add(
        this._editor.onDidChangeModelTokens(
          (e) => this._onTokensChange(e)
        )
      );
      this._sessionStore.add(
      ...
      if (this._focused) {
        if (this._focusedStickyElementIndex === -1) {
          ...
          if (this._stickyScrollWidget.lineNumberCount === 0) {
            ...
            const previousFocusedLineNumberExists = this._stickyScrollWidget.lineNumbers.includes(
  ...
  _updateAndGetMinRebuildFromLine(rebuildFromLine) {
    if (rebuildFromLine !== void 0) {
      const minRebuildFromLineOrInfinity = this._minRebuildFromLine !== void 0 ? this._minRebuildFromLine : Number.POSITIVE_INFINITY;
      this._minRebuildFromLine = Math.min(
        rebuildFromLine,
        minRebuildFromLineOrInfinity
      );
    }
  ...
  findScrollWidgetState() {
    ...
    if (arrayVisibleRanges.length !== 0) {
      ...
      const candidateRanges = this._stickyLineCandidateProvider.getCandidateStickyLinesIntersecting(
        fullVisibleRange
      );
