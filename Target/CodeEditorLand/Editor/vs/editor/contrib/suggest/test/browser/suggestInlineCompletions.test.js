...
suite("Suggest Inline Completions", () => {
  ...
  setup(() => {
    ...
    model = createTextModel(
      ...
      void 0,
      void 0,
      URI.from({ scheme: "foo", path: "foo.bar" })
    );
    ...
    insta.invokeFunction((accessor) => {
      disposables.add(
        accessor.get(ILanguageFeaturesService).completionProvider.register(
          { pattern: "*.bar", scheme: "foo" },
          new class {
  ...
  test("Aggressive inline completions when typing within line #146948", async () => {
    const completions = disposables.add(
      insta.createInstance(SuggestInlineCompletions)
    );
    {
      const result = await completions.provideInlineCompletions(
        model,
        new Position(1, 3),
        {
  ...
  test("Snippets show in inline suggestions even though they are turned off #175190", async () => {
    const completions = disposables.add(
      insta.createInstance(SuggestInlineCompletions)
    );
    {
