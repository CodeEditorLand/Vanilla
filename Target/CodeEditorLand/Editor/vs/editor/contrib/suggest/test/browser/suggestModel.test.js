...
} from "../../../../test/common/testTextModel.js";
...
function createMockEditor(model, languageFeaturesService) {
  ...
  const ctrl = editor.registerAndInstantiateContribution(
...
suite("SuggestModel - Context", () => {
  ...
  const assertAutoTrigger = (model, offset, expected, message) => {
    ...
    assert.strictEqual(
      LineContext.shouldAutoTrigger(editor),
      expected,
      message
    );
    editor.dispose();
  ...
  test("Context - shouldAutoTrigger", () => {
    ...
    assertAutoTrigger(
      ...
      1,
      true,
      "typing a single character before a word: D|as"
    );
  ...
  test("shouldAutoTrigger at embedded language boundaries", () => {
    ...
    assertAutoTrigger(
      ...
      1,
      true,
      "a|<x \u2014 should trigger at end of word"
    );
    assertAutoTrigger(
      model,
      2,
      false,
      "a<|x \u2014 should NOT trigger at start of word"
    );
    assertAutoTrigger(
      model,
      3,
      true,
      "a<x|x \u2014  should trigger after typing a single character before a word"
    );
    assertAutoTrigger(
      model,
      4,
      true,
      "a<xx|> \u2014 should trigger at boundary between languages"
    );
    assertAutoTrigger(
      model,
      5,
      false,
      "a<xx>|a \u2014 should NOT trigger at start of word"
    );
    assertAutoTrigger(
      model,
      6,
      true,
      "a<xx>a|< \u2014 should trigger at end of word"
    );
    assertAutoTrigger(
      model,
      8,
      true,
      "a<xx>a<x|> \u2014 should trigger at end of word at boundary"
    );
    disposables2.dispose();
  });
});
suite("SuggestModel - TriggerAndCancelOracle", () => {
  ...
  function withOracle(callback) {
    return new Promise((resolve, reject) => {
      ...
      const oracle = editor.invokeWithinContext(
        (accessor) => accessor.get(IInstantiationService).createInstance(SuggestModel, editor)
      );
  ...
  test("events - cancel/trigger", () => withOracle((model2) => {
    return Promise.all([
      assertEvent(
        model2.onDidTrigger,
        () => {
  ...
  test("events - suggest/empty", () => {
    disposables.add(
      registry.register({ scheme: "test" }, alwaysEmptySupport)
    );
      ...
      return Promise.all([
        ...
        assertEvent(
          ...
          (event) => {
            ...
            assert.strictEqual(
              event.completionModel.items.length,
              0
            );
          }
        )
  ...
  test("trigger - on type", () => {
    disposables.add(
      registry.register({ scheme: "test" }, alwaysSomethingSupport)
    );
  ...
  test("#17400: Keep filtering suggestModel.ts after space", () => {
    disposables.add(
      registry.register(
        { scheme: "test" },
        {
        ...
        (event) => {
          return assertEvent(
            ...
            () => {
              ...
              editor.trigger("keyboard", Handler.Type, {
                text: "My"
              });
            },
            (event2) => {
              ...
              assert.strictEqual(
                event2.completionModel.items.length,
                1
              );
              const [first] = event2.completionModel.items;
              assert.strictEqual(
                first.completion.label,
                "My Table"
              );
              return assertEvent(
                model2.onDidSuggest,
                ...
                (event3) => {
                  assert.strictEqual(
                    event3.triggerOptions.auto,
                    true
                  );
                  assert.strictEqual(
                    event3.completionModel.items.length,
                    1
                  );
                  const [first2] = event3.completionModel.items;
                  assert.strictEqual(
                    first2.completion.label,
                    "My Table"
                  );
                }
              );
  ...
  test("#21484: Trigger character always force a new completion session", () => {
    disposables.add(
      registry.register(
        { scheme: "test" },
        {
              ...
              suggestions: [
                {
                  ...
                  range: Range.fromPositions(
                    pos.delta(
                      0,
                      doc.getLineContent(pos.lineNumber)[pos.column - 2] === "." ? 0 : -1
                    ),
                    pos
                  )
                }
  ...
  test("Intellisense Completion doesn't respect space after equal sign (.html file), #29353 [1/2]", () => {
    disposables.add(
      registry.register({ scheme: "test" }, alwaysSomethingSupport)
    );
    return withOracle((model2, editor) => {
  ...
  test("Intellisense Completion doesn't respect space after equal sign (.html file), #29353 [2/2]", () => {
    disposables.add(
      registry.register({ scheme: "test" }, alwaysSomethingSupport)
    );
    return withOracle((model2, editor) => {
  ...
  test("Incomplete suggestion results cause re-triggering when typing w/o further context, #28400 (1/2)", () => {
    disposables.add(
      registry.register(
        { scheme: "test" },
        {
    ...
    return withOracle((model2, editor) => {
      ...
      return assertEvent(
        ...
        (event) => {
          assert.strictEqual(event.triggerOptions.auto, false);
          assert.strictEqual(
            event.completionModel.getIncompleteProvider().size,
            1
          );
          assert.strictEqual(event.completionModel.items.length, 1);
          return assertEvent(
  ...
  test("Incomplete suggestion results cause re-triggering when typing w/o further context, #28400 (2/2)", () => {
    disposables.add(
      registry.register(
        { scheme: "test" },
        {
      ...
      return assertEvent(
        ...
        (event) => {
          ...
          return assertEvent(
            ...
            (event2) => {
              assert.strictEqual(
                event2.triggerOptions.auto,
                false
              );
              assert.strictEqual(
                event2.completionModel.getIncompleteProvider().size,
                1
              );
              assert.strictEqual(
                event2.completionModel.items.length,
  ...
  test("Trigger character is provided in suggest context", () => {
  ...
  test("Mac press and hold accent character insertion does not update suggestions, #35269", () => {
    disposables.add(
      registry.register(
        { scheme: "test" },
        {
    ...
    return withOracle((model2, editor) => {
      return assertEvent(
        ...
        (event) => {
          assert.strictEqual(event.completionModel.items.length, 1);
          assert.strictEqual(
            event.completionModel.items[0].completion.label,
            "abc"
          );
          return assertEvent(
            model2.onDidSuggest,
            ...
            (event2) => {
              ...
              assert.strictEqual(
                event2.completionModel.items[0].completion.label,
                "\xE4bc"
              );
            }
          );
  ...
  test("Backspace should not always cancel code completion, #36491", () => {
    disposables.add(
      registry.register({ scheme: "test" }, alwaysSomethingSupport)
    );
    return withOracle(async (model2, editor) => {
  ...
  test("Text changes for completion CodeAction are affected by the completion #39893", () => {
    disposables.add(
      registry.register(
        { scheme: "test" },
        {
  ...
  test("Completion unexpectedly triggers on second keypress of an edit group in a snippet #43523", () => {
    disposables.add(
      registry.register({ scheme: "test" }, alwaysSomethingSupport)
    );
    return withOracle((model2, editor) => {
  ...
  test("Fails to render completion details #47988", () => {
    let disposeA = 0;
    let disposeB = 0;
    disposables.add(
      registry.register(
  ...
  test("Trigger (full) completions when (incomplete) completions are already active #99504", () => {
    let countA = 0;
    let countB = 0;
    disposables.add(
      registry.register(
    ...
    return withOracle(async (model2, editor) => {
      await assertEvent(
        ...
        (event) => {
          ...
          assert.strictEqual(event.completionModel.items.length, 1);
          assert.strictEqual(
            event.completionModel.items[0].textLabel,
            "Z aaa"
          );
        }
      );
      await assertEvent(
        ...
        (event) => {
          ...
          assert.strictEqual(
            event.completionModel.items[1].textLabel,
            "aaa"
          );
  ...
  test("registerCompletionItemProvider with letters as trigger characters block other completion items to show up #127815", async () => {
    disposables.add(
      registry.register(
        { scheme: "test" },
        {
  ...
  test("Unexpected suggest scoring #167242", async () => {
    disposables.add(
      registry.register("*", {
        // word-based
        _debugDisplayName: "test",
  ...
  test("Completion list closes unexpectedly when typing a digit after a word separator #169390", () => {
  ...
  test("Set refilter-flag, keep triggerKind", () => {
    disposables.add(
      registry.register(
        { scheme: "test" },
        {
    ...
    return withOracle(async (model2, editor) => {
      await assertEvent(
        ...
        (event) => {
          assert.strictEqual(event.triggerOptions.auto, true);
          assert.strictEqual(
            event.triggerOptions.triggerCharacter,
            void 0
          );
          assert.strictEqual(
            event.triggerOptions.triggerKind,
            void 0
          );
          assert.strictEqual(event.completionModel.items.length, 1);
        }
  ...
  test("Snippets gone from IntelliSense #173244", () => {
