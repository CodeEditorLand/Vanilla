...
const CutAction = supportsCut ? registerCommand(
  new MultiCommand({
...
const CopyAction = supportsCopy ? registerCommand(
  new MultiCommand({
...
const PasteAction = supportsPaste ? registerCommand(
  new MultiCommand({
  ...
  PasteAction.addImplementation(
    ...
    (accessor, args) => {
      ...
      if (focusedEditor && focusedEditor.hasTextFocus()) {
        ...
        if (result) {
          return CopyPasteController.get(
            focusedEditor
          ...
          return (async () => {
            ...
            if (clipboardText !== "") {
              const metadata = InMemoryClipboardMetadataManager.INSTANCE.get(
                clipboardText
              );
