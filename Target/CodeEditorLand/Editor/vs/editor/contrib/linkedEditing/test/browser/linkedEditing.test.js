...
suite("linked editing", () => {
  ...
  setup(() => {
    ...
    languageFeaturesService = instantiationService.get(
  ...
  function testCase(name, initialState, operations, expectedEndText) {
    test(name, async () => {
      await runWithFakedTimers({}, async () => {
        disposables.add(
          languageFeaturesService.linkedEditingRangeProvider.register(
            mockFileSelector,
            {
        ...
        const linkedEditingContribution = disposables.add(
        ...
        const testEditor = {
          ...
          trigger(source, handlerId, payload) {
            if (handlerId === Handler.Type || handlerId === Handler.Paste) {
              ...
              instantiationService.invokeFunction(
                (accessor) => new DeleteWordLeft().runEditorCommand(
                  accessor,
                  editor,
                  payload
                )
              ...
              instantiationService.invokeFunction(
                (accessor) => new DeleteAllLeftAction().runEditorCommand(
                  accessor,
                  editor,
                  payload
                )
          ...
          undo() {
            CoreEditingCommands.Undo.runEditorCommand(
              null,
              editor,
              null
            );
          ...
          redo() {
            CoreEditingCommands.Redo.runEditorCommand(
              null,
              editor,
              null
            );
  ...
  testCase(
    "Breakout with stop pattern - insert stop char, respos",
    state4,
    async (editor) => {
      const pos = new Position(1, 2);
      await editor.setPosition(pos);
