...
suite("SnippetParser", () => {
  ...
  test("Parser, TM text", () => {
    ...
    const [, placeholder] = new SnippetParser().parse(
      "foo${1:bar\\}${2:foo}}"
    ).children;
  ...
  test("Marker, toTextmateString() <-> identity", () => {
    function assertIdent(input) {
      ...
      function checkCheckChildren(marker1, marker2) {
        assert.ok(
          marker1 instanceof Object.getPrototypeOf(marker2).constructor
        );
        assert.ok(
          marker2 instanceof Object.getPrototypeOf(marker1).constructor
        );
        assert.strictEqual(
          marker1.children.length,
          marker2.children.length
        );
  ...
  test("Parser, choise marker", () => {
    const { placeholders } = new SnippetParser().parse(
      "${1|one,two,three|}"
    );
    ...
    assert.strictEqual(
      placeholders[0].children[0].options.length,
      3
    );
  ...
  test("Parser, transform example", () => {
    const { children } = new SnippetParser().parse(
  ...
  test("parser, parent node", () => {
    let snippet = new SnippetParser().parse(
  ...
  test("TextmateSnippet#offset", () => {
    ...
    assert.strictEqual(snippet.offset(snippet.children[0]), 0);
    assert.strictEqual(
      snippet.offset(snippet.children[0].children[0]),
      0
    );
  ...
  test("TextmateSnippet#replace 1/2", () => {
    ...
    const newEnclosing = snippet.enclosingPlaceholders(
      snippet.placeholders[1]
    );
  ...
  test("Snippets: make parser ignore `${0|choice|}`, #31599", () => {
  ...
  test("Transform -> FormatString#resolve", () => {
    ...
    assert.strictEqual(
      new FormatString(1, "downcase").resolve("FOO"),
      "foo"
    );
    assert.strictEqual(
      new FormatString(1, "capitalize").resolve("bar"),
      "Bar"
    );
    assert.strictEqual(
      new FormatString(1, "capitalize").resolve("bar no repeat"),
      "Bar no repeat"
    );
    assert.strictEqual(
      new FormatString(1, "pascalcase").resolve("bar-foo"),
      "BarFoo"
    );
    assert.strictEqual(
      new FormatString(1, "pascalcase").resolve("bar-42-foo"),
      "Bar42Foo"
    );
    assert.strictEqual(
      new FormatString(1, "pascalcase").resolve("snake_AndPascalCase"),
      "SnakeAndPascalCase"
    );
    assert.strictEqual(
      new FormatString(1, "pascalcase").resolve("kebab-AndPascalCase"),
      "KebabAndPascalCase"
    );
    assert.strictEqual(
      new FormatString(1, "pascalcase").resolve("_justPascalCase"),
      "JustPascalCase"
    );
    assert.strictEqual(
      new FormatString(1, "camelcase").resolve("bar-foo"),
      "barFoo"
    );
    assert.strictEqual(
      new FormatString(1, "camelcase").resolve("bar-42-foo"),
      "bar42Foo"
    );
    assert.strictEqual(
      new FormatString(1, "camelcase").resolve("snake_AndCamelCase"),
      "snakeAndCamelCase"
    );
    assert.strictEqual(
      new FormatString(1, "camelcase").resolve("kebab-AndCamelCase"),
      "kebabAndCamelCase"
    );
    assert.strictEqual(
      new FormatString(1, "camelcase").resolve("_JustCamelCase"),
      "justCamelCase"
    );
    assert.strictEqual(
      new FormatString(1, "notKnown").resolve("input"),
      "input"
    );
    assert.strictEqual(
      new FormatString(1, void 0, "foo", void 0).resolve(void 0),
      ""
    );
    assert.strictEqual(
      new FormatString(1, void 0, "foo", void 0).resolve(""),
      ""
    );
    assert.strictEqual(
      new FormatString(1, void 0, "foo", void 0).resolve("bar"),
      "foo"
    );
    assert.strictEqual(
      new FormatString(1, void 0, void 0, "foo").resolve(void 0),
      "foo"
    );
    assert.strictEqual(
      new FormatString(1, void 0, void 0, "foo").resolve(""),
      "foo"
    );
    assert.strictEqual(
      new FormatString(1, void 0, void 0, "foo").resolve("bar"),
      "bar"
    );
    assert.strictEqual(
      new FormatString(1, void 0, "bar", "foo").resolve(void 0),
      "foo"
    );
    assert.strictEqual(
      new FormatString(1, void 0, "bar", "foo").resolve(""),
      "foo"
    );
    assert.strictEqual(
      new FormatString(1, void 0, "bar", "foo").resolve("baz"),
      "bar"
    );
  });
  test("Snippet variable transformation doesn't work if regex is complicated and snippet body contains '$$' #55627", () => {
    const snippet = new SnippetParser().parse(
      'const fileName = "${TM_FILENAME/(.*)\\..+$/$1/}"'
    );
    assert.strictEqual(
  ...
  test("[BUG] HTML attribute suggestions: Snippet session does not have end-position set, #33147", () => {
  ...
  test("Snippet optional transforms are not applied correctly when reusing the same variable, #37702", () => {
  ...
  test("Variable transformation doesn't work if undefined variables are used in the same snippet #51769", () => {
    const transform = new Transform();
    transform.appendChild(new Text("bar"));
    transform.regexp = /foo/gi;
  ...
  test("Mirroring sequence of nested placeholders not selected properly on backjumping #58736", () => {
    const snippet = new SnippetParser().parse(
  ...
  test("Backspace can't be escaped in snippet variable transforms #65412", () => {
    const snippet = new SnippetParser().parse(
      "namespace ${TM_DIRECTORY/[\\/]/\\\\/g};"
    );
    assertMarker(snippet, Text, Variable, Text);
  });
  test("Snippet cannot escape closing bracket inside conditional insertion variable replacement #78883", () => {
    const snippet = new SnippetParser().parse(
    ...
    assert.strictEqual(
      variable.transform.children[0].ifValue,
      "import { hello } from world"
    );
    assert.strictEqual(
      variable.transform.children[0].elseValue,
      void 0
    );
  });
  test("Snippet escape backslashes inside conditional insertion variable replacement #80394", () => {
    const snippet = new SnippetParser().parse(
      "${CURRENT_YEAR/(.+)/${1:+\\\\}/}"
    );
    const variable = snippet.children[0];
  ...
  test("Snippet placeholder empty right after expansion #152553", () => {
    ...
    const snippet2 = new SnippetParser().parse(
      "${1:prog}: ${3:${2:$1.cc}.33} - $2 $3"
    );
    const actual2 = snippet2.toString();
    assert.strictEqual(actual2, "prog: prog.cc.33 - prog.cc prog.cc.33");
    const snippet3 = new SnippetParser().parse(
      "${1:$2.one} <> ${2:$1.two}"
    );
