...
let CodeLensContribution = class {
  ...
  _getLayoutInfo() {
    const lineHeightFactor = Math.max(
      1.3,
      this._editor.getOption(EditorOption.lineHeight) / this._editor.getOption(EditorOption.fontSize)
    );
  ...
  _onModelChange() {
    ...
    const scheduler = new RunOnceScheduler(() => {
      const t1 = Date.now();
      this._getCodeLensModelPromise?.cancel();
      this._getCodeLensModelPromise = createCancelablePromise(
        (token) => getCodeLensModel(
  ...
  async getModel() {
    ...
    return this._currentCodeLensModel?.isDisposed ? void 0 : this._currentCodeLensModel;
  }
};
...
registerEditorAction(
  class ShowLensesInCurrentLine extends EditorAction {
    ...
    async run(accessor, editor) {
      ...
      const codelensController = editor.getContribution(
        CodeLensContribution.ID
      );
      ...
      if (model.isDisposed) {
        ...
        const newLens = newModel?.lenses.find(
          (lens) => lens.symbol.range.startLineNumber === lineNumber && lens.symbol.command?.title === command.title
        );
