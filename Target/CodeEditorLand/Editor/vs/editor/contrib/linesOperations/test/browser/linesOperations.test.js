...
import {
  ...
  TransposeAction,
  UpperCaseAction
} from "../../browser/linesOperations.js";
...
suite("Editor Contrib - Line Operations", () => {
  ...
  suite("SortLinesAscendingAction", () => {
    test("should sort selected lines in ascending order", () => {
    ...
    test("should sort lines in ascending order", () => {
      withTestCodeEditor(["omicron", "beta", "alpha"], {}, (editor) => {
        const model = editor.getModel();
        const sortLinesAscendingAction = new SortLinesAscendingAction();
        executeAction(sortLinesAscendingAction, editor);
    ...
    test("should sort multiple selections in ascending order", () => {
      withTestCodeEditor(
        ["omicron", "beta", "alpha", "", "omicron", "beta", "alpha"],
        {},
        (editor) => {
          const model = editor.getModel();
          const sortLinesAscendingAction = new SortLinesAscendingAction();
          ...
          editor.getSelections().forEach((actualSelection, index) => {
            assert.deepStrictEqual(
              ...
              expectedSelections[index].toString()
            );
          });
        }
      );
  ...
  suite("SortLinesDescendingAction", () => {
    test("should sort selected lines in descending order", () => {
    ...
    test("should sort multiple selections in descending order", () => {
      withTestCodeEditor(
        ["alpha", "beta", "omicron", "", "alpha", "beta", "omicron"],
        {},
        (editor) => {
          const model = editor.getModel();
          const sortLinesDescendingAction = new SortLinesDescendingAction();
  ...
  suite("DeleteDuplicateLinesAction", () => {
    test("should remove duplicate lines within selection", () => {
      withTestCodeEditor(
        ["alpha", "beta", "beta", "beta", "alpha", "omicron"],
        {},
        (editor) => {
          const model = editor.getModel();
    ...
    test("should remove duplicate lines in multiple selections", () => {
      withTestCodeEditor(
        [
          "alpha",
          "beta",
        ...
        (editor) => {
          ...
          const deleteDuplicateLinesAction = new DeleteDuplicateLinesAction();
          editor.setSelections([
            new Selection(1, 2, 4, 3),
            new Selection(6, 2, 8, 3)
          ]);
          executeAction(deleteDuplicateLinesAction, editor);
          assert.deepStrictEqual(model.getLinesContent(), [
            "alpha",
  ...
  suite("DeleteAllLeftAction", () => {
    test("should delete to the left of the cursor", () => {
    ...
    test("should jump to the previous line when on first column", () => {
      withTestCodeEditor(["one", "two", "three"], {}, (editor) => {
        const model = editor.getModel();
        const deleteAllLeftAction = new DeleteAllLeftAction();
    ...
    test("should keep deleting lines in multi cursor mode", () => {
      withTestCodeEditor(
        [
    ...
    test("should work in multi cursor mode", () => {
      withTestCodeEditor(
        [
          "hello",
          "world",
    ...
    test("issue #36234: should push undo stop", () => {
      withTestCodeEditor(["one", "two", "three"], {}, (editor) => {
        ...
        editor.trigger("keyboard", Handler.Type, {
          text: "Typing some text here on line "
        });
        assert.strictEqual(
          model.getLineContent(1),
  ...
  suite("JoinLinesAction", () => {
    test("should join lines and insert space if necessary", () => {
      withTestCodeEditor(
        [
          "hello",
          "world",
    ...
    test("#50471 Join lines at the end of document", () => {
  ...
  test("toggle case", () => {
    withTestCodeEditor(
      ...
      (editor) => {
        ...
        executeAction(snakecaseAction, editor);
        assert.strictEqual(
          model.getValueInRange(new Selection(14, 1, 17, 15)),
    ...
    withTestCodeEditor(
      ...
      (editor) => {
        ...
        assert.strictEqual(
          ...
          "ReTainAnyCAPitalization"
        );
        editor.setSelection(new Selection(6, 1, 6, 23));
        executeAction(camelcaseAction, editor);
        assert.strictEqual(
          model.getLineContent(6),
          "myVar.testFunction()"
        );
  ...
  test("empty selection in middle of lines", () => {
    testDeleteLinesCommand(
      ...
      ["first", "third line", "fourth line", "fifth"],
      new Selection(2, 3, 2, 3)
    );
  });
  test("empty selection at top of lines", () => {
    testDeleteLinesCommand(
      ["first", "second line", "third line", "fourth line", "fifth"],
      new Selection(1, 5, 1, 5),
      ["second line", "third line", "fourth line", "fifth"],
      new Selection(1, 5, 1, 5)
    );
  });
