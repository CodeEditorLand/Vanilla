...
} from "../../../../test/common/modes/supports/autoClosingPairsRules.js";
import {
  cppBracketRules,
  goBracketRules,
  htmlBracketRules,
  ...
  vbBracketRules
} from "../../../../test/common/modes/supports/bracketRules.js";
import {
  goIndentationRules,
...
} from "../../../../test/common/modes/supports/indentationRules.js";
import {
  cppOnEnterRules,
  htmlOnEnterRules,
  javascriptOnEnterRules,
  phpOnEnterRules
} from "../../../../test/common/modes/supports/onEnterRules.js";
...
function testIndentationToSpacesCommand(lines, selection, tabSize, expectedLines, expectedSelection) {
  testCommand(
    ...
    null,
    selection,
    (accessor, sel) => new IndentationToSpacesCommand(sel, tabSize),
    expectedLines,
    expectedSelection
  );
}
function testIndentationToTabsCommand(lines, selection, tabSize, expectedLines, expectedSelection) {
  testCommand(
    ...
    null,
    selection,
    (accessor, sel) => new IndentationToTabsCommand(sel, tabSize),
    expectedLines,
    expectedSelection
  );
}
...
suite("Change Indentation to Spaces - TypeScript/Javascript", () => {
  ...
  test("single tabs only at start of line", () => {
  ...
  test("multiple tabs at start of line", () => {
    testIndentationToSpacesCommand(
      [
        "		first",
        "	second line",
...
suite("Change Indentation to Tabs -  TypeScript/Javascript", () => {
  ensureNoDisposablesAreLeakedInTestSuite();
  test("spaces only at start of line", () => {
  ...
  test("multiple spaces at start of line", () => {
    testIndentationToTabsCommand(
      [
        "first",
        "   second line",
...
suite("Auto Indent On Paste - TypeScript/JavaScript", () => {
      ...
      (editor, viewModel, instantiationService) => {
        ...
        const tokens = [
          ...
          [
            ...
            {
              startIndex: 3,
              standardTokenType: StandardTokenType.Other
            }
          ],
          [
            {
        ...
        const autoIndentOnPasteController = editor.registerAndInstantiateContribution(
  ...
  test("issue #29803: do not indent when pasting text with only one line", () => {
    const model = createTextModel(
      ["const linkHandler = new Class(a, b, c,", "    d)"].join("\n"),
      languageId,
      {}
    );
    disposables.add(model);
...
suite("Auto Indent On Type - TypeScript/JavaScript", () => {
  ...
  test("issue #208215: indent after arrow function 2", () => {
    const model = createTextModel(
      ["const array = [1, 2, 3, 4, 5];", "array.map(", "    v =>"].join(
        "\n"
      ),
      languageId,
      {}
  ...
  test("issue #36090", () => {
    ...
    withTestCodeEditor(
      model,
      { autoIndent: "advanced", serviceCollection },
      (editor, viewModel) => {
  ...
  test("issue #43244: indent when lambda arrow function is detected, outdent when end is reached", () => {
    const model = createTextModel(
      ["const array = [1, 2, 3, 4, 5];", "array.map(_)"].join("\n"),
      languageId,
      {}
    );
    disposables.add(model);
  ...
  test.skip("issue #43244: keep indentation when methods called on object/array", () => {
    const model = createTextModel(
      ["const array = [1, 2, 3]", "    .filter(() => true)"].join("\n"),
      languageId,
      {}
    );
    disposables.add(model);
  ...
  test.skip("issue #67678: indent on typing curly brace", () => {
    const model = createTextModel(
      ["if (true) {", 'console.log("a")', 'console.log("b")', ""].join(
        "\n"
      ),
      languageId,
      {}
  ...
  test.skip("issue #125261: typing closing brace does not keep the current indentation", () => {
    ...
    withTestCodeEditor(
      model,
      { autoIndent: "keep", serviceCollection },
      (editor, viewModel) => {
        editor.setSelection(new Selection(2, 5, 2, 5));
        viewModel.type("}", "keyboard");
...
suite("Auto Indent On Type - CPP", () => {
  ...
  test.skip("issue #178334: incorrect outdent of } when signature spans multiple lines", () => {
    const model = createTextModel(
      ["int WINAPI WinMain(bool instance,", "    int nshowcmd) {}"].join(
        "\n"
      ),
      languageId,
      {}
  ...
  test.skip('issue #111265: auto indentation set to "none" still changes the indentation', () => {
    ...
    withTestCodeEditor(
      model,
      { autoIndent: "none", serviceCollection },
      (editor, viewModel) => {
...
suite("Auto Indent On Type - Visual Basic", () => {
  ...
  test.skip("issue #118932: no indentation in visual basic files", () => {
    const model = createTextModel(
      ["if True then", "    Some code", "    end i"].join("\n"),
      languageId,
      {}
    );
    disposables.add(model);
    withTestCodeEditor(
      ...
      (editor, viewModel, instantiationService) => {
        ...
        assert.strictEqual(
          model.getValue(),
          ["if True then", "    Some code", "end if"].join("\n")
        );
      }
    );
  });
