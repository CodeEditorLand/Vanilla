...
class CodeMain {
  ...
  createServices() {
    ...
    const policyService = isWindows && productService.win32RegValueName ? disposables.add(
      new NativePolicyService(
        logService,
        productService.win32RegValueName
      )
    ) : environmentMainService.policyFile ? disposables.add(
    ...
    services.set(
      ILifecycleMainService,
      new SyncDescriptor(LifecycleMainService, void 0, false)
    );
    ...
    services.set(
      IRequestService,
      new SyncDescriptor(RequestService, [networkLogger], true)
    );
    ...
    await Promises.settled([
      ...
      Promise.all(
        [
          ...
          (path) => path ? promises.mkdir(path, { recursive: true }) : void 0
        )
      ),
      // State service
      stateService.init(),
  ...
  async claimInstance(logService, environmentMainService, lifecycleMainService, instantiationService, productService, retry) {
    ...
    try {
      ...
      const otherInstanceLaunchMainService = ProxyChannel.toService(
        client.getChannel("launch"),
        { disableMarshalling: true }
      );
      const otherInstanceDiagnosticsMainService = ProxyChannel.toService(
  ...
  handleStartupDataDirError(environmentMainService, productService, error) {
    if (error.code === "EACCES" || error.code === "EPERM") {
      const directories = coalesce([
        ...
        (folder) => getPathLabel(URI.file(folder), {
          os: OS,
  ...
  doValidatePaths(args, gotoLineMode) {
    ...
    const distinctPaths = distinct(
      result,
      (path) => path && caseInsensitive ? path.toLowerCase() : path || ""
    );
