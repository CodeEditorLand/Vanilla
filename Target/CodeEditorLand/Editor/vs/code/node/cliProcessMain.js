  ...
  async initServices() {
    ...
    await Promise.all(
      [
        ...
        (path) => path ? fs.promises.mkdir(path, { recursive: true }) : void 0
      )
    );
    ...
    const policyService = isWindows && productService.win32RegValueName ? this._register(
      new NativePolicyService(
        logService,
        productService.win32RegValueName
      )
    ) : environmentService.policyFile ? this._register(
    ...
    services.set(
      ...
      new SyncDescriptor(NativeLanguagePackService, void 0, false)
    );
    const appenders = [];
