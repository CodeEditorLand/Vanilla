  ...
  *_streams() {
  ...
  handleFragment(fragment) {
    ...
    if (fragment.part.type === "text") {
      out2 = new extHostTypes.LanguageModelTextPart(fragment.part.value);
    } else {
      out2 = new extHostTypes.LanguageModelToolCallPart(
        fragment.part.name,
        fragment.part.toolCallId,
        fragment.part.parameters
      );
    }
    res.stream.emitOne(out2);
  }
  reject(err) {
    this._isDone = true;
...
let ExtHostLanguageModels = class {
  ...
  createLanguageModelAccessInformation(from) {
    ...
    return {
      ...
      canSendRequest(chat) {
        let metadata;
        for (const [_, value] of that._allLanguageModelData) {
