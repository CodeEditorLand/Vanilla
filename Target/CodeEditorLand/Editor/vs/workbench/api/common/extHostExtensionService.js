...
let AbstractExtHostExtensionService = class extends Disposable {
      ...
      new ExtensionsActivator(
        ...
        this._globalRegistry,
        {
          ...
          actualActivateExtension: async (extensionId, reason) => {
            if (ExtensionDescriptionRegistry.isHostExtension(
              extensionId,
              this._myRegistry,
              this._globalRegistry
            )) {
            ...
            const extensionDescription = this._myRegistry.getExtensionDescription(
              extensionId
            );
  ...
  async _handleWorkspaceContainsEagerExtension(folders, desc) {
    ...
    const host = {
      ...
      checkExists: (folders2, includes, token) => this._mainThreadWorkspaceProxy.$checkExists(
        folders2,
        includes,
        token
      )
