...
function toSCMHistoryItemDto(historyItem) {
  const labels = historyItem.labels?.map((l) => ({
    title: l.title,
    icon: getHistoryItemIconDto(l.icon)
  }));
...
let ExtHostSCM = class {
  ...
  async $resolveHistoryItemGroupCommonAncestor(sourceControlHandle, historyItemGroupIds, token) {
    const historyProvider = this._sourceControls.get(sourceControlHandle)?.historyProvider;
    return await historyProvider?.resolveHistoryItemGroupCommonAncestor(
      historyItemGroupIds,
      token
    ) ?? void 0;
  }
