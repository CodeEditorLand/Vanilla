...
CommandsRegistry.registerCommand(
  ...
  async (accessor, args) => {
    ...
    const cliService = instantiationService.createChild(
    ...
    if (args.list) {
      ...
      const revive = /* @__PURE__ */ __name((inputs) => inputs.map(
        (input) => isString(input) ? input : URI.revive(input)
      ), "revive");
...
let RemoteExtensionManagementCLI = class extends ExtensionManagementCLI {
  ...
  validateExtensionKind(manifest) {
    if (!this._extensionManifestPropertiesService.canExecuteOnWorkspace(
      manifest
    ) && // Web extensions installed on remote can be run in web worker extension host
    !(isWeb && this._extensionManifestPropertiesService.canExecuteOnWeb(
      manifest
    ))) {
      this.logger.info(
        localize(
