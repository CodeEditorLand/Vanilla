...
function toISCMHistoryItem(historyItemDto) {
  const labels = historyItemDto.labels?.map((l) => ({
    title: l.title,
    icon: getIconFromIconDto(l.icon)
  }));
...
class MainThreadSCMHistoryProvider {
  ...
  _currentHistoryItemGroup = observableValueOpts(
    {
      owner: this,
  ...
  async resolveHistoryItemGroupCommonAncestor(historyItemGroupIds) {
  ...
  $onDidChangeCurrentHistoryItemGroup(historyItemGroup) {
    this._currentHistoryItemGroup.set(historyItemGroup, void 0);
  }
}
