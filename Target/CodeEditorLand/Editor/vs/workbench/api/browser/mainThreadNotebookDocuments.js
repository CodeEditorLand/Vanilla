...
let MainThreadNotebookDocuments = class {
  constructor(extHostContext, _notebookEditorModelResolverService, _uriIdentityService) {
    ...
    this._disposables.add(
      this._notebookEditorModelResolverService.onDidChangeDirty(
        (model) => this._proxy.$acceptDirtyStateChanged(
          model.resource,
          model.isDirty()
        )
      )
    ...
    this._disposables.add(
      this._notebookEditorModelResolverService.onDidSaveNotebook(
        (e) => this._proxy.$acceptModelSaved(e)
      )
    );
    this._disposables.add(
      _notebookEditorModelResolverService.onWillFailWithConflict((e) => {
