...
suite("ExtHostSearch", () => {
  ...
  async function registerTestTextSearchProvider(provider, scheme = "file") {
    disposables.add(
      extHostSearch.registerTextSearchProviderOld(scheme, provider)
    );
    await rpcProtocol.sync();
  }
  async function registerTestFileSearchProvider(provider, scheme = "file") {
    disposables.add(
      extHostSearch.registerFileSearchProviderOld(scheme, provider)
    );
    await rpcProtocol.sync();
  }
  async function runFileSearch(query, cancel = false) {
    ...
    try {
      ...
      const p = extHostSearch.$provideFileSearchResults(
        mockMainThreadSearch.lastHandle,
        0,
        query,
        cancellation.token
      );
    ...
    await rpcProtocol.sync();
    return {
      results: mockMainThreadSearch.results.map(
        (r) => URI.revive(r)
      ),
      stats
    };
  ...
  async function runTextSearch(query) {
    ...
    try {
      const cancellation = new CancellationTokenSource();
      const p = extHostSearch.$provideTextSearchResults(
        mockMainThreadSearch.lastHandle,
        0,
        query,
        cancellation.token
  ...
  setup(() => {
    ...
    mockPFS = {};
    extHostSearch = disposables.add(
      new class extends NativeExtHostSearch {
        constructor() {
          super(
            rpcProtocol,
  ...
  const fancySchemeFolderA = URI.from({
    scheme: fancyScheme,
  ...
  suite("File:", () => {
    ...
    test("session cancellation should work", async () => {
      ...
      await registerTestFileSearchProvider({
        provideFileSearchResults(query, options, token) {
          disposables2.push(
            options.session?.onCancellationRequested(() => {
              numSessionCancelled++;
            })
          );
    ...
    test("all provider calls get global include/excludes", async () => {
      await registerTestFileSearchProvider({
        provideFileSearchResults(query2, options, token) {
          assert(
            options.excludes.length === 2 && options.includes.length === 2,
            "Missing global include/excludes"
          );
          return Promise.resolve(null);
        }
      });
      ...
      await registerTestFileSearchProvider({
        provideFileSearchResults(query2, options, token) {
          if (options.folder.toString() === rootFolderA.toString()) {
            assert.deepStrictEqual(options.includes.sort(), [
              "*.ts",
              "foo"
            ]);
    ...
    test("basic sibling exclude clause", async () => {
      ...
      await registerTestFileSearchProvider({
        provideFileSearchResults(query2, options, token) {
          return Promise.resolve(
            reportedResults.map(
              (relativePath) => joinPath(options.folder, relativePath)
            )
          );
        }
      });
        ...
        provideFileSearchResults(query2, options, token) {
          ...
          if (options.folder.fsPath === rootFolderA.fsPath) {
            reportedResults = [
              ...
              (relativePath) => joinPath(rootFolderA, relativePath)
            );
          } else {
            reportedResults = [
              "fileB.ts",
              ...
              (relativePath) => joinPath(rootFolderB, relativePath)
            );
          }
          return Promise.resolve(reportedResults);
        }
    ...
    test("provider returns maxResults exactly", async () => {
      ...
      assert(
        !wasCanceled,
        "Expected not to be canceled when just reaching limit"
      );
    });
    test("multiroot max results", async () => {
      ...
      await registerTestFileSearchProvider({
        async provideFileSearchResults(query2, options, token) {
          ...
          return ["file1.ts", "file2.ts", "file3.ts"].map(
            (relativePath) => joinPath(options.folder, relativePath)
          );
        }
      });
      ...
      assert.strictEqual(
        cancels,
        2,
        "Expected all invocations to be canceled when hitting limit"
      );
    });
  ...
  suite("Text:", () => {
    ...
    test("no results", async () => {
      ...
      const { results, stats } = await runTextSearch(
        getSimpleQuery("foo")
      );
      assert(!stats.limitHit);
      assert(!results.length);
    ...
    test("multiroot max results", async () => {
      ...
      await registerTestTextSearchProvider({
        async provideTextSearchResults(query2, options, progress, token) {
          ...
          await new Promise((r) => process.nextTick(r));
          ["file1.ts", "file2.ts", "file3.ts"].forEach(
            (f) => progress.report(makeTextResult(options.folder, f))
          );
          return null;
        }
      });
