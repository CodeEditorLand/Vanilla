...
suite("ExtHostTypes", () => {
  ...
  test("Position, isBefore(OrEqual)?", () => {
  ...
  test("Range", () => {
    ...
    assert.throws(
      () => new types.Range(new types.Position(0, 0), void 0)
    );
    assert.throws(() => new types.Range(new types.Position(0, 0), null));
    assert.throws(
      () => new types.Range(void 0, new types.Position(0, 0))
    );
  ...
  test("Range, isEmpty|isSingleLine", () => {
    let range = new types.Range(1, 0, 0, 0);
  ...
  test("Range, contains (no instanceof)", () => {
    const range = new types.Range(1, 1, 2, 11);
    const startLike = {
  ...
  test("Range, with", () => {
    ...
    assert.strictEqual(res.start.character, 1);
    res = range.with({
      end: new types.Position(9, 8),
      start: new types.Position(2, 3)
    });
    assert.strictEqual(res.end.line, 9);
    assert.strictEqual(res.end.character, 8);
    assert.strictEqual(res.start.line, 2);
  ...
  test("WorkspaceEdit - keep order of text and file changes", () => {
    const edit = new types.WorkspaceEdit();
  ...
  test("WorkspaceEdit - two edits for one resource", () => {
    const edit = new types.WorkspaceEdit();
  ...
  test("WorkspaceEdit - set with metadata accepts undefined", () => {
    const edit = new types.WorkspaceEdit();
    const uri = URI.parse("foo:bar");
    edit.set(uri, [
      [
  ...
  test("DocumentLink", () => {
    ...
    assert.throws(
      () => new types.DocumentLink(new types.Range(1, 1, 1, 1), null)
    );
  });
  test("toJSON & stringify", () => {
    ...
    assertToJSON(
      new types.Location(URI.file("u.ts"), new types.Position(3, 4)),
      {
    ...
    assertToJSON(
      new types.Location(URI.file("u.ts"), new types.Range(1, 2, 3, 4)),
      {
        uri: URI.parse("file:///u.ts").toJSON(),
        range: [
          { line: 1, character: 2 },
    ...
    assertToJSON(
      new types.CodeLens(new types.Range(7, 8, 9, 10), {
        command: "id",
        title: "title"
      }),
      {
    ...
    assertToJSON(new types.CompletionItem("complete"), {
      label: "complete"
    });
  ...
  test("SymbolInformation, old ctor", () => {
    const info = new types.SymbolInformation(
      "foo",
      types.SymbolKind.Array,
      new types.Range(1, 1, 2, 3)
    );
    assert.ok(info.location instanceof types.Location);
    assert.strictEqual(info.location.uri, void 0);
  });
  test("SnippetString, builder-methods", () => {
    let string;
    string = new types.SnippetString();
    assert.strictEqual(
      string.appendText("I need $ and $").value,
      "I need \\$ and \\$"
    );
    string = new types.SnippetString();
    assert.strictEqual(
      string.appendText("I need \\$").value,
      "I need \\\\\\$"
    );
    string = new types.SnippetString();
    ...
    string = new types.SnippetString();
    string.appendText("foo").appendPlaceholder(
      (b) => b.appendText("abc").appendPlaceholder("nested")
    ).appendText("bar");
  ...
  test("instanceof doesn't work for FileSystemError #49386", () => {
  ...
  test("CodeActionKind contains", () => {
    assert.ok(
      types.CodeActionKind.RefactorExtract.contains(
        types.CodeActionKind.RefactorExtract
      )
    );
    assert.ok(
      types.CodeActionKind.RefactorExtract.contains(
        types.CodeActionKind.RefactorExtract.append("other")
      )
    );
    assert.ok(
      !types.CodeActionKind.RefactorExtract.contains(
    ...
    assert.ok(
      !types.CodeActionKind.RefactorExtract.contains(
        types.CodeActionKind.Refactor.append("other")
      )
    );
    assert.ok(
      !types.CodeActionKind.RefactorExtract.contains(
        types.CodeActionKind.Empty.append("other").append("refactor")
      )
    );
    assert.ok(
      !types.CodeActionKind.RefactorExtract.contains(
        types.CodeActionKind.Empty.append("refactory")
      )
    );
  });
  test("CodeActionKind intersects", () => {
    assert.ok(
      types.CodeActionKind.RefactorExtract.intersects(
        types.CodeActionKind.RefactorExtract
      )
    );
    assert.ok(
  ...
  test("SemanticTokensBuilder simple", () => {
    ...
    assert.deepStrictEqual(
      toArr(builder.build().data),
      [1, 0, 5, 1, 1, 0, 10, 4, 2, 2, 1, 2, 3, 2, 2]
    );
  });
  test("SemanticTokensBuilder no modifier", () => {
    ...
    assert.deepStrictEqual(
      toArr(builder.build().data),
      [1, 0, 5, 1, 0, 0, 10, 4, 2, 0, 1, 2, 3, 2, 0]
    );
  });
  test("SemanticTokensBuilder out of order 1", () => {
    ...
    assert.deepStrictEqual(
      toArr(builder.build().data),
      [1, 0, 4, 4, 4, 1, 0, 5, 1, 1, 0, 10, 1, 2, 2, 0, 5, 2, 3, 3]
    );
  });
  ...
  test("Markdown codeblock rendering is swapped #111604", () => {
  ...
  test("NotebookCellOutputItem - factories", () => {
    assert.throws(() => {
    ...
    assert.deepStrictEqual(
      item.data,
      new TextEncoder().encode(JSON.stringify(1))
    );
    ...
    assert.deepStrictEqual(
      item.data,
      new TextEncoder().encode(JSON.stringify(true))
    );
    ...
    assert.deepStrictEqual(
      ...
      new TextEncoder().encode(
        JSON.stringify([true, 1, "ddd"], void 0, "	")
      )
    );
  ...
  test("FileDecoration#validate", () => {
    ...
    assert.throws(
      () => types.FileDecoration.validate({ badge: "\u0BAA\u0BC1\u0BA9\u0BCD\u0B9A\u0BBF\u0BB0\u0BBF\u0BAA\u0BCD\u0BAA\u0BCB\u0B9F\u0BC1" })
    );
