...
import {
  ...
  ExtHostDiagnostics
} from "../../common/extHostDiagnostics.js";
import {
  Diagnostic,
...
suite("ExtHostDiagnostics", () => {
  ...
  test("diagnostic collection, forEach, clear, has", () => {
  ...
  test("diagnostic collection, immutable read", () => {
    const collection = new DiagnosticCollection(
      "test",
      "test",
      100,
  ...
  test("diagnostics collection, set with dupliclated tuples", () => {
    const collection = new DiagnosticCollection(
      "test",
      "test",
      100,
  ...
  test("diagnostics collection, set tuple overrides, #11547", () => {
    let lastEntries;
    const collection = new DiagnosticCollection(
      "test",
      "test",
    ...
    const uri = URI.parse("sc:hightower");
    collection.set([
      [uri, [new Diagnostic(new Range(0, 0, 1, 1), "error")]]
    ]);
    ...
    lastEntries = void 0;
    collection.set([
      [uri, [new Diagnostic(new Range(0, 0, 1, 1), "warning")]]
    ]);
    assert.strictEqual(collection.get(uri).length, 1);
  ...
  test("do send message when not making a change", () => {
    let changeCount = 0;
    let eventCount = 0;
  ...
  test("diagnostics collection, tuples and undefined (small array), #15585", () => {
    const collection = new DiagnosticCollection(
      "test",
      "test",
      100,
  ...
  test("diagnostics collection, tuples and undefined (large array), #15585", () => {
    const collection = new DiagnosticCollection(
      "test",
      "test",
      100,
  ...
  test("diagnostic capping (max per file)", () => {
    let lastEntries;
    const collection = new DiagnosticCollection(
      "test",
      "test",
    ...
    assert.strictEqual(
      lastEntries[0][1][200].severity,
      MarkerSeverity.Warning
    );
    assert.strictEqual(
      lastEntries[0][1][250].severity,
      MarkerSeverity.Info
    );
  });
  test("diagnostic capping (max files)", () => {
    let lastEntries;
    const collection = new DiagnosticCollection(
      "test",
      "test",
  ...
  test("diagnostic eventing", async () => {
    const emitter = new Emitter();
    const collection = new DiagnosticCollection(
      "ddd",
      "test",
  ...
  test("vscode.languages.onDidChangeDiagnostics Does Not Provide Document URI #49582", async () => {
    const emitter = new Emitter();
    const collection = new DiagnosticCollection(
      "ddd",
      "test",
  ...
  test("diagnostics with related information", (done) => {
    const collection = new DiagnosticCollection(
      "ddd",
      "test",
      100,
      ...
      new class extends DiagnosticsShape {
        $changeMany(owner, entries) {
          ...
          assert.strictEqual(
            diag2.relatedInformation[0].message,
            "more1"
          );
          assert.strictEqual(
            diag2.relatedInformation[1].message,
            "more2"
          );
          done();
        }
    ...
    diag.relatedInformation = [
      new DiagnosticRelatedInformation(
        new Location(URI.parse("cc:dd"), new Range(0, 0, 0, 0)),
        "more1"
      ),
      new DiagnosticRelatedInformation(
        new Location(URI.parse("cc:ee"), new Range(0, 0, 0, 0)),
        "more2"
      )
    ];
    collection.set(URI.parse("aa:bb"), [diag]);
  });
  test("vscode.languages.getDiagnostics appears to return old diagnostics in some circumstances #54359", () => {
    const ownerHistory = [];
    const diags = new ExtHostDiagnostics(
      new class {
        getProxy(id) {
    ...
    const collection1 = diags.createDiagnosticCollection(
      nullExtensionDescription.identifier,
      "foo"
    );
    const collection2 = diags.createDiagnosticCollection(
      nullExtensionDescription.identifier,
      "foo"
    );
    collection1.clear();
  ...
  test("Error updating diagnostics from extension #60394", () => {
    let callCount = 0;
    const collection = new DiagnosticCollection(
      "ddd",
      "test",
  ...
  test("Version id is set whenever possible", () => {
    const all = [];
    const collection = new DiagnosticCollection(
      "ddd",
      "test",
    ...
    const allVersions = all.flatMap(
      (tuple) => tuple[1].map((t) => t.modelVersionId)
    );
    assert.deepStrictEqual(allVersions, [7, 7, 7, 7]);
  });
  test("Diagnostics created by tasks aren't accessible to extensions #47292", async () => runWithFakedTimers({}, async () => {
    const diags = new ExtHostDiagnostics(
      new class {
        getProxy(id) {
          return {};
  ...
  test("languages.getDiagnostics doesn't handle case insensitivity correctly #128198", () => {
    const diags = new ExtHostDiagnostics(
      new class {
        getProxy(id) {
    ...
    const col = diags.createDiagnosticCollection(
      nullExtensionDescription.identifier
    );
    const uriSensitive = URI.from({ scheme: "foo", path: "/SOME/path" });
    const uriSensitiveCaseB = uriSensitive.with({
      path: uriSensitive.path.toUpperCase()
    });
    const uriInSensitive = URI.from({
      scheme: "insensitive",
      path: "/SOME/path"
    });
    const uriInSensitiveUpper = uriInSensitive.with({
      path: uriInSensitive.path.toUpperCase()
    });
    col.set(uriSensitive, [
      new Diagnostic(new Range(0, 0, 0, 0), "sensitive")
    ]);
    col.set(uriInSensitive, [
      new Diagnostic(new Range(0, 0, 0, 0), "insensitive")
    ]);
    ...
    assert.strictEqual(
      diags.getDiagnostics(uriInSensitiveUpper)?.length,
      1
    );
    const fromForEach = [];
    ...
    assert.strictEqual(
      fromForEach[1].toString(),
      uriInSensitive.toString()
    );
  });
});
