...
suite("ExtHostDocumentSaveParticipant", () => {
  ...
  test("event delivery", () => {
    ...
    let event;
    const sub = participant.getOnWillSaveTextDocumentEvent(
  ...
  test("event delivery, bad listener doesn't prevent more events", () => {
    ...
    const sub1 = participant.getOnWillSaveTextDocumentEvent(
      nullExtensionDescription
    )((e) => {
      throw new Error("\u{1F480}");
    });
    let event;
    const sub2 = participant.getOnWillSaveTextDocumentEvent(
      nullExtensionDescription
    )((e) => {
      event = e;
    });
  ...
  test("event delivery, overall timeout", async () => {
    const participant = new ExtHostDocumentSaveParticipant(
      nullLogService,
      documents,
      mainThreadBulkEdits,
    ...
    const sub3 = participant.getOnWillSaveTextDocumentEvent(
      nullExtensionDescription
    )((event) => {
      calls.push(3);
    });
    const values = await participant.$participateInSave(
      resource,
      SaveReason.EXPLICIT
    );
    sub1.dispose();
  ...
  test("event delivery, pushEdits sync", () => {
    ...
    const sub = participant.getOnWillSaveTextDocumentEvent(
      ...
      e.waitUntil(
        Promise.resolve([TextEdit.insert(new Position(0, 0), "bar")])
      );
      e.waitUntil(
        Promise.resolve([TextEdit.setEndOfLine(EndOfLine.CRLF)])
      );
    });
    return participant.$participateInSave(resource, SaveReason.EXPLICIT).then(() => {
      sub.dispose();
