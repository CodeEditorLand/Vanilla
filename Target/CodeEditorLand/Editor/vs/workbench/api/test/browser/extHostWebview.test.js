...
suite("ExtHostWebview", () => {
  ...
  function createWebview(rpcProtocol2, remoteAuthority) {
    const extHostWebviews = disposables.add(
      new ExtHostWebviews(
        rpcProtocol2,
        {
    ...
    const extHostWebviewPanels = disposables.add(
      new ExtHostWebviewPanels(rpcProtocol2, extHostWebviews, void 0)
    );
    return disposables.add(
      extHostWebviewPanels.createWebviewPanel(
        {
  ...
  test("Cannot register multiple serializers for the same view type", async () => {
    ...
    const extHostWebviewPanels = disposables.add(
      new ExtHostWebviewPanels(rpcProtocol, extHostWebviews, void 0)
    );
    let lastInvokedDeserializer;
    ...
    const serializerARegistration = extHostWebviewPanels.registerWebviewPanelSerializer(
      extension,
      viewType,
      serializerA
    );
    await extHostWebviewPanels.$deserializeWebviewPanel(
      "x",
      viewType,
      {
        title: "title",
    ...
    assert.throws(
      () => disposables.add(
        extHostWebviewPanels.registerWebviewPanelSerializer(
          extension,
          viewType,
          serializerB
        )
  ...
  test("asWebviewUri for local file paths", () => {
    ...
    assert.strictEqual(
      webview.webview.asWebviewUri(
        URI.parse("file://localhost/Users/codey/file.html")
      ).toString(),
