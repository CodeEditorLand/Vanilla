...
suite("ExtHostEditorTabs", () => {
  const defaultTabDto = {
    id: "uniquestring",
    input: {
      kind: TabInputKind.TextInput,
      uri: URI.parse("file://abc/def.txt")
    },
    isActive: true,
    isDirty: true,
    isPinned: true,
  ...
  test("Ensure empty model throws when accessing active group", () => {
  ...
  test("Ensure tabGroup change events fires", () => {
    const extHostEditorTabs = new ExtHostEditorTabs(
      SingleProxyRPCProtocol(
        new class extends mock() {
          // override/implement $moveTab or $closeTab
  ...
  test("Check TabGroupChangeEvent properties", () => {
    const extHostEditorTabs = new ExtHostEditorTabs(
      SingleProxyRPCProtocol(
        new class extends mock() {
          // override/implement $moveTab or $closeTab
    ...
    const events = [];
    store.add(
      extHostEditorTabs.tabGroups.onDidChangeTabGroups(
        (e) => events.push(e)
      )
    );
  ...
  test("Ensure reference equality for activeTab and activeGroup", () => {
    const extHostEditorTabs = new ExtHostEditorTabs(
      SingleProxyRPCProtocol(
        new class extends mock() {
          // override/implement $moveTab or $closeTab
  ...
  test("TextMergeTabInput surfaces in the UI", () => {
    const extHostEditorTabs = new ExtHostEditorTabs(
      SingleProxyRPCProtocol(
        new class extends mock() {
          // override/implement $moveTab or $closeTab
  ...
  test("Ensure reference stability", () => {
    const extHostEditorTabs = new ExtHostEditorTabs(
      SingleProxyRPCProtocol(
        new class extends mock() {
          // override/implement $moveTab or $closeTab
    ...
    let all = extHostEditorTabs.tabGroups.all.flatMap(
      (group) => group.tabs
    );
    ...
    assert.strictEqual(
      apiTab1.input.uri.toString(),
      URI.revive(dtoResource).toString()
    );
    ...
    all = extHostEditorTabs.tabGroups.all.flatMap((group) => group.tabs);
    assert.strictEqual(all.length, 1);
    const apiTab2 = all[0];
    assert.ok(apiTab1.input instanceof TextTabInput);
    assert.strictEqual(
  ...
  test("Tab.isActive working", () => {
    ...
    const tabDtoAAA = createTabDto({
      ...
      input: {
        kind: TabInputKind.TextInput,
        uri: URI.parse("file://abc/AAA.txt")
      },
      editorId: "default"
    });
    const tabDtoBBB = createTabDto({
      ...
      input: {
        kind: TabInputKind.TextInput,
        uri: URI.parse("file://abc/BBB.txt")
      },
      editorId: "default"
    });
    extHostEditorTabs.$acceptEditorTabModel([
    ...
    const all = extHostEditorTabs.tabGroups.all.flatMap(
      (group) => group.tabs
    );
    ...
    assert.strictEqual(
      activeTab1?.input?.uri.toString(),
      URI.revive(dtoAAAResource)?.toString()
    );
    ...
    assert.strictEqual(
      activeTab2?.input?.uri.toString(),
      URI.revive(dtoBBBResource)?.toString()
    );
    assert.strictEqual(activeTab2?.isActive, true);
    assert.strictEqual(activeTab1?.isActive, false);
  });
  test("vscode.window.tagGroups is immutable", () => {
    const extHostEditorTabs = new ExtHostEditorTabs(
      SingleProxyRPCProtocol(
        new class extends mock() {
          // override/implement $moveTab or $closeTab
  ...
  test("Ensure close is called with all tab ids", () => {
    const closedTabIds = [];
    const extHostEditorTabs = new ExtHostEditorTabs(
      SingleProxyRPCProtocol(
        new class extends mock() {
  ...
  test("Update tab only sends tab change event", async () => {
    const closedTabIds = [];
    const extHostEditorTabs = new ExtHostEditorTabs(
      SingleProxyRPCProtocol(
        new class extends mock() {
    ...
    assert.strictEqual(extHostEditorTabs.tabGroups.all.length, 1);
    assert.strictEqual(
      extHostEditorTabs.tabGroups.all.flatMap((g) => g.tabs).length,
      1
    );
    const tab = extHostEditorTabs.tabGroups.all[0].tabs[0];
    const p = new Promise(
      (resolve) => store.add(extHostEditorTabs.tabGroups.onDidChangeTabs(resolve))
    );
    extHostEditorTabs.$acceptTabOperation({
      groupId: 12,
      index: 0,
  ...
  test("Active tab", () => {
    ...
    assert.strictEqual(
      extHostEditorTabs.tabGroups.activeTabGroup?.activeTab,
      extHostEditorTabs.tabGroups.activeTabGroup?.tabs[0]
    );
    tab1.isActive = false;
    tab2.isActive = true;
    extHostEditorTabs.$acceptTabOperation({
    ...
    assert.strictEqual(
      extHostEditorTabs.tabGroups.activeTabGroup?.activeTab,
      extHostEditorTabs.tabGroups.activeTabGroup?.tabs[1]
    );
    tab3.isActive = true;
    extHostEditorTabs.$acceptEditorTabModel([
      {
  ...
  test("Tab operations patches open and close correctly", () => {
    const extHostEditorTabs = new ExtHostEditorTabs(
      SingleProxyRPCProtocol(
        new class extends mock() {
          // override/implement $moveTab or $closeTab
    ...
    assert.strictEqual(
      extHostEditorTabs.tabGroups.all[0]?.activeTab?.label,
      "label3"
    );
    extHostEditorTabs.$acceptTabOperation({
      groupId: 12,
    ...
    assert.strictEqual(
      extHostEditorTabs.tabGroups.all[0]?.tabs[1]?.label,
      "label2"
    );
  });
  test("Tab operations patches move correctly", () => {
    const extHostEditorTabs = new ExtHostEditorTabs(
      SingleProxyRPCProtocol(
        new class extends mock() {
          // override/implement $moveTab or $closeTab
    ...
    assert.strictEqual(
      extHostEditorTabs.tabGroups.all[0]?.tabs[0]?.label,
      "label2"
    );
    extHostEditorTabs.$acceptTabOperation({
      groupId: 12,
    ...
    assert.strictEqual(
      extHostEditorTabs.tabGroups.all[0]?.tabs[2]?.label,
      "label1"
    );
  });
});
