...
} from "../../common/extHostTextEditor.js";
import {
  Range,
...
suite("ExtHostTextEditor", () => {
  let editor;
  const doc = new ExtHostDocumentData(
    void 0,
    URI.file(""),
    ["aaaa bbbb+cccc abc"],
    "\n",
  ...
  test("API [bug]: registerTextEditorCommand clears redo stack even if no edits are made #55163", async () => {
    let applyCount = 0;
...
suite("ExtHostTextEditorOptions", () => {
  ...
  setup(() => {
    ...
    opts = new ExtHostTextEditorOptions(
      mockProxy,
      "1",
      {
        tabSize: 4,
  ...
  test("can do bulk updates 1", () => {
    ...
    assert.deepStrictEqual(calls, [
      { tabSize: "auto", insertSpaces: true }
    ]);
  });
