...
suite("ExtHostLanguageFeatures", () => {
  ...
  setup(() => {
    ...
    rpcProtocol.set(
      ...
      disposables.add(
        inst.createInstance(MainThreadCommands, rpcProtocol)
      )
    );
    ...
    mainThread = rpcProtocol.set(
      ...
      disposables.add(
        inst.createInstance(MainThreadLanguageFeatures, rpcProtocol)
      )
    );
  });
  teardown(() => {
  ...
  test("DocumentSymbols, register/deregister", async () => {
    assert.strictEqual(
      languageFeaturesService.documentSymbolProvider.all(model).length,
      0
    );
    const d1 = extHost.registerDocumentSymbolProvider(
      defaultExtension,
      defaultSelector,
      new class {
  ...
  test("DocumentSymbols, evil provider", async () => {
    ...
    await rpcProtocol.sync();
    const value = (await OutlineModel.create(
  ...
  test("Quick Outline uses a not ideal sorting, #138502", async () => {
    const symbols = [
      {
        name: "containers",
        range: {
  ...
  test("CodeLens, evil provider", async () => {
    ...
    disposables.add(
      extHost.registerCodeLensProvider(
        ...
        new class {
          provideCodeLenses() {
            return [
              new types.CodeLens(new types.Range(0, 0, 0, 0))
            ];
          }
        }()
      )
    ...
    await rpcProtocol.sync();
    const value = await getCodeLensModel(
      languageFeaturesService.codeLensProvider,
      model,
      CancellationToken.None
    );
  ...
  test("CodeLens, do not resolve a resolved lens", async () => {
    ...
    const symbol = await Promise.resolve(
      data.provider.resolveCodeLens(
        model,
        data.symbol,
        CancellationToken.None
      )
  ...
  test("Definition, data conversion", async () => {
    ...
    await rpcProtocol.sync();
    const value = await getDefinitionsAtPosition(
      languageFeaturesService.definitionProvider,
      model,
      new EditorPosition(1, 1),
      false,
      CancellationToken.None
    );
    assert.strictEqual(value.length, 1);
  ...
  test("Declaration, data conversion", async () => {
    ...
    await rpcProtocol.sync();
    const value = await getDeclarationsAtPosition(
  ...
  test("Implementation, data conversion", async () => {
    ...
    await rpcProtocol.sync();
    const value = await getImplementationsAtPosition(
  ...
  test("Type Definition, data conversion", async () => {
    ...
    await rpcProtocol.sync();
    const value = await getTypeDefinitionsAtPosition(
  ...
  test("HoverProvider, registration order", async () => {
    ...
    await rpcProtocol.sync();
    const value = await getHoversPromise(
      languageFeaturesService.hoverProvider,
      model,
      new EditorPosition(1, 1),
      CancellationToken.None
  ...
  test("Occurrences, data conversion", async () => {
    ...
    await rpcProtocol.sync();
    const value = await getOccurrencesAtPosition(
  ...
  test("References, registration order", async () => {
    ...
    await rpcProtocol.sync();
    const value = await getReferencesAtPosition(
  ...
  test("Quick Fix, command data conversion", async () => {
    ...
    await rpcProtocol.sync();
    const value = await getCodeActions(
  ...
  test("Navigate types, evil provider", async () => {
    disposables.add(
      extHost.registerWorkspaceSymbolProvider(
        defaultExtension,
        new class {
  ...
  test("Rename, evil provider 0/2", async () => {
    ...
    try {
      await rename(
        languageFeaturesService.renameProvider,
        model,
        new EditorPosition(1, 1),
        "newName"
      );
  ...
  test("Multiple RenameProviders don't respect all possible PrepareRename handlers 1/2, #98352", async () => {
  ...
  test("Multiple RenameProviders don't respect all possible PrepareRename handlers 2/2, #98352", async () => {
  ...
  test("Parameter Hints, order", async () => {
    ...
    await rpcProtocol.sync();
    const value = await provideSignatureHelp(
  ...
  test("Suggest, order 1/3", async () => {
    ...
    await rpcProtocol.sync();
    const value = await provideSuggestionItems(
      languageFeaturesService.completionProvider,
      model,
      new EditorPosition(1, 1),
  ...
  test("Suggest, order 2/3", async () => {
    ...
    assert.strictEqual(value.items.length, 1);
    assert.strictEqual(
      value.items[0].completion.insertText,
      "weak-selector"
    );
    value.disposable.dispose();
  });
        ...
        new class {
          provideCompletionItems() {
            return new types.CompletionList(
              [new types.CompletionItem("hello")],
              true
            );
          }
        }(),
  ...
  test("Format Doc, data conversion", async () => {
    ...
    await rpcProtocol.sync();
    const value = await getDocumentFormattingEditsUntilResult(
      NullWorkerService,
      languageFeaturesService,
      model,
  ...
  test("Format Range, data conversion", async () => {
    ...
    await rpcProtocol.sync();
    const value = await getDocumentRangeFormattingEditsUntilResult(
      NullWorkerService,
      languageFeaturesService,
      model,
      new EditorRange(1, 1, 1, 1),
      { insertSpaces: true, tabSize: 4 },
      CancellationToken.None
    );
    assert.strictEqual(value.length, 1);
  ...
  test("Format on Type, data conversion", async () => {
    ...
    await rpcProtocol.sync();
    const value = await getOnTypeFormattingEdits(
      NullWorkerService,
      languageFeaturesService,
      model,
      new EditorPosition(1, 1),
  ...
  test("Links, data conversion", async () => {
    ...
    await rpcProtocol.sync();
    const { links } = disposables.add(
      await getLinks(
        languageFeaturesService.linkProvider,
        model,
        CancellationToken.None
      )
    );
  ...
  test("Document colors, data conversion", async () => {
    ...
    await rpcProtocol.sync();
    const value = await getColors(
      languageFeaturesService.colorProvider,
      model,
      CancellationToken.None
    );
    assert.strictEqual(value.length, 1);
    const [first] = value;
    assert.deepStrictEqual(first.colorInfo.color, {
      red: 0.1,
      green: 0.2,
      blue: 0.3,
      alpha: 0.4
  ...
  test("Selection Ranges, data conversion", async () => {
    ...
    await rpcProtocol.sync();
    provideSelectionRanges(
      languageFeaturesService.selectionRangeProvider,
      model,
      [new Position(1, 17)],
