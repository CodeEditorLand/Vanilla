...
suite("MainThreadWorkspace", () => {
  ...
  test("simple", () => {
    instantiationService.stub(ISearchService, {
      fileSearch(query) {
        ...
        assert.strictEqual(
          query.folderQueries[0].disregardIgnoreFiles,
          true
        );
  ...
  test("exclude defaults", () => {
    ...
    instantiationService.stub(ISearchService, {
      fileSearch(query) {
        ...
        assert.strictEqual(
          query.folderQueries[0].excludePattern?.length,
          1
        );
        assert.deepStrictEqual(
          query.folderQueries[0].excludePattern[0].pattern,
          { filesExclude: true }
        );
        return Promise.resolve({ results: [], messages: [] });
      }
  ...
  test("disregard excludes", () => {
    ...
    instantiationService.stub(ISearchService, {
      fileSearch(query) {
        assert.deepStrictEqual(
          query.folderQueries[0].excludePattern,
          []
        );
  ...
  test("Valid revived URI after moving to EH", () => {
    ...
    instantiationService.stub(ISearchService, {
      fileSearch(query) {
        ...
        assert.strictEqual(
          query.folderQueries[0].folder.path,
          "/Users/username/Downloads"
        );
        assert.strictEqual(
          query.folderQueries[0].folder.scheme,
          "test"
        );
        return Promise.resolve({ results: [], messages: [] });
      }
