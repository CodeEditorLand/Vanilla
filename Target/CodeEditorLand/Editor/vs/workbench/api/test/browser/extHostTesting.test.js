...
import {
  ...
  TestItemImpl
} from "../../common/extHostTestItem.js";
import {
  ExtHostTesting,
  TestRunCoordinator,
  TestRunDto,
  TestRunProfileImpl
} from "../../common/extHostTesting.js";
...
const assertTreesEqual = (a, b) => {
  if (!a) {
    throw new assert.AssertionError({
      message: "Expected a to be defined",
      actual: a
    });
  }
  if (!b) {
    throw new assert.AssertionError({
      message: "Expected b to be defined",
      actual: b
    });
  }
  ...
  assert.strictEqual(
    aChildren.length,
    bChildren.length,
    `expected ${a.label}.children.length == ${b.label}.children.length`
  );
  aChildren.forEach(
    (key) => assertTreesEqual(
      a.children.get(key),
      b.children.get(key)
    )
  );
...
suite("ExtHost Testing", () => {
  ...
  setup(() => {
    resolveCalls = [];
    single = ds.add(
      new TestExtHostTestItemCollection("ctrlId", "root", {
        getDocument: () => void 0
      })
    );
    single.resolveHandler = (item) => {
      ...
      if (item === void 0) {
        ...
        item.children.add(
          new TestItemImpl("ctrlId", "id-aa", "aa", URI.file("/"))
        );
        item.children.add(
          new TestItemImpl("ctrlId", "id-ab", "ab", URI.file("/"))
        );
      }
    };
    ds.add(
      single.onDidGenerateDiff(
        (d) => single.setDiff(
          d
  ...
  suite("OwnedTestCollection", () => {
    test("adds a root recursively", async () => {
      await single.expand(single.root.id, Number.POSITIVE_INFINITY);
      ...
      assert.deepStrictEqual(single.collectDiff(), [
        {
          ...
          item: {
            controllerId: "ctrlId",
            expand: TestItemExpandState.BusyExpanding,
        ...
        },
        {
          ...
          item: {
            ...
            item: convert.TestItem.from(
              a.children.get("id-aa")
            )
          }
        },
        {
    ...
    test("parents are set correctly", () => {
      single.expand(single.root.id, Number.POSITIVE_INFINITY);
      single.collectDiff();
      const a = single.root.children.get("id-a");
    ...
    test("manages tags correctly", () => {
      ...
      a.children.replace([]);
      assert.deepStrictEqual(
        single.collectDiff().filter((t) => t.op === TestDiffOpType.RemoveTag),
    ...
    test("treats in-place replacement as mutation", () => {
      ...
      single.root.children.replace([
        ...
        new TestItemImpl(
          "ctrlId",
          "id-b",
          single.root.children.get("id-b").label,
          uri
        )
      ]);
      assert.deepStrictEqual(single.collectDiff(), [
    ...
    test("treats in-place replacement as mutation deeply", () => {
      ...
      single.root.children.replace([
        newA,
        single.root.children.get("id-b")
      ]);
      assert.deepStrictEqual(single.collectDiff(), [
        {
          op: TestDiffOpType.Update,
          item: {
            extId: TestId.fromExtHostTestItem(
              oldAB,
              "ctrlId"
            ).toString(),
            item: { label: "Hello world" }
    ...
    test("moves an item to be a new child", async () => {
      ...
      assert.deepStrictEqual(
        [...single.root.children].map(([_, item]) => item),
        [single.root.children.get("id-a")]
      );
  ...
  suite("TestRunTracker", () => {
    ...
    setup(async () => {
      ...
      configuration = new TestRunProfileImpl(
        mockObject()(),
        /* @__PURE__ */ new Map(),
        /* @__PURE__ */ new Set(),
        Event.None,
    ...
    test("tracks a run started from a main thread request", () => {
      const tracker = ds.add(
        c.prepareForMainThreadTestRun(
          ext,
          req,
          dto,
          configuration,
    ...
    test("tracks a run started from an extension request", () => {
      ...
      const task3Detached = c.createTestRun(
        ext,
        "ctrl",
        single,
        { ...req },
      ...
      assert.strictEqual(tracker.hasRunningTasks, true);
      task2.end();
      assert.deepStrictEqual(proxy.$finishedExtensionTestRun.args, [
        [tracker.id]
      ]);
      assert.strictEqual(tracker.hasRunningTasks, false);
      task3Detached.end();
    ...
    test("adds tests to run smartly", () => {
      ...
      task1.passed(
        single.root.children.get("id-a").children.get("id-aa")
      );
      expectedArgs.push([
        "ctrlId",
        tracker.id,
      ...
      assert.deepStrictEqual(proxy.$addTestsToRun.args, expectedArgs);
      task1.enqueued(
        single.root.children.get("id-a").children.get("id-ab")
      );
      expectedArgs.push([
        "ctrlId",
        tracker.id,
    ...
    test("adds test messages to run", () => {
      const test1 = new TestItemImpl(
        "ctrlId",
        "id-c",
        "test c",
        URI.file("/testc.txt")
      );
      const test2 = new TestItemImpl(
        "ctrlId",
        "id-d",
        "test d",
        URI.file("/testd.txt")
      );
      test1.range = test2.range = new Range(
        new Position(0, 0),
        new Position(1, 0)
      );
      ...
      message1.location = new Location(
        URI.file("/a.txt"),
        new Position(0, 0)
      );
        ...
        [
          {
            ...
            expected: void 0,
            actual: void 0,
            location: convert.location.from({
              uri: test2.uri,
              range: test2.range
            }),
            stackTrace: void 0
    ...
    test("sets state of test with identical local IDs (#131827)", () => {
      ...
      assert.deepStrictEqual(proxy.$addTestsToRun.args, [
        [
          "ctrl",
          tracker.id,
          [single.root, testA, childA].map(
            (t) => convert.TestItem.from(t)
          )
        ],
        [
          "ctrl",
          tracker.id,
          [single.root, testB, childB].map(
            (t) => convert.TestItem.from(t)
          )
        ]
      ]);
  ...
  suite("service", () => {
    ...
    setup(() => {
      ...
      ctrl = ds.add(
        new TestExtHostTesting(
          ...
          new ExtHostDocumentsAndEditors(
            rpcProtocol,
            new NullLogService()
          )
        )
    ...
    test("exposes active profiles correctly", async () => {
      ...
      const profAA = ds.add(
        ctrlA.createRunProfile("aa", TestRunProfileKind.Run, () => {
        })
      );
      const profAB = ds.add(
        ctrlA.createRunProfile("ab", TestRunProfileKind.Run, () => {
        })
      );
      const ctrlB = ds.add(ctrl.createTestController(extB, "b", "ctrlb"));
      const profBA = ds.add(
        ctrlB.createRunProfile("ba", TestRunProfileKind.Run, () => {
        })
      );
      const profBB = ds.add(
        ctrlB.createRunProfile("bb", TestRunProfileKind.Run, () => {
        })
      );
      ...
      const changeA = Event.toPromise(
        profAA.onDidChangeDefault
      );
      const changeBA = Event.toPromise(
        profBA.onDidChangeDefault
      );
      const changeBB = Event.toPromise(
        profBB.onDidChangeDefault
      );
      ...
      ctrl.$setDefaultRunProfiles({
        ...
        b: [
          ctrl.getProfileInternalId(ctrlB, profBA),
          ctrl.getProfileInternalId(ctrlB, profBB)
        ]
      });
