...
const simplify = /* @__PURE__ */ __name((item) => ({
  id: item.id,
  label: item.label,
  uri: item.uri,
  range: item.range
}), "simplify");
const assertTreesEqual = /* @__PURE__ */ __name((a, b) => {
  if (!a) {
...
suite("ExtHost Testing", () => {
  class TestExtHostTestItemCollection extends ExtHostTestItemCollection {
    static {
      __name(this, "TestExtHostTestItemCollection");
    }
    setDiff(diff) {
      this.diff = diff;
    }
  ...
  setup(() => {
    ...
    single = ds.add(new TestExtHostTestItemCollection("ctrlId", "root", {
      getDocument: /* @__PURE__ */ __name(() => void 0, "getDocument")
    }));
  ...
  suite("OwnedTestCollection", () => {
    ...
    suite("expandibility restoration", () => {
      const doReplace = /* @__PURE__ */ __name(async (canResolveChildren = true) => {
        const uri = single.root.children.get("id-a").uri;
        const newA = new TestItemImpl("ctrlId", "id-a", "Hello world", uri);
  ...
  suite("service", () => {
    ...
    class TestExtHostTesting extends ExtHostTesting {
      static {
        __name(this, "TestExtHostTesting");
      }
