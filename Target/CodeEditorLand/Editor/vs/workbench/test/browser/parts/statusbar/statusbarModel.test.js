...
suite("Workbench status bar model", () => {
  ...
  test("basics", () => {
    ...
    const model = disposables.add(
      new StatusbarViewModel(disposables.add(new TestStorageService()))
    );
    ...
    assert.strictEqual(
      model.getEntries(StatusbarAlignment.RIGHT).length,
      1
    );
  ...
  test("entry with reference to other entry (existing)", () => {
    ...
    let entry = {
      ...
      name: "3",
      priority: {
        primary: { id: "a", alignment: StatusbarAlignment.LEFT },
        secondary: 1
      },
      container,
      labelContainer: container,
    ...
    entry = {
      ...
      name: "3",
      priority: {
        primary: { id: "a", alignment: StatusbarAlignment.RIGHT },
        secondary: 1
      },
      container,
      labelContainer: container,
  ...
  test("entry with reference to other entry but different alignment does not explode", () => {
    ...
    const relativeEntryLeft = {
      ...
      name: "relative",
      priority: {
        primary: { id: "1-right", alignment: StatusbarAlignment.LEFT },
        secondary: 1
      },
      container,
      labelContainer: container,
    ...
    assert.strictEqual(
      model.getEntries(StatusbarAlignment.LEFT)[2],
      relativeEntryLeft
    );
    assert.strictEqual(
      model.getEntries(StatusbarAlignment.RIGHT).length,
