...
suite("Workbench editor utils", () => {
  class TestEditorInputWithPreferredResource extends TestEditorInput {
    ...
    static {
      __name(this, "TestEditorInputWithPreferredResource");
    }
  }
  ...
  test("EditorInputCapabilities", () => {
    ...
    const sideBySideInput = instantiationService.createInstance(SideBySideEditorInput, "name", void 0, testInput1, testInput2);
  ...
  test("EditorResourceAccessor - typed inputs", () => {
    ...
    const diffInput = instantiationService.createInstance(DiffEditorInput, "name", "description", untitled, file, void 0);
  ...
  async function testWhenEditorClosed(sideBySide, custom, ...resources) {
    ...
    for (const resource of resources) {
      if (custom) {
        await accessor2.editorService.openEditor(new TestFileEditorInput(resource, "testTypeId"), { pinned: true });
      } else if (sideBySide) {
        await accessor2.editorService.openEditor(instantiationService.createInstance(SideBySideEditorInput, "testSideBySideEditor", void 0, new TestFileEditorInput(resource, "testTypeId"), new TestFileEditorInput(resource, "testTypeId")), { pinned: true });
      } else {
        await accessor2.editorService.openEditor({ resource, options: { pinned: true } });
      }
    }
    const closedPromise = accessor2.instantitionService.invokeFunction((accessor3) => whenEditorClosed(accessor3, resources));
    accessor2.editorGroupService.activeGroup.closeAllEditors();
    await closedPromise;
  }
  __name(testWhenEditorClosed, "testWhenEditorClosed");
