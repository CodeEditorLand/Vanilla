...
suite("TextResourceEditorInput", () => {
  ...
  test("basics", async () => {
    ...
    const input = disposables.add(instantiationService.createInstance(TextResourceEditorInput, resource, "The Name", "The Description", void 0, void 0));
  ...
  test("preferred language (via ctor)", async () => {
    ...
    const resource = URI.from({ scheme: "inmemory", authority: null, path: "thePath" });
    accessor.modelService.createModel("function test() {}", accessor.languageService.createById(PLAINTEXT_LANGUAGE_ID), resource);
    const input = disposables.add(instantiationService.createInstance(TextResourceEditorInput, resource, "The Name", "The Description", "resource-input-test", void 0));
    const model = disposables.add(await input.resolve());
    assert.ok(model);
  ...
  test("preferred contents (via ctor)", async () => {
    const resource = URI.from({ scheme: "inmemory", authority: null, path: "thePath" });
    accessor.modelService.createModel("function test() {}", accessor.languageService.createById(PLAINTEXT_LANGUAGE_ID), resource);
    const input = disposables.add(instantiationService.createInstance(TextResourceEditorInput, resource, "The Name", "The Description", void 0, "My Resource Input Contents"));
    const model = disposables.add(await input.resolve());
    assert.ok(model);
