...
suite("QuickAccess", () => {
  ...
  let TestProviderDefault = class {
    ...
    provide(picker, token) {
      ...
      token.onCancellationRequested(
        () => providerDefaultCanceled = true
      );
      setTimeout(
        () => this.quickInputService.quickAccess.show(
          providerDescriptor3.prefix
        )
      );
  ...
  const providerDescriptorDefault = {
    ctor: TestProviderDefault,
    prefix: "",
    helpEntries: []
  };
  const providerDescriptor1 = {
    ctor: TestProvider1,
    prefix: "test",
    helpEntries: []
  };
  const providerDescriptor2 = {
    ctor: TestProvider2,
    prefix: "test something",
    helpEntries: []
  };
  const providerDescriptor3 = {
    ctor: TestProvider3,
    prefix: "changed",
    helpEntries: []
  };
  ...
  test("registry", () => {
    ...
    disposables2.add(
      registry.registerQuickAccessProvider(providerDescriptorDefault)
    );
    assert(
      registry.getQuickAccessProvider("") === providerDescriptorDefault
    );
    assert(
      registry.getQuickAccessProvider("test") === providerDescriptorDefault
    );
    const disposable = disposables2.add(
      registry.registerQuickAccessProvider(providerDescriptor1)
    );
  ...
  test("provider", async () => {
    ...
    disposables2.add(
      registry.registerQuickAccessProvider(providerDescriptor2)
    );
    disposables2.add(
      registry.registerQuickAccessProvider(providerDescriptor3)
    );
  ...
  test("quick pick access - show()", async () => {
    ...
    const disposables2 = new DisposableStore();
    disposables2.add(
      registry.registerQuickAccessProvider(fastProviderDescriptor)
    );
    disposables2.add(
      registry.registerQuickAccessProvider(slowProviderDescriptor)
    );
    disposables2.add(
      registry.registerQuickAccessProvider(fastAndSlowProviderDescriptor)
    );
  ...
  test("PickerEditorState can properly restore editors", async () => {
    ...
    const editorViewState = disposables.add(
    ...
    assert.strictEqual(
      part.activeGroup.activeEditor?.resource,
      input1.resource
    );
    assert.deepStrictEqual(
      part.activeGroup.getEditors(EditorsOrder.MOST_RECENTLY_ACTIVE).map((e) => e.resource),
      [input1.resource, input2.resource]
    );
    if (part.activeGroup.activeEditorPane?.getSelection) {
      assert.deepStrictEqual(
        part.activeGroup.activeEditorPane?.getSelection(),
        input1.options.selection
      );
    }
