...
suite("Untitled text editors", () => {
  ...
  test("basics", async () => {
    ...
    assert.ok(
      !accessor.untitledTextEditorService.isUntitledWithAssociatedResource(
        input1.resource
      )
    );
    ...
    assert.strictEqual(
      ...
      input1.getModel().resource.toString()
    );
    ...
    assert.strictEqual(
      await service.resolve({ untitledResource: input2.resource }),
      model
    );
    ...
    const resourcePromise = awaitDidChangeDirty(
      accessor.untitledTextEditorService
    );
  ...
  test("associated resource is dirty", async () => {
    ...
    let onDidChangeDirtyModel;
    disposables.add(
      service.onDidChangeDirty((model2) => {
  ...
  test("via create options", async () => {
    ...
    assert.strictEqual(
      snapshotToString(model2.createSnapshot()),
      "Hello World"
    );
    const input = disposables.add(
      instantiationService.createInstance(
  ...
  test("service#onDidChangeEncoding", async () => {
    ...
    let counter = 0;
    disposables.add(
      service.onDidChangeEncoding((model2) => {
        counter++;
        assert.strictEqual(
  ...
  test("service#onDidChangeLabel", async () => {
    ...
    let counter = 0;
    disposables.add(
      service.onDidChangeLabel((model2) => {
        counter++;
        assert.strictEqual(
          model2.resource.toString(),
          input.resource.toString()
  ...
  test("service#getValue", async () => {
    ...
    model1.dispose();
    assert.strictEqual(
      service.getValue(URI.parse("https://www.microsoft.com")),
      void 0
    );
  });
  test("model#onDidChangeContent", async () => {
    const service = accessor.untitledTextEditorService;
    const input = disposables.add(
      instantiationService.createInstance(
        UntitledTextEditorInput,
  ...
  test("model#onDidRevert and input disposed when reverted", async () => {
    const service = accessor.untitledTextEditorService;
    const input = disposables.add(
      instantiationService.createInstance(
        UntitledTextEditorInput,
  ...
  test("model#onDidChangeName and input name", async () => {
    const service = accessor.untitledTextEditorService;
    const input = disposables.add(
      instantiationService.createInstance(
        UntitledTextEditorInput,
    ...
    model.textEditorModel?.setValue(
      "123456789012345678901234567890123456789\u{1F31E}"
    );
    assert.strictEqual(
      input.getName(),
  ...
  test("model#onDidChangeDirty", async () => {
    const service = accessor.untitledTextEditorService;
    const input = disposables.add(
      instantiationService.createInstance(
        UntitledTextEditorInput,
  ...
  test("model#onDidChangeEncoding", async () => {
    const service = accessor.untitledTextEditorService;
    const input = disposables.add(
      instantiationService.createInstance(
        UntitledTextEditorInput,
    ...
    assert.strictEqual(
      counter,
      1,
      "Another change to same encoding does not fire event"
    );
  });
  test("canDispose with dirty model", async () => {
    const service = accessor.untitledTextEditorService;
    const input = disposables.add(
      instantiationService.createInstance(
        UntitledTextEditorInput,
    ...
    const model = disposables.add(await input.resolve());
    model.textEditorModel?.setValue("foo");
    const canDisposePromise = service.canDispose(
      model
    );
    ...
    const canDispose2 = service.canDispose(
      model
    );
