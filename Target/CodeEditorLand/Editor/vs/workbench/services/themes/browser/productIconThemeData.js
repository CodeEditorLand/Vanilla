...
class ProductIconThemeData {
  ...
  static STORAGE_KEY = "productIconThemeData";
  id;
  label;
  settingsId;
  description;
  ...
  watch;
  iconThemeDocument = {
    iconDefinitions: /* @__PURE__ */ new Map()
  };
  styleSheetContent;
  ...
  ensureLoaded(fileService, logService) {
    return this.isLoaded ? Promise.resolve(this.styleSheetContent) : this.load(fileService, logService);
  }
  ...
  static _defaultProductIconTheme = null;
      ...
      if (Array.isArray(iconDefinitions) && isObject(iconFontDefinitions)) {
        ...
        for (const entry of iconDefinitions) {
          ...
          if (isString(id) && isString(fontCharacter)) {
            if (isString(fontId)) {
              const iconFontDefinition = IconFontDefinition.fromJSONObject(
...
function _resolveIconDefinition(iconContribution, iconThemeDocument) {
  ...
  let definition = iconDefinitions.get(
    iconContribution.id
  );
