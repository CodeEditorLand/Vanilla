...
class FileIconThemeData {
  ...
  ensureLoaded(themeLoader) {
    return this.isLoaded ? Promise.resolve(this.styleSheetContent) : this.load(themeLoader);
  }
  reload(themeLoader) {
...
class FileIconThemeLoader {
  ...
  processIconThemeDocument(id, iconThemeDocumentLocation, iconThemeDocument) {
    ...
    if (Array.isArray(fonts)) {
      ...
      fonts.forEach((font) => {
        const src = font.src.map(
          (l) => `${asCSSUrl(resolvePath(l.path))} format('${l.format}')`
        ).join(", ");
        cssRules.push(
          `@font-face { src: ${src}; font-family: '${font.id}'; font-weight: ${font.weight}; font-style: ${font.style}; font-display: block; }`
        );
        if (font.size !== void 0 && font.size !== defaultFontSize) {
