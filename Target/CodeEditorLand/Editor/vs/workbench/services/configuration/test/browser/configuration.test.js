...
suite("DefaultConfiguration", () => {
    ...
    const environmentService = new BrowserWorkbenchEnvironmentService(
      ...
      ),
      {
        configurationDefaults: {
          "test.configurationDefaultsOverride": "envOverrideValue"
        }
      },
      TestProductService
    );
  ...
  test("configuration default overrides are read from cache", async () => {
    ...
    await configurationCache.write(
      cacheKey,
      JSON.stringify({
        "test.configurationDefaultsOverride": "overrideValue"
      })
    );
    const testObject = disposables.add(
      new DefaultConfiguration(
    ...
    assert.deepStrictEqual(
      actual.getValue("test.configurationDefaultsOverride"),
      "overrideValue"
    );
    assert.deepStrictEqual(
      testObject.configurationModel.getValue(
  ...
  test("configuration default overrides are written in cache", async () => {
    ...
    configurationRegistry.registerDefaultConfigurations([
      {
        overrides: {
          "test.configurationDefaultsOverride": "newoverrideValue"
        }
      }
    ]);
    await promise;
