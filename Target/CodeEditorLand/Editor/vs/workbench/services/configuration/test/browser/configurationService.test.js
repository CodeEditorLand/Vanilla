...
__name(convertToWorkspacePayload, "convertToWorkspacePayload");
...
suite("WorkspaceContextService - Folder", () => {
  ...
  setup(async () => {
    ...
    testObject = disposables.add(new WorkspaceService(
      ...
      disposables.add(new RemoteAgentService(
        ...
        environmentService,
        TestProductService,
        disposables.add(new RemoteAuthorityResolverService(false, void 0, void 0, void 0, TestProductService, logService)),
  ...
  test("getWorkspaceFolder() - queries in workspace folder", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    const fileSystemProvider = disposables.add(new InMemoryFileSystemProvider());
    disposables.add(fileService.registerProvider(ROOT.scheme, fileSystemProvider));
    const folder2 = joinPath(ROOT, folderName).with({ query: "myquery=1" });
    ...
    disposables.add(fileService.registerProvider(Schemas.vscodeUserData, disposables.add(new FileUserDataProvider(ROOT.scheme, fileSystemProvider, Schemas.vscodeUserData, userDataProfilesService, uriIdentityService, new NullLogService()))));
    const userDataProfileService = disposables.add(new UserDataProfileService(userDataProfilesService.defaultProfile));
    const testObject2 = disposables.add(new WorkspaceService(
      { configurationCache: new ConfigurationCache() },
      environmentService,
      userDataProfileService,
      userDataProfilesService,
      fileService,
      disposables.add(new RemoteAgentService(new RemoteSocketFactoryService(), userDataProfileService, environmentService, TestProductService, disposables.add(new RemoteAuthorityResolverService(false, void 0, void 0, void 0, TestProductService, logService)), new SignService(TestProductService), new NullLogService())),
      uriIdentityService,
      new NullLogService(),
      new NullPolicyService()
    ));
  ...
  test("getWorkspaceFolder() - queries in resource", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    await testObject2.initialize(convertToWorkspacePayload(folder2));
    const actual = testObject2.getWorkspaceFolder(joinPath(folder2, "a").with({ query: "myquery=1" }));
    assert.strictEqual(actual, testObject2.getWorkspace().folders[0]);
  }));
...
suite("WorkspaceConfigurationService - Folder", () => {
  ...
  test("policy settings when policy value is not set", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    assert.strictEqual(testObject.inspect("configurationService.folder.policySetting").policyValue, void 0);
  }));
  ...
  test("update user configuration", () => {
    return testObject.updateValue("configurationService.folder.testSetting", "value", ConfigurationTarget.USER).then(() => assert.strictEqual(testObject.getValue("configurationService.folder.testSetting"), "value"));
  });
  test("update workspace configuration", () => {
    return testObject.updateValue("tasks.service.testSetting", "value", ConfigurationTarget.WORKSPACE).then(() => assert.strictEqual(testObject.getValue(TasksSchemaProperties.ServiceTestSetting), "value"));
  });
  test("update resource configuration", () => {
    return testObject.updateValue("configurationService.folder.testSetting", "value", { resource: workspaceService.getWorkspace().folders[0].uri }, ConfigurationTarget.WORKSPACE_FOLDER).then(() => assert.strictEqual(testObject.getValue("configurationService.folder.testSetting"), "value"));
  });
  ...
  test("update application setting into workspace configuration in a workspace is not supported", () => {
    return testObject.updateValue("configurationService.folder.applicationSetting", "workspaceValue", {}, ConfigurationTarget.WORKSPACE, { donotNotifyError: true }).then(() => assert.fail("Should not be supported"), (e) => assert.strictEqual(e.code, ConfigurationEditingErrorCode.ERROR_INVALID_WORKSPACE_CONFIGURATION_APPLICATION));
  });
  test("update machine setting into workspace configuration in a workspace is not supported", () => {
    return testObject.updateValue("configurationService.folder.machineSetting", "workspaceValue", {}, ConfigurationTarget.WORKSPACE, { donotNotifyError: true }).then(() => assert.fail("Should not be supported"), (e) => assert.strictEqual(e.code, ConfigurationEditingErrorCode.ERROR_INVALID_WORKSPACE_CONFIGURATION_MACHINE));
  });
  test("update tasks configuration", () => {
    return testObject.updateValue("tasks", { "version": "1.0.0", tasks: [{ "taskName": "myTask" }] }, ConfigurationTarget.WORKSPACE).then(() => assert.deepStrictEqual(testObject.getValue(TasksSchemaProperties.Tasks), { "version": "1.0.0", tasks: [{ "taskName": "myTask" }] }));
  });
  test("update user configuration should trigger change event before promise is resolve", () => {
    const target = sinon.spy();
    disposables.add(testObject.onDidChangeConfiguration(target));
    return testObject.updateValue("configurationService.folder.testSetting", "value", ConfigurationTarget.USER).then(() => assert.ok(target.called));
  });
  test("update workspace configuration should trigger change event before promise is resolve", () => {
    const target = sinon.spy();
    disposables.add(testObject.onDidChangeConfiguration(target));
    return testObject.updateValue("configurationService.folder.testSetting", "value", ConfigurationTarget.WORKSPACE).then(() => assert.ok(target.called));
  });
  test("update memory configuration", () => {
    return testObject.updateValue("configurationService.folder.testSetting", "memoryValue", ConfigurationTarget.MEMORY).then(() => assert.strictEqual(testObject.getValue("configurationService.folder.testSetting"), "memoryValue"));
  });
  test("update memory configuration should trigger change event before promise is resolve", () => {
    const target = sinon.spy();
    disposables.add(testObject.onDidChangeConfiguration(target));
    return testObject.updateValue("configurationService.folder.testSetting", "memoryValue", ConfigurationTarget.MEMORY).then(() => assert.ok(target.called));
  });
  ...
  test("update user configuration to default value when target is not passed", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    assert.strictEqual(testObject.inspect("configurationService.folder.testSetting").userValue, void 0);
  }));
  ...
  test("update task configuration should trigger change event before promise is resolve", () => {
    const target = sinon.spy();
    disposables.add(testObject.onDidChangeConfiguration(target));
    return testObject.updateValue("tasks", { "version": "1.0.0", tasks: [{ "taskName": "myTask" }] }, ConfigurationTarget.WORKSPACE).then(() => assert.ok(target.called));
  });
  ...
  test("creating workspace settings", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    await new Promise((c, e) => {
      const disposable = testObject.onDidChangeConfiguration((e2) => {
        assert.ok(e2.affectsConfiguration("configurationService.folder.testSetting"));
        assert.strictEqual(testObject.getValue("configurationService.folder.testSetting"), "workspaceValue");
        disposable.dispose();
        c();
      });
  ...
  test("restricted setting is read from workspace when workspace is trusted", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    assert.strictEqual(testObject.restrictedSettings.userLocal, void 0);
    assert.strictEqual(testObject.restrictedSettings.userRemote, void 0);
...
suite("WorkspaceConfigurationService-Multiroot", () => {
  ...
  test("update application setting into workspace configuration in a workspace is not supported", () => {
    return testObject.updateValue("configurationService.workspace.applicationSetting", "workspaceValue", {}, ConfigurationTarget.WORKSPACE, { donotNotifyError: true }).then(() => assert.fail("Should not be supported"), (e) => assert.strictEqual(e.code, ConfigurationEditingErrorCode.ERROR_INVALID_WORKSPACE_CONFIGURATION_APPLICATION));
  });
  test("update machine setting into workspace configuration in a workspace is not supported", () => {
    return testObject.updateValue("configurationService.workspace.machineSetting", "workspaceValue", {}, ConfigurationTarget.WORKSPACE, { donotNotifyError: true }).then(() => assert.fail("Should not be supported"), (e) => assert.strictEqual(e.code, ConfigurationEditingErrorCode.ERROR_INVALID_WORKSPACE_CONFIGURATION_MACHINE));
  });
  test("update workspace folder configuration", () => {
    const workspace = workspaceContextService.getWorkspace();
    return testObject.updateValue("configurationService.workspace.testResourceSetting", "workspaceFolderValue", { resource: workspace.folders[0].uri }, ConfigurationTarget.WORKSPACE_FOLDER).then(() => assert.strictEqual(testObject.getValue("configurationService.workspace.testResourceSetting", { resource: workspace.folders[0].uri }), "workspaceFolderValue"));
  });
  ...
  test("remove setting from all targets", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    await testObject.updateValue(key, "workspaceValue", ConfigurationTarget.WORKSPACE);
    await testObject.updateValue(key, "userValue", ConfigurationTarget.USER);
    await testObject.updateValue(key, void 0, { resource: workspace.folders[0].uri });
    await testObject.reloadConfiguration();
  ...
  test("restricted setting is read from workspace folders when workspace is trusted", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    assert.strictEqual(testObject.restrictedSettings.workspaceFolder?.size, 1);
    assert.strictEqual(testObject.restrictedSettings.workspaceFolder?.get(testObject.getWorkspace().folders[0].uri), void 0);
  ...
  test("remove an unregistered setting", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    await testObject.reloadConfiguration();
    await testObject.updateValue(key, void 0, { resource: workspaceContextService.getWorkspace().folders[0].uri });
    let actual = testObject.inspect(key, { resource: workspaceContextService.getWorkspace().folders[0].uri });
    assert.strictEqual(actual.userValue, void 0);
    assert.strictEqual(actual.workspaceValue, void 0);
    assert.strictEqual(actual.workspaceFolderValue, void 0);
    await testObject.updateValue(key, void 0, { resource: workspaceContextService.getWorkspace().folders[1].uri });
...
suite("WorkspaceConfigurationService - Remote Folder", () => {
  ...
  setup(async () => {
    ...
    instantiationService = workbenchInstantiationService(void 0, disposables);
    environmentService = TestEnvironmentService;
    const remoteEnvironmentPromise = new Promise((c) => resolveRemoteEnvironment = /* @__PURE__ */ __name(() => c({ settingsPath: remoteSettingsResource }), "resolveRemoteEnvironment"));
    const remoteAgentService = instantiationService.stub(IRemoteAgentService, { getEnvironment: /* @__PURE__ */ __name(() => remoteEnvironmentPromise, "getEnvironment") });
    const configurationCache = { read: /* @__PURE__ */ __name(() => Promise.resolve(""), "read"), write: /* @__PURE__ */ __name(() => Promise.resolve(), "write"), remove: /* @__PURE__ */ __name(() => Promise.resolve(), "remove"), needsCaching: /* @__PURE__ */ __name(() => false, "needsCaching") };
    const uriIdentityService = disposables.add(new UriIdentityService(fileService));
    const userDataProfilesService = instantiationService.stub(IUserDataProfilesService, disposables.add(new UserDataProfilesService(environmentService, fileService, uriIdentityService, logService)));
    disposables.add(fileService.registerProvider(Schemas.vscodeUserData, disposables.add(new FileUserDataProvider(ROOT.scheme, fileSystemProvider, Schemas.vscodeUserData, userDataProfilesService, uriIdentityService, new NullLogService()))));
    userDataProfileService = instantiationService.stub(IUserDataProfileService, disposables.add(new UserDataProfileService(userDataProfilesService.defaultProfile)));
  ...
  __name(registerRemoteFileSystemProvider, "registerRemoteFileSystemProvider");
  ...
  __name(registerRemoteFileSystemProviderOnActivation, "registerRemoteFileSystemProviderOnActivation");
