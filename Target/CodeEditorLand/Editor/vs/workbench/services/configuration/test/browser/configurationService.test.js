...
} from "../../../keybinding/common/keybindingEditing.js";
...
suite("WorkspaceContextService - Folder", () => {
  ...
  setup(async () => {
    ...
    const userDataProfileService = disposables.add(
      ...
      new WorkspaceService(
        ...
        disposables.add(
          new RemoteAgentService(
            ...
            new SignService(TestProductService),
            new NullLogService()
          )
        ),
        uriIdentityService,
    ...
    await testObject.initialize(
      convertToWorkspacePayload(folder)
    );
  });
  test("getWorkspace()", () => {
  ...
  test("getWorkspaceFolder() - queries in workspace folder", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    const folder2 = joinPath(ROOT, folderName).with({
      query: "myquery=1"
    });
    await fileService.createFolder(folder2);
    const environmentService = TestEnvironmentService;
    const uriIdentityService = disposables.add(
      new UriIdentityService(fileService)
    );
    ...
    const testObject2 = disposables.add(
      new WorkspaceService(
        { configurationCache: new ConfigurationCache() },
        environmentService,
        userDataProfileService,
    ...
    await testObject2.initialize(
      convertToWorkspacePayload(folder2)
    );
    const actual = testObject2.getWorkspaceFolder(joinPath(folder2, "a"));
    assert.strictEqual(actual, testObject2.getWorkspace().folders[0]);
  }));
  test("getWorkspaceFolder() - queries in resource", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    const folder2 = joinPath(ROOT, folderName);
    await fileService.createFolder(folder2);
    const environmentService = TestEnvironmentService;
    const uriIdentityService = disposables.add(
      new UriIdentityService(fileService)
    ...
    await testObject2.initialize(convertToWorkspacePayload(folder2));
    const actual = testObject2.getWorkspaceFolder(
      joinPath(folder2, "a").with({ query: "myquery=1" })
    );
    assert.strictEqual(actual, testObject2.getWorkspace().folders[0]);
  }));
  ...
  test("isCurrentWorkspace() => false", () => {
    assert.ok(
      !testObject.isCurrentWorkspace(joinPath(dirname(folder), "abc"))
    );
  });
...
suite("WorkspaceContextService - Workspace", () => {
  ...
  setup(async () => {
    ...
    await fileService.writeFile(
      configResource,
      VSBuffer.fromString(JSON.stringify(workspace, null, "	"))
    );
    ...
    const environmentService = TestEnvironmentService;
    const remoteAgentService = disposables.add(
      disposables.add(
        instantiationService.createInstance(RemoteAgentService)
      )
    );
...
suite("WorkspaceContextService - Workspace Editing", () => {
  ...
  setup(async () => {
    ...
    instantiationService.stub(
      IJSONEditingService,
      instantiationService.createInstance(JSONEditingService)
    );
    testObject.acquireInstantiationService(instantiationService);
  });
  ...
  test("add folders (at specific index)", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    await testObject.addFolders(
      [{ uri: joinPath(ROOT, "d") }, { uri: joinPath(ROOT, "c") }],
      0
    );
    const actual = testObject.getWorkspace().folders;
    assert.strictEqual(actual.length, 4);
  ...
  test("add folders triggers change event", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    assert.strictEqual(
      target.callCount,
      2,
      `Should be called only once but called ${target.callCount} times`
    );
    const actual_1 = target.args[1][0];
    assert.deepStrictEqual(
      actual_1.added.map((r) => r.uri.toString()),
      addedFolders.map((a) => a.uri.toString())
    );
  ...
  test("remove folders", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    await testObject.removeFolders([
      testObject.getWorkspace().folders[0].uri
    ]);
    const actual = testObject.getWorkspace().folders;
  ...
  test("remove folders triggers change event", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    assert.deepStrictEqual(
      actual_1.removed.map((r) => r.uri.toString()),
      [removedFolder.uri.toString()]
    );
    assert.deepStrictEqual(
      actual_1.changed.map((c) => c.uri.toString()),
      [testObject.getWorkspace().folders[0].uri.toString()]
    );
  }));
  test("remove folders and add them back by writing into the file", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    const promise = new Promise((resolve, reject) => {
      disposables.add(
        testObject.onDidChangeWorkspaceFolders((actual) => {
          try {
            assert.deepStrictEqual(
              actual.added.map((r) => r.uri.toString()),
    ...
    const workspace = {
      folders: [
        ...
        { path: folders[1].uri.path }
      ]
    };
    await fileService.writeFile(
      testObject.getWorkspace().configuration,
      VSBuffer.fromString(JSON.stringify(workspace, null, "	"))
    );
    await promise;
  }));
  test("update folders (remove last and add to end)", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    const removedFolders = [testObject.getWorkspace().folders[1]].map(
      (f) => f.uri
    );
    ...
    assert.deepStrictEqual(
      actual_1.removed.map((r_1) => r_1.uri.toString()),
      removedFolders.map((a_1) => a_1.toString())
    );
    assert.deepStrictEqual(actual_1.changed, []);
  }));
  test("update folders (rename first via add and remove)", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    disposables.add(testObject.onDidChangeWorkspaceFolders(target));
    const addedFolders = [
      { uri: joinPath(ROOT, "a"), name: "The Folder" }
    ];
    const removedFolders = [testObject.getWorkspace().folders[0]].map(
      (f) => f.uri
    );
    ...
    assert.deepStrictEqual(actual_1.removed, []);
    assert.deepStrictEqual(
      actual_1.changed.map((r) => r.uri.toString()),
      removedFolders.map((a) => a.toString())
    );
  }));
  test("update folders (remove first and add to end)", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    const changedFolders = [testObject.getWorkspace().folders[1]].map(
      (f) => f.uri
    );
    await testObject.updateFolders(addedFolders, removedFolders);
    assert.strictEqual(
    ...
    assert.deepStrictEqual(
      actual_1.changed.map((r_2) => r_2.uri.toString()),
      changedFolders.map((a_2) => a_2.toString())
    );
  }));
  test("reorder folders trigger change event", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    assert.deepStrictEqual(
      actual_1.changed.map((c) => c.uri.toString()),
      testObject.getWorkspace().folders.map((f) => f.uri.toString()).reverse()
    );
  }));
...
suite("WorkspaceService - Initialization", () => {
  ...
  setup(async () => {
    ...
    userDataProfileService = instantiationService.stub(
  ...
  (isMacintosh ? test.skip : test)(
    ...
    () => runWithFakedTimers({ useFakeTimers: true }, async () => {
      await fileService.writeFile(
        ...
        VSBuffer.fromString(
          '{ "initialization.testSetting1": "userValue" }'
        )
      );
      await testObject.reloadConfiguration();
      const target = sinon.spy();
      ...
      assert.strictEqual(
        testObject.getValue("initialization.testSetting1"),
        "userValue"
      );
      assert.strictEqual(target.callCount, 4);
      assert.deepStrictEqual(target.args[0], [WorkbenchState.FOLDER]);
      assert.deepStrictEqual(target.args[1], [void 0]);
      assert.deepStrictEqual(
        target.args[3][0].added.map(
          (f) => f.uri.toString()
        ),
        [folder.toString()]
      );
      assert.deepStrictEqual(
        target.args[3][0].removed,
        []
      );
      assert.deepStrictEqual(
        target.args[3][0].changed,
        []
      );
    })
  );
  (isMacintosh ? test.skip : test)(
    ...
    () => runWithFakedTimers({ useFakeTimers: true }, async () => {
      ...
      await fileService.writeFile(
        ...
        VSBuffer.fromString(
          '{ "initialization.testSetting1": "workspaceValue" }'
        )
      );
      await testObject.initialize(convertToWorkspacePayload(folder));
      assert.strictEqual(
      ...
      assert.deepStrictEqual(
        [
          ...target.args[0][0].affectedKeys
        ],
        ["initialization.testSetting1"]
      );
      assert.deepStrictEqual(target.args[1], [WorkbenchState.FOLDER]);
      assert.deepStrictEqual(target.args[2], [void 0]);
      assert.deepStrictEqual(
        target.args[4][0].added.map(
          (f) => f.uri.toString()
        ),
        [folder.toString()]
      );
      assert.deepStrictEqual(
        target.args[4][0].removed,
        []
      );
      assert.deepStrictEqual(
        target.args[4][0].changed,
        []
      );
    })
  );
  (isMacintosh ? test.skip : test)(
    ...
    () => runWithFakedTimers({ useFakeTimers: true }, async () => {
      ...
      disposables.add(testObject.onDidChangeConfiguration(target));
      await testObject.initialize(
        getWorkspaceIdentifier(configResource)
      );
      assert.strictEqual(target.callCount, 4);
      assert.deepStrictEqual(target.args[0], [
      ...
      assert.deepStrictEqual(
        ...
        [
          joinPath(ROOT, "a").toString(),
          joinPath(ROOT, "b").toString()
        ]
      );
      assert.deepStrictEqual(
        target.args[3][0].removed,
  ...
  (isMacintosh ? test.skip : test)(
    ...
    () => runWithFakedTimers({ useFakeTimers: true }, async () => {
      ...
      disposables.add(testObject.onDidChangeConfiguration(target));
      await fileService.writeFile(
        joinPath(ROOT, "a", ".vscode", "settings.json"),
        VSBuffer.fromString(
          '{ "initialization.testSetting1": "workspaceValue1" }'
        )
      );
      await fileService.writeFile(
        joinPath(ROOT, "b", ".vscode", "settings.json"),
        VSBuffer.fromString(
          '{ "initialization.testSetting2": "workspaceValue2" }'
        )
      );
      await testObject.initialize(
        getWorkspaceIdentifier(configResource)
      ...
      assert.deepStrictEqual(target.args[1], [
        WorkbenchState.WORKSPACE
      ]);
      assert.deepStrictEqual(target.args[2], [void 0]);
      assert.deepStrictEqual(
  ...
  (isMacintosh ? test.skip : test)(
    ...
    () => runWithFakedTimers({ useFakeTimers: true }, async () => {
      await testObject.initialize(
        convertToWorkspacePayload(joinPath(ROOT, "a"))
      );
      await fileService.writeFile(
        userDataProfileService.currentProfile.settingsResource,
        VSBuffer.fromString(
      ...
      disposables.add(testObject.onDidChangeConfiguration(target));
      await testObject.initialize(
        convertToWorkspacePayload(joinPath(ROOT, "b"))
      );
      assert.strictEqual(
        testObject.getValue("initialization.testSetting1"),
        "userValue"
      ...
      assert.deepStrictEqual(
        target.args[1][0].added.map(
          (folder_1) => folder_1.uri.toString()
        ),
        [joinPath(ROOT, "b").toString()]
      );
      assert.deepStrictEqual(
        target.args[1][0].removed.map((folder_2) => folder_2.uri.toString()),
        [joinPath(ROOT, "a").toString()]
      );
      assert.deepStrictEqual(
        target.args[1][0].changed,
        []
      );
    })
  );
  (isMacintosh ? test.skip : test)(
    ...
    () => runWithFakedTimers({ useFakeTimers: true }, async () => {
      ...
      await fileService.writeFile(
        joinPath(ROOT, "b", ".vscode", "settings.json"),
        VSBuffer.fromString(
          '{ "initialization.testSetting1": "workspaceValue2" }'
        )
      );
      await testObject.initialize(
        convertToWorkspacePayload(joinPath(ROOT, "b"))
      ...
      assert.deepStrictEqual(
        target.args[2][0].added.map(
          (folder_1) => folder_1.uri.toString()
        ),
        [joinPath(ROOT, "b").toString()]
      );
      assert.deepStrictEqual(
        target.args[2][0].removed.map((folder_2) => folder_2.uri.toString()),
        [joinPath(ROOT, "a").toString()]
      );
      assert.deepStrictEqual(
        target.args[2][0].changed,
        []
      );
    })
  );
...
suite("WorkspaceConfigurationService - Folder", () => {
  ...
  setup(async () => {
    ...
    workspaceService = testObject = disposables.add(
      new WorkspaceService(
        { configurationCache: new ConfigurationCache() },
        environmentService,
        userDataProfileService,
    ...
    instantiationService.stub(
      IKeybindingEditingService,
      disposables.add(
        instantiationService.createInstance(KeybindingsEditingService)
      )
    );
    instantiationService.stub(
      ITextFileService,
  ...
  test("globals override defaults", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    await testObject.reloadConfiguration();
    assert.strictEqual(
      testObject.getValue("configurationService.folder.testSetting"),
      "userValue"
    );
  }));
  test("globals", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    await testObject.reloadConfiguration();
    assert.strictEqual(
      testObject.getValue("testworkbench.editor.tabs"),
      true
    );
  }));
  test("workspace settings", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    await fileService.writeFile(
      joinPath(
        workspaceService.getWorkspace().folders[0].uri,
        ".vscode",
        "settings.json"
      ),
      VSBuffer.fromString('{ "testworkbench.editor.icons": true }')
    );
    await testObject.reloadConfiguration();
    assert.strictEqual(
      testObject.getValue("testworkbench.editor.icons"),
      true
    );
  }));
  ...
  test("machine overridable settings override user Settings", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    await fileService.writeFile(
      userDataProfileService.currentProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.folder.machineOverridableSetting": "userValue" }'
      )
    );
    await fileService.writeFile(
      joinPath(
      ...
      VSBuffer.fromString(
        '{ "configurationService.folder.machineOverridableSetting": "workspaceValue" }'
      )
    );
    await testObject.reloadConfiguration();
    assert.strictEqual(
  ...
  test("workspace settings override user settings after defaults are registered ", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    assert.strictEqual(
      testObject.getValue("configurationService.folder.newSetting"),
      "workspaceValue"
    );
  }));
  test("machine overridable settings override user settings after defaults are registered ", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    await fileService.writeFile(
      userDataProfileService.currentProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.folder.newMachineOverridableSetting": "userValue" }'
      )
    );
    await fileService.writeFile(
      joinPath(
      ...
      VSBuffer.fromString(
        '{ "configurationService.folder.newMachineOverridableSetting": "workspaceValue" }'
      )
    );
    await testObject.reloadConfiguration();
    configurationRegistry.registerConfiguration({
  ...
  test("application settings are not read from workspace", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    await fileService.writeFile(
      userDataProfileService.currentProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.folder.applicationSetting": "userValue" }'
      )
    );
    await fileService.writeFile(
      joinPath(
      ...
      VSBuffer.fromString(
        '{ "configurationService.folder.applicationSetting": "workspaceValue" }'
      )
    );
    await testObject.reloadConfiguration();
    assert.strictEqual(
  ...
  test("machine settings are not read from workspace", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    await fileService.writeFile(
      userDataProfileService.currentProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.folder.machineSetting": "userValue" }'
      )
    );
    await fileService.writeFile(
      joinPath(
      ...
      VSBuffer.fromString(
        '{ "configurationService.folder.machineSetting": "workspaceValue" }'
      )
    );
    await testObject.reloadConfiguration();
    assert.strictEqual(
  ...
  test("get application scope settings are not loaded after defaults are registered", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    await fileService.writeFile(
      userDataProfileService.currentProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.folder.applicationSetting-2": "userValue" }'
      )
    );
    await fileService.writeFile(
      joinPath(
      ...
      VSBuffer.fromString(
        '{ "configurationService.folder.applicationSetting-2": "workspaceValue" }'
      )
    );
    await testObject.reloadConfiguration();
    assert.strictEqual(
  ...
  test("get application scope settings are not loaded after defaults are registered when workspace folder uri is passed", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    await fileService.writeFile(
      userDataProfileService.currentProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.folder.applicationSetting-3": "userValue" }'
      )
    );
    await fileService.writeFile(
      joinPath(
      ...
      VSBuffer.fromString(
        '{ "configurationService.folder.applicationSetting-3": "workspaceValue" }'
      )
    );
    await testObject.reloadConfiguration();
    assert.strictEqual(
  ...
  test("get machine scope settings are not loaded after defaults are registered", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    await fileService.writeFile(
      userDataProfileService.currentProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.folder.machineSetting-2": "userValue" }'
      )
    );
    await fileService.writeFile(
      joinPath(
      ...
      VSBuffer.fromString(
        '{ "configurationService.folder.machineSetting-2": "workspaceValue" }'
      )
    );
    await testObject.reloadConfiguration();
    assert.strictEqual(
  ...
  test("get machine scope settings are not loaded after defaults are registered when workspace folder uri is passed", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    await fileService.writeFile(
      userDataProfileService.currentProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.folder.machineSetting-3": "userValue" }'
      )
    );
    await fileService.writeFile(
      joinPath(
      ...
      VSBuffer.fromString(
        '{ "configurationService.folder.machineSetting-3": "workspaceValue" }'
      )
    );
    await testObject.reloadConfiguration();
    assert.strictEqual(
  ...
  test("policy value override all", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    const result = await runWithFakedTimers(
      { useFakeTimers: true },
      async () => {
        ...
        await fileService.writeFile(
          environmentService.policyFile,
          VSBuffer.fromString(
            '{ "configurationService.folder.policySetting": "policyValue" }'
          )
        );
        return promise;
      }
    ...
    assert.strictEqual(
      testObject.inspect("configurationService.folder.policySetting").policyValue,
      "policyValue"
    );
  }));
  test("policy settings when policy value is not set", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    await fileService.writeFile(
      userDataProfileService.currentProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.folder.policySetting": "userValue" }'
      )
    );
    await fileService.writeFile(
      joinPath(
      ...
      VSBuffer.fromString(
        '{ "configurationService.folder.policySetting": "workspaceValue" }'
      )
    );
    await testObject.reloadConfiguration();
    assert.strictEqual(
  ...
  test("inspect", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    assert.strictEqual(actual.defaultValue, void 0);
    assert.strictEqual(actual.application, void 0);
    assert.strictEqual(actual.userValue, void 0);
    assert.strictEqual(actual.workspaceValue, void 0);
    assert.strictEqual(actual.workspaceFolderValue, void 0);
    assert.strictEqual(actual.value, void 0);
    actual = testObject.inspect(
      "configurationService.folder.testSetting"
    );
    ...
    await fileService.writeFile(
      ...
      VSBuffer.fromString(
        '{ "configurationService.tasks.testSetting": "tasksValue" }'
      )
    );
    await testObject.reloadConfiguration();
  ...
  test("inspect restricted settings", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    await fileService.writeFile(
      userDataProfileService.currentProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.folder.restrictedSetting": "userRestrictedValue" }'
      )
    );
    await testObject.reloadConfiguration();
    let actual = testObject.inspect(
      "configurationService.folder.restrictedSetting"
    );
    assert.strictEqual(actual.defaultValue, "isSet");
    assert.strictEqual(actual.application, void 0);
    ...
    await fileService.writeFile(
      ...
      VSBuffer.fromString(
        '{ "configurationService.folder.restrictedSetting": "workspaceRestrictedValue" }'
      )
    );
    await testObject.reloadConfiguration();
    actual = testObject.inspect(
  ...
  test("inspect restricted settings after change", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    const actual = testObject.inspect(
      "configurationService.folder.restrictedSetting"
    );
    assert.strictEqual(actual.defaultValue, "isSet");
    assert.strictEqual(actual.application, void 0);
  ...
  test("update workspace configuration", () => {
    return testObject.updateValue(
      ...
      () => assert.strictEqual(
        testObject.getValue(
          TasksSchemaProperties.ServiceTestSetting
        ),
        "value"
      )
    );
  ...
  test("update language configuration for multiple languages", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    await testObject.updateValue(
      "configurationService.folder.languageSetting",
      "multiLangValue",
      { overrideIdentifiers: ["xyzlang", "deflang"] },
      ConfigurationTarget.USER
    );
    assert.strictEqual(
      testObject.getValue(
    ...
    assert.deepStrictEqual(
      testObject.getValue(
        keyFromOverrideIdentifiers(["deflang", "xyzlang"])
      ),
      {
        "configurationService.folder.languageSetting": "multiLangValue"
      }
    );
  }));
  test("update language configuration for multiple languages when already set", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    await fileService.writeFile(
      userDataProfileService.currentProfile.settingsResource,
      VSBuffer.fromString(
        '{ "[deflang][xyzlang]": { "configurationService.folder.languageSetting": "userValue" }}'
      )
    );
    await testObject.updateValue(
      "configurationService.folder.languageSetting",
    ...
    const actualContent = (await fileService.readFile(
      userDataProfileService.currentProfile.settingsResource
    )).value.toString();
  ...
  test("update tasks configuration", () => {
    return testObject.updateValue(
      "tasks",
      { version: "1.0.0", tasks: [{ taskName: "myTask" }] },
      ConfigurationTarget.WORKSPACE
    ).then(
      () => assert.deepStrictEqual(
        testObject.getValue(TasksSchemaProperties.Tasks),
        { version: "1.0.0", tasks: [{ taskName: "myTask" }] }
      )
    );
  });
  ...
  test("remove setting from all targets", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    await testObject.updateValue(key, void 0);
    await testObject.reloadConfiguration();
  ...
  test("update user configuration to default value when target is not passed", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    assert.strictEqual(
      testObject.inspect("configurationService.folder.testSetting").userValue,
      void 0
    );
  }));
  ...
  test("change event when there are global tasks", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    await fileService.writeFile(
      joinPath(environmentService.userRoamingDataHome, "tasks.json"),
      VSBuffer.fromString(
        '{ "version": "1.0.0", "tasks": [{ "taskName": "myTask" }'
      )
    );
  ...
  test("creating workspace settings", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    await new Promise((c, e) => {
      const disposable = testObject.onDidChangeConfiguration((e2) => {
        assert.ok(
          e2.affectsConfiguration(
            "configurationService.folder.testSetting"
          )
        );
        assert.strictEqual(
  ...
  test("deleting workspace settings", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    const workspaceSettingsResource = joinPath(
      workspaceService.getWorkspace().folders[0].uri,
      ".vscode",
      "settings.json"
    );
    ...
    await testObject.reloadConfiguration();
    const e = await new Promise((c, e2) => {
      Event.once(testObject.onDidChangeConfiguration)(c);
      fileService.del(workspaceSettingsResource).catch(e2);
    });
    assert.ok(
  ...
  test("restricted setting is read from workspace when workspace is trusted", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    await fileService.writeFile(
      userDataProfileService.currentProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.folder.restrictedSetting": "userValue" }'
      )
    );
    await fileService.writeFile(
      joinPath(
      ...
      VSBuffer.fromString(
        '{ "configurationService.folder.restrictedSetting": "workspaceValue" }'
      )
    );
    await testObject.reloadConfiguration();
    assert.strictEqual(
    ...
    assert.strictEqual(
      testObject.restrictedSettings.userLocal,
      void 0
    );
    assert.strictEqual(
      testObject.restrictedSettings.userRemote,
      void 0
    );
    ...
    assert.strictEqual(
      testObject.restrictedSettings.workspaceFolder?.size,
      1
    );
    assert.deepStrictEqual(
      testObject.restrictedSettings.workspaceFolder?.get(
        workspaceService.getWorkspace().folders[0].uri
      ),
      ["configurationService.folder.restrictedSetting"]
    );
  ...
  test("adding an restricted setting triggers change event", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    testObject.updateWorkspaceTrust(false);
    const promise = Event.toPromise(
      testObject.onDidChangeRestrictedSettings
    );
    await fileService.writeFile(
      joinPath(
        workspaceService.getWorkspace().folders[0].uri,
  ...
  test("remove an unregistered setting", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    await fileService.writeFile(
      userDataProfileService.currentProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.folder.unknownSetting": "userValue" }'
      )
    );
    await fileService.writeFile(
      joinPath(
      ...
      VSBuffer.fromString(
        '{ "configurationService.folder.unknownSetting": "workspaceValue" }'
      )
    );
    await testObject.reloadConfiguration();
    await testObject.updateValue(key, void 0);
...
suite("WorkspaceConfigurationService - Profiles", () => {
  ...
  setup(async () => {
      ...
      disposables.add(
        new UserDataProfileService(
          toUserDataProfile(
            ...
            joinPath(environmentService.cacheHome, "profilesCache")
          )
        )
      )
    );
    ...
    await fileService.writeFile(
      ...
      VSBuffer.fromString(
        '{ "configurationService.profiles.applicationSetting2": "applicationValue", "configurationService.profiles.testSetting2": "userValue" }'
      )
    );
    await fileService.writeFile(
      userDataProfileService.currentProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.profiles.applicationSetting2": "profileValue", "configurationService.profiles.testSetting2": "profileValue" }'
      )
    );
    await workspaceService.initialize(convertToWorkspacePayload(folder));
    instantiationService.stub(
  ...
  test("inspect", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    assert.strictEqual(actual.value, "isSet");
    await fileService.writeFile(
      instantiationService.get(IUserDataProfilesService).defaultProfile.settingsResource,
      VSBuffer.fromString(
    ...
    await fileService.writeFile(
      userDataProfileService.currentProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.profiles.applicationSetting": "profileValue" }'
      )
    );
    await testObject.reloadConfiguration();
    actual = testObject.inspect(
    ...
    await fileService.writeFile(
      instantiationService.get(IUserDataProfilesService).defaultProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.profiles.testSetting": "applicationValue" }'
      )
    );
    await fileService.writeFile(
      userDataProfileService.currentProfile.settingsResource,
    ...
    assert.strictEqual(actual.defaultValue, "isSet");
    assert.strictEqual(actual.applicationValue, void 0);
    assert.strictEqual(actual.userValue, "profileValue");
    assert.strictEqual(actual.workspaceValue, void 0);
    assert.strictEqual(actual.workspaceFolderValue, void 0);
  ...
  test("registering normal setting after init", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    await fileService.writeFile(
      instantiationService.get(IUserDataProfilesService).defaultProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.profiles.testSetting3": "defaultProfile" }'
      )
    );
    await testObject.reloadConfiguration();
    configurationRegistry.registerConfiguration({
  ...
  test("registering application scope setting after init", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    await fileService.writeFile(
      instantiationService.get(IUserDataProfilesService).defaultProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.profiles.applicationSetting3": "defaultProfile" }'
      )
    );
    await testObject.reloadConfiguration();
    configurationRegistry.registerConfiguration({
  ...
  test("setting applied to all profiles is registered later", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    await fileService.writeFile(
      instantiationService.get(IUserDataProfilesService).defaultProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.profiles.testSetting4": "userValue" }'
      )
    );
    await fileService.writeFile(
      userDataProfileService.currentProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.profiles.testSetting4": "profileValue" }'
      )
    );
    await testObject.updateValue(
      APPLY_ALL_PROFILES_SETTING,
  ...
  test("test isSettingAppliedToAllProfiles", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    assert.strictEqual(
      testObject.isSettingAppliedForAllProfiles(
        "configurationService.profiles.applicationSetting2"
      ),
      true
    );
  ...
  test("switch to default profile", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    await fileService.writeFile(
      instantiationService.get(IUserDataProfilesService).defaultProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.profiles.applicationSetting": "applicationValue", "configurationService.profiles.testSetting": "userValue" }'
      )
    );
    await fileService.writeFile(
      userDataProfileService.currentProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.profiles.applicationSetting": "profileValue", "configurationService.profiles.testSetting": "profileValue" }'
      )
    );
    await testObject.reloadConfiguration();
    const promise = Event.toPromise(
    ...
    await userDataProfileService.updateCurrentProfile(
      instantiationService.get(IUserDataProfilesService).defaultProfile
    );
    const changeEvent = await promise;
    assert.deepStrictEqual(
      [...changeEvent.affectedKeys],
  ...
  test("switch to non default profile", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    await fileService.writeFile(
      ...
      VSBuffer.fromString(
        '{ "configurationService.profiles.applicationSetting": "profileValue2", "configurationService.profiles.testSetting": "profileValue2" }'
      )
    );
    const promise = Event.toPromise(
      testObject.onDidChangeConfiguration
  ...
  test("switch to non default profile using settings from default profile", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    await fileService.writeFile(
      profile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.profiles.applicationSetting": "applicationValue2", "configurationService.profiles.testSetting": "profileValue2" }'
      )
    );
    const promise = Event.toPromise(
      testObject.onDidChangeConfiguration
  ...
  test("In non-default profile, changing application settings shall include only application scope settings in the change event", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    await fileService.writeFile(
      instantiationService.get(IUserDataProfilesService).defaultProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.profiles.applicationSetting": "applicationValue", "configurationService.profiles.testSetting": "applicationValue" }'
      )
    );
    const changeEvent = await promise;
    assert.deepStrictEqual(
  ...
  test("switch to non default profile with settings applied to all profiles", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    await fileService.writeFile(
      profile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.profiles.testSetting": "profileValue", "configurationService.profiles.testSetting2": "profileValue2" }'
      )
    );
    const promise = Event.toPromise(
      testObject.onDidChangeConfiguration
...
suite("WorkspaceConfigurationService-Multiroot", () => {
  ...
  test("application settings are not read from workspace", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    await jsonEditingServce.write(
      workspaceContextService.getWorkspace().configuration,
      [
        {
          path: ["settings"],
          value: {
            "configurationService.workspace.applicationSetting": "workspaceValue"
          }
        }
      ],
      true
      ...
      [
        {
          path: ["settings"],
          value: {
            "configurationService.workspace.machineSetting": "workspaceValue"
          }
        }
      ],
      true
      ...
      [
        {
          path: ["settings"],
          value: {
            "configurationService.workspace.newSetting": "workspaceValue"
          }
        }
      ],
      true
  ...
  test("get application scope settings are not loaded after defaults are registered when workspace folder is passed", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    await fileService.writeFile(
      userDataProfileService.currentProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.workspace.newSetting-2": "userValue" }'
      )
    );
    await jsonEditingServce.write(
      workspaceContextService.getWorkspace().configuration,
      [
        {
          path: ["settings"],
          value: {
            "configurationService.workspace.newSetting-2": "workspaceValue"
          }
        }
      ],
      true
  ...
  test("application settings are not read from workspace folder", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    await fileService.writeFile(
      workspaceContextService.getWorkspace().folders[0].toResource(".vscode/settings.json"),
      VSBuffer.fromString(
        '{ "configurationService.workspace.applicationSetting": "workspaceFolderValue" }'
      )
    );
    await testObject.reloadConfiguration();
    assert.strictEqual(
  ...
  test("machine settings are not read from workspace folder", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    await fileService.writeFile(
      workspaceContextService.getWorkspace().folders[0].toResource(".vscode/settings.json"),
      VSBuffer.fromString(
        '{ "configurationService.workspace.machineSetting": "workspaceFolderValue" }'
      )
    );
    await testObject.reloadConfiguration();
    assert.strictEqual(
  ...
  test("application settings are not read from workspace folder after defaults are registered", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    await fileService.writeFile(
      userDataProfileService.currentProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.workspace.testNewApplicationSetting": "userValue" }'
      )
    );
    await fileService.writeFile(
      workspaceContextService.getWorkspace().folders[0].toResource(".vscode/settings.json"),
      VSBuffer.fromString(
        '{ "configurationService.workspace.testNewApplicationSetting": "workspaceFolderValue" }'
      )
    );
    await testObject.reloadConfiguration();
    assert.strictEqual(
  ...
  test("machine settings are not read from workspace folder after defaults are registered", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    await fileService.writeFile(
      userDataProfileService.currentProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.workspace.testNewMachineSetting": "userValue" }'
      )
    );
    await fileService.writeFile(
      workspaceContextService.getWorkspace().folders[0].toResource(".vscode/settings.json"),
      VSBuffer.fromString(
        '{ "configurationService.workspace.testNewMachineSetting": "workspaceFolderValue" }'
      )
    );
    await testObject.reloadConfiguration();
    assert.strictEqual(
      ...
      [
        {
          path: ["settings"],
          value: {
            "configurationService.workspace.testNewResourceSetting2": "workspaceValue"
          }
        }
      ],
      true
  ...
  test("resource language setting in folder is read after it is registered later", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    await fileService.writeFile(
      workspaceContextService.getWorkspace().folders[0].toResource(".vscode/settings.json"),
      VSBuffer.fromString(
        '{ "configurationService.workspace.testNewResourceLanguageSetting2": "workspaceFolderValue" }'
      )
    );
    await jsonEditingServce.write(
      workspaceContextService.getWorkspace().configuration,
      ...
      [
        {
          path: ["settings"],
          value: {
            "configurationService.workspace.testResourceSetting": "workspaceValue"
          }
        }
      ],
      true
  ...
  test("inspect restricted settings", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    await fileService.writeFile(
      workspaceContextService.getWorkspace().folders[0].toResource(".vscode/settings.json"),
      VSBuffer.fromString(
        '{ "configurationService.workspace.testRestrictedSetting1": "workspaceFolderRestrictedValue" }'
      )
    );
    await testObject.reloadConfiguration();
    actual = testObject.inspect(
    ...
    assert.strictEqual(
      actual.workspaceFolderValue,
      "workspaceFolderRestrictedValue"
    );
    assert.strictEqual(actual.value, "isSet");
    testObject.updateWorkspaceTrust(true);
    await testObject.reloadConfiguration();
  ...
  test("inspect restricted settings after change", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    await fileService.writeFile(
      userDataProfileService.currentProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.workspace.testRestrictedSetting1": "userRestrictedValue" }'
      )
    );
    await testObject.reloadConfiguration();
  ...
  test("get launch configuration", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    await jsonEditingServce.write(
      workspaceContextService.getWorkspace().configuration,
      [{ path: ["launch"], value: expectedLaunchConfiguration }],
      true
    );
    await testObject.reloadConfiguration();
  ...
  test("get tasks configuration", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    await jsonEditingServce.write(
      workspaceContextService.getWorkspace().configuration,
      [{ path: ["tasks"], value: expectedTasksConfiguration }],
      true
    );
    await testObject.reloadConfiguration();
  ...
  test("update launch configuration in a workspace", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    await testObject.updateValue(
      "launch",
      { version: "1.0.0", configurations: [{ name: "myLaunch" }] },
      { resource: workspace.folders[0].uri },
      ConfigurationTarget.WORKSPACE,
      { donotNotifyError: true }
    );
  ...
  test("configuration of newly added folder is available on configuration change event", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    await fileService.writeFile(
      joinPath(uri, ".vscode", "settings.json"),
      VSBuffer.fromString(
        '{ "configurationService.workspace.testResourceSetting": "workspaceFolderValue" }'
      )
    );
  ...
  test("restricted setting is read from workspace folders when workspace is trusted", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    await fileService.writeFile(
      userDataProfileService.currentProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.workspace.testRestrictedSetting1": "userValue", "configurationService.workspace.testRestrictedSetting2": "userValue" }'
      )
    );
    await jsonEditingServce.write(
      workspaceContextService.getWorkspace().configuration,
      [
        {
          path: ["settings"],
          value: {
            "configurationService.workspace.testRestrictedSetting1": "workspaceValue"
          }
        }
      ],
      true
    ...
    await fileService.writeFile(
      joinPath(
        testObject.getWorkspace().folders[1].uri,
        ".vscode",
        "settings.json"
      ),
      VSBuffer.fromString(
        '{ "configurationService.workspace.testRestrictedSetting2": "workspaceFolder2Value" }'
      )
    );
    await testObject.reloadConfiguration();
    assert.strictEqual(
  ...
  test("remove an unregistered setting", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    await fileService.writeFile(
      userDataProfileService.currentProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.workspace.unknownSetting": "userValue" }'
      )
    );
    await jsonEditingServce.write(
      workspaceContextService.getWorkspace().configuration,
      [
        {
          path: ["settings"],
          value: {
            "configurationService.workspace.unknownSetting": "workspaceValue"
          }
        }
      ],
      true
    ...
    await fileService.writeFile(
      ...
      VSBuffer.fromString(
        '{ "configurationService.workspace.unknownSetting": "workspaceFolderValue1" }'
      )
    );
    await fileService.writeFile(
      joinPath(
        workspaceContextService.getWorkspace().folders[1].uri,
        ".vscode",
        "settings.json"
      ),
      VSBuffer.fromString(
        '{ "configurationService.workspace.unknownSetting": "workspaceFolderValue2" }'
      )
    );
    await testObject.reloadConfiguration();
    await testObject.updateValue(key, void 0, {
...
suite("WorkspaceConfigurationService - Remote Folder", () => {
  ...
  setup(async () => {
    ...
    environmentService = TestEnvironmentService;
    const remoteEnvironmentPromise = new Promise(
      (c) => resolveRemoteEnvironment = () => c({ settingsPath: remoteSettingsResource })
    );
    const remoteAgentService = instantiationService.stub(
      IRemoteAgentService,
      {
        getEnvironment: () => remoteEnvironmentPromise
      }
    );
    ...
    testObject = disposables.add(
      new WorkspaceService(
        { configurationCache, remoteAuthority },
        environmentService,
        userDataProfileService,
        userDataProfilesService,
        fileService,
  ...
  function registerRemoteFileSystemProvider() {
    disposables.add(
      instantiationService.get(IFileService).registerProvider(
  ...
  function registerRemoteFileSystemProviderOnActivation() {
    const disposable = disposables.add(
      instantiationService.get(IFileService).onWillActivateFileSystemProvider((e) => {
  ...
  test("remote settings override globals", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    await fileService.writeFile(
      ...
      VSBuffer.fromString(
        '{ "configurationService.remote.machineSetting": "remoteValue" }'
      )
    );
    registerRemoteFileSystemProvider();
  ...
  test("machine settings in local user settings does not override defaults", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    await fileService.writeFile(
      userDataProfileService.currentProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.remote.machineSetting": "globalValue" }'
      )
    );
    registerRemoteFileSystemProvider();
    resolveRemoteEnvironment();
  ...
  test("machine overridable settings in local user settings does not override defaults", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    await fileService.writeFile(
      userDataProfileService.currentProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.remote.machineOverridableSetting": "globalValue" }'
      )
    );
    registerRemoteFileSystemProvider();
    resolveRemoteEnvironment();
  ...
  test("machine setting is written in remote settings", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    await testObject.reloadConfiguration();
    assert.strictEqual(
      testObject.inspect("configurationService.remote.machineSetting").userRemoteValue,
      "machineValue"
    );
  }));
  ...
  test("machine settings in local user settings does not override defaults after defalts are registered ", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    await fileService.writeFile(
      userDataProfileService.currentProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.remote.newMachineSetting": "userValue" }'
      )
    );
    registerRemoteFileSystemProvider();
    resolveRemoteEnvironment();
  ...
  test("machine overridable settings in local user settings does not override defaults after defaults are registered ", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    await fileService.writeFile(
      userDataProfileService.currentProfile.settingsResource,
      VSBuffer.fromString(
        '{ "configurationService.remote.newMachineOverridableSetting": "userValue" }'
      )
    );
    registerRemoteFileSystemProvider();
    resolveRemoteEnvironment();
...
function getWorkspaceId(configPath) {
  ...
  if (!isLinux) {
    workspaceConfigPath = workspaceConfigPath.toLowerCase();
  }
