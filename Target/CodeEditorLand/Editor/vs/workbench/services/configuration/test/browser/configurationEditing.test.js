...
suite("ConfigurationEditing", () => {
  ...
  setup(async () => {
    ...
    environmentService.policyFile = joinPath(
      workspaceFolder,
      "policies.json"
    );
    ...
    workspaceService = disposables.add(
    ...
    await workspaceService.initialize(
      getSingleFolderWorkspaceIdentifier(workspaceFolder)
    );
    ...
    testObject = instantiationService.createInstance(
      ConfigurationEditing,
      null
    );
  });
  test("errors cases - invalid key", async () => {
    try {
      await testObject.writeConfiguration(
        EditableConfigurationTarget.WORKSPACE,
        { key: "unknown.key", value: "value" },
  ...
  test("errors cases - invalid global tasks configuration", async () => {
    const resource = joinPath(
      ...
      USER_STANDALONE_CONFIGURATIONS["tasks"]
    );
    await fileService.writeFile(
      resource,
      VSBuffer.fromString(",,,,,,,,,,,,,,")
    ...
    try {
      await testObject.writeConfiguration(
        EditableConfigurationTarget.USER_LOCAL,
        {
          key: "tasks.configurationEditing.service.testSetting",
          value: "value"
        },
        { donotNotifyError: true }
      );
  ...
  test("errors cases - ERROR_POLICY_CONFIGURATION", async () => {
    await runWithFakedTimers({ useFakeTimers: true }, async () => {
      const promise = Event.toPromise(
        instantiationService.get(IConfigurationService).onDidChangeConfiguration
      );
      await fileService.writeFile(
        environmentService.policyFile,
        VSBuffer.fromString(
          '{ "configurationEditing.service.policySetting": "policyValue" }'
        )
      );
      await promise;
    });
    try {
      await testObject.writeConfiguration(
        EditableConfigurationTarget.USER_LOCAL,
        {
          key: "configurationEditing.service.policySetting",
          value: "value"
        },
        { donotNotifyError: true }
      );
  ...
  test("write policy setting - when not set", async () => {
    ...
    assert.strictEqual(
      parsed["configurationEditing.service.policySetting"],
      "value"
    );
  });
  test("write one setting - empty file", async () => {
    ...
    const parsed = json.parse(contents.value.toString());
    assert.strictEqual(
      parsed["configurationEditing.service.testSetting"],
      "value"
    );
  });
  test("write one setting - existing file", async () => {
    await fileService.writeFile(
      userDataProfileService.currentProfile.settingsResource,
      VSBuffer.fromString('{ "my.super.setting": "my.super.value" }')
    );
    await testObject.writeConfiguration(
      EditableConfigurationTarget.USER_LOCAL,
      { key: "configurationEditing.service.testSetting", value: "value" }
  ...
  test("remove an existing setting - existing file", async () => {
    await fileService.writeFile(
      userDataProfileService.currentProfile.settingsResource,
      VSBuffer.fromString(
        '{ "my.super.setting": "my.super.value", "configurationEditing.service.testSetting": "value" }'
      )
    );
    await testObject.writeConfiguration(
      EditableConfigurationTarget.USER_LOCAL,
  ...
  test("write overridable settings to user settings", async () => {
    const key = "[language]";
    const value = {
      "configurationEditing.service.testSetting": "overridden value"
    };
    await testObject.writeConfiguration(
      EditableConfigurationTarget.USER_LOCAL,
      { key, value }
  ...
  test("write overridable settings to workspace folder settings", async () => {
    ...
    const folderSettingsFile = joinPath(
      workspaceService.getWorkspace().folders[0].uri,
      FOLDER_SETTINGS_PATH
    );
    await testObject.writeConfiguration(
      EditableConfigurationTarget.WORKSPACE_FOLDER,
      { key, value },
      { scopes: { resource: folderSettingsFile } }
    );
  ...
  test("write workspace standalone setting - empty file", async () => {
    const target = joinPath(
      workspaceService.getWorkspace().folders[0].uri,
      WORKSPACE_STANDALONE_CONFIGURATIONS["tasks"]
    );
    await testObject.writeConfiguration(
      EditableConfigurationTarget.WORKSPACE,
      { key: "tasks.service.testSetting", value: "value" }
    );
  ...
  test("write workspace standalone setting - existing file with JSON errors - full JSON", async () => {
    ...
    await fileService.writeFile(
      target,
      VSBuffer.fromString('{ "my.super.setting": ')
    );
    await testObject.writeConfiguration(
      EditableConfigurationTarget.WORKSPACE,
      {
  ...
  test("write workspace standalone setting should replace complete file", async () => {
    ...
    const expected = JSON.stringify(
      { version: "1.0.0", tasks: [{ taskName: "myTask1" }] },
      null,
      "	"
    );
