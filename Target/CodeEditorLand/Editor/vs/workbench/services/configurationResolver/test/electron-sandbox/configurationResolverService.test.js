...
suite("Configuration Resolver Service", () => {
  ...
  setup(() => {
    ...
    containingWorkspace = testWorkspace(
      URI.parse("file:///VSCode/workspaceLocation")
    );
    workspace = containingWorkspace.folders[0];
    configurationResolverService = new TestConfigurationResolverService(
      nullContext,
      Promise.resolve(envVariables),
      editorService,
      new MockInputsConfigurationService(),
      mockCommandService,
      new TestContextService(containingWorkspace),
      quickInputService,
      labelService,
      pathService,
      extensionService,
    ...
    if (platform.isWindows) {
      assert.strictEqual(
        await configurationResolverService.resolveAsync(
          workspace,
          "abc ${workspaceFolder} xyz"
        ),
        "abc \\VSCode\\workspaceLocation xyz"
      );
    } else {
      assert.strictEqual(
        await configurationResolverService.resolveAsync(
  ...
  test("workspace folder with argument", async () => {
    if (platform.isWindows) {
      assert.strictEqual(
        await configurationResolverService.resolveAsync(
          workspace,
          "abc ${workspaceFolder:workspaceLocation} xyz"
        ),
        "abc \\VSCode\\workspaceLocation xyz"
      );
    } else {
  ...
  test("workspace folder with invalid argument", () => {
    assert.rejects(
      async () => await configurationResolverService.resolveAsync(
        workspace,
        "abc ${workspaceFolder:invalidLocation} xyz"
      )
    );
  });
  ...
  test("workspace root folder name", async () => {
    assert.strictEqual(
      await configurationResolverService.resolveAsync(
        workspace,
        "abc ${workspaceRootFolderName} xyz"
      ),
      "abc workspaceLocation xyz"
    );
  });
  ...
  test("relative file with argument", async () => {
    assert.strictEqual(
      await configurationResolverService.resolveAsync(
        workspace,
        "abc ${relativeFile:workspaceLocation} xyz"
      ),
      "abc file xyz"
    );
  });
  test("relative file with invalid argument", () => {
    assert.rejects(
      async () => await configurationResolverService.resolveAsync(
        workspace,
        "abc ${relativeFile:invalidLocation} xyz"
      )
    );
  });
  test("relative file with undefined workspace folder", async () => {
    if (platform.isWindows) {
      assert.strictEqual(
        ...
        "abc \\VSCode\\workspaceLocation\\file xyz"
      );
    } else {
      assert.strictEqual(
        await configurationResolverService.resolveAsync(
  ...
  test("substitute many", async () => {
    if (platform.isWindows) {
      assert.strictEqual(
        ...
        "\\VSCode\\workspaceLocation - \\VSCode\\workspaceLocation"
      );
    } else {
      assert.strictEqual(
        await configurationResolverService.resolveAsync(
  ...
  test("substitute one env variable", async () => {
    if (platform.isWindows) {
      assert.strictEqual(
        await configurationResolverService.resolveAsync(
          workspace,
          "abc ${workspaceFolder} ${env:key1} xyz"
        ),
        "abc \\VSCode\\workspaceLocation Value for key1 xyz"
      );
    } else {
      assert.strictEqual(
        await configurationResolverService.resolveAsync(
  ...
  test("substitute many env variable", async () => {
    if (platform.isWindows) {
      assert.strictEqual(
        await configurationResolverService.resolveAsync(
          workspace,
          "${workspaceFolder} - ${workspaceFolder} ${env:key1} - ${env:key2}"
        ),
        "\\VSCode\\workspaceLocation - \\VSCode\\workspaceLocation Value for key1 - Value for key2"
      );
    } else {
      assert.strictEqual(
        await configurationResolverService.resolveAsync(
  ...
  test("disallows nested keys (#77289)", async () => {
    assert.strictEqual(
      await configurationResolverService.resolveAsync(
        workspace,
        "${env:key1} ${env:key1${env:key2}}"
      ),
      "Value for key1 ${env:key1${env:key2}}"
    );
  });
  test("supports extensionDir", async () => {
    ...
    assert.strictEqual(
      await configurationResolverService.resolveAsync(
        workspace,
        "${extensionInstallFolder:publisher.extId}"
      ),
  ...
  test("substitute one configuration variable", async () => {
    ...
    const service = new TestConfigurationResolverService(
      nullContext,
      Promise.resolve(envVariables),
      disposables.add(new TestEditorServiceWithActiveEditor()),
    ...
    assert.strictEqual(
      await service.resolveAsync(
        workspace,
        "abc ${config:editor.fontFamily} xyz"
      ),
      "abc foo xyz"
    );
  });
  ...
  test("substitute many configuration variables", async () => {
    ...
    assert.strictEqual(
      await service.resolveAsync(
        workspace,
        "abc ${config:editor.fontFamily} ${config:terminal.integrated.fontFamily} xyz"
      ),
      "abc foo bar xyz"
    );
  });
  test("substitute one env variable and a configuration variable", async () => {
    ...
    if (platform.isWindows) {
      assert.strictEqual(
        await service.resolveAsync(
          workspace,
          "abc ${config:editor.fontFamily} ${workspaceFolder} ${env:key1} xyz"
        ),
        "abc foo \\VSCode\\workspaceLocation Value for key1 xyz"
      );
    } else {
      assert.strictEqual(
        await service.resolveAsync(
  ...
  test("substitute many env variable and a configuration variable", async () => {
    ...
    if (platform.isWindows) {
      assert.strictEqual(
        await service.resolveAsync(
          workspace,
          "${config:editor.fontFamily} ${config:terminal.integrated.fontFamily} ${workspaceFolder} - ${workspaceFolder} ${env:key1} - ${env:key2}"
        ),
        "foo bar \\VSCode\\workspaceLocation - \\VSCode\\workspaceLocation Value for key1 - Value for key2"
      );
    } else {
      assert.strictEqual(
        await service.resolveAsync(
  ...
  test("mixed types of configuration variables", async () => {
    ...
    assert.strictEqual(
      await service.resolveAsync(
        workspace,
        "abc ${config:editor.fontFamily} ${config:editor.lineNumbers} ${config:editor.insertSpaces} xyz"
      ),
  ...
  test("uses original variable as fallback", async () => {
    ...
    assert.strictEqual(
      await service.resolveAsync(workspace, "abc ${unknownVariable} xyz"),
      "abc ${unknownVariable} xyz"
    );
    assert.strictEqual(
      await service.resolveAsync(
        workspace,
        "abc ${env:unknownVariable} xyz"
      ),
      "abc  xyz"
    );
  });
  test("configuration variables with invalid accessor", () => {
    ...
    assert.rejects(
      async () => await service.resolveAsync(workspace, "abc ${env} xyz")
    );
    assert.rejects(
      async () => await service.resolveAsync(workspace, "abc ${env:} xyz")
    );
    assert.rejects(
      async () => await service.resolveAsync(workspace, "abc ${config} xyz")
    );
    assert.rejects(
      async () => await service.resolveAsync(workspace, "abc ${config:} xyz")
    );
    ...
    assert.rejects(
      async () => await service.resolveAsync(
        workspace,
        "abc ${config:editor.none.none2} xyz"
      )
    );
  });
  ...
  test("an old style command variable", () => {
    ...
    return configurationResolverService.resolveWithInteractionReplace(
      void 0,
      configuration,
      void 0,
      commandVariables
  ...
  test("resolveWithEnvironment", async () => {
    ...
    const resolvedResult = await configurationResolverService.resolveWithEnvironment(
      { ...env },
      void 0,
      configuration
    );
