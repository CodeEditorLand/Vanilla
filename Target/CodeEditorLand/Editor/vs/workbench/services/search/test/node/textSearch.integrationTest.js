  ...
  return engine.search(
    ...
    if (typeof expectedResultCount === "function") {
      ...
      assert.strictEqual(
        c,
        expectedResultCount,
        `rg ${c} !== ${expectedResultCount}`
      );
    }
    return results;
  });
}
flakySuite("TextSearch-integration", () => {
  ...
  test("Multiple matches on line: h\\d,", () => {
    ...
    return doSearchTest(config, 15).then((results) => {
      ...
      assert.strictEqual(
        match.rangeLocations.map((e) => e.source).length,
        5
      );
    });
  });
  test("Search with context matches", () => {
    ...
    return doSearchTest(config, 3).then((results) => {
      assert.strictEqual(results.length, 3);
      assert.strictEqual(
        results[0].results[0].lineNumber,
        24
      );
      assert.strictEqual(
      ...
      assert.strictEqual(
        results[2].results[0].lineNumber,
        26
      );
      assert.strictEqual(
        results[2].results[0].text,
        "        compiler.emit();"
      );
    });
  });
    ...
    test("invalid encoding", () => {
      ...
      return doSearchTest(config, 0).then(
        ...
        (err) => {
          ...
          assert.strictEqual(
            searchError.message,
            "Unknown encoding: invalidEncoding"
          );
          assert.strictEqual(
            searchError.code,
    ...
    test("invalid regex case 1", () => {
      ...
      return doSearchTest(config, 0).then(
        ...
        (err) => {
          ...
          assert.strictEqual(
            searchError.message,
            regexParseErrorForUnclosedParenthesis
          );
          assert.strictEqual(
            searchError.code,
