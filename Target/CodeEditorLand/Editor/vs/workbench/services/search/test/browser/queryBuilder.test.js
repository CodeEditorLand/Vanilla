...
suite("QueryBuilder", () => {
  ...
  setup(() => {
    ...
    mockWorkspace = new Workspace("workspace", [
      toWorkspaceFolder(ROOT_1_URI)
    ]);
  ...
  test("splits include pattern when expandPatterns enabled", () => {
    assertEqualQueries(
      queryBuilder.file([ROOT_1_NAMED_FOLDER], {
  ...
  test("simple exclude setting", () => {
    ...
    assertEqualTextQueries(
      queryBuilder.text(PATTERN_INFO, [ROOT_1_URI], {
        expandPatterns: true
  ...
  test("multiroot exclude settings", () => {
    ...
    mockWorkspace.folders = toWorkspaceFolders(
      [
        ...
        { path: ROOT_2_URI.fsPath },
        { path: ROOT_3_URI.fsPath }
      ],
      WS_CONFIG_PATH,
      extUriBiasedIgnorePathCase
    );
    mockWorkspace.configuration = uri.file(fixPath("/config"));
    mockConfigService.setUserConfiguration(
      "search",
      {
        ...DEFAULT_USER_CONFIG,
        exclude: { "foo/**/*.js": true }
    ...
    assertEqualTextQueries(
      ...
      ]),
      {
        ...
        folderQueries: [
          {
            folder: ROOT_1_URI,
            excludePattern: makeExcludePatternFromPatterns("foo/**/*.js")
          },
          {
            folder: ROOT_2_URI,
            excludePattern: makeExcludePatternFromPatterns("bar")
          },
          { folder: ROOT_3_URI }
        ],
        type: QueryType.Text
  ...
  test("file pattern trimming", () => {
    ...
    assertEqualQueries(
      queryBuilder.file([], { filePattern: ` ${content} ` }),
      {
        folderQueries: [],
        filePattern: content,
        type: QueryType.File
    ...
    assertEqualTextQueries(
      ...
      }),
      {
        ...
        folderQueries: [
          {
            folder: ROOT_1_URI,
            excludePattern: makeExcludePatternFromPatterns(
              "bar",
              "bar/**"
            )
          }
  ...
  suite("parseSearchPaths 1", () => {
    test("simple includes", () => {
      ...
      [
        ...
        ([includePattern, expectedPatterns]) => testSimpleIncludes(
          includePattern,
          expectedPatterns
        )
      );
    ...
    test("relative includes w/two root folders", () => {
      ...
      mockWorkspace.folders = toWorkspaceFolders(
        [{ path: ROOT_1_URI.fsPath }, { path: getUri(ROOT_2).fsPath }],
        WS_CONFIG_PATH,
        extUriBiasedIgnorePathCase
      );
      mockWorkspace.configuration = uri.file(fixPath("config"));
    ...
    (isWindows ? test.skip : test)("includes with tilde", () => {
      ...
      const cases = [
        [
          ...
          {
            searchPaths: [
              { searchPath: getUri(userHome.fsPath, "/foo/bar") }
            ]
          }
        ],
        [
...
function assertEqualQueries(actual, expected) {
  ...
  const folderQueryToCompareObject = (fq) => {
    const excludePattern = fq.excludePattern?.map(
      (e) => normalizeExpression(e.pattern)
    );
    return {
  ...
  if (expected.folderQueries) {
    assert.deepStrictEqual(
      actual.folderQueries.map(folderQueryToCompareObject),
  ...
  if (expected.extraFileResources) {
    assert.deepStrictEqual(
      actual.extraFileResources.map((extraFile) => extraFile.fsPath),
...
function assertEqualSearchPathResults(actual, expected, message) {
  ...
  assert.strictEqual(
    ...
    expected.searchPaths && expected.searchPaths.length
  );
  if (actual.searchPaths) {
    actual.searchPaths.forEach((searchPath, i) => {
      const expectedSearchPath = expected.searchPaths[i];
      assert.deepStrictEqual(
        searchPath.pattern && { ...searchPath.pattern },
        expectedSearchPath.pattern
      );
      assert.strictEqual(
        searchPath.searchPath.toString(),
