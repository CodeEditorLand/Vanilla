...
suite("RipgrepTextSearchEngine", () => {
  ...
  test("unicodeEscapesToPCRE2", async () => {
    ...
    assert.strictEqual(
      unicodeEscapesToPCRE2("foo\\\\\\u1234"),
      "foo\\\\\\x{1234}"
    );
    assert.strictEqual(unicodeEscapesToPCRE2("\\u{1234}"), "\\x{1234}");
    assert.strictEqual(
      unicodeEscapesToPCRE2("\\u{1234}\\u{0001}"),
      "\\x{1234}\\x{0001}"
    );
    assert.strictEqual(
      unicodeEscapesToPCRE2("foo\\u{1234}bar"),
      "foo\\x{1234}bar"
    );
    assert.strictEqual(
      unicodeEscapesToPCRE2("[\\u00A0-\\u00FF]"),
      "[\\x{00A0}-\\x{00FF}]"
    );
    assert.strictEqual(
      unicodeEscapesToPCRE2("foo\\u{123456}7bar"),
      "foo\\u{123456}7bar"
    );
  ...
  suite("RipgrepParser", () => {
    ...
    function testParser(inputData, expectedResults) {
      const testParser2 = new RipgrepParser(
        1e3,
        TEST_FOLDER,
        ...
        data: {
          ...
          submatches: matchRanges.map((mr) => {
            return {
              ...mr,
              match: {
                text: text.substring(mr.start, mr.end)
              }
            };
          })
        }
    ...
    test("multiple results", () => {
      testParser(
        [
          ...
          makeRgMatch("app/file2.js", "foobar", 4, [
            { start: 3, end: 6 }
          ]),
          makeRgMatch("app2/file3.js", "foobar", 4, [
            { start: 3, end: 6 }
          ])
        ],
        [
  ...
  suite("getRgArgs", () => {
    test("simple includes", () => {
      ...
      [
        ...
        ([includes, expectedFromIncludes]) => testGetRgArgs(
          includes,
          expectedFromIncludes
        )
      );
  ...
  test("brace expansion for ripgrep", () => {
    ...
    [
      ...
      ([includePattern, expectedPatterns]) => testBraceExpansion(
        includePattern,
        expectedPatterns
      )
    );
