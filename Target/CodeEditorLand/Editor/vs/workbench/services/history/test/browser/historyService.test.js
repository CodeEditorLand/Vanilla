...
import {
  ...
  HistoryService
} from "../../browser/historyService.js";
...
suite("HistoryService", () => {
  ...
  async function createServices(scope = GoScope.DEFAULT, configureSearchExclude = false) {
    ...
    const historyService = disposables.add(
      instantiationService.createInstance(HistoryService)
    );
  ...
  test("back / forward: is editor group aware", async function() {
    ...
    const pane1 = await editorService.openEditor({
      resource,
      options: { pinned: true }
    });
    const pane2 = await editorService.openEditor(
      { resource, options: { pinned: true } },
      SIDE_GROUP
    );
    assert.notStrictEqual(pane1, pane2);
    await editorService.openEditor(
      { resource: otherResource, options: { pinned: true } },
      pane2?.group
    );
    await historyService.goBack();
  ...
  test("back / forward: in-editor text selection changes (user)", async function() {
    ...
    await setTextSelection(
      historyService,
      pane,
      new Selection(15, 1, 15, 1)
    );
    await setTextSelection(
      historyService,
      pane,
      new Selection(16, 1, 16, 1)
    );
    await setTextSelection(
      historyService,
      pane,
      new Selection(17, 1, 17, 1)
    );
    await setTextSelection(
      historyService,
      pane,
      new Selection(30, 5, 30, 8)
    );
    await setTextSelection(
      historyService,
      pane,
      new Selection(40, 1, 40, 1)
    );
  ...
  test("back / forward: in-editor text selection changes (navigation)", async function() {
    ...
    await setTextSelection(
      historyService,
      pane,
      new Selection(2, 2, 2, 10)
    );
    await setTextSelection(
      historyService,
      pane,
      new Selection(5, 3, 5, 20),
      EditorPaneSelectionChangeReason.NAVIGATION
    );
    await setTextSelection(
      historyService,
      pane,
      new Selection(120, 8, 120, 18),
      EditorPaneSelectionChangeReason.NAVIGATION
    );
    await setTextSelection(
      historyService,
      pane,
      new Selection(300, 3, 300, 20)
    );
    await setTextSelection(
      historyService,
      pane,
      new Selection(500, 3, 500, 20)
    );
    await setTextSelection(
      historyService,
      pane,
      new Selection(200, 3, 200, 20)
    );
  ...
  test("back / forward: in-editor text selection changes (jump)", async function() {
    ...
    await setTextSelection(
      ...
      pane,
      new Selection(5, 3, 5, 20),
      EditorPaneSelectionChangeReason.JUMP
    );
    await setTextSelection(
      historyService,
      pane,
  ...
  async function setTextSelection(historyService, pane, selection, reason = EditorPaneSelectionChangeReason.USER) {
    const promise = Event.toPromise(
      historyService.onDidChangeEditorNavigationStack
    );
  ...
  test("back / forward: tracks editor moves across groups", async function() {
    ...
    const editorChangePromise = Event.toPromise(
  ...
  test("back / forward: tracks group removals", async function() {
    ...
    const pane2 = await editorService.openEditor(
      { resource: resource2, options: { pinned: true } },
      SIDE_GROUP
    );
    assert.notStrictEqual(pane1, pane2);
  ...
  test("back / forward: editor navigation stack - navigation", async function() {
    ...
    const stack = instantiationService.createInstance(
      EditorNavigationStack,
      GoFilter.NONE,
      GoScope.DEFAULT
    );
  ...
  test("back / forward: editor navigation stack - mutations", async function() {
    ...
    const resource = toResource.call(this, "/path/index.txt");
    const otherResource = toResource.call(this, "/path/index.html");
    const unrelatedResource = toResource.call(
      this,
      "/path/unrelated.html"
    );
    const pane = await editorService.openEditor({
    ...
    assert.strictEqual(stack.canGoForward(), true);
    stack.remove(
      new FileOperationEvent(unrelatedResource, FileOperation.DELETE)
    );
    assert.strictEqual(stack.canGoForward(), true);
    await stack.goForward();
    assert.strictEqual(stack.canGoBack(), true);
  ...
  test("back / forward: editor group scope", async function() {
    ...
    const sideGroup = part.addGroup(part.activeGroup, GroupDirection.RIGHT);
    const pane2 = await editorService.openEditor(
      { resource: resource1, options: { pinned: true } },
      sideGroup
    );
    await editorService.openEditor({
      resource: resource2,
    ...
    await editorService.openEditor(
      { resource: resource3, options: { pinned: true } },
      pane1?.group
    );
    await historyService.goBack();
    await historyService.goBack();
  ...
  test("back / forward: editor  scope", async function() {
    ...
    await setTextSelection(
      historyService,
      pane,
      new Selection(12, 2, 12, 10)
    );
    await setTextSelection(
      historyService,
      pane,
      new Selection(150, 3, 150, 20)
    );
    await historyService.goBack();
  ...
  test("go to last edit location", async function() {
    ...
    const model = await textFileService.files.resolve(
      resource
    );
    ...
    assert.strictEqual(
      editorService.activeEditor?.resource?.toString(),
      resource.toString()
    );
    return workbenchTeardown(instantiationService);
  });
  ...
  test("getHistory", async () => {
    ...
    const [part, historyService, editorService, , instantiationService] = await createServices(void 0, true);
    let history = historyService.getHistory();
    assert.strictEqual(history.length, 0);
    const input1 = disposables.add(
      new TestFileEditorInput(
        URI.parse("foo://bar1/node_modules/test.txt"),
        TEST_EDITOR_INPUT_ID
      )
    );
    await part.activeGroup.openEditor(input1, { pinned: true });
    ...
    const input3 = disposables.add(
      new TestFileEditorInputWithUntyped(
        URI.parse("foo://bar3"),
        TEST_EDITOR_INPUT_ID
      )
    );
    ...
    assert.strictEqual(
      isResourceEditorInput(history[0]) && !(history[0] instanceof EditorInput),
      true
    );
    assert.strictEqual(
      history[0].options?.override,
      "testOverride"
    );
    ...
    assert.strictEqual(
      history[0].resource?.toString(),
      input4.resource.toString()
    );
    input1.dispose();
    history = historyService.getHistory();
    assert.strictEqual(history.length, 2);
    const input5 = disposables.add(
      new TestFileEditorInputWithUntyped(
        URI.parse("file://bar5"),
        TEST_EDITOR_INPUT_ID
      )
    );
    const input6 = disposables.add(
      new TestFileEditorInputWithUntyped(
        URI.file("file://bar1/node_modules/test.txt"),
        TEST_EDITOR_INPUT_ID
      )
    );
    const input7 = new SideBySideEditorInput(
      void 0,
      void 0,
      input6,
      input5,
  ...
  test("getLastActiveFile", async () => {
    ...
    await part.activeGroup.openEditor(input2, { pinned: true });
    assert.strictEqual(
      historyService.getLastActiveFile("foo")?.toString(),
      input2.resource.toString()
    );
    assert.strictEqual(
      historyService.getLastActiveFile("foo", "bar2")?.toString(),
      input2.resource.toString()
    );
    assert.strictEqual(
      historyService.getLastActiveFile("foo", "bar1")?.toString(),
  ...
  test("open next/previous recently used editor (single group)", async () => {
    ...
    await part.activeGroup.openEditor(input2, { pinned: true });
    assert.strictEqual(part.activeGroup.activeEditor, input2);
    let editorChangePromise = Event.toPromise(
      editorService.onDidActiveEditorChange
    );
    historyService.openPreviouslyUsedEditor();
    await editorChangePromise;
    assert.strictEqual(part.activeGroup.activeEditor, input1);
    editorChangePromise = Event.toPromise(
      editorService.onDidActiveEditorChange
    );
