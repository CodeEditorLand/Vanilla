...
} from "../../browser/storageService.js";
async function createStorageService() {
  ...
  const profilesRoot = URI.file("/profiles").with({
  ...
  const inMemoryExtraProfile = {
    ...
    globalStorageHome: joinPath(
      inMemoryExtraProfileRoot,
      "globalStorageHome"
    ),
    settingsResource: joinPath(
      inMemoryExtraProfileRoot,
      "settingsResource"
    ),
    ...
    extensionsResource: joinPath(
      inMemoryExtraProfileRoot,
      "extensionsResource"
    ),
    cacheHome: joinPath(inMemoryExtraProfileRoot, "cache")
  };
  const storageService = disposables.add(
    new BrowserStorageService(
      { id: "workspace-storage-test" },
      disposables.add(new UserDataProfileService(inMemoryExtraProfile)),
      logService
    )
  );
...
flakySuite("StorageService (browser)", () => {
  const disposables = new DisposableStore();
  let storageService;
  createSuite({
    setup: async () => {
...
flakySuite("IndexDBStorageDatabase (browser)", () => {
  ...
  const disposables = new DisposableStore();
  teardown(async () => {
    const storage = disposables.add(
      await IndexedDBStorageDatabase.create({ id }, logService)
    );
    await storage.clear();
    disposables.clear();
  });
  test("Basics", async () => {
    let storage = disposables.add(
      new Storage(
        disposables.add(
          await IndexedDBStorageDatabase.create({ id }, logService)
        )
    ...
    strictEqual(
      storage.get("barNumber", "undefinedNumber"),
      "undefinedNumber"
    );
    strictEqual(
      storage.get("barBoolean", "undefinedBoolean"),
      "undefinedBoolean"
    );
  ...
  test("Inserts and Deletes at the same time", async () => {
    ...
    const largeItem = JSON.stringify({
      largeItem: "Hello World".repeat(1e3)
    });
  ...
  test("Storage change event", async () => {
    ...
    let storageValueChangeEvent = storageChangeEvents.find(
      (e) => e.key === "notExternal"
    );
    strictEqual(storageValueChangeEvent?.external, false);
    storageChangeEvents = [];
    storage.set("isExternal", 42, true);
    storageValueChangeEvent = storageChangeEvents.find(
      (e) => e.key === "isExternal"
    );
