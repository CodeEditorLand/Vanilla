...
let TextMateTokenizationFeature = class extends Disposable {
  ...
  async _createTokenizationSupport(languageId) {
    ...
    try {
      ...
      const disposable = tokenization.onDidEncounterLanguage((encodedLanguageId2) => {
  ...
  async _loadVSCodeOnigurumaWASM() {
    if (isWeb) {
      const response = await fetch(isESM ? resolveAmdNodeModulePath("vscode-oniguruma", "release/onig.wasm") : FileAccess.asBrowserUri("vscode-oniguruma/../onig.wasm").toString(true));
