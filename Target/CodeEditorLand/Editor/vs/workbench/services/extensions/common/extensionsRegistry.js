...
class ExtensionPointUserDelta {
  ...
  static compute(previous, current) {
    ...
    const removed = previous.filter(
      (user) => !currentSet.has(user.description.identifier)
    );
...
const schema = {
  properties: {
    ...
    enabledApiProposals: {
      ...
      items: {
        type: "string",
        enum: Object.keys(allApiProposals).map(
          (proposalName) => proposalName
        ),
        markdownEnumDescriptions: Object.values(allApiProposals).map(
      ...
      properties: {
        virtualWorkspaces: {
          ...
          defaultSnippets: [
            ...
            {
              label: "false",
              body: { supported: false, description: "${2}" }
            }
          ],
...
class ExtensionsRegistryImpl {
  ...
  _extensionPoints = /* @__PURE__ */ new Map();
  registerExtensionPoint(desc) {
    ...
    const result = new ExtensionPoint(
      desc.extensionPoint,
      desc.defaultExtensionKind
    );
    this._extensionPoints.set(desc.extensionPoint, result);
    if (desc.activationEventsGenerator) {
      ImplicitActivationEvents.register(
        desc.extensionPoint,
        desc.activationEventsGenerator
      );
    }
...
const ExtensionsRegistry = Registry.as(
