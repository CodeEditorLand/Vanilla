...
class ViewDescriptorSequence {
  ...
  constructor(model) {
    ...
    model.onDidAddVisibleViewDescriptors(
      (added) => added.forEach(
        ({ viewDescriptor, index }) => this.elements.splice(index, 0, viewDescriptor)
      ),
      null,
      this.disposables
    );
    model.onDidRemoveVisibleViewDescriptors(
      (removed) => removed.sort((a, b) => b.index - a.index).forEach(({ index }) => this.elements.splice(index, 1)),
      null,
      this.disposables
    );
    model.onDidMoveVisibleViewDescriptors(
      ({ from, to }) => move(this.elements, from.index, to.index),
      null,
      this.disposables
    );
  }
...
suite("ViewContainerModel", () => {
  ...
  test("register/unregister", () => {
    ...
    const testObject = viewDescriptorService.getViewContainerModel(container);
    const target = disposableStore.add(
      new ViewDescriptorSequence(testObject)
    );
    assert.strictEqual(testObject.visibleViewDescriptors.length, 0);
    ...
    assert.deepStrictEqual(
      testObject.visibleViewDescriptors[0],
      viewDescriptor
    );
  ...
  test("when contexts", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    ViewsRegistry.registerViews([viewDescriptor], container);
    assert.strictEqual(
      testObject.visibleViewDescriptors.length,
      0,
      "view should not appear since context isnt in"
    );
    assert.strictEqual(target.elements.length, 0);
    const key = contextKeyService.createKey(
      "showview1",
      false
    );
    assert.strictEqual(
  ...
  test("when contexts - multiple", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    assert.deepStrictEqual(
      testObject.visibleViewDescriptors,
      [view1],
  ...
  test("view states", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    storageService.store(
      `${container.id}.state.hidden`,
      JSON.stringify([{ id: "view1", isHidden: true }]),
  ...
  test("add event is not triggered if view was set visible (when visible) and not active", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    assert.ok(
      !targetEvent.called,
      "add event should not be called since it is already visible"
    );
    assert.strictEqual(testObject.visibleViewDescriptors.length, 0);
    assert.strictEqual(target.elements.length, 0);
  }));
  test("remove event is not triggered if view was hidden and not active", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    assert.ok(
      !targetEvent.called,
      "add event should not be called since it is disabled"
    );
    assert.strictEqual(testObject.visibleViewDescriptors.length, 0);
    assert.strictEqual(target.elements.length, 0);
  }));
  ...
  test("add event is triggered only once when view is set visible while it is set active", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    disposableStore.add(
      Event.once(testObject.onDidChangeActiveViewDescriptors)(
        () => testObject.setVisible("view1", true)
      )
    );
    key.set(true);
  ...
  test("#142087: view descriptor visibility is not reset", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    storageService.store(
      getViewsStateStorageId("test.state"),
      JSON.stringify([
        {
    ...
    assert.strictEqual(
      testObject.activeViewDescriptors[0].id,
      viewDescriptor.id
    );
    assert.strictEqual(testObject.visibleViewDescriptors.length, 0);
  }));
  test("remove event is triggered properly if multiple views are hidden at the same time", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    ViewsRegistry.registerViews(
      [viewDescriptor1, viewDescriptor2, viewDescriptor3],
      container
    );
  ...
  test("add and remove events are triggered properly if multiple views are hidden and added at the same time", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    assert.ok(
      removeEvent.calledOnce,
      "remove event should be called once"
    );
