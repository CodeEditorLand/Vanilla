...
suite("ViewDescriptorService", () => {
  ...
  setup(() => {
    ...
    instantiationService.stub(
      IContextKeyService,
      disposables.add(
        instantiationService.createInstance(ContextKeyService)
      )
    );
  });
  teardown(() => {
    for (const viewContainer of ViewContainersRegistry.all) {
      if (viewContainer.id.startsWith(viewContainerIdPrefix)) {
        ViewsRegistry.deregisterViews(
          ViewsRegistry.getViews(viewContainer),
          viewContainer
        );
      }
    }
  ...
  test("Empty Containers", () => {
    ...
    assert.strictEqual(
      sidebarViews.allViewDescriptors.length,
      0,
      "The sidebar container should have no views yet."
    );
    assert.strictEqual(
      panelViews.allViewDescriptors.length,
      0,
  ...
  test("Register/Deregister", () => {
    ...
    assert.strictEqual(
      sidebarViews.activeViewDescriptors.length,
      2,
      "Sidebar should have 2 views"
    );
    assert.strictEqual(
      panelViews.activeViewDescriptors.length,
      1,
  ...
  test("move views to existing containers", async () => {
    const testObject = aViewDescriptorService();
    const viewDescriptors = [
      {
        id: "view1",
    ...
    ViewsRegistry.registerViews(viewDescriptors.slice(2), panelContainer);
    testObject.moveViewsToContainer(
      viewDescriptors.slice(2),
      sidebarContainer
    );
    testObject.moveViewsToContainer(
      viewDescriptors.slice(0, 2),
  ...
  test("move views to generated containers", async () => {
    const testObject = aViewDescriptorService();
    const viewDescriptors = [
      {
        id: "view1",
    ...
    const generatedPanel = assertIsDefined(
...
`;
    ...
    testObject.moveViewsToContainer(
      [viewDescriptors[1], viewDescriptors[2]],
      panelContainer
    );
    assert.strictEqual(
      actualSequence,
  ...
  test("reset", async () => {
    ...
    assert.deepStrictEqual(
      testObject.getViewContainerById(generatedPanel.id),
      null
    );
    assert.deepStrictEqual(
      testObject.getViewContainerById(generatedSidebar.id),
      null
    );
  });
  test("initialize with custom locations", async () => {
    ...
    assert.deepStrictEqual(
      testObject.getViewContainerLocation(viewContainer1),
  ...
  test("orphan view containers", async () => {
    const storageService = instantiationService.get(IStorageService);
    ...
    testObject.whenExtensionsRegistered();
    assert.deepStrictEqual(
      testObject.getViewContainerById(generatedViewContainerId),
      null
    );
    assert.deepStrictEqual(
      testObject.isViewContainerRemovedPermanently(
        generatedViewContainerId
      ),
      true
    );
    const actual = JSON.parse(
      storageService.get("views.customizations", StorageScope.PROFILE)
    );
    assert.deepStrictEqual(actual, {
      viewContainerLocations: {},
      viewLocations: {},
  ...
  test("custom locations take precedence when default view container of views change", async () => {
    const storageService = instantiationService.get(IStorageService);
    const viewContainer1 = ViewContainersRegistry.registerViewContainer(
      {
        id: `${viewContainerIdPrefix}-${generateUuid()}`,
    ...
    ViewsRegistry.registerViews(viewDescriptors.slice(3), viewContainer1);
    const testObject = aViewDescriptorService();
    ViewsRegistry.moveViews(
      [viewDescriptors[0], viewDescriptors[1]],
      panelContainer
    );
    const sidebarViews = testObject.getViewContainerModel(sidebarContainer);
    assert.deepStrictEqual(
  ...
  test("view containers with not existing views are not removed from customizations", async () => {
    const storageService = instantiationService.get(IStorageService);
    const viewContainer1 = ViewContainersRegistry.registerViewContainer(
      {
        id: `${viewContainerIdPrefix}-${generateUuid()}`,
  ...
  test("storage change also updates locations even if views do not exists and views are registered later", async () => {
    const storageService = instantiationService.get(IStorageService);
    const testObject = aViewDescriptorService();
    ...
    const generateViewContainer = testObject.getViewContainerById(
      generateViewContainerId
    );
    assert.deepStrictEqual(
      testObject.getViewContainerLocation(generateViewContainer),
      ViewContainerLocation.AuxiliaryBar
    );
    const generatedViewContainerModel = testObject.getViewContainerModel(
  ...
  test("storage change move views and retain visibility state", async () => {
    const storageService = instantiationService.get(IStorageService);
    const testObject = aViewDescriptorService();
    const viewContainer = ViewContainersRegistry.registerViewContainer(
      {
