...
class FileDialogService extends AbstractFileDialogService {
  ...
  getFilePickerTypes(filters) {
    return filters?.filter((filter) => {
      ...
      accept[getMediaOrTextMime(`fileName.${filter.extensions[0]}`) ?? "text/plain"] = extensions.map(
  ...
  async showSaveDialog(options) {
    ...
    try {
      fileHandle = await activeWindow.showSaveFilePicker({
        types: this.getFilePickerTypes(options.filters),
        ...options.defaultUri ? { suggestedName: basename(options.defaultUri) } : void 0,
        ...{ startIn }
      });
    } catch (error) {
      return void 0;
  ...
  async showOpenDialog(options) {
    ...
    try {
      if (options.canSelectFiles) {
        ...
        uri = await this.fileSystemProvider.registerDirectoryHandle(
          handle
        );
      }
    } catch (error) {
    ...
    if (context === "open") {
      buttons.push({
        ...
        run: async () => {
          ...
          if (files) {
            const filesData = (await this.instantiationService.invokeFunction(
              (accessor) => extractFileListData(accessor, files)
