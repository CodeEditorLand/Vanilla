...
suite("EditorsObserver", () => {
  ...
  async function createEditorObserver(scoped = false) {
    ...
    const observer = disposables.add(
      new EditorsObserver(
        scoped ? part : void 0,
        part,
  ...
  async function testSingleGroupBasics(scoped = false) {
    ...
    disposables.add(
      observer.onDidMostRecentlyActiveEditorsChange(() => {
    ...
    const input1 = new TestFileEditorInput(
      URI.parse("foo://bar1"),
      TEST_SERIALIZABLE_EDITOR_INPUT_ID
    );
    ...
    const input2 = new TestFileEditorInput(
      URI.parse("foo://bar2"),
      TEST_SERIALIZABLE_EDITOR_INPUT_ID
    );
    const input3 = new TestFileEditorInput(
      URI.parse("foo://bar3"),
      TEST_SERIALIZABLE_EDITOR_INPUT_ID
    );
  ...
  test("basics (multi group)", async () => {
    ...
    let currentEditorsMRU = observer.editors;
    assert.strictEqual(currentEditorsMRU.length, 0);
    const sideGroup = disposables.add(
      part.addGroup(rootGroup, GroupDirection.RIGHT)
    );
    const input1 = new TestFileEditorInput(
      URI.parse("foo://bar1"),
      TEST_SERIALIZABLE_EDITOR_INPUT_ID
    );
    await rootGroup.openEditor(input1, {
      pinned: true,
      activation: EditorActivation.ACTIVATE
    });
    await sideGroup.openEditor(input1, {
      pinned: true,
      activation: EditorActivation.ACTIVATE
    });
    currentEditorsMRU = observer.editors;
  ...
  test("hasEditor/hasEditors - same resource, different type id", async () => {
    ...
    const input2 = disposables.add(
      new TestFileEditorInput(input1.resource, "otherTypeId")
    );
    assert.strictEqual(observer.hasEditors(input1.resource), false);
    assert.strictEqual(
      observer.hasEditor({
  ...
  test("hasEditor/hasEditors - side by side editor support", async () => {
    ...
    const primary = disposables.add(
      new TestFileEditorInput(
        URI.parse("foo://bar1"),
        TEST_SERIALIZABLE_EDITOR_INPUT_ID
      )
    );
    const secondary = disposables.add(
      new TestFileEditorInput(URI.parse("foo://bar2"), "otherTypeId")
    );
  ...
  test("copy group", async () => {
    ...
    const copiedGroup = part.copyGroup(
      rootGroup,
      rootGroup,
  ...
  test("initial editors are part of observer and state is persisted & restored (single group)", async () => {
    ...
    const observer = disposables.add(
      new EditorsObserver(void 0, part, storage)
    );
    ...
    const restoredObserver = disposables.add(
      new EditorsObserver(void 0, part, storage)
    );
    await part.whenReady;
  ...
  test("observer closes editors when limit reached (in group)", async () => {
    const [part] = await createPart();
    disposables.add(
      part.enforcePartOptions({
        limit: { enabled: true, value: 3, perEditorGroup: true }
      })
    );
    const storage = disposables.add(new TestStorageService());
    const observer = disposables.add(
    ...
    disposables.add(
      part.enforcePartOptions({
        limit: { enabled: true, value: 1, perEditorGroup: true }
      })
    );
    await timeout(10);
    assert.strictEqual(rootGroup.count, 1);
