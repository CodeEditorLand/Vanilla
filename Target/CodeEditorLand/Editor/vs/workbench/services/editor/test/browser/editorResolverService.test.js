...
suite("EditorResolverService", () => {
  ...
  async function createEditorResolverService(instantiationService = workbenchInstantiationService(
    void 0,
    disposables
  )) {
    const part = await createEditorPart(instantiationService, disposables);
    instantiationService.stub(IEditorGroupsService, part);
    const editorResolverService = instantiationService.createInstance(
  ...
  test("Simple Resolve", async () => {
    ...
    const registeredEditor = service.registerEditor(
      ...
      {},
      {
        createEditorInput: ({ resource, options }, group) => ({
          editor: new TestFileEditorInput(
    ...
    const resultingResolution = await service.resolveEditor(
      { resource: URI.file("my://resource-basics.test") },
      part.activeGroup
    );
  ...
  test("Untitled Resolve", async () => {
    ...
    const registeredEditor = service.registerEditor(
      ...
      {},
      {
        ...
        createUntitledEditorInput: ({ resource, options }, group) => ({
          editor: new TestFileEditorInput(
            resource ? resource : URI.from({ scheme: Schemas.untitled }),
            UNTITLED_TEST_EDITOR_INPUT_ID
          )
        })
      }
    );
    ...
    resultingResolution = await service.resolveEditor(
      { resource: URI.file("/fake.test"), forceUntitled: true },
      part.activeGroup
    );
    assert.ok(resultingResolution);
    assert.notStrictEqual(typeof resultingResolution, "number");
  ...
  test("Side by side Resolve", async () => {
    ...
    const registeredEditorPrimary = service.registerEditor(
      ...
      {
        createEditorInput: ({ resource, options }, group) => ({
          editor: constructDisposableFileEditorInput(
            URI.parse(resource.toString()),
            TEST_EDITOR_INPUT_ID,
            disposables
          )
    ...
    const resultingResolution = await service.resolveEditor(
      {
        primary: {
          resource: URI.file("my://resource-basics.test-primary")
        },
        secondary: {
          resource: URI.file("my://resource-basics.test-secondary")
        }
      },
      part.activeGroup
    );
  ...
  test("Diff editor Resolve", async () => {
      ...
      {},
      {
        ...
        createDiffEditorInput: ({ modified, original, options }, group) => ({
          editor: accessor.instantiationService.createInstance(
            ...
            "name",
            "description",
            constructDisposableFileEditorInput(
              URI.parse(original.toString()),
              TEST_EDITOR_INPUT_ID,
              disposables
            ),
            constructDisposableFileEditorInput(
              URI.parse(modified.toString()),
              TEST_EDITOR_INPUT_ID,
              disposables
            ),
            void 0
    ...
    const resultingResolution = await service.resolveEditor(
      {
        original: {
          resource: URI.file("my://resource-basics.test-diff")
        },
        modified: {
          resource: URI.file("my://resource-basics.test-diff")
        }
    ...
    if (resultingResolution !== ResolvedStatus.ABORT && resultingResolution !== ResolvedStatus.NONE) {
      assert.strictEqual(
        resultingResolution.editor.typeId,
        "workbench.editors.diffEditorInput"
      );
      resultingResolution.editor.dispose();
    } else {
      assert.fail();
  ...
  test("Diff editor Resolve - Different Types", async () => {
    ...
    resultingResolution = await service.resolveEditor(
      {
        original: {
          resource: URI.file("my://resource-basics.test-secondDiff")
        },
        modified: {
          resource: URI.file("my://resource-basics.test-secondDiff")
        }
  ...
  test("Registry & Events", async () => {
    ...
    assert.strictEqual(
      service.getEditors().some((editor) => editor.id === "TEST_EDITOR"),
      true
    );
    registeredEditor.dispose();
