...
suite("EditorGroupsService", () => {
  ...
  async function createParts(instantiationService = workbenchInstantiationService(
    void 0,
    disposables
  )) {
    ...
    const parts = await createEditorParts(
  ...
  test("groups basics", async () => {
    ...
    const rightGroupModelChangeListener = rightGroup.onDidModelChange(
      (e) => {
        if (e.kind === GroupModelChangeKind.GROUP_ACTIVE) {
  ...
  test("save & restore state", async () => {
    ...
    const rightGroup = part.addGroup(rootGroup, GroupDirection.RIGHT);
    const downGroup = part.addGroup(rightGroup, GroupDirection.DOWN);
    const rootGroupInput = createTestFileEditorInput(
      URI.file("foo/bar1"),
      TEST_EDITOR_INPUT_ID
    );
    await rootGroup.openEditor(rootGroupInput, { pinned: true });
    const rightGroupInput = createTestFileEditorInput(
      URI.file("foo/bar2"),
      TEST_EDITOR_INPUT_ID
    );
  ...
  test("groups index / labels", async () => {
  ...
  test("groups label", async () => {
    ...
    const rightGroupLabelChangeListener = rightGroup.onDidModelChange(
      (e) => {
        if (e.kind === GroupModelChangeKind.GROUP_LABEL) {
  ...
  test("copy/merge groups", async () => {
    ...
    const rightGroup = part.addGroup(rootGroup, GroupDirection.RIGHT);
    part.activateGroup(rightGroup);
    const downGroup = part.copyGroup(
      rootGroup,
      rightGroup,
      GroupDirection.DOWN
    );
    ...
    let res = part.mergeGroup(rootGroup, rightGroup, {
    ...
    res = part.mergeGroup(rootGroup, rightGroup, {
      mode: MergeGroupMode.MOVE_EDITORS
    });
    assert.strictEqual(res, true);
  ...
  test("editor basics", async () => {
    ...
    const inputInactive = createTestFileEditorInput(
      URI.file("foo/bar/inactive"),
      TEST_EDITOR_INPUT_ID
    );
    ...
    assert.strictEqual(
      editorOpenEvents[0].editorIndex,
      0
    );
    assert.strictEqual(
      editorOpenEvents[1].editorIndex,
      1
    );
    ...
    assert.strictEqual(
      editorCloseEvents[0].editorIndex,
      1
    );
  ...
  test("moveEditor (same group)", async () => {
    ...
    assert.strictEqual(
      moveEvents[0].editorIndex,
      0
    );
    assert.strictEqual(
      moveEvents[0].oldEditorIndex,
      1
    );
    ...
    const res = group.moveEditors(
      [{ editor: inputInactive, options: { index: 1 } }],
      group
    );
    assert.strictEqual(res, true);
    assert.strictEqual(moveEvents.length, 2);
    assert.strictEqual(
      moveEvents[1].editorIndex,
      1
    );
    assert.strictEqual(
      moveEvents[1].oldEditorIndex,
      0
    );
  ...
  test("copyEditors (across groups)", async () => {
    ...
    assert.strictEqual(group.getEditorByIndex(2), input3);
    group.copyEditors(
      [{ editor: input1 }, { editor: input2 }, { editor: input3 }],
      rightGroup
    );
  ...
  test("replaceEditors", async () => {
    ...
    await group.replaceEditors([
      { editor: input, replacement: inputInactive }
    ]);
    assert.strictEqual(group.count, 1);
    assert.strictEqual(group.getEditorByIndex(0), inputInactive);
  });
  ...
  test("replaceEditors - forceReplaceDirty flag", async () => {
    ...
    accessor.fileDialogService.setConfirmResult(ConfirmResult.CANCEL);
    await group.replaceEditors([
      { editor: input1, replacement: input2, forceReplaceDirty: false }
    ]);
    assert.strictEqual(group.activeEditor, input1);
    assert.ok(!input1.gotDisposed);
    await group.replaceEditors([
      { editor: input1, replacement: input2, forceReplaceDirty: true }
    ]);
    assert.strictEqual(group.activeEditor, input2);
    assert.ok(input1.gotDisposed);
  });
  test("replaceEditors - proper index handling", async () => {
    ...
    const input5 = createTestFileEditorInput(
      URI.file("foo/bar5"),
      TEST_EDITOR_INPUT_ID
    );
    const input6 = createTestFileEditorInput(
      URI.file("foo/bar6"),
      TEST_EDITOR_INPUT_ID
    );
    const input7 = createTestFileEditorInput(
      URI.file("foo/bar7"),
      TEST_EDITOR_INPUT_ID
    );
    const input8 = createTestFileEditorInput(
      URI.file("foo/bar8"),
      TEST_EDITOR_INPUT_ID
    );
  ...
  test("replaceEditors - should be able to replace when side by side editor is involved with same input side by side", async () => {
    ...
    assert.strictEqual(group.getEditorByIndex(0), input);
    await group.replaceEditors([
      { editor: input, replacement: sideBySideInput }
    ]);
    assert.strictEqual(group.count, 1);
    assert.strictEqual(group.getEditorByIndex(0), sideBySideInput);
    await group.replaceEditors([
      { editor: sideBySideInput, replacement: input }
    ]);
    assert.strictEqual(group.count, 1);
    assert.strictEqual(group.getEditorByIndex(0), input);
  });
  ...
  test("find neighbour group (left/right)", async () => {
    const [part] = await createPart();
    const rootGroup = part.activeGroup;
    const rightGroup = part.addGroup(rootGroup, GroupDirection.RIGHT);
    assert.strictEqual(
      rightGroup,
      part.findGroup({ direction: GroupDirection.RIGHT }, rootGroup)
    );
    assert.strictEqual(
      rootGroup,
      part.findGroup({ direction: GroupDirection.LEFT }, rightGroup)
    );
  });
  test("find neighbour group (up/down)", async () => {
    const [part] = await createPart();
    const rootGroup = part.activeGroup;
    const downGroup = part.addGroup(rootGroup, GroupDirection.DOWN);
    assert.strictEqual(
      downGroup,
      part.findGroup({ direction: GroupDirection.DOWN }, rootGroup)
    );
    assert.strictEqual(
      rootGroup,
      part.findGroup({ direction: GroupDirection.UP }, downGroup)
    );
  });
  test("find group by location (left/right)", async () => {
    const [part] = await createPart();
    const rootGroup = part.activeGroup;
    const rightGroup = part.addGroup(rootGroup, GroupDirection.RIGHT);
    const downGroup = part.addGroup(rightGroup, GroupDirection.DOWN);
    assert.strictEqual(
      rootGroup,
      part.findGroup({ location: GroupLocation.FIRST })
    );
    assert.strictEqual(
      downGroup,
      part.findGroup({ location: GroupLocation.LAST })
    );
    assert.strictEqual(
      rightGroup,
      part.findGroup({ location: GroupLocation.NEXT }, rootGroup)
    );
    assert.strictEqual(
      rootGroup,
      part.findGroup({ location: GroupLocation.PREVIOUS }, rightGroup)
    );
    assert.strictEqual(
      downGroup,
      part.findGroup({ location: GroupLocation.NEXT }, rightGroup)
    );
    assert.strictEqual(
      rightGroup,
      part.findGroup({ location: GroupLocation.PREVIOUS }, downGroup)
    );
  });
  test("applyLayout (2x2)", async () => {
    const [part] = await createPart();
    part.applyLayout({
  ...
  test("sticky editors", async () => {
    ...
    const inputSticky = createTestFileEditorInput(
      URI.file("foo/bar/sticky"),
      TEST_EDITOR_INPUT_ID
    );
  ...
  test("selection: setSelection, isSelected, selectedEditors", async () => {
    ...
    await group.openEditors(
      [input1, input2, input3].map((editor) => ({
        editor,
        options: { pinned: true }
      }))
    );
  ...
  test("moveEditor with context (across groups)", async () => {
    ...
    const thirdInput = createTestFileEditorInput(
      URI.file("foo/bar/third"),
      TEST_EDITOR_INPUT_ID
    );
  ...
  test("onWillOpenEditor", async () => {
    ...
    const secondInput = createTestFileEditorInput(
      URI.file("foo/bar/second"),
      TEST_EDITOR_INPUT_ID
    );
    const thirdInput = createTestFileEditorInput(
      URI.file("foo/bar/third"),
  ...
  test("locked groups - auto locking via setting", async () => {
    ...
    await configurationService.setUserConfiguration("workbench", {
      editor: {
        autoLockGroups: { testEditorInputForEditorGroupService: true }
      }
    });
    ...
    let input1 = createTestFileEditorInput(
      URI.file("foo/bar1"),
      TEST_EDITOR_INPUT_ID
    );
    let input2 = createTestFileEditorInput(
      URI.file("foo/bar2"),
      TEST_EDITOR_INPUT_ID
    );
    ...
    input2 = createTestFileEditorInput(
      URI.file("foo/bar2"),
      TEST_EDITOR_INPUT_ID
    );
    await rootGroup.openEditor(input1, { pinned: true });
  ...
  test("maximize editor group", async () => {
    ...
    let maximizedValue;
    const maxiizeGroupEventDisposable = part.onDidChangeGroupMaximized(
      (maximized) => {
    ...
    assert.deepStrictEqual(part.getSize(rightGroup), {
      width: 0,
      height: 0
    });
    assert.deepStrictEqual(part.getSize(rightBottomGroup), {
      width: 0,
      height: 0
    });
    ...
    assert.deepStrictEqual(
      part.getSize(rightBottomGroup),
      sizeRightBottomGroup
    );
  ...
  test("transient editors - basics", async () => {
    ...
    const inputTransient = createTestFileEditorInput(
      URI.file("foo/bar/inactive"),
      TEST_EDITOR_INPUT_ID
    );
    await group.openEditor(input, { pinned: true });
  ...
  test("transient editors - overrides enablePreview setting", async () => {
    const instantiationService = workbenchInstantiationService(
      void 0,
      disposables
    );
  ...
  test("working sets - create / apply state", async () => {
    const [part] = await createPart();
    const input = createTestFileEditorInput(
      URI.file("foo/bar"),
      TEST_EDITOR_INPUT_ID
    ...
    const pane1 = await part.activeGroup.openEditor(input, {
      pinned: true
    });
  ...
  test("context key provider", async () => {
    ...
    const rawContextKey = new RawContextKey(
      "testContextKey",
      parts.activeGroup.id
    );
    ...
    let globalContextKeyValue = rootContextKeyService.getContextKeyValue(
      rawContextKey.key
    );
    let group1ContextKeyValue = group1.scopedContextKeyService.getContextKeyValue(
      rawContextKey.key
    );
    let group2ContextKeyValue = group2.scopedContextKeyService.getContextKeyValue(
      rawContextKey.key
    );
    ...
    globalContextKeyValue = rootContextKeyService.getContextKeyValue(
      rawContextKey.key
    );
    group1ContextKeyValue = group1.scopedContextKeyService.getContextKeyValue(
      rawContextKey.key
    );
    group2ContextKeyValue = group2.scopedContextKeyService.getContextKeyValue(
      rawContextKey.key
    );
    ...
    const group3ContextKeyValue = group3.scopedContextKeyService.getContextKeyValue(
      rawContextKey.key
    );
  ...
  test("context key provider: onDidChange", async () => {
    const disposables2 = new DisposableStore();
    const instantiationService = workbenchInstantiationService(
      {
        contextKeyService: (instantiationService2) => instantiationService2.createInstance(
  ...
  test("context key provider: active editor change", async () => {
    const disposables2 = new DisposableStore();
    const instantiationService = workbenchInstantiationService(
      {
        contextKeyService: (instantiationService2) => instantiationService2.createInstance(
    ...
    disposables2.add(parts.registerContextKeyProvider(contextKeyProvider));
    assert.strictEqual(
      isEqual(group1.activeEditor?.resource, input1.resource),
      true
    );
    let globalContextKeyValue = rootContextKeyService.getContextKeyValue(
      rawContextKey.key
