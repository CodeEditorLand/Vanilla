...
} from "../../../extensions/common/extensionManifestPropertiesService.js";
...
function createStorageService(instantiationService, disposableStore) {
  ...
  if (!service) {
    let workspaceContextService = instantiationService.get(
      IWorkspaceContextService
    );
    if (!workspaceContextService) {
      workspaceContextService = instantiationService.stub(
        IWorkspaceContextService,
        {
    ...
    service = instantiationService.stub(
      IStorageService,
...
class TestExtensionEnablementService extends ExtensionEnablementService {
  constructor(instantiationService) {
    ...
    const extensionManagementServerService = instantiationService.get(IExtensionManagementServerService) || instantiationService.stub(
      IExtensionManagementServerService,
      anExtensionManagementServerService(
        {
          ...
          extensionManagementService: {
            onInstallExtension: disposables.add(
              new Emitter()
            ).event,
            onDidInstallExtensions: disposables.add(
              new Emitter()
            ).event,
            onUninstallExtension: disposables.add(
              new Emitter()
            ).event,
            onDidUninstallExtension: disposables.add(
              new Emitter()
            ).event,
            onDidChangeProfile: disposables.add(
              new Emitter()
            ).event,
            onDidUpdateExtensionMetadata: disposables.add(
              new Emitter()
            ).event
          }
        },
    ...
    const extensionManagementService = disposables.add(
      instantiationService.createInstance(ExtensionManagementService)
    );
    const workbenchExtensionManagementService = instantiationService.get(IWorkbenchExtensionManagementService) || instantiationService.stub(
      IWorkbenchExtensionManagementService,
      extensionManagementService
    );
    const workspaceTrustManagementService = instantiationService.get(IWorkspaceTrustManagementService) || instantiationService.stub(
      IWorkspaceTrustManagementService,
    ...
    super(
      storageService,
      disposables.add(
        new GlobalExtensionEnablementService(
          storageService,
          extensionManagementService
        )
      ),
      ...
      instantiationService.get(IUserDataSyncEnablementService) || instantiationService.stub(IUserDataSyncEnablementService, {
        isEnabled() {
          return false;
        }
      }),
      instantiationService.get(IUserDataSyncAccountService) || instantiationService.stub(
        IUserDataSyncAccountService,
        UserDataSyncAccountService
      ),
      instantiationService.get(ILifecycleService) || instantiationService.stub(
        ILifecycleService,
        disposables.add(new TestLifecycleService())
      ),
      instantiationService.get(INotificationService) || instantiationService.stub(
      ...
      instantiationService.get(IExtensionManifestPropertiesService) || instantiationService.stub(
        IExtensionManifestPropertiesService,
        disposables.add(
          new ExtensionManifestPropertiesService(
            TestProductService,
  ...
  reset() {
    ...
    if (workspaceEnabledExtensions.length) {
      extensions = extensions.filter(
        (r) => !workspaceEnabledExtensions.some(
          (e) => areSameExtensions(e, r)
        )
      );
    }
    extensions.forEach(
      (d) => this.setEnablement(
        [aLocalExtension(d.id)],
...
suite("ExtensionEnablementService Test", () => {
  ...
  test("test disable an extension globally triggers the change event", async () => {
    ...
    assert.deepStrictEqual(target.args[0][0][0].identifier, {
      id: "pub.a"
    });
  });
  ...
  test("test state of globally disabled extension", () => {
    return testObject.setEnablement(
      ...
      () => assert.strictEqual(
        testObject.getEnablementState(aLocalExtension("pub.a")),
        EnablementState.DisabledGlobally
      )
    );
  });
  ...
  test("test enable an extension globally return truthy promise", async () => {
    ...
    const value = await testObject.setEnablement(
      [aLocalExtension("pub.a")],
      EnablementState.EnabledGlobally
    );
  ...
  test("test enable an extension also enables dependencies", async () => {
    ...
    assert.strictEqual(
      testObject.getEnablementState(target),
      EnablementState.EnabledGlobally
    );
    assert.strictEqual(
      testObject.getEnablementState(dep),
      EnablementState.EnabledGlobally
    );
  });
  ...
  test("test enable an extension with a dependency extension that cannot be enabled", async () => {
    ...
    const localWorkspaceDepExtension = aLocalExtension2(
      "pub.b",
      { extensionKind: ["workspace"] },
      { location: URI.file(`pub.b`) }
    );
    const remoteWorkspaceExtension = aLocalExtension2(
      "pub.a",
      { extensionKind: ["workspace"], extensionDependencies: ["pub.b"] },
      {
    ...
    const remoteWorkspaceDepExtension = aLocalExtension2(
      "pub.b",
      { extensionKind: ["workspace"] },
      {
        location: URI.file(`pub.b`).with({
          scheme: Schemas.vscodeRemote
        })
      }
    );
    installed.push(
      localWorkspaceDepExtension,
      remoteWorkspaceExtension,
      remoteWorkspaceDepExtension
    );
    testObject = disposableStore.add(
      new TestExtensionEnablementService(instantiationService)
    );
    ...
    assert.strictEqual(
      testObject.getEnablementState(remoteWorkspaceExtension),
      EnablementState.EnabledGlobally
    );
  });
  test("test enable an extension also enables packed extensions", async () => {
    installed.push(
      ...[
        aLocalExtension2("pub.a", { extensionPack: ["pub.b"] }),
        aLocalExtension("pub.b")
      ]
    );
    const target = installed[0];
  ...
  test("test isEnabled return false extension is disabled globally", () => {
    return testObject.setEnablement(
      ...
      () => assert.ok(!testObject.isEnabled(aLocalExtension("pub.a")))
    );
  });
  ...
  test("test isEnabled return true extension is not disabled", () => {
    return testObject.setEnablement(
      ...
      () => assert.ok(testObject.isEnabled(aLocalExtension("pub.b")))
    );
  });
  test("test canChangeEnablement return false for language packs", () => {
    assert.strictEqual(
      testObject.canChangeEnablement(
        aLocalExtension("pub.a", {
          localizations: [
            {
              languageId: "gr",
              translations: [{ id: "vscode", path: "path" }]
            }
          ]
        })
      ),
  ...
  test("test canChangeWorkspaceEnablement return true", () => {
    assert.strictEqual(
      testObject.canChangeWorkspaceEnablement(aLocalExtension("pub.a")),
      true
    );
  });
  ...
  test("test canChangeWorkspaceEnablement return false for auth extension", () => {
    assert.strictEqual(
      testObject.canChangeWorkspaceEnablement(
        aLocalExtension("pub.a", {
          authentication: [{ id: "a", label: "a" }]
        })
      ),
  ...
  test("test canChangeEnablement return true for system extensions when extensions are disabled in environment", () => {
    ...
    const extension = aLocalExtension(
      "pub.a",
      void 0,
  ...
  test("test extension does not support vitrual workspace is not enabled in virtual workspace", async () => {
    ...
    instantiationService.stub(IWorkspaceContextService, "getWorkspace", {
      folders: [
        { uri: URI.file("worskapceA").with({ scheme: "virtual" }) }
      ]
    });
    testObject = disposableStore.add(
      new TestExtensionEnablementService(instantiationService)
  ...
  test("test web extension from web extension management server and does not support vitrual workspace is enabled in virtual workspace", async () => {
    instantiationService.stub(
      ...
      anExtensionManagementServerService(
        ...
        anExtensionManagementServer("web", instantiationService)
      )
    );
    const extension = aLocalExtension2(
      "pub.a",
      {
        capabilities: { virtualWorkspaces: false },
        browser: "browser.js"
      },
      { location: URI.file(`pub.a`).with({ scheme: "web" }) }
    );
    instantiationService.stub(IWorkspaceContextService, "getWorkspace", {
      folders: [
        { uri: URI.file("worskapceA").with({ scheme: "virtual" }) }
  ...
  test("test web extension from remote extension management server and does not support vitrual workspace is disabled in virtual workspace", async () => {
    ...
    const extension = aLocalExtension2(
      ...
      { location: URI.file(`pub.a`).with({ scheme: "vscode-remote" }) }
    );
    instantiationService.stub(IWorkspaceContextService, "getWorkspace", {
      folders: [
        { uri: URI.file("worskapceA").with({ scheme: "virtual" }) }
  ...
  test("test enable a remote workspace extension and local ui extension that is a dependency of remote", async () => {
    ...
    const localUIExtension = aLocalExtension2(
      "pub.a",
      { main: "main.js", extensionKind: ["ui"] },
      { location: URI.file(`pub.a`) }
    );
    const remoteUIExtension = aLocalExtension2(
      "pub.a",
      { main: "main.js", extensionKind: ["ui"] },
      { location: URI.file(`pub.a`).with({ scheme: "vscode-remote" }) }
    );
    const target = aLocalExtension2(
      "pub.b",
      { main: "main.js", extensionDependencies: ["pub.a"] },
      { location: URI.file(`pub.b`).with({ scheme: "vscode-remote" }) }
    );
    testObject = disposableStore.add(
      new TestExtensionEnablementService(instantiationService)
    );
    ...
    assert.strictEqual(
      testObject.getEnablementState(localUIExtension),
      EnablementState.EnabledGlobally
    );
  });
  ...
  test("test extension does not support untrusted workspaces is disabled in untrusted workspace", () => {
    const extension = aLocalExtension2("pub.a", {
      main: "main.js",
      capabilities: {
        untrustedWorkspaces: { supported: false, description: "hello" }
      }
    });
  ...
  test("test local workspace extension is disabled by kind", async () => {
    instantiationService.stub(
      IExtensionManagementServerService,
      aMultiExtensionManagementServerService(instantiationService)
    );
    const localWorkspaceExtension = aLocalExtension2(
      "pub.a",
      { extensionKind: ["workspace"] },
      { location: URI.file(`pub.a`) }
    );
    ...
    assert.deepStrictEqual(
      testObject.getEnablementState(localWorkspaceExtension),
  ...
  test("test canChangeEnablement return true when the local workspace extension is disabled by kind", () => {
    ...
    assert.strictEqual(
      testObject.canChangeEnablement(localWorkspaceExtension),
      false
    );
  });
  ...
  test("test web extension on local server is disabled by kind when web worker is not enabled", async () => {
    ...
    assert.strictEqual(
      testObject.isEnabled(localWorkspaceExtension),
      false
    );
    assert.deepStrictEqual(
      testObject.getEnablementState(localWorkspaceExtension),
  ...
  test("test web extension on web server is not disabled by kind", async () => {
    ...
    const webExtension = aLocalExtension2(
      "pub.a",
      { browser: "browser.js" },
      { location: URI.file(`pub.a`).with({ scheme: "web" }) }
    );
    ...
    assert.deepStrictEqual(
      testObject.getEnablementState(webExtension),
      EnablementState.EnabledGlobally
    );
  });
  ...
  test("test extension is disabled by dependency if it has a dependency that is disabled", async () => {
    ...
    assert.strictEqual(
      testObject.getEnablementState(installed[1]),
  ...
  test("test extension is disabled by dependency if it has a dependency that is disabled by virtual workspace", async () => {
    ...
    await testObject.waitUntilInitialized();
    assert.strictEqual(
      testObject.getEnablementState(installed[0]),
      EnablementState.DisabledByVirtualWorkspace
    );
    assert.strictEqual(
      testObject.getEnablementState(installed[1]),
  ...
  test("test extension is not disabled by dependency if it has a dependency that is disabled by extension kind", async () => {
    ...
    const remoteWorkspaceExtension = aLocalExtension2(
      "pub.n",
      { extensionKind: ["workspace"], extensionDependencies: ["pub.a"] },
      {
        location: URI.file(`pub.a`).with({
          scheme: Schemas.vscodeRemote
        })
    ...
    assert.strictEqual(
      testObject.getEnablementState(remoteUIExtension),
      EnablementState.DisabledByExtensionKind
    );
    assert.strictEqual(
      testObject.getEnablementState(remoteWorkspaceExtension),
  ...
  test("test override workspace to trusted when getting extensions enablements", async () => {
    ...
    assert.strictEqual(
      testObject.getEnablementStates([extension], { trusted: true })[0],
      EnablementState.EnabledGlobally
    );
  });
  test("test override workspace to not trusted when getting extensions enablements", async () => {
    ...
    assert.strictEqual(
      testObject.getEnablementStates([extension], { trusted: false })[0],
      EnablementState.DisabledByTrustRequirement
    );
  });
  test("test update extensions enablements on trust change triggers change events for extensions depending on workspace trust", async () => {
    ...
    assert.deepStrictEqual(target.args[0][0][1].identifier, {
      id: "pub.c"
    });
  });
  test("test adding an extension that was disabled", async () => {
    ...
    const target = sinon.spy();
    disposableStore.add(testObject.onEnablementChanged(target));
    didChangeProfileExtensionsEvent.fire({
      added: [extension],
      removed: []
    });
    assert.ok(!testObject.isEnabled(extension));
...
function aMultiExtensionManagementServerService(instantiationService) {
  const localExtensionManagementServer = anExtensionManagementServer(
    "vscode-local",
    instantiationService
  );
  const remoteExtensionManagementServer = anExtensionManagementServer(
    "vscode-remote",
    instantiationService
  );
...
function aLocalExtension(id, contributes, type) {
  return aLocalExtension2(
    id,
    contributes ? { contributes } : {},
    isUndefinedOrNull(type) ? {} : { type }
  );
}
