...
import {
  createValidator,
  getInvalidTypeError
} from "../../common/preferencesValidation.js";
suite("Preferences Validation", () => {
  ...
  test("numerical objects work", () => {
    ...
    }
    {
      const obj = new Tester({
        type: "object",
        properties: {
          b: { type: "integer", minimum: 2, maximum: 5.5 }
        }
      });
      obj.accepts({ b: 2 });
      obj.accepts({ b: 3 });
  ...
  test("pattern", () => {
    const arr = new ArrayTester({
      type: "array",
      items: { type: "string", pattern: "^(hello)*$" }
    });
    arr.accepts(["hello"]);
  ...
  test("Unicode pattern", () => {
    ...
    arr.rejects(["hello", "#world"]).withMessage(
      `Value '#world' must match regex`
    );
  });
