...
suite("KeybindingsEditorModel", () => {
  ...
  setup(() => {
    ...
    testObject = disposables.add(
      instantiationService.createInstance(KeybindingsEditorModel, OS)
    );
    disposables.add(
  ...
  test("fetch returns default keybindings", async () => {
    const expected = prepareKeybindingService(
      aResolvedKeybindingItem({
        ...
        firstChord: { keyCode: KeyCode.Escape }
      }),
      aResolvedKeybindingItem({
        command: "b" + uuid.generateUuid(),
        firstChord: { keyCode: KeyCode.Escape },
        secondChord: { keyCode: KeyCode.Escape }
      })
    );
    await testObject.resolve(/* @__PURE__ */ new Map());
  ...
  test("fetch returns default keybindings at the top", async () => {
    ...
    await testObject.resolve(/* @__PURE__ */ new Map());
    const actuals = asResolvedKeybindingItems(
      testObject.fetch("").slice(0, 2),
      true
    );
    assertKeybindingItems(actuals, expected);
  });
  test("fetch returns default keybindings sorted by command id", async () => {
    const keybindings = prepareKeybindingService(
      ...
      aResolvedKeybindingItem({
        command: "a" + uuid.generateUuid(),
        firstChord: { keyCode: KeyCode.Backspace }
      })
    );
  ...
  test("convert keybinding without title to entry", async () => {
    ...
    assert.strictEqual(
      actual.keybindingItem.keybinding.getAriaLabel(),
      expected.resolvedKeybinding.getAriaLabel()
    );
    assert.strictEqual(
      actual.keybindingItem.when,
      expected.when.serialize()
    );
  });
  ...
  test("convert without title and binding to entry", async () => {
    ...
    await testObject.resolve(/* @__PURE__ */ new Map());
    const actual = testObject.fetch("").filter(
      (element) => element.keybindingItem.command === "command_without_keybinding"
    )[0];
    assert.strictEqual(
      actual.keybindingItem.command,
      "command_without_keybinding"
    );
    assert.strictEqual(actual.keybindingItem.commandLabel, "");
    assert.strictEqual(actual.keybindingItem.commandDefaultLabel, null);
    assert.strictEqual(actual.keybindingItem.keybinding, void 0);
  ...
  test("filter by modifiers and key", async () => {
    ...
    const expected = aResolvedKeybindingItem({
      ...
      firstChord: {
        keyCode: KeyCode.Escape,
        modifiers: { altKey: true, metaKey: true }
      },
      when: "whenContext1 && whenContext2",
      isDefault: false
    });
  ...
  test("filter by modifiers in random order and key", async () => {
    ...
    const expected = aResolvedKeybindingItem({
      ...
      firstChord: {
        keyCode: KeyCode.Escape,
        modifiers: { shiftKey: true, metaKey: true }
      },
      when: "whenContext1 && whenContext2",
      isDefault: false
    });
  ...
  test("filter by first part", async () => {
    ...
    const expected = aResolvedKeybindingItem({
      ...
      secondChord: { keyCode: KeyCode.Delete },
      when: "whenContext1 && whenContext2",
      isDefault: false
    });
    prepareKeybindingService(
  ...
  test("filter matches first part and in chord part", async () => {
    ...
    prepareKeybindingService(
      ...
      aResolvedKeybindingItem({
        ...
        secondChord: { keyCode: KeyCode.UpArrow },
        when: "whenContext1 && whenContext2",
        isDefault: false
      })
    );
  ...
  test("filter exact matches", async () => {
    ...
    const expected = aResolvedKeybindingItem({
      command,
      firstChord: { keyCode: KeyCode.KeyC, modifiers: { ctrlKey: true } },
      when: "whenContext1 && whenContext2",
      isDefault: false
    });
    prepareKeybindingService(
  ...
  test("filter exact matches with user settings label", async () => {
    ...
    const expected = aResolvedKeybindingItem({
      command,
      firstChord: { keyCode: KeyCode.DownArrow }
    });
    prepareKeybindingService(
      expected,
      aResolvedKeybindingItem({
  ...
  test("filter exact matches also return chords", async () => {
    ...
    const expected = aResolvedKeybindingItem({
      command,
      firstChord: { keyCode: KeyCode.KeyK, modifiers: { ctrlKey: true } },
      secondChord: {
        keyCode: KeyCode.KeyC,
        modifiers: { ctrlKey: true }
      },
  ...
  test("filter by extension", async () => {
    ...
    extensions.push(
      {
        identifier: new ExtensionIdentifier("foo"),
        displayName: "foo bar"
      },
      {
        identifier: new ExtensionIdentifier("bar"),
        displayName: "bar foo"
      }
    );
    disposables.add(
      MenuRegistry.addCommand({
        ...
        category: "category",
        source: {
          id: extensions[1].identifier.value,
          title: extensions[1].displayName
        }
      })
    );
    const expected = aResolvedKeybindingItem({
    ...
    prepareKeybindingService(
      expected,
      aResolvedKeybindingItem({ command: command2, isDefault: true })
    );
    await testObject.resolve(/* @__PURE__ */ new Map());
  ...
  function assertKeybindingItem(actual, expected) {
    ...
    if (actual.resolvedKeybinding) {
      ...
      assert.strictEqual(
        actual.resolvedKeybinding.getLabel(),
        expected.resolvedKeybinding.getLabel()
      );
    } else {
      assert.ok(!expected.resolvedKeybinding);
    }
  }
  function aResolvedKeybindingItem({
    command,
    when,
    isDefault,
    firstChord,
    ...
    const keybinding = chords.length > 0 ? new USLayoutResolvedKeybinding(chords, OS) : void 0;
    return new ResolvedKeybindingItem(
      keybinding,
      command || "some command",
      null,
      when ? ContextKeyExpr.deserialize(when) : void 0,
      isDefault === void 0 ? true : isDefault,
      extensionId ?? null,
      false
    );
  }
  function asResolvedKeybindingItems(keybindingEntries, keepUnassigned = false) {
    if (!keepUnassigned) {
      keybindingEntries = keybindingEntries.filter(
        (keybindingEntry) => !!keybindingEntry.keybindingItem.keybinding
      );
    }
    return keybindingEntries.map(
      (entry) => entry.keybindingItem.keybindingItem
    );
  }
});
