...
suite("WorkingCopyFileService", () => {
  ...
  test("move - source identical to target", async function() {
    const sourceModel = instantiationService.createInstance(
      TextFileEditorModel,
      toResource.call(this, "/path/file.txt"),
      "utf8",
      void 0
    );
    accessor.textFileService.files.add(
    ...
    const eventCounter = await testEventsMoveOrCopy(
      [
        {
          file: {
            source: sourceModel.resource,
            target: sourceModel.resource
          },
          overwrite: true
        }
      ],
  ...
  test("move - one source == target and another source != target", async function() {
    const sourceModel1 = instantiationService.createInstance(
      TextFileEditorModel,
      toResource.call(this, "/path/file1.txt"),
      "utf8",
      void 0
    );
    const sourceModel2 = instantiationService.createInstance(
      TextFileEditorModel,
      toResource.call(this, "/path/file2.txt"),
      "utf8",
      void 0
    );
    const targetModel2 = instantiationService.createInstance(
      TextFileEditorModel,
    ...
    const eventCounter = await testEventsMoveOrCopy(
      [
        {
          file: {
            source: sourceModel1.resource,
            target: sourceModel1.resource
          },
          overwrite: true
        },
        {
          file: {
            source: sourceModel2.resource,
            target: targetModel2.resource
          },
          overwrite: true
        }
      ],
  ...
  test("registerWorkingCopyProvider", async function() {
    ...
    assert.strictEqual(
      dirty.length,
      1,
      "Should have unregistered our provider"
    );
    assert.strictEqual(dirty[0], model1);
  });
  test("createFolder", async function() {
    ...
    disposables.add(
      accessor.workingCopyFileService.onWillRunWorkingCopyFileOperation(
        (e) => {
    ...
    disposables.add(
      accessor.workingCopyFileService.onDidRunWorkingCopyFileOperation(
        (e) => {
          assert.strictEqual(e.files.length, 1);
          const file = e.files[0];
          assert.strictEqual(
  ...
  test("cancellation of participants", async function() {
    ...
    let promise = accessor.workingCopyFileService.create(
      [{ resource }],
      cts.token
    );
    cts.cancel();
    ...
    canceled = false;
    cts = new CancellationTokenSource();
    promise = accessor.workingCopyFileService.delete(
      [{ resource }],
      cts.token
    );
    cts.cancel();
  ...
  async function testEventsMoveOrCopy(files, move) {
    ...
    const listener2 = accessor.workingCopyFileService.onDidRunWorkingCopyFileOperation(
      (e) => {
        eventCounter++;
      }
    );
  ...
  async function testMoveOrCopy(files, move, targetDirty) {
    ...
    const models = await Promise.all(
      files.map(async ({ source, target }, i) => {
        ...
        const targetModel = instantiationService.createInstance(
          TextFileEditorModel,
          target,
          "utf8",
          void 0
        ...
        assert.ok(
          accessor.textFileService.isDirty(sourceModel.resource)
        );
        if (targetDirty) {
          await targetModel.resolve();
          targetModel.textEditorModel.setValue("bar" + i);
          assert.ok(
            accessor.textFileService.isDirty(targetModel.resource)
          );
        }
    ...
    if (move) {
      await accessor.workingCopyFileService.move(
        models.map((model) => ({
          file: {
            source: model.sourceModel.resource,
            target: model.targetModel.resource
          },
    ...
    for (let i = 0; i < models.length; i++) {
      ...
      assert.strictEqual(
        targetModel.textEditorModel.getValue(),
        "foo" + i
      );
      if (move) {
        assert.ok(
