...
suite("WorkingCopyHistoryTracker", () => {
  ...
  setup(async () => {
    testDir = URI.file(
      randomPath(join("vsctests", "workingcopyhistorytracker"))
    ).with({ scheme: Schemas.inMemory });
    historyHome = joinPath(testDir, "User", "History");
    workHome = joinPath(testDir, "work");
    workingCopyHistoryService = disposables.add(
      new TestWorkingCopyHistoryService(disposables)
    );
    ...
    await fileService.writeFile(
      testFile1Path,
      VSBuffer.fromString(testFile1PathContents)
    );
    await fileService.writeFile(
      testFile2Path,
      VSBuffer.fromString(testFile2PathContents)
    );
  });
  function createTracker() {
    return new WorkingCopyHistoryTracker(
      ...
      disposables.add(
        new UriIdentityService(disposables.add(new TestFileService()))
      ),
  ...
  test("history entry added on save", async () => {
    const workingCopy1 = disposables.add(
      new TestWorkingCopy(testFile1Path)
    );
    const workingCopy2 = disposables.add(
      new TestWorkingCopy(testFile2Path)
    );
    const stat1 = await fileService.resolve(workingCopy1.resource, {
      resolveMetadata: true
    });
    const stat2 = await fileService.resolve(workingCopy2.resource, {
      resolveMetadata: true
    });
    ...
    disposables.add(
      workingCopyHistoryService.onDidAddEntry((e) => {
        if (isEqual(
          e.entry.workingCopy.resource,
          workingCopy1.resource
    ...
    disposables.add(
      workingCopyHistoryService.onDidAddEntry((e) => {
        if (isEqual(e.entry.workingCopy.resource, workingCopy1.resource)) {
          assert.fail(
            "Unexpected working copy history entry: " + e.entry.workingCopy.resource.toString()
          );
        }
  ...
  test("entries moved (file rename)", async () => {
    const entriesMoved = Event.toPromise(
      workingCopyHistoryService.onDidMoveEntries
    );
    ...
    let entries = await workingCopyHistoryService.getEntries(
      workingCopy.resource,
      CancellationToken.None
    );
    assert.strictEqual(entries.length, 3);
    const renamedWorkingCopyResource = joinPath(
      dirname(workingCopy.resource),
      "renamed.txt"
    );
    await workingCopyHistoryService._fileService.move(
      workingCopy.resource,
      renamedWorkingCopyResource
    );
    await entriesMoved;
    entries = await workingCopyHistoryService.getEntries(
      workingCopy.resource,
      CancellationToken.None
    );
    ...
    const all = await workingCopyHistoryService.getAll(
      CancellationToken.None
    );
  ...
  test("entries moved (folder rename)", async () => {
    ...
    const renamedWorkingCopy1Resource = joinPath(
      renamedWorkHome,
      basename(workingCopy1.resource)
    );
    const renamedWorkingCopy2Resource = joinPath(
      renamedWorkHome,
      basename(workingCopy2.resource)
    );
    await entriesMoved;
    entries = await workingCopyHistoryService.getEntries(
      workingCopy1.resource,
    ...
    for (const resource of all) {
      if (resource.toString() !== renamedWorkingCopy1Resource.toString() && resource.toString() !== renamedWorkingCopy2Resource.toString()) {
        assert.fail(
          `Unexpected history resource: ${resource.toString()}`
        );
      }
    }
  });
