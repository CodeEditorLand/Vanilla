...
suite("StoredFileWorkingCopy (with custom save)", () => {
  ...
  setup(() => {
    ...
    workingCopy = disposables.add(
      new StoredFileWorkingCopy(
        "testStoredFileWorkingCopyType",
        resource,
        basename(resource),
        factory,
        (options) => workingCopy.resolve(options),
  ...
  test("save (custom implemented)", async () => {
    ...
    assert.strictEqual(
      workingCopy.hasState(StoredFileWorkingCopyState.ERROR),
      true
    );
  });
...
suite("StoredFileWorkingCopy", () => {
  ...
  function createWorkingCopy(uri = resource) {
    const workingCopy2 = new StoredFileWorkingCopy(
      "testStoredFileWorkingCopyType",
      uri,
      basename(uri),
      factory,
      (options) => workingCopy2.resolve(options),
      accessor.fileService,
      accessor.logService,
      accessor.workingCopyFileService,
      accessor.filesConfigurationService,
  ...
  test("orphaned tracking", async () => {
    return runWithFakedTimers({}, async () => {
      assert.strictEqual(
        workingCopy.hasState(StoredFileWorkingCopyState.ORPHAN),
        false
      );
  ...
  test("dirty / modified", async () => {
    ...
    assert.strictEqual(workingCopy.isDirty(), false);
    assert.strictEqual(
      workingCopy.hasState(StoredFileWorkingCopyState.DIRTY),
      false
    );
    await workingCopy.resolve();
  ...
  test("dirty - working copy marks non-dirty when undo reaches saved version ID", async () => {
    ...
    workingCopy.model?.fireContentChangeEvent({
      isRedoing: false,
      isUndoing: true
    });
    assert.strictEqual(workingCopy.isDirty(), false);
  ...
  test("resolve (without backup)", async () => {
    ...
    await workingCopy.resolve({
      contents: bufferToStream(
        VSBuffer.fromString("hello initial contents")
      )
    });
    assert.strictEqual(workingCopy.isDirty(), true);
    assert.strictEqual(
      workingCopy.model?.contents,
      "hello initial contents"
    );
  ...
  test("resolve (with backup)", async () => {
    await workingCopy.resolve({
      contents: bufferToStream(VSBuffer.fromString("hello backup"))
    });
  ...
  test("resolve (with backup, preserves metadata and orphaned state)", async () => {
    return runWithFakedTimers({}, async () => {
      ...
      const orphanedPromise = Event.toPromise(
        workingCopy.onDidChangeOrphaned
      );
      accessor.fileService.notExistsSet.set(resource, true);
      accessor.fileService.fireFileChanges(
  ...
  test("resolve (updates orphaned state accordingly)", async () => {
    return runWithFakedTimers({}, async () => {
      ...
      try {
        accessor.fileService.readShouldThrowError = new FileOperationError(
          "file not found",
  ...
  test("stat.readonly and stat.locked can change when decreased mtime is ignored", async () => {
    await workingCopy.resolve();
    ...
    assert.strictEqual(
      getLastResolvedFileStat(workingCopy)?.mtime,
      stat.mtime,
      "mtime should not decrease"
    );
    assert.notStrictEqual(
      getLastResolvedFileStat(workingCopy)?.readonly,
      stat.readonly,
      "readonly should have changed despite simultaneous attempt to decrease mtime"
    );
    assert.notStrictEqual(
      getLastResolvedFileStat(workingCopy)?.locked,
      stat.locked,
      "locked should have changed despite simultaneous attempt to decrease mtime"
    );
  });
  ...
  test("resolve (FILE_NOT_MODIFIED_SINCE still updates readonly state)", async () => {
    ...
    await workingCopy.resolve();
    assert.strictEqual(workingCopy.isReadonly(), false);
    const stat = await accessor.fileService.resolve(workingCopy.resource, {
  ...
  test("save (no errors) - save reason", async () => {
    ...
    await workingCopy.resolve();
    workingCopy.model?.updateContents("hello save");
    const source = SaveSourceRegistry.registerSource(
      "testSource",
      "Hello Save"
    );
  ...
  test("save (errors)", async () => {
    ...
    assert.strictEqual(
      workingCopy.hasState(StoredFileWorkingCopyState.SAVED),
      false
    );
    assert.strictEqual(
    ...
    assert.strictEqual(
      workingCopy.hasState(StoredFileWorkingCopyState.CONFLICT),
      false
    );
    assert.strictEqual(workingCopy.isDirty(), true);
  ...
  test("save - returns false when save fails", async () => {
    await workingCopy.resolve();
    try {
      accessor.fileService.writeShouldThrowError = new FileOperationError(
        "write error",
  ...
  test("save participant", async () => {
    await workingCopy.resolve();
    assert.strictEqual(
      accessor.workingCopyFileService.hasSaveParticipants,
      false
    );
  ...
  test("Save Participant, calling save from within is unsupported but does not explode (sync save)", async () => {
    await workingCopy.resolve();
    await testSaveFromSaveParticipant(workingCopy, false);
  });
  test("Save Participant, calling save from within is unsupported but does not explode (async save)", async () => {
    await workingCopy.resolve();
  ...
  test("Save Participant carries context", async () => {
    await workingCopy.resolve();
    const from = URI.file("testFrom");
    assert.strictEqual(
      accessor.workingCopyFileService.hasSaveParticipants,
  ...
  test("state", async () => {
    ...
    await workingCopy.resolve({
      contents: bufferToStream(VSBuffer.fromString("hello state"))
    });
    assert.strictEqual(
      workingCopy.hasState(StoredFileWorkingCopyState.DIRTY),
      true
