...
suite("WorkingCopyBackupTracker (native)", function() {
  ...
  setup(async () => {
    testDir = URI.file(
      join(generateUuid(), "vsctests", "workingcopybackuptracker")
    ...
    workspaceBackupPath = joinPath(
      backupHome,
      hash(workspaceResource.toString()).toString(16)
    );
    ...
    return accessor.fileService.writeFile(
      workspacesJsonPath,
      VSBuffer.fromString("")
    );
  });
  ...
  async function createTracker(autoSaveEnabled = false) {
    ...
    accessor = instantiationService.createInstance(TestServiceAccessor);
    const tracker = instantiationService.createInstance(
      TestWorkingCopyBackupTracker
    );
    const cleanup = async () => {
  ...
  async function trackBackupsTest(resource, autoSave) {
    ...
    assert.strictEqual(
      accessor2.workingCopyBackupService.hasBackupSync(fileModel),
      true
    );
    fileModel.dispose();
  ...
  test("onWillShutdown - veto if user cancels (hot.exit: off)", async function() {
    ...
    accessor2.filesConfigurationService.testOnFilesConfigurationChange({
      files: { hotExit: "off" }
    });
    await model?.resolve();
  ...
  test("onWillShutdown - no veto and backups cleaned up if user does not want to save (hot.exit: off)", async function() {
    ...
    assert.ok(!veto);
    assert.ok(
      accessor2.workingCopyBackupService.discardedBackups.length > 0
    );
    await cleanup();
  });
  test("onWillShutdown - no backups discarded when shutdown without dirty but tracker not ready", async () => {
    const { accessor: accessor2, cleanup } = await createTracker();
    const event = new TestBeforeShutdownEvent();
    accessor2.lifecycleService.fireBeforeShutdown(event);
    const veto = await event.value;
  ...
  test("onWillShutdown - backups discarded when shutdown without dirty", async () => {
  ...
  suite("Hot Exit", () => {
    ...
    async function hotExitTest(setting, shutdownReason, multipleWindows, workspace, shouldVeto) {
      ...
      if (!workspace) {
        accessor2.contextService.setWorkspace(
          new Workspace("empty:1508317022751")
        );
      }
      if (multipleWindows) {
      ...
      assert.strictEqual(
        accessor2.workingCopyBackupService.discardedBackups.length,
        0
      );
