...
} from "../../common/fileWorkingCopyManager.js";
import {
  NO_TYPE_ID,
...
} from "./storedFileWorkingCopy.test.js";
import {
...
suite("UntitledFileWorkingCopyManager", () => {
  ...
  setup(() => {
    ...
    disposables.add(
      accessor.fileService.registerProvider(
        Schemas.file,
        disposables.add(new TestInMemoryFileSystemProvider())
      )
    );
    disposables.add(
      accessor.fileService.registerProvider(
  ...
  test("basics", async () => {
    ...
    assert.strictEqual(
      manager.untitled.get(URI.file("/some/invalidPath")),
      void 0
    );
    assert.strictEqual(
      manager.untitled.get(
        URI.file("/some/invalidPath").with({
    ...
    assert.strictEqual(
      manager.untitled.get(workingCopy1.resource),
      workingCopy1
    );
    assert.strictEqual(
      manager.untitled.get(workingCopy2.resource),
      workingCopy2
    );
  ...
  test("resolve - with initial value", async () => {
    ...
    const workingCopy1 = await manager.untitled.resolve({
      contents: {
        value: bufferToStream(VSBuffer.fromString("Hello World"))
      }
    });
    assert.strictEqual(workingCopy1.isModified(), true);
    assert.strictEqual(workingCopy1.isDirty(), true);
  ...
  test("resolve begins counter from 1 for disposed untitled", async () => {
    ...
    untitled1.dispose();
    const untitled1Again = disposables.add(
      await manager.untitled.resolve()
    );
    assert.strictEqual(
      untitled1.resource.toString(),
      untitled1Again.resource.toString()
    );
  });
  test("resolve - existing", async () => {
    ...
    const workingCopy2 = await manager.untitled.resolve({
      untitledResource: workingCopy1.resource
    });
  ...
  test("resolve - untitled resource used for new working copy", async () => {
    ...
    const validUntitledResource = invalidUntitledResource.with({
      scheme: Schemas.untitled
    });
    const workingCopy1 = await manager.untitled.resolve({
    ...
    assert.notStrictEqual(
      workingCopy1.resource.toString(),
      invalidUntitledResource.toString()
    );
    const workingCopy2 = await manager.untitled.resolve({
      untitledResource: validUntitledResource
    });
    assert.strictEqual(
      workingCopy2.resource.toString(),
      validUntitledResource.toString()
    );
    workingCopy1.dispose();
    workingCopy2.dispose();
  });
  ...
  test("save - without associated resource", async () => {
    ...
    accessor.fileDialogService.setPickFileToSave(
      URI.file("simple/file.txt")
    );
    const result = await workingCopy.save();
    assert.ok(result);
    assert.strictEqual(
      manager.untitled.get(workingCopy.resource),
      void 0
    );
    workingCopy.dispose();
  });
  test("save - with associated resource", async () => {
    ...
    workingCopy.model?.updateContents(
      "Simple Save with associated resource"
    );
    accessor.fileService.notExistsSet.set(
      URI.from({ scheme: Schemas.file, path: "/some/associated.txt" }),
      true
    );
    const result = await workingCopy.save();
    assert.ok(result);
  ...
  test("manager with different types produce different URIs", async () => {
    try {
      ...
      const untitled1OriginalType = disposables.add(
        await manager.untitled.resolve()
      );
      const untitled1OtherType = disposables.add(
        await manager.untitled.resolve()
      );
      assert.notStrictEqual(
        untitled1OriginalType.resource.toString(),
        untitled1OtherType.resource.toString()
      );
    } finally {
      manager.destroy();
    }
