...
class TestWorkingCopyHistoryService extends NativeWorkingCopyHistoryService {
  static {
    __name(this, "TestWorkingCopyHistoryService");
  }
  _fileService;
...
suite("WorkingCopyHistoryService", () => {
  ...
  test("getEntries - corrupt meta.json is no problem", async () => {
    ...
    assertEntryEqual(
      ...
      entry1,
      false
      /* skip timestamp that is unreliable when entries.json is gone */
    );
  });
  ...
  __name(assertEntryEqual, "assertEntryEqual");
  ...
  test("entries are merged when source is same", async () => {
    ...
    assert.strictEqual(entries.length, 1);
    assertEntryEqual(entries[0], entry3);
    service._configurationService.setUserConfiguration("workbench.localHistory.mergeWindow", void 0);
    await addEntry({ resource: workingCopy1.resource, source: "test-source" }, CancellationToken.None);
    await addEntry({ resource: workingCopy1.resource, source: "test-source" }, CancellationToken.None);
    entries = await service.getEntries(workingCopy1.resource, CancellationToken.None);
    assert.strictEqual(entries.length, 3);
