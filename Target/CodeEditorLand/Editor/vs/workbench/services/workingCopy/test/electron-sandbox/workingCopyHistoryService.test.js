...
suite("WorkingCopyHistoryService", () => {
  ...
  setup(async () => {
    testDir = URI.file(
      join(generateUuid(), "vsctests", "workingcopyhistoryservice")
    ...
    await fileService.writeFile(
      testFile3Path,
      VSBuffer.fromString(testFile3PathContents)
    );
  });
  ...
  test("addEntry", async () => {
    ...
    const entry2B = await addEntry(
      { resource: workingCopy2.resource },
      CancellationToken.None
    );
    assert.strictEqual(
  ...
  test("renameEntry", async () => {
    ...
    service.dispose();
    service = disposables.add(
      new TestWorkingCopyHistoryService(disposables, fileService)
    );
    entries = await service.getEntries(
      workingCopy1.resource,
      CancellationToken.None
  ...
  test("removeEntry - deletes history entries folder when last entry removed", async () => {
    ...
    assert.strictEqual(
      await fileService.exists(dirname(entry.location)),
      true
    );
    entry = (await service.getEntries(
      workingCopy1.resource,
      CancellationToken.None
    )).at(0);
    assert.ok(entry);
  ...
  test("getAll", async () => {
    ...
    const workingCopy3 = disposables.add(
      new TestWorkingCopy(testFile3Path)
    );
    await addEntry(
  ...
  function assertEntryEqual(entryA, entryB, assertTimestamp = true) {
    ...
    assert.strictEqual(
      entryA.location.toString(),
      entryB.location.toString()
    );
    if (assertTimestamp) {
  ...
  test("entries are merged when source is same", async () => {
    ...
    assert.strictEqual(replaced, void 0);
    const entry2 = await addEntry(
      { resource: workingCopy1.resource, source: "test-source" },
      CancellationToken.None
    );
  ...
  test("move entries (file rename)", async () => {
    ...
    assert.ok(entries[3].sourceDescription);
  ...
  test("move entries (file rename) - preserves previous entries (no new entries)", async () => {
    const workingCopyTarget = disposables.add(
      new TestWorkingCopy(testFile1Path)
    );
    const workingCopySource = disposables.add(
      new TestWorkingCopy(testFile2Path)
    );
    const entry1 = await addEntry(
  ...
  test("move entries (file rename) - preserves previous entries (new entries)", async () => {
    ...
    assert.ok(entries[6].sourceDescription);
    const all = await service.getAll(CancellationToken.None);
    assert.strictEqual(all.length, 1);
    assert.strictEqual(
      all[0].toString(),
