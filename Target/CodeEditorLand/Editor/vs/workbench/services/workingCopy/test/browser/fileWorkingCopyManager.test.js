...
suite("FileWorkingCopyManager", () => {
  ...
  test("onDidCreate, get, workingCopies", async () => {
    ...
    assert.strictEqual(
      manager.get(fileWorkingCopy.resource),
      fileWorkingCopy
    );
    assert.strictEqual(
      manager.get(untitledFileWorkingCopy.resource),
      untitledFileWorkingCopy
    );
    const sameFileWorkingCopy = disposables.add(
      await manager.resolve(fileUri)
    );
    const sameUntitledFileWorkingCopy = disposables.add(
      await manager.resolve({
        untitledResource: untitledFileWorkingCopy.resource
      })
    );
    assert.strictEqual(sameFileWorkingCopy, fileWorkingCopy);
    assert.strictEqual(
      sameUntitledFileWorkingCopy,
      untitledFileWorkingCopy
    );
    assert.strictEqual(manager.workingCopies.length, 2);
    assert.strictEqual(createCounter, 2);
  ...
  test("resolve", async () => {
    const fileWorkingCopy = disposables.add(
      await manager.resolve(URI.file("/test.html"))
    );
    assert.ok(fileWorkingCopy instanceof StoredFileWorkingCopy);
    assert.strictEqual(
      await manager.stored.resolve(fileWorkingCopy.resource),
      fileWorkingCopy
    );
    const untitledFileWorkingCopy = disposables.add(
      await manager.resolve()
    );
    ...
    assert.strictEqual(
      await manager.resolve(untitledFileWorkingCopy.resource),
      untitledFileWorkingCopy
    );
  });
  test("destroy", async () => {
    ...
    await manager.resolve({
      contents: {
        value: bufferToStream(VSBuffer.fromString("Hello Untitled"))
      }
    });
  ...
  async function testSaveAsFile(source, target, resolveSource, resolveTarget) {
    ...
    } else if (resolveSource || resolveTarget) {
      assert.strictEqual(target.toString(), result?.resource.toString());
    } else if (accessor.uriIdentityService.extUri.isEqual(source, target)) {
