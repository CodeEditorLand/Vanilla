...
let ExtensionIgnoredRecommendationsService = class extends Disposable {
  ...
  _onDidChangeGlobalIgnoredRecommendation = this._register(
    new Emitter()
  );
  ...
  toggleGlobalIgnoredRecommendation(extensionId, shouldIgnore) {
    ...
    this._globalIgnoredRecommendations = shouldIgnore ? [...this._globalIgnoredRecommendations, extensionId] : this._globalIgnoredRecommendations.filter(
      (id) => id !== extensionId
    );
  ...
  getCachedIgnoredRecommendations() {
    const ignoredRecommendations = JSON.parse(
