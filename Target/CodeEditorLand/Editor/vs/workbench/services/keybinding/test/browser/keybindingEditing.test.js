...
suite("KeybindingsEditing", () => {
  ...
  setup(async () => {
    ...
    instantiationService = workbenchInstantiationService({
      ...
      configurationService: /* @__PURE__ */ __name(() => configService, "configurationService"),
      environmentService: /* @__PURE__ */ __name(() => environmentService, "environmentService")
    }, disposables);
  ...
  test("errors cases - parse errors", async () => {
    ...
    try {
      await testObject.editKeybinding(aResolvedKeybindingItem({ firstChord: { keyCode: KeyCode.Escape } }), "alt+c", void 0);
  ...
  test("errors cases - dirty", () => {
    ...
    return testObject.editKeybinding(aResolvedKeybindingItem({ firstChord: { keyCode: KeyCode.Escape } }), "alt+c", void 0).then(
  ...
  test("edit a default keybinding to an empty file", async () => {
    ...
    await testObject.editKeybinding(aResolvedKeybindingItem({ firstChord: { keyCode: KeyCode.Escape }, command: "a" }), "alt+c", void 0);
  ...
  test("add another keybinding", async () => {
    ...
    await testObject.addKeybinding(aResolvedKeybindingItem({ firstChord: { keyCode: KeyCode.Escape }, command: "a" }), "alt+c", void 0);
    return assert.deepStrictEqual(await getUserKeybindings(), expected);
  });
  test("add a new default keybinding", async () => {
    const expected = [{ key: "alt+c", command: "a" }];
    await testObject.addKeybinding(aResolvedKeybindingItem({ command: "a" }), "alt+c", void 0);
    return assert.deepStrictEqual(await getUserKeybindings(), expected);
  });
  test("add a new default keybinding using edit", async () => {
    const expected = [{ key: "alt+c", command: "a" }];
    await testObject.editKeybinding(aResolvedKeybindingItem({ command: "a" }), "alt+c", void 0);
    assert.deepStrictEqual(await getUserKeybindings(), expected);
  });
  test("edit an user keybinding", async () => {
    await writeToKeybindingsFile({ key: "escape", command: "b" });
    const expected = [{ key: "alt+c", command: "b" }];
    await testObject.editKeybinding(aResolvedKeybindingItem({ firstChord: { keyCode: KeyCode.Escape }, command: "b", isDefault: false }), "alt+c", void 0);
    assert.deepStrictEqual(await getUserKeybindings(), expected);
  });
  ...
  test("add a new keybinding to unassigned keybinding", async () => {
    ...
    await testObject.editKeybinding(aResolvedKeybindingItem({ command: "a", isDefault: false }), "shift+alt+c", void 0);
    assert.deepStrictEqual(await getUserKeybindings(), expected);
  });
  ...
  __name(writeToKeybindingsFile, "writeToKeybindingsFile");
  ...
  function aResolvedKeybindingItem({ command, when, isDefault, firstChord, secondChord }) {
    const aSimpleKeybinding = /* @__PURE__ */ __name(function(chord) {
    ...
    return new ResolvedKeybindingItem(keybinding, command || "some command", null, when ? ContextKeyExpr.deserialize(when) : void 0, isDefault === void 0 ? true : isDefault, null, false);
  }
  __name(aResolvedKeybindingItem, "aResolvedKeybindingItem");
});
