...
suite("Task Terminal Status", () => {
  ...
  setup(() => {
    ...
    taskTerminalStatus = store.add(
      new TaskTerminalStatus(
        taskService,
        accessibilitySignalService
      )
    );
    testTerminal = store.add(
      instantiationService.createInstance(TestTerminal)
    );
    testTask = instantiationService.createInstance(
      TestTask
    );
    problemCollector = store.add(
      instantiationService.createInstance(TestProblemCollector)
    );
  });
  test("Should add failed status when there is an exit code on task end", async () => {
    taskTerminalStatus.addTerminal(
      testTask,
      testTerminal,
      problemCollector
    );
  ...
  test("Should drop status when a background task exits", async () => {
    ...
    await poll(
      async () => Promise.resolve(),
      () => testTerminal?.statusList.statuses?.includes(
        SUCCEEDED_TASK_STATUS
      ) === false,
...
async function poll(fn, acceptFn, timeoutMessage, retryCount = 200, retryInterval = 10) {
  ...
  while (true) {
    if (trial > retryCount) {
      throw new Error(
        `Timeout: ${timeoutMessage} after ${retryCount * retryInterval / 1e3} seconds.\r${lastError}`
      );
    }
    let result;
    try {
