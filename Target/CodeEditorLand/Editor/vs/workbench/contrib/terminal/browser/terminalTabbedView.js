...
import { TerminalTabList, TerminalTabsListSizes } from "./terminalTabsList.js";
...
let TerminalTabbedView = class extends Disposable {
  ...
  _refreshShowTabs() {
    ...
    } else if (this._splitView.length === 2 && !this._terminalTabsMouseContextKey.get()) {
  ...
  _getLastListWidth() {
    ...
    if (!storedValue || !Number.parseInt(storedValue)) {
    ...
    return Number.parseInt(storedValue);
  }
  _handleOnDidSashReset() {
    ...
    if (ctx) {
      ...
      const maxInstanceWidth = this._terminalGroupService.instances.reduce((p, c) => {
        return Math.max(
          p,
          ctx.measureText(c.title + (c.description || "")).width + this._getAdditionalWidth(c)
        );
      }, 0);
    ...
    this._splitView.addView(
      {
        ...
        layout: /* @__PURE__ */ __name((width) => this._terminalGroupService.groups.forEach(
          (tab) => tab.layout(width, this._height || 0)
        ), "layout"),
        minimumSize: 120,
  ...
  _attachEventListeners(parentDomElement, terminalContainer) {
      ...
      dom.addDisposableListener(
        ...
        (event) => {
          ...
          if (!this._cancelContextMenu) {
            openContextMenu(
              dom.getWindow(terminalContainer),
              event,
      ...
      dom.addDisposableListener(
        ...
        (event) => {
          ...
          if (!this._cancelContextMenu) {
            ...
            if (focusedInstance) {
              selectedInstances.splice(
                selectedInstances.findIndex(
                  (e) => e.instanceId === focusedInstance.instanceId
                ),
                1
              );
            ...
            openContextMenu(
              dom.getWindow(this._tabContainer),
              event,
              selectedInstances,
              emptyList ? this._tabsListEmptyMenu : this._tabsListMenu,
              this._contextMenuService,
              emptyList ? this._getTabActions() : void 0
            );
          }
          event.preventDefault();
          event.stopImmediatePropagation();
    ...
    this._register(
      dom.addDisposableListener(
        terminalContainer.ownerDocument,
        "keydown",
        (event) => {
  ...
  _getTabActions() {
    return [
      new Separator(),
      this._configurationService.inspect(TerminalSettingId.TabsLocation).userValue === "left" ? new Action(
        "moveRight",
        localize("moveTabsRight", "Move Tabs Right"),
        void 0,
        void 0,
        async () => {
