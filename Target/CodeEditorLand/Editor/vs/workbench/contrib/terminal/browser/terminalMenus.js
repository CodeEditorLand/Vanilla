...
function getTerminalActionBarArgs(location, profiles, defaultProfileName, contributedProfiles, terminalService, dropdownMenu) {
  ...
  for (const p of profiles) {
    ...
    dropdownActions.push(new Action(TerminalCommandId.NewWithProfile, isDefault ? localize("defaultTerminalProfile", "{0} (Default)", sanitizedProfileName) : sanitizedProfileName, void 0, true, async () => {
      const instance = await terminalService.createTerminal(options);
      terminalService.setActiveInstance(instance);
      await terminalService.focusActiveInstance();
    }));
    submenuActions.push(new Action(TerminalCommandId.Split, isDefault ? localize("defaultTerminalProfile", "{0} (Default)", sanitizedProfileName) : sanitizedProfileName, void 0, true, async () => {
  ...
  for (const contributed of contributedProfiles) {
    ...
    dropdownActions.push(new Action("contributed", title, void 0, true, () => terminalService.createTerminal({
      config: {
    ...
    submenuActions.push(new Action("contributed-split", title, void 0, true, () => terminalService.createTerminal({
      config: {
        extensionIdentifier: contributed.extensionIdentifier,
        id: contributed.id,
        title
