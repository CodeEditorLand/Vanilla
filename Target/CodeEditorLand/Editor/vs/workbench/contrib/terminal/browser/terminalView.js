            ...
            const choices = [
              {
                ...
                run: () => this.configurationService.updateValue(
      ...
      dom.reset(
        ...
        ...renderLabelWithIcons(
          this._instantiationService.invokeFunction(
            ...
            ThemeIcon.isThemeIcon(this._commandAction.item.icon) ? this._commandAction.item.icon : void 0
          )
        )
      );
      if (this._altCommand) {
...
function getSingleTabTitle(instance, separator) {
  ...
  return instance.description ? `${instance.title} ${separator} ${instance.description}` : instance.title;
}
...
let SingleTabHoverDelegate = class {
  ...
  placement = "element";
  get delay() {
    return Date.now() - this._lastHoverHideTime < 200 ? 0 : this._configurationService.getValue(
