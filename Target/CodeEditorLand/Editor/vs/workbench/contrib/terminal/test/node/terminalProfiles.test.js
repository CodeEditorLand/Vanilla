...
} from "../../../../../platform/terminal/node/terminalProfiles.js";
function profilesEqual(actualProfiles, expectedProfiles) {
  strictEqual(
    actualProfiles.length,
    expectedProfiles.length,
    `Actual: ${actualProfiles.map((e) => e.profileName).join(",")}
Expected: ${expectedProfiles.map((e) => e.profileName).join(",")}`
  );
  for (const expected of expectedProfiles) {
    const actual = actualProfiles.find(
      (e) => e.profileName === expected.profileName
    );
