...
suite("TerminalCapabilityStore", () => {
  ...
  setup(() => {
    ...
    store.add(
      capabilityStore.onDidAddCapabilityType((e) => addEvents.push(e))
    );
    store.add(
      capabilityStore.onDidRemoveCapabilityType(
        (e) => removeEvents.push(e)
      )
    );
    addEvents = [];
    removeEvents = [];
  ...
  test("has should return whether a capability is present", () => {
    deepStrictEqual(
      capabilityStore.has(TerminalCapability.CwdDetection),
      false
    );
    capabilityStore.add(TerminalCapability.CwdDetection, {});
    deepStrictEqual(
...
suite("TerminalCapabilityStoreMultiplexer", () => {
  ...
  test("items should return items from all stores", () => {
    ...
    deepStrictEqual(
      ...
      [TerminalCapability.CwdDetection].sort()
    );
  ...
  test("has should return whether a capability is present", () => {
    deepStrictEqual(
      multiplexer.has(TerminalCapability.CwdDetection),
      false
    );
    multiplexer.add(store1);
    store1.add(TerminalCapability.CwdDetection, {});
