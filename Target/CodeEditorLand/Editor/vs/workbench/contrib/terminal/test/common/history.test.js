  ...
  suite("fetchBashHistory", () => {
    ...
    suite("remote", () => {
      ...
      test("Windows", async () => {
        ...
        strictEqual(
          await instantiationService.invokeFunction(fetchBashHistory),
          void 0
        );
      });
      test("macOS", async () => {
  ...
  suite("fetchZshHistory", () => {
    ...
    suite("remote", () => {
      ...
      test("Windows", async () => {
        remoteEnvironment = { os: OperatingSystem.Windows };
        strictEqual(
          await instantiationService.invokeFunction(fetchZshHistory),
          void 0
        );
      });
      test("macOS", async () => {
    ...
    setup(() => {
      ...
      instantiationService.stub(IFileService, {
        async readFile(resource) {
          ...
          strictEqual(
            resource.toString().replaceAll("%5C", "/"),
            expected.toString().replaceAll("%5C", "/")
          );
          return { value: VSBuffer.fromString(fileContent) };
        }
      });
  ...
  suite("fetchFishHistory", () => {
    ...
    suite("remote", () => {
      ...
      test("Windows", async () => {
        remoteEnvironment = { os: OperatingSystem.Windows };
        strictEqual(
          await instantiationService.invokeFunction(fetchFishHistory),
          void 0
        );
      });
      test("macOS", async () => {
