...
suite("ShellIntegrationAddon", () => {
  ...
  suite("cwd detection", () => {
    test("should activate capability on the cwd sequence (OSC 633 ; P ; Cwd=<cwd> ST)", async () => {
      strictEqual(
        capabilities.has(TerminalCapability.CwdDetection),
        false
      );
  ...
  suite("BufferMarkCapability", () => {
    test("SetMark", async () => {
      strictEqual(
        capabilities.has(TerminalCapability.BufferMarkDetection),
        false
      );
      await writeP(xterm, "foo");
      strictEqual(
    ...
    suite("parseMarkSequence", () => {
      test("basic", async () => {
        deepEqual(parseMarkSequence(["", ""]), {
          id: void 0,
          hidden: false
        });
      });
      test("ID", async () => {
        deepEqual(parseMarkSequence(["Id=3", ""]), {
          id: "3",
          hidden: false
        });
      });
      test("hidden", async () => {
        deepEqual(parseMarkSequence(["", "Hidden"]), {
          id: void 0,
          hidden: true
        });
      });
      test("ID + hidden", async () => {
        deepEqual(parseMarkSequence(["Id=4555", "Hidden"]), {
          id: "4555",
          hidden: true
        });
      });
