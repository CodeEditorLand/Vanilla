...
suite("EnvironmentVariable - EnvironmentVariableService", () => {
  ...
  setup(() => {
    ...
    environmentVariableService = store.add(
  ...
  test("should persist collections to the storage service and be able to restore from them", () => {
    ...
    environmentVariableService.set("ext1", {
      map: collection,
      persistent: true
    });
    deepStrictEqual(
      [
        ...environmentVariableService.mergedCollection.getVariableMap(void 0).entries()
      ],
      [
        [
          "A",
    ...
    const service2 = store.add(
      instantiationService.createInstance(TestEnvironmentVariableService)
    );
    deepStrictEqual(
      [...service2.mergedCollection.getVariableMap(void 0).entries()],
      [
        [
          "A",
          [
  ...
  suite("mergedCollection", () => {
    test("should overwrite any other variable with the first extension that replaces", () => {
      ...
      environmentVariableService.set("ext2", {
        map: collection2,
        persistent: true
      });
      environmentVariableService.set("ext3", {
        map: collection3,
        persistent: true
      });
      deepStrictEqual(
    ...
    test("should correctly apply the environment values from multiple extension contributions in the correct order", async () => {
      ...
      const env = { A: "foo" };
      await environmentVariableService.mergedCollection.applyToProcessEnvironment(
        env,
        void 0
      );
    ...
    test("should correctly apply the workspace specific environment values from multiple extension contributions in the correct order", async () => {
      ...
      deepStrictEqual(
        [
          ...environmentVariableService.mergedCollection.getVariableMap(scope1).entries()
        ],
        [
          [
            "A",
