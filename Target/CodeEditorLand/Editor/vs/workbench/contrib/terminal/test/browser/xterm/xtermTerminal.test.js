...
suite("XtermTerminal", () => {
  ...
  setup(async () => {
    ...
    themeService = instantiationService.get(
      IThemeService
    );
    XTermBaseCtor = (await importAMDNodeModule(
      "@xterm/xterm",
      "lib/xterm.js"
    )).Terminal;
  ...
  suite("renderers", () => {
    test.skip("should re-evaluate gpu acceleration auto when the setting is changed", async () => {
      ...
      if (isSafari) {
        strictEqual(
          TestWebglAddon.isEnabled,
          false,
          "The webgl renderer is always disabled on Safari"
        );
      } else {
