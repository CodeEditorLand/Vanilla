...
import {
  ...
  parseExitResult
} from "../../browser/terminalInstance.js";
...
suite("Workbench - TerminalInstance", () => {
  ...
  suite("TerminalInstance", () => {
    ...
    test("should create an instance of TerminalInstance with env from default profile", async () => {
      ...
      instantiationService.set(
        ITerminalProfileResolverService,
        new MockTerminalProfileResolverService()
      );
      ...
      instantiationService.stub(
        ITerminalInstanceService,
        store.add(new TestTerminalInstanceService())
      );
      terminalInstance = store.add(
        instantiationService.createInstance(
          TerminalInstance,
      ...
      deepStrictEqual(terminalInstance.shellLaunchConfig.env, {
        TEST: "TEST"
      });
    });
  });
  suite("parseExitResult", () => {
    ...
    test("should ignore pty host-related errors", () => {
      deepStrictEqual(
        parseExitResult(
          { message: "Could not find pty with id 16" },
          {},
          ProcessState.KilledDuringLaunch,
          void 0
        ),
  ...
  suite("TerminalLabelComputer", () => {
    ...
    function createLabelComputer(configuration) {
      ...
      return store.add(
        instantiationService.createInstance(TerminalLabelComputer)
      );
    }
    test('should resolve to "" when the template variables are empty', () => {
      const terminalLabelComputer = createLabelComputer({
        terminal: {
          integrated: {
            tabs: { separator: " - ", title: "", description: "" }
          }
        }
      });
      terminalLabelComputer.refreshLabel(
        createInstance({ capabilities, processName: "" })
      );
    ...
    test("should resolve cwd", () => {
      ...
      terminalLabelComputer.refreshLabel(
        createInstance({ capabilities, cwd: ROOT_1 })
      );
    ...
    test("should resolve process", () => {
      ...
      terminalLabelComputer.refreshLabel(
        createInstance({ capabilities, processName: "zsh" })
      );
    ...
    test("should resolve sequence", () => {
      ...
      terminalLabelComputer.refreshLabel(
        createInstance({ capabilities, sequence: "sequence" })
      );
