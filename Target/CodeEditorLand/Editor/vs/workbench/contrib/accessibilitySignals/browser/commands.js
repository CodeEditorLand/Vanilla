...
class ShowSignalSoundHelp extends Action2 {
  ...
  async run(accessor) {
    ...
    qp.selectedItems = items.filter(
      (i) => accessibilitySignalService.isSoundEnabled(i.signal) || userGestureSignals.includes(i.signal) && configurationService.getValue(
...
class ShowAccessibilityAnnouncementHelp extends Action2 {
  ...
  async run(accessor) {
    ...
    qp.items = items;
    qp.selectedItems = items.filter(
      (i) => accessibilitySignalService.isAnnouncementEnabled(i.signal) || userGestureSignals.includes(i.signal) && configurationService.getValue(
    ...
    disposables.add(
      qp.onDidAccept(() => {
        ...
        const disabledAnnouncements = AccessibilitySignal.allAccessibilitySignals.filter(
          (cue) => !!cue.legacyAnnouncementSettingsKey && !enabledAnnouncements.includes(cue)
        );
