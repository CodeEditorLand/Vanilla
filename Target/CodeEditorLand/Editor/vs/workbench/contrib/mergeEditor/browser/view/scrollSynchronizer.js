  ...
  constructor(viewModel, input1View, input2View, baseView, inputResultView, layout) {
    ...
    const handleInput1OnScroll = this.updateScrolling = () => {
      ...
      if (baseView2) {
        if (this.shouldAlignBase) {
          this.baseView.get()?.editor.setScrollTop(
            this.input1View.editor.getScrollTop(),
            ScrollType.Immediate
          );
        } else {
      ...
      this.input1View.editor.onDidScrollChange(
        this.reentrancyBarrier.makeExclusiveOrSkip((c) => {
          ...
          if (c.scrollLeftChanged) {
            this.baseView.get()?.editor.setScrollLeft(
              c.scrollLeft,
              ScrollType.Immediate
            );
    ...
    this._store.add(
      autorunWithStore((reader, store) => {
        ...
        if (baseView2) {
          store.add(
            baseView2.editor.onDidScrollChange(
              this.reentrancyBarrier.makeExclusiveOrSkip((c) => {
                if (c.scrollTopChanged) {
                  if (!this.model) {
                    return;
                  ...
                  if (this.shouldAlignBase) {
                    ...
                    const baseInput1Mapping = new DocumentLineRangeMap(
                      this.model.baseInput1Diffs.get(),
                      -1
                    );
                    this.synchronizeScrolling(
                    ...
                    const baseInput2Mapping = new DocumentLineRangeMap(
                      this.model.baseInput2Diffs.get(),
                      -1
                    );
                    this.synchronizeScrolling(
