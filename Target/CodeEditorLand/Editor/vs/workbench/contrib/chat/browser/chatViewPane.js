...
let ChatViewPane = class extends ViewPane {
  ...
  updateModel(model) {
    ...
    model = model ?? (this.chatService.transferredSessionData?.sessionId ? this.chatService.getOrRestoreSession(
  ...
  shouldShowWelcome() {
    if (!this.chatAgentService.getContributedDefaultAgent(
      ChatAgentLocation.Panel
    )) {
      return true;
    }
  ...
  renderBody(parent) {
    try {
      ...
      const sessionId = this.getSessionId();
      const disposeListener = sessionId ? this._register(
