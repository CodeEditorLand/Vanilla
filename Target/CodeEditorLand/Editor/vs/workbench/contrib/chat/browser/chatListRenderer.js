...
let ChatListItemRenderer = class extends Disposable {
  constructor(editorOptions, location, rendererOptions, delegate, codeBlockModelCollection, overflowWidgetsDomNode, instantiationService, configService, logService, contextKeyService, themeService, commandService, hoverService) {
    ...
    this.markdownDecorationsRenderer = this.instantiationService.createInstance(
  ...
  _onDidClickFollowup = this._register(
    new Emitter()
  );
  ...
  _onDidChangeItemHeight = this._register(
    new Emitter()
  );
  ...
  renderTemplate(container) {
    ...
    const hoverOptions = getChatAgentHoverOptions(
      () => isResponseVM(template.currentElement) ? template.currentElement.agent : void 0,
      this.commandService
    );
  ...
  renderChatTreeItem(element, index, templateData) {
    ...
    templateData.rowContainer.classList.toggle(
      ...
      isResponseVM(element) && !element.isComplete && !element.progressMessages.length
    );
    ...
    if (isResponseVM(element) && index === this.delegate.getListLength() - 1 && (!element.isComplete || element.renderData) && element.response.value.length) {
      ...
      const runProgressiveRender = /* @__PURE__ */ __name((initial) => {
        try {
          if (this.doNextProgressiveRender(
            element,
            index,
            templateData,
            !!initial
  ...
  basicRenderElement(element, index, templateData) {
    let value = [];
    if (isRequestVM(element) && !element.confirmation) {
      const markdown = "message" in element.message ? element.message.message : this.markdownDecorationsRenderer.convertParsedRequestToMarkdown(
        element.message
      );
      value = [
        {
  ...
  getDataForProgressiveRender(element) {
    ...
    Math.floor(
      (Date.now() - renderData.lastRenderTime) / 1e3 * rate
    );
    return {
      numWordsToRender,
      rate
  ...
  diff(renderedParts, contentToRender, element) {
    ...
    for (let i = 0; i < contentToRender.length; i++) {
      ...
      if (!renderedPart || !renderedPart.hasSameContent(
        content,
  ...
  renderMarkdown(markdown, templateData, context) {
    ...
    const codeBlockStartIndex = context.preceedingContentParts.reduce(
      (acc, part) => acc + (part instanceof ChatMarkdownContentPart ? part.codeblocks.length : 0),
      0
    );
    ...
    markdownPart.addDisposable(
      toDisposable(() => {
        const codeBlocksByResponseId2 = this.codeBlocksByResponseId.get(
          element.id
        );
        if (codeBlocksByResponseId2) {
          markdownPart.codeblocks.forEach(
            (info, i) => delete codeBlocksByResponseId2[codeBlockStartIndex + i]
          );
        }
      })
    );
