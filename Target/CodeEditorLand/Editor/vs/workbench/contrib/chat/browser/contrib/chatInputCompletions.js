      ...
      this.languageFeaturesService.completionProvider.register(
        ...
        },
        {
          ...
          provideCompletionItems: /* @__PURE__ */ __name(async (model, position, _context, _token) => {
            ...
            const agents = this.chatAgentService.getAgents().filter((a) => !a.isDefault).filter(
              (a) => a.locations.includes(widget.location)
            );
            return {
      ...
      this.languageFeaturesService.completionProvider.register(
        ...
        },
        {
          ...
          provideCompletionItems: /* @__PURE__ */ __name(async (model, position, _context, token) => {
            ...
            const agents = this.chatAgentService.getAgents().filter(
              (a) => a.locations.includes(widget.location)
            );
            ...
            return {
              suggestions: justAgents.concat(
                agents.flatMap(
                  (agent) => agent.slashCommands.map((c, i) => {
                    const { label: agentLabel, isDupe } = this.getAgentCompletionDetails(
                      agent
                    );
                    ...
                    const item = {
                      label: {
                        ...
                        detail: isDupe ? ` (${agent.publisherDisplayName})` : void 0
                      },
        ...
        },
        {
          ...
          provideCompletionItems: /* @__PURE__ */ __name(async (model, position, _context, _token) => {
            ...
            for (const value of Object.values(ChatAgentLocation)) {
              if (typeof value === "string" && configService.getValue(
            ...
            ).filter((v) => !usedVariableNames.has(v.name)).filter((v) => !v.isSlow || slowSupported).map((v) => {
            ...
            if (!usedAgent || usedAgent.agent.supportsToolReferences) {
              toolItems.push(
                ...Array.from(toolsService.getTools()).filter((t) => t.canBeInvokedManually).filter(
