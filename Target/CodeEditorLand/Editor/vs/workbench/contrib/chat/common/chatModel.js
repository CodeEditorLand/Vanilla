  ...
  _updateRepr(quiet) {
    this._responseRepr = this._responseParts.map((part) => {
      if (part.kind === "treeData") {
        ...
        return basename(
          "uri" in part.inlineReference ? part.inlineReference.uri : part.inlineReference
        );
...
let ChatModel = class extends Disposable {
  ...
  get requesterUsername() {
    return (this._defaultAgent ? this._defaultAgent.metadata.requester?.name : this.initialData?.requesterUsername) ?? "";
  }
  get responderUsername() {
    return (this._defaultAgent ? this._defaultAgent.fullName : this.initialData?.responderUsername) ?? "";
  }
  _initialRequesterAvatarIconUri;
  get requesterAvatarIconUri() {
    return this._defaultAgent ? this._defaultAgent.metadata.requester?.icon : this._initialRequesterAvatarIconUri;
  }
  _initialResponderAvatarIconUri;
  get responderAvatarIcon() {
    return this._defaultAgent ? this._defaultAgent?.metadata.themeIcon : this._initialResponderAvatarIconUri;
  }
  get initState() {
  ...
  acceptResponseProgress(request, progress, quiet) {
    ...
    if (progress.kind === "markdownContent" || progress.kind === "treeData" || progress.kind === "inlineReference" || progress.kind === "markdownVuln" || progress.kind === "progressMessage" || progress.kind === "command" || progress.kind === "textEdit" || progress.kind === "warning" || progress.kind === "progressTask" || progress.kind === "confirmation") {
