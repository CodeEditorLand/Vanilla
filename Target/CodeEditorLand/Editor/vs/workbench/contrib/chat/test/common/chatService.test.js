...
suite("ChatService", () => {
  ...
  setup(async () => {
    ...
    instantiationService.stub(
      IChatSlashCommandService,
      testDisposables.add(
        instantiationService.createInstance(ChatSlashCommandService)
      )
    );
    instantiationService.stub(
    ...
    testDisposables.add(
      chatAgentService.registerAgent("testAgent", {
    ...
    testDisposables.add(
      chatAgentService.registerAgent(
        chatAgentWithUsedContextId,
        getAgentData(chatAgentWithUsedContextId)
      )
    );
    testDisposables.add(
      chatAgentService.registerAgentImplementation("testAgent", agent)
    );
  ...
  test("retrieveSession", async () => {
    const testService = testDisposables.add(
      instantiationService.createInstance(ChatService)
    );
    const session1 = testDisposables.add(
    ...
    session1.addRequest(
      { parts: [], text: "request 1" },
      { variables: [] },
      0
    );
    const session2 = testDisposables.add(
      testService.startSession(
        ChatAgentLocation.Panel,
        CancellationToken.None
      )
    ...
    session2.addRequest(
      { parts: [], text: "request 2" },
      { variables: [] },
      0
    );
    storageService.flush();
    const testService2 = testDisposables.add(
      instantiationService.createInstance(ChatService)
    );
    const retrieved1 = testDisposables.add(
      testService2.getOrRestoreSession(session1.sessionId)
    );
    await retrieved1.waitForInitialization();
    const retrieved2 = testDisposables.add(
      testService2.getOrRestoreSession(session2.sessionId)
    );
    await retrieved2.waitForInitialization();
    assert.deepStrictEqual(
      retrieved1.getRequests()[0]?.message.text,
      "request 1"
    );
    assert.deepStrictEqual(
      retrieved2.getRequests()[0]?.message.text,
      "request 2"
    );
  });
  test("addCompleteRequest", async () => {
    ...
    await testService.addCompleteRequest(
      model.sessionId,
      "test request",
      void 0,
      0,
    ...
    assert.strictEqual(
      model.getRequests()[0].response?.response.toString(),
      "test response"
    );
  });
  test("sendRequest fails", async () => {
    ...
    const response = await testService.sendRequest(
      model.sessionId,
      `@${chatAgentWithUsedContextId} test request`
    );
    assert(response);
  ...
  test("history", async () => {
    ...
    testDisposables.add(
      chatAgentService.registerAgent("defaultAgent", {
        ...getAgentData("defaultAgent"),
        isDefault: true
      })
    );
    testDisposables.add(
      chatAgentService.registerAgent("agent2", getAgentData("agent2"))
    );
    testDisposables.add(
    ...
    assert.strictEqual(model.getRequests().length, 1);
    assert.strictEqual(
      model.getRequests()[0].response?.result?.metadata?.historyLength,
      0
    );
    const response2 = await testService.sendRequest(
      model.sessionId,
      `test request`,
      { agentId: "agent2" }
    );
    ...
    assert.strictEqual(
      model.getRequests()[1].response?.result?.metadata?.historyLength,
      0
    );
    const response3 = await testService.sendRequest(
      model.sessionId,
      `test request`,
      { agentId: "defaultAgent" }
    );
    ...
    assert.strictEqual(
      model.getRequests()[2].response?.result?.metadata?.historyLength,
      2
    );
  });
  ...
  test("can deserialize", async () => {
    ...
    );
    {
      ...
      const chatModel1 = testDisposables.add(
        testService.startSession(
          ChatAgentLocation.Panel,
          CancellationToken.None
        )
