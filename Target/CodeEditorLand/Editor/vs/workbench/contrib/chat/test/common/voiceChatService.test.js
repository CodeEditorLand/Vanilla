...
suite("VoiceChat", () => {
  ...
  async function createSession(options) {
    ...
    const session = await service.createVoiceChatSession(
      cts.token,
      options
    );
    disposables.add(
  ...
  test("Agent and slash command detection (useAgents: false)", async () => {
    await testAgentsAndSlashCommandsDetection({
      usesAgents: false,
      model: {}
    });
  });
  ...
  async function testAgentsAndSlashCommandsDetection(options) {
    ...
    assert.strictEqual(
      event?.text,
      options.usesAgents ? "@workspace" : "At workspace"
    );
    ...
    emitter.fire({
      status: SpeechToTextStatus.Recognizing,
      text: "At workspace help"
    });
    assert.strictEqual(event?.status, SpeechToTextStatus.Recognizing);
    assert.strictEqual(
      event?.text,
      options.usesAgents ? "@workspace help" : "At workspace help"
    );
    ...
    assert.strictEqual(
      event?.text,
      options.usesAgents ? "@workspace /fix" : "/fix"
    );
    ...
    emitter.fire({
      status: SpeechToTextStatus.Recognizing,
      text: "At code slash search help"
    });
    assert.strictEqual(event?.status, SpeechToTextStatus.Recognizing);
    assert.strictEqual(
      event?.text,
  ...
  test("waiting for input", async () => {
    ...
    emitter.fire({
      status: SpeechToTextStatus.Recognizing,
      text: "At workspace slash explain"
    });
    assert.strictEqual(event?.status, SpeechToTextStatus.Recognizing);
