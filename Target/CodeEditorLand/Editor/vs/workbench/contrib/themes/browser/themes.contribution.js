...
let MarketplaceThemesPicker = class {
    ...
    try {
      ...
      for (let i = 0; i < pager.total && i < 1; i++) {
        ...
        for (let i2 = 0; i2 < allThemes.length; i2++) {
          ...
          for (const theme of allThemes[i2]) {
            this._marketplaceThemes.push({
              ...
              theme,
              label: theme.label,
              description: `${ext.displayName} \xB7 ${ext.publisherDisplayName}`,
        ...
        if (nThemes !== this._marketplaceThemes.length) {
          this._marketplaceThemes.sort(
            (t1, t2) => t1.label.localeCompare(t2.label)
          );
          this._onDidChange.fire();
        }
      }
  ...
  openQuickPick(value, currentTheme, selectTheme) {
    ...
    return new Promise((s, _) => {
      ...
      disposables.add(
        quickpick.onDidAccept(async (_2) => {
  ...
  async openQuickPick(picks, currentTheme) {
    ...
    if (this.extensionGalleryService.isEnabled()) {
      if (this.extensionResourceLoaderService.supportsExtensionGalleryResources && this.options.browseMessage) {
        marketplaceThemePicker = this.instantiationService.createInstance(
          MarketplaceThemesPicker,
      ...
      return new Promise((s, _) => {
            ...
            if (!theme || theme.configureItem) {
              if (!theme || theme.configureItem === "extensions" /* EXTENSIONS_VIEW */) {
                ...
                if (marketplaceThemePicker) {
                  const res = await marketplaceThemePicker.openQuickPick(
                    quickpick.value,
                    currentTheme,
                    selectTheme
                  );
        ...
        disposables.add(
          quickpick.onDidChangeActive(
            (themes) => selectTheme(themes[0]?.theme, false)
          )
        );
        disposables.add(
          quickpick.onDidHide(() => {
...
registerAction2(
  class extends Action2 {
    ...
    async run(accessor) {
      ...
      const setTheme = /* @__PURE__ */ __name((theme, settingsTarget) => themeService.setColorTheme(
        theme,
        settingsTarget
      ), "setTheme");
      const getMarketplaceColorThemes = /* @__PURE__ */ __name((publisher, name, version) => themeService.getMarketplaceColorThemes(
        publisher,
        name,
        version
...
registerAction2(
  class extends Action2 {
    ...
    async run(accessor) {
      ...
      const setTheme = /* @__PURE__ */ __name((theme, settingsTarget) => themeService.setFileIconTheme(
        theme,
        settingsTarget
      ), "setTheme");
      const getMarketplaceColorThemes = /* @__PURE__ */ __name((publisher, name, version) => themeService.getMarketplaceFileIconThemes(
        publisher,
        name,
        version
      ), "getMarketplaceColorThemes");
...
registerAction2(
  class extends Action2 {
    ...
    async run(accessor) {
      ...
      const setTheme = /* @__PURE__ */ __name((theme, settingsTarget) => themeService.setProductIconTheme(
        theme,
        settingsTarget
      ), "setTheme");
      const getMarketplaceColorThemes = /* @__PURE__ */ __name((publisher, name, version) => themeService.getMarketplaceProductIconThemes(
        publisher,
        name,
        version
      ), "getMarketplaceColorThemes");
...
CommandsRegistry.registerCommand(
  ...
  async (accessor, extension, themeSettingsId) => {
    const themeService = accessor.get(IWorkbenchThemeService);
...
function findBuiltInThemes(themes, extension) {
  return themes.filter(
    ({ extensionData }) => extensionData && extensionData.extensionIsBuiltin && equalsIgnoreCase(
...
let DefaultThemeUpdatedNotificationContribution = class {
  ...
  async _tryNewThemeNotification() {
    ...
    const theme = (await this._workbenchThemeService.getColorThemes()).find(
      (theme2) => theme2.settingsId === newThemeSettingsId
    );
    if (theme) {
      const choices = [
        {
