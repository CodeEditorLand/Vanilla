...
class LocalAddressColumn {
  ...
  label = nls.localize("tunnel.addressColumn.label", "Forwarded Address");
  tooltip = nls.localize("tunnel.addressColumn.tooltip", "The address that the forwarded port is available at.");
  weight = 1;
  templateId = "actionbar";
  project(row) {
    if (row.tunnelType === TunnelType.Add) {
...
let TunnelPanel = class extends ViewPane {
  ...
  static TITLE = nls.localize2("remote.tunnel", "Ports");
  panelContainer;
  table;
  tableDisposables = this._register(new DisposableStore());
  tunnelTypeContext;
