...
} from "../../browser/snippetCompletionProvider.js";
...
suite("SnippetsService", () => {
  ...
  async function asCompletionModel(model, position, provider, context = defaultCompletionContext) {
    ...
    const result = new CompletionModel(
      ...
      position.column,
      {
        ...
        leadingLineContent: model.getLineContent(position.lineNumber).substring(0, position.column - 1)
      },
      WordDistance.None,
  ...
  test("snippet completions - simple", async () => {
    ...
    const model = disposables.add(
      instantiateTextModel(instantiationService, "", "fooLang")
    );
    ...
    const completions = await asCompletionModel(
      model,
      new Position(1, 1),
      provider
    );
  ...
  test("snippet completions - simple 2", async () => {
    const provider = new SnippetCompletionProvider(
      languageService,
      snippetService,
      disposables.add(new TestLanguageConfigurationService())
    ...
    const model = disposables.add(
      instantiateTextModel(instantiationService, "hello ", "fooLang")
    );
    await provider.provideCompletionItems(
      model,
      new Position(1, 6),
    ...
    const completions1 = await asCompletionModel(
      model,
      new Position(1, 6),
      provider
    );
    assert.strictEqual(completions1.items.length, 0);
    const completions2 = await asCompletionModel(
      model,
      new Position(1, 7),
      provider
    );
  ...
  test("snippet completions - with prefix", async () => {
    const provider = new SnippetCompletionProvider(
      languageService,
      snippetService,
      disposables.add(new TestLanguageConfigurationService())
    ...
    const model = disposables.add(
      instantiateTextModel(instantiationService, "bar", "fooLang")
    );
    await provider.provideCompletionItems(
      model,
      new Position(1, 4),
      ...
      assert.strictEqual(
        result.suggestions[0].range.insert.startColumn,
        1
      );
      assert.strictEqual(
    ...
    assert.strictEqual(
      completions.items[0].completion.range.insert.startColumn,
      1
    );
    assert.strictEqual(
      completions.items[0].completion.insertText,
      "barCodeSnippet"
    );
  });
  test("snippet completions - with different prefixes", async () => {
    snippetService = new SimpleSnippetService([
      new Snippet(
        false,
        ["fooLang"],
    ...
    const model = disposables.add(
      instantiateTextModel(instantiationService, "bar-bar", "fooLang")
    );
    {
      await provider.provideCompletionItems(
        model,
        ...
        assert.strictEqual(
          result.suggestions[1].range.insert.startColumn,
          1
        );
      });
      const completions = await asCompletionModel(
      ...
      assert.strictEqual(
        completions.items[1].completion.insertText,
        "s2"
      );
      assert.strictEqual(
        completions.items[1].completion.range.insert.startColumn,
        1
      );
    }
    {
      await provider.provideCompletionItems(
        ...
        assert.strictEqual(
          first2.range.insert.startColumn,
          5
        );
        ...
        assert.strictEqual(
          second2.range.insert.startColumn,
          1
        );
      });
      const completions = await asCompletionModel(
      ...
      assert.strictEqual(
        first.range.insert.startColumn,
        1
      );
      ...
      assert.strictEqual(
        second.range.insert.startColumn,
        5
      );
    }
    {
  ...
  test('Cannot use "<?php" as user snippet prefix anymore, #26275', async () => {
    snippetService = new SimpleSnippetService([
      new Snippet(
        false,
        ["fooLang"],
    ...
    let model = instantiateTextModel(
    ...
    assert.strictEqual(
      completions2.items[0].completion.range.insert.startColumn,
      2
    );
    model.dispose();
    ...
    const completions3 = await asCompletionModel(
      model,
      new Position(1, 4),
      provider
    );
    ...
    assert.strictEqual(
      completions3.items[0].completion.range.insert.startColumn,
      2
    );
    model.dispose();
  });
  test("No user snippets in suggestions, when inside the code, #30508", async () => {
    snippetService = new SimpleSnippetService([
      new Snippet(
        false,
        ["fooLang"],
  ...
  test("SnippetSuggest - ensure extension snippets come last ", async () => {
    snippetService = new SimpleSnippetService([
      new Snippet(
        false,
        ["fooLang"],
  ...
  test("Dash in snippets prefix broken #53945", async () => {
    snippetService = new SimpleSnippetService([
      new Snippet(
        false,
        ["fooLang"],
    ...
    const model = disposables.add(
      instantiateTextModel(instantiationService, "p-", "fooLang")
    );
    ...
    let completions = await asCompletionModel(
      model,
      new Position(1, 2),
      provider
    );
    ...
    completions = await asCompletionModel(
      model,
      new Position(1, 3),
      provider
    );
  ...
  test("No snippets suggestion on long lines beyond character 100 #58807", async () => {
    snippetService = new SimpleSnippetService([
      new Snippet(
        false,
        ["fooLang"],
    ...
    const model = disposables.add(
      instantiateTextModel(
        instantiationService,
        "Thisisaverylonglinegoingwithmore100bcharactersandthismakesintellisensebecomea Thisisaverylonglinegoingwithmore100bcharactersandthismakesintellisensebecomea b",
        "fooLang"
      )
    );
  ...
  test("Type colon will trigger snippet #60746", async () => {
    snippetService = new SimpleSnippetService([
      new Snippet(
        false,
        ["fooLang"],
  ...
  test("substring of prefix can't trigger snippet #60737", async () => {
    snippetService = new SimpleSnippetService([
      new Snippet(
        false,
        ["fooLang"],
    ...
    const model = disposables.add(
      instantiateTextModel(instantiationService, "template", "fooLang")
    );
    const result = await provider.provideCompletionItems(
      model,
      new Position(1, 9),
  ...
  test("No snippets suggestion beyond character 100 if not at end of line #60247", async () => {
    snippetService = new SimpleSnippetService([
      new Snippet(
        false,
        ["fooLang"],
    ...
    const model = disposables.add(
      instantiateTextModel(
        instantiationService,
        "Thisisaverylonglinegoingwithmore100bcharactersandthismakesintellisensebecomea Thisisaverylonglinegoingwithmore100bcharactersandthismakesintellisensebecomea b text_after_b",
        "fooLang"
      )
    );
    const result = await provider.provideCompletionItems(
  ...
  test("issue #61296: VS code freezes when editing CSS fi`le with emoji", async () => {
    const languageConfigurationService = disposables.add(
      new TestLanguageConfigurationService()
    );
    disposables.add(
      languageConfigurationService.register("fooLang", {
    ...
    const model = disposables.add(
      instantiateTextModel(instantiationService, ".\u{1F437}-a-b", "fooLang")
    );
    const result = await provider.provideCompletionItems(
      model,
      new Position(1, 8),
  ...
  test("No snippets shown when triggering completions at whitespace on line that already has text #62335", async () => {
    snippetService = new SimpleSnippetService([
      new Snippet(
        false,
        ["fooLang"],
    ...
    const model = disposables.add(
      instantiateTextModel(instantiationService, "a ", "fooLang")
    );
    const result = await provider.provideCompletionItems(
      model,
      new Position(1, 3),
  ...
  test("Snippet prefix with special chars and numbers does not work #62906", async () => {
    snippetService = new SimpleSnippetService([
      new Snippet(
        false,
        ["fooLang"],
  ...
  test("Snippet replace range", async () => {
    snippetService = new SimpleSnippetService([
      new Snippet(
        false,
        ["fooLang"],
  ...
  test("Snippet replace-range incorrect #108894", async () => {
    snippetService = new SimpleSnippetService([
      new Snippet(
        false,
        ["fooLang"],
  ...
  test("Snippet will replace auto-closing pair if specified in prefix", async () => {
    const languageConfigurationService = disposables.add(
      new TestLanguageConfigurationService()
    );
    disposables.add(
  ...
  test("Leading whitespace in snippet prefix #123860", async () => {
    snippetService = new SimpleSnippetService([
      new Snippet(
        false,
        ["fooLang"],
  ...
  test("still show suggestions in string when disable string suggestion #136611", async () => {
    snippetService = new SimpleSnippetService([
      new Snippet(
        false,
        ["fooLang"],
  ...
  test("still show suggestions in string when disable string suggestion #136611 (part 2)", async () => {
    snippetService = new SimpleSnippetService([
      new Snippet(
        false,
        ["fooLang"],
  ...
  test("Snippet suggestions are too eager #138707 (word)", async () => {
    snippetService = new SimpleSnippetService([
      new Snippet(
        false,
        ["fooLang"],
    ...
    assert.strictEqual(result.suggestions.length, 1);
    assert.strictEqual(
      result.suggestions[0].label.label,
      "hell_or_tell"
    );
    const completions = await asCompletionModel(
      model,
  ...
  test("Snippet suggestions are too eager #138707 (no word)", async () => {
    snippetService = new SimpleSnippetService([
      new Snippet(
        false,
        ["fooLang"],
  ...
  test("Snippet suggestions are too eager #138707 (word/word)", async () => {
    snippetService = new SimpleSnippetService([
      new Snippet(
        false,
        ["fooLang"],
  ...
  test("Strange and useless autosuggestion #region/#endregion PHP #140039", async () => {
    snippetService = new SimpleSnippetService([
      new Snippet(
        false,
        ["fooLang"],
  ...
  test.skip("Snippets disappear with . key #145960", async () => {
    snippetService = new SimpleSnippetService([
      new Snippet(
        false,
        ["fooLang"],
  ...
  test("Hyphen in snippet prefix de-indents snippet #139016", async () => {
    snippetService = new SimpleSnippetService([
      new Snippet(
        false,
        ["fooLang"],
  ...
  test("Autocomplete suggests based on the last letter of a word and it depends on the typing speed #191070", async () => {
    snippetService = new SimpleSnippetService([
      new Snippet(
        false,
        ["fooLang"],
