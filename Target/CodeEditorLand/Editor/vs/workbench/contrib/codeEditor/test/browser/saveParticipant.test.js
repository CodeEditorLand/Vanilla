...
suite("Save Participants", function() {
  ...
  test("insert final new line", async function() {
    const model = disposables.add(instantiationService.createInstance(TextFileEditorModel, toResource.call(this, "/path/final_new_line.txt"), "utf8", void 0));
    await model.resolve();
    const configService = new TestConfigurationService();
    configService.setUserConfiguration("files", { "insertFinalNewline": true });
    const participant = new FinalNewLineParticipant(configService, void 0);
    let lineContent = "";
  ...
  test("trim final new lines", async function() {
    const model = disposables.add(instantiationService.createInstance(TextFileEditorModel, toResource.call(this, "/path/trim_final_new_line.txt"), "utf8", void 0));
    await model.resolve();
    const configService = new TestConfigurationService();
    configService.setUserConfiguration("files", { "trimFinalNewlines": true });
    const participant = new TrimFinalNewLinesParticipant(configService, void 0);
  ...
  test("trim whitespace", async function() {
    ...
    const participant = new TrimWhitespaceParticipant(configService, void 0);
    const textContent = "Test";
