...
const modifySearchEditorContextLinesCommand = (accessor, increase) => {
  ...
  const input = editorService.activeEditor;
  if (input instanceof SearchEditorInput) {
    editorService.activeEditorPane.modifyContextLines(
      increase
    );
  }
};
...
const openNewSearchEditor = async (accessor, _args = {}, toSide = false) => {
  ...
  if (activeEditorControl) {
    ...
    if (selection?.isEmpty() && configurationService.getValue(
      "search"
    ).seedWithNearestWord) {
  ...
  const args = {
    query: seedSearchStringFromSelection ? selected : void 0
  };
...
const createEditorFromSearchResult = async (accessor, searchResult, rawIncludePattern, rawExcludePattern, onlySearchInOpenEditors) => {
  ...
  const configurationService = accessor.get(IConfigurationService);
  const sortOrder = configurationService.getValue(
    "search"
  ).sortOrder;
