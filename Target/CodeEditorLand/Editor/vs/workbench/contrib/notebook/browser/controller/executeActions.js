  ...
  class ExecuteNotebookAction extends NotebookAction {
    ...
    async runWithContext(accessor, context) {
      ...
      const editor = editorService.getEditors(EditorsOrder.MOST_RECENTLY_ACTIVE).find(
        (editor2) => editor2.editor instanceof NotebookEditorInput && editor2.editor.viewType === context.notebookEditor.textModel.viewType && editor2.editor.resource.toString() === context.notebookEditor.textModel.uri.toString()
      );
  ...
  class ExecuteAboveCells extends NotebookMultiCellAction {
    ...
    async runWithContext(accessor, context) {
      ...
      if (context.ui) {
        ...
        endCellIdx = Math.min(
          ...context.selectedCells.map(
            (cell) => context.notebookEditor.getCellIndex(cell)
          )
        );
      }
...
registerAction2(
  class RevealRunningCellAction extends NotebookAction {
    ...
    async runWithContext(accessor, context) {
      ...
      const executingCells = notebookExecutionStateService.getCellExecutionsForNotebook(
        notebook
      );
      if (executingCells[0]) {
...
registerAction2(
  class RevealLastFailedCellAction extends NotebookAction {
    ...
    async runWithContext(accessor, context) {
      ...
      const notebook = context.notebookEditor.textModel.uri;
      const lastFailedCellHandle = notebookExecutionStateService.getLastFailedCellForNotebook(
        notebook
      );
      if (lastFailedCellHandle !== void 0) {
        const lastFailedCell = context.notebookEditor.getCellByHandle(
