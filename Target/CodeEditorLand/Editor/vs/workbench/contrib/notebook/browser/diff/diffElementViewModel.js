...
class SideBySideDiffElementViewModel extends DiffElementCellViewModelBase {
  ...
  async computeInputEditorHeightWithUnchangedLinesHidden() {
    if (this.checkIfInputModified()) {
      this.editorHeightWithUnchangedLinesCollapsed = this._layoutInfo.editorHeight = await this.unchangedRegionsService.computeEditorHeight(
        this.original.uri,
        this.modified.uri
      );
    }
  ...
  async computeMetadataEditorHeightWithUnchangedLinesHidden() {
    if (this.checkMetadataIfModified()) {
      ...
      this._layoutInfo.metadataHeight = await this.unchangedRegionsService.computeEditorHeight(
        originalMetadataUri,
        modifiedMetadataUri
      );
    }
  ...
  async computeEditorHeights() {
    ...
    await Promise.all([
      this.computeInputEditorHeightWithUnchangedLinesHidden(),
      this.computeMetadataEditorHeightWithUnchangedLinesHidden()
    ]);
  }
}
