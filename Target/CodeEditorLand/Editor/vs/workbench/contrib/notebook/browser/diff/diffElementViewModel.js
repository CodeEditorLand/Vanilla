  ...
  constructor(mainDocumentTextModel, editorEventDispatcher, initData) {
    ...
    this._register(
      this.editorEventDispatcher.onDidChangeLayout(
        (e) => this._layoutInfoEmitter.fire({ outerWidth: true })
      )
    );
  }
  _layoutInfoEmitter = this._register(
    new Emitter()
  );
...
class DiffElementCellViewModelBase extends DiffElementViewModelBase {
  constructor(mainDocumentTextModel, original, modified, type, editorEventDispatcher, initData, notebookService, configurationService) {
    super(mainDocumentTextModel, editorEventDispatcher, initData);
    this.type = type;
    this.configurationService = configurationService;
    this.original = original ? this._register(
      new DiffNestedCellViewModel(original, notebookService)
    ) : void 0;
    this.modified = modified ? this._register(
  ...
  outputFoldingState;
  _stateChangeEmitter = this._register(
    new Emitter()
  );
  ...
  _layout(delta) {
    ...
    const outputHeight = this.ignoreOutputs ? 0 : delta.recomputeOutput || delta.rawOutputHeight !== void 0 || delta.outputMetadataHeight !== void 0 ? this._getOutputTotalHeight(
      rawOutputHeight,
  ...
  getHeight(lineHeight) {
    if (this._layoutInfo.layoutState === CellLayoutState.Uninitialized) {
      const editorHeight = this.cellFoldingState === 1 /* Collapsed */ ? 0 : this.estimateEditorHeight(lineHeight);
...
class SideBySideDiffElementViewModel extends DiffElementCellViewModelBase {
  constructor(mainDocumentTextModel, otherDocumentTextModel, original, modified, type, editorEventDispatcher, initData, notebookService, configurationService) {
    super(
