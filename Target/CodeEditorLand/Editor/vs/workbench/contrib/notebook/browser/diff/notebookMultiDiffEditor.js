...
let NotebookMultiTextDiffEditor = class extends EditorPane {
  ...
  modelSpecificResources = this._register(
    new DisposableStore()
  );
  _model;
  viewModel;
  ...
  ctxAllCollapsed = this._parentContextKeyService.createKey(
    NOTEBOOK_DIFF_CELLS_COLLAPSED.key,
    false
  );
  ctxHasUnchangedCells = this._parentContextKeyService.createKey(
    NOTEBOOK_DIFF_HAS_UNCHANGED_CELLS.key,
    false
  );
  ctxHiddenUnchangedCells = this._parentContextKeyService.createKey(
  ...
  async setInput(input, options, context, token) {
    ...
    this.viewModel = this.modelSpecificResources.add(
      new NotebookDiffViewModel(
        ...
        UnchangedEditorRegionsService.Empty,
        void 0,
        true
      )
    );
