...
class AbstractElementRenderer extends Disposable {
  ...
  _metadataLocalDisposable = this._register(
    new DisposableStore()
  );
  _outputLocalDisposable = this._register(
    new DisposableStore()
  );
  ...
  buildBody() {
    ...
    this._ignoreOutputs = this.configurationService.getValue(
  ...
  async _buildMetadataEditor() {
    ...
    if (this.cell instanceof SideBySideDiffElementViewModel) {
      ...
      const originalMetadataModel = await this.textModelService.createModelReference(
      ...
      const modifiedMetadataModel = await this.textModelService.createModelReference(
        CellUri.generateCellPropertyUri(
      ...
      if (originalOutputsSource !== modifiedOutputsSource) {
        ...
        this._outputEditorDisposeStore.add(
          this.cell.modified.textModel.onDidChangeOutputs(() => {
            const modifiedOutputsSource2 = getFormattedOutputJSON(
              this.cell.modified?.outputs || []
            );
...
let ModifiedElement = class extends AbstractElementRenderer {
  ...
  _buildOutputRendererContainer() {
    if (!this._outputViewContainer) {
      ...
      const outputMetadataChangeOnly = outputModified && outputModified.kind === OutputComparison.Metadata && this.cell.original.outputs.length === 1 && this.cell.modified.outputs.length === 1 && outputEqual(
      ...
      if (outputMetadataChangeOnly) {
        ...
        this._outputMetadataEditor = this.instantiationService.createInstance(
          DiffEditorWidget,
  ...
  _showOutputsRenderer() {
    if (this._outputViewContainer) {
      ...
      this._outputMetadataEditor?.layout({
        width: this._editor?.getViewWidth() || this.cell.getComputedCellContainerWidth(
          this.notebookEditor.getLayoutInfo(),
          false,
          true
        ),
  ...
  updateSourceEditor() {
    ...
    this._cellHeader = this._register(
      this.instantiationService.createInstance(
        ...
        this.notebookEditor,
        {
          updateInfoRendering: () => renderSourceEditor(),
          checkIfModified: (cell) => {
            return cell.modified?.textModel.getTextBufferHash() !== cell.original?.textModel.getTextBufferHash() ? { reason: void 0 } : false;
          },
          getFoldingState: (cell) => cell.cellFoldingState,
          updateFoldingState: (cell, state) => cell.cellFoldingState = state,
          unChangedLabel: "Input",
    ...
    const ignore = this.textConfigurationService.getValue(
    ...
    const refreshToolbar = () => {
      const ignore2 = this.textConfigurationService.getValue(
        this.cell.modified.uri,
        "diffEditor.ignoreTrimWhitespace"
      );
  ...
  async _initializeSourceDiffEditor() {
    ...
    const modifiedCell = this.cell.modified;
    const originalRef = await this.textModelService.createModelReference(
      originalCell.uri
    );
    const modifiedRef = await this.textModelService.createModelReference(
      modifiedCell.uri
  ...
  updateEditorOptionsForWhitespace() {
    ...
    const ignoreTrimWhitespace = this.textConfigurationService.getValue(
      uri,
      "diffEditor.ignoreTrimWhitespace"
    );
    ...
    this._register(
      this.textConfigurationService.onDidChangeConfiguration((e) => {
        if (e.affectsConfiguration(uri, "diffEditor") && e.affectedKeys.has("diffEditor.ignoreTrimWhitespace")) {
          const ignoreTrimWhitespace2 = this.textConfigurationService.getValue(
            uri,
            "diffEditor.ignoreTrimWhitespace"
          );
