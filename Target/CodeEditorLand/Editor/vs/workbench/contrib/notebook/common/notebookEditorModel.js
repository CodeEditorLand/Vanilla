...
let SimpleNotebookEditorModel = class extends EditorModel {
  ...
  _workingCopy;
  _workingCopyListeners = this._register(
    new DisposableStore()
  );
  scratchPad;
  dispose() {
  ...
  async canDispose() {
    ...
    if (SimpleNotebookEditorModel._isStoredFileWorkingCopy(
      this._workingCopy
    )) {
  ...
  isOrphaned() {
    return SimpleNotebookEditorModel._isStoredFileWorkingCopy(
      this._workingCopy
...
class NotebookFileWorkingCopyModel extends Disposable {
  ...
  async update(stream, token) {
    ...
    this._notebookLogService.info(
      "WorkingCopyModel",
      "Notebook content updated from file system - " + this._notebookModel.uri.toString()
    );
...
let NotebookFileWorkingCopyModelFactory = class {
  ...
  async createModel(resource, stream, token) {
    const notebookModel = this._notebookService.getNotebookTextModel(resource) ?? await this._notebookService.createNotebookTextModel(
      this._viewType,
      resource,
      stream
    );
