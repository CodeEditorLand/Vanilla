...
import {
  ...
  NotebookCellList
} from "../../browser/view/notebookCellList.js";
...
} from "../../browser/viewModel/notebookOutlineDataSourceFactory.js";
import {
  NotebookViewModel
} from "../../browser/viewModel/notebookViewModelImpl.js";
...
function setupInstantiationService(disposables) {
  ...
  instantiationService.stub(
    IListService,
    disposables.add(instantiationService.createInstance(ListService))
  );
  ...
  instantiationService.stub(
    ...
    disposables.add(new TestWorkspaceTrustRequestService(true))
  );
  instantiationService.stub(
    INotebookExecutionStateService,
    new TestNotebookExecutionStateService()
  );
  instantiationService.stub(IKeybindingService, new MockKeybindingService());
  instantiationService.stub(
    INotebookCellStatusBarService,
    disposables.add(new NotebookCellStatusBarService())
  );
  instantiationService.stub(
    ICodeEditorService,
    disposables.add(new TestCodeEditorService(testThemeService))
  );
  instantiationService.stub(
    INotebookCellOutlineDataSourceFactory,
  ...
  instantiationService.stub(
    ...
    new class MockLanguageDetectionService {
      _serviceBrand;
...
function _createTestNotebookEditor(instantiationService, disposables, cells) {
  ...
  const notebook = disposables.add(
    instantiationService.createInstance(
      NotebookTextModel,
      viewType,
      URI.parse("test://test"),
  ...
  const notebookOptions = disposables.add(
    new NotebookOptions(
      mainWindow,
      false,
      void 0,
  ...
  const viewModel = disposables.add(
    instantiationService.createInstance(
      NotebookViewModel,
      viewType,
      model.notebook,
  ...
  const cellList = disposables.add(
    createNotebookCellList(instantiationService, disposables, viewContext)
  );
  cellList.attachViewModel(viewModel);
  const listViewInfoAccessor = disposables.add(
...
async function withTestNotebookDiffModel(originalCells, modifiedCells, callback) {
  ...
  const originalNotebook = createTestNotebookEditor(
    instantiationService,
    disposables,
    originalCells
  );
  const modifiedNotebook = createTestNotebookEditor(
    instantiationService,
    disposables,
    modifiedCells
  );
...
async function withTestNotebook(cells, callback, accessor) {
  ...
  const notebookEditor = _createTestNotebookEditor(
    instantiationService,
    disposables,
    cells
  );
...
function createNotebookCellList(instantiationService, disposables, viewContext) {
  ...
  const notebookOptions = !!viewContext ? viewContext.notebookOptions : disposables.add(
    new NotebookOptions(
      mainWindow,
      false,
      void 0,
