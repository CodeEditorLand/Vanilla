...
suite("Notebook Clipboard", () => {
  ...
  test.skip("Cut multiple selected cells", async () => {
    await withTestNotebook(
      ...
      async (editor, viewModel, _ds, accessor) => {
        ...
        const clipboardContrib = new NotebookClipboardContribution(
          createEditorService(editor)
        );
  ...
  test.skip("Cut should take folding info into account", async () => {
    await withTestNotebook(
      [
  ...
  test.skip("Copy should take folding info into account", async () => {
    await withTestNotebook(
      [
        ["# header a", "markdown", CellKind.Markup, [], {}],
        ["var b = 1;", "javascript", CellKind.Code, [], {}],
      ...
      async (editor, viewModel, _ds, accessor) => {
        ...
        assert.strictEqual(
          viewModel.cellAt(8)?.getText(),
          "var b = 1;"
        );
      }
    );
  });
  test.skip("#119773, cut last item should not focus on the top first cell", async () => {
    await withTestNotebook(
      [
        ["# header 1", "markdown", CellKind.Markup, [], {}],
        ["paragraph 1", "markdown", CellKind.Markup, [], {}],
  ...
  test.skip("#119771, undo paste should restore selections", async () => {
    await withTestNotebook(
      [
        ["# header 1", "markdown", CellKind.Markup, [], {}],
        ["paragraph 1", "markdown", CellKind.Markup, [], {}],
  ...
  test("copy cell from ui still works if the target cell is not part of a selection", async () => {
    await withTestNotebook(
      ...
      async (editor, viewModel, _ds, accessor) => {
        ...
        assert.deepStrictEqual(_toCopy, [
          viewModel.cellAt(0).model,
          viewModel.cellAt(1).model
        ]);
