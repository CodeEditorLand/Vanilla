...
suite("NotebookCellAnchor", () => {
  ...
  setup(() => {
    ...
    cellAnchor = store.add(
      new NotebookCellAnchor(executionService, config, scrollEvent.event)
    );
  });
  class MockListView {
    focusedCellTop = 100;
  ...
  test("Basic anchoring", async () => {
    focusedCell.focusMode = CellFocusMode.Editor;
    const listView = new MockListView();
    assert(
      cellAnchor.shouldAnchor(listView, 1, -10, resizingCell),
      "should anchor if cell editor is focused"
    );
    assert(
      cellAnchor.shouldAnchor(listView, 1, 10, resizingCell),
      "should anchor if cell editor is focused"
    ...
    assert(
      cellAnchor.shouldAnchor(listView, 1, 10, resizingCell),
      "should anchor if cell is growing"
    );
    ...
    assert(
      !cellAnchor.shouldAnchor(listView, 1, -10, resizingCell),
      "should not anchor if not growing and editor not focused"
    );
    config.setUserConfiguration(NotebookSetting.scrollToRevealCell, "none");
    assert(
      !cellAnchor.shouldAnchor(listView, 1, 10, resizingCell),
      "should not anchor if scroll on execute is disabled"
    );
  });
  test("Anchor during execution until user scrolls up", async () => {
    const listView = new MockListView();
    ...
    assert(
      cellAnchor.shouldAnchor(listView, 1, 10, resizingCell),
      "cell should still be anchored after scrolling down"
    );
    scrollEvent.fire(scrollUp);
    assert(
      !cellAnchor.shouldAnchor(listView, 1, 10, resizingCell),
      "cell should not be anchored after scrolling up"
    );
    focusedCell.focusMode = CellFocusMode.Editor;
    assert(
      cellAnchor.shouldAnchor(listView, 1, 10, resizingCell),
      "cell should anchor again if the editor is focused"
    );
    focusedCell.focusMode = CellFocusMode.Container;
    onDidStopExecution.fire();
    assert(
      cellAnchor.shouldAnchor(listView, 1, 10, resizingCell),
      "cell should anchor for new execution"
    );
  });
  test("Only anchor during when the focused cell will be pushed out of view", async () => {
    const mockListView = new MockListView();
    mockListView.focusedCellTop = 50;
    const listView = mockListView;
    assert(
      !cellAnchor.shouldAnchor(listView, 1, 10, resizingCell),
      "should not anchor if focused cell will still be fully visible after resize"
    );
    focusedCell.focusMode = CellFocusMode.Editor;
    assert(
      cellAnchor.shouldAnchor(listView, 1, 10, resizingCell),
      "cell should always anchor if the editor is focused"
    );
    assert(
      cellAnchor.shouldAnchor(listView, 1, 150, resizingCell),
      "cell should be anchored if focused cell will be pushed out of view"
    );
