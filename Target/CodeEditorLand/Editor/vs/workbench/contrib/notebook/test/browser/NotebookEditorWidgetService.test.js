...
suite("NotebookEditorWidgetService", () => {
  ...
  test("Retrieve widget within group", async () => {
    const notebookEditorInput = createNotebookInput("/test.np", "type1");
    const notebookEditorService = disposables.add(
      instantiationService.createInstance(
        TestNotebookEditorWidgetService
      )
    );
    const widget = notebookEditorService.retrieveWidget(
      instantiationService,
      1,
      notebookEditorInput
    );
    const value = widget.value;
    const widget2 = notebookEditorService.retrieveWidget(
      instantiationService,
      1,
      notebookEditorInput
    );
    ...
    assert.strictEqual(
      value,
      widget2.value,
      "should return the same widget"
    );
    assert.strictEqual(
      widget.value,
      void 0,
      "initial borrow should no longer have widget"
    );
  });
  test("Retrieve independent widgets", async () => {
    ...
    const widgetDiffGroup = notebookEditorService.retrieveWidget(
      instantiationService,
      2,
      inputType1
    );
    const widgetDiffType = notebookEditorService.retrieveWidget(
      instantiationService,
      1,
      inputType2
    );
    ...
    assert.notStrictEqual(
      widgetDiffGroup.value,
      void 0,
      "should create a widget"
    );
    assert.notStrictEqual(
      widgetDiffType.value,
      void 0,
      "should create a widget"
    );
    assert.notStrictEqual(
      widget.value,
      widgetDiffGroup.value,
      "should return a different widget"
    );
    assert.notStrictEqual(
      widget.value,
      widgetDiffType.value,
  ...
  test("Only relevant widgets get disposed", async () => {
    const inputType1 = createNotebookInput("/test.np", "type1");
    const inputType2 = createNotebookInput("/test.np", "type2");
    const notebookEditorService = disposables.add(
      instantiationService.createInstance(
    ...
    assert.strictEqual(
      widget.value,
      void 0,
      "widgets in group should get disposed"
    );
    assert.strictEqual(
      widgetDiffType.value,
      void 0,
    ...
    assert.notStrictEqual(
      widgetDiffGroup.value,
      void 0,
      "other group should not be disposed"
    );
  });
  test("Widget should move between groups when editor is moved", async () => {
    const inputType1 = createNotebookInput(
      "/test.np",
    ...
    const initialValue = notebookEditorService.retrieveWidget(
      instantiationService,
      1,
      inputType1
    ).value;
    ...
    const widgetFirstGroup = notebookEditorService.retrieveWidget(
      instantiationService,
      1,
      inputType1
    );
    ...
    assert.strictEqual(
      widgetDiffGroup.value,
      initialValue,
      "widget should be reused in new group"
    );
    assert.notStrictEqual(
      widgetFirstGroup.value,
      initialValue,
      "should create a new widget in the first group"
    );
  });
});
