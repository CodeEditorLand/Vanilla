...
} from "../../browser/contrib/notebookVariables/notebookVariablesDataSource.js";
suite("NotebookVariableDataSource", () => {
  ...
  test("Root element should return children", async () => {
    const variables = await dataSource.getChildren({
      kind: "root",
  ...
  test("Get children for large list", async () => {
    const parent = {
      ...
      value: "[...]",
      hasNamedChildren: false,
      indexedChildrenCount: 2e3
    };
    results = [];
    const variables = await dataSource.getChildren(parent);
    assert(
      variables.length > 1,
      "We should have results for groups of children"
    );
    assert(
      !provideVariablesCalled,
      "provideVariables should not be called"
    );
    assert.equal(
      variables[0].extHostId,
      parent.extHostId,
      "ExtHostId should match the parent since we will use it to get the real children"
    );
  });
  test("Get children for very large list", async () => {
    ...
    assert(
      children.length === 100,
      "We should have a full page of child groups"
    );
    assert(
      !provideVariablesCalled,
      "provideVariables should not be called"
  ...
  test("Cancel while enumerating through children", async () => {
    ...
    assert.equal(
      variables.length,
      5,
      "Iterating should have been cancelled"
    );
  });
});
