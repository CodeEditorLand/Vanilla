...
suite("notebookCellDiagnostics", () => {
  ...
  setup(() => {
    ...
    const config = instantiationService.get(
  ...
  test("diagnostic is added for cell execution failure", async () => {
    await withTestNotebook(
      ...
      async (editor, viewModel, store, accessor) => {
        ...
        testExecutionService.fireExecutionChanged(
          editor.textModel.uri,
          cell.handle
        );
        await waitForState(cell.excecutionError, (error) => !!error);
        assert.strictEqual(
          cell?.excecutionError.get()?.message,
          "error"
        );
  ...
  test("diagnostics are cleared only for cell with new execution", async () => {
    await withTestNotebook(
      [
        ["print(x)", "python", CellKind.Code, [], {}],
      ...
      async (editor, viewModel, store, accessor) => {
        ...
        assert.strictEqual(
          cell2?.excecutionError.get()?.message,
          "another error",
