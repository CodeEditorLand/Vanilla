...
import {
  ...
  prettyChanges
} from "../../browser/diff/notebookDiffViewModel.js";
import {
  CellKind
...
suite("NotebookDiff", () => {
    ...
    await withTestNotebookDiffModel(
          ...
          [
            {
              ...
              outputs: [
                {
                  mime: Mimes.text,
                  data: VSBuffer.wrap(new Uint8Array([3]))
                }
              ]
            }
          ],
      ...
      async (model, disposables2, accessor) => {
        const diff = new LcsDiff(
          new CellSequence(model.original.notebook),
          new CellSequence(model.modified.notebook)
        );
        ...
        diffViewModel = disposables2.add(
          new NotebookDiffViewModel(
            model,
            notebookEditorWorkerService,
            configurationService,
            eventDispatcher,
            accessor.get(INotebookService),
            void 0
          )
        );
  ...
  test("diff different output", async () => {
    await withTestNotebookDiffModel(
          ...
          [
            {
              ...
              outputs: [
                {
                  mime: Mimes.text,
                  data: VSBuffer.wrap(new Uint8Array([5]))
                }
              ]
            }
          ],
      ...
      async (model, disposables2, accessor) => {
        ...
        let eventArgs;
        disposables2.add(
          diffViewModel.onDidChangeItems((e) => eventArgs = e)
        );
        await diffViewModel.computeDiff(token);
          ...
          [
            {
              ...
              outputs: [
                {
                  mime: Mimes.text,
                  data: VSBuffer.wrap(new Uint8Array([6]))
                }
              ]
            }
          ],
          ...
          [
            {
              ...
              outputs: [
                {
                  mime: Mimes.text,
                  data: VSBuffer.wrap(new Uint8Array([2]))
                }
              ]
            }
          ],
  ...
  test("diff insert 2", async () => {
    await withTestNotebookDiffModel(
      ...
      async (model, disposables2, accessor) => {
        const eventDispatcher2 = disposables2.add(
          new NotebookDiffEditorEventDispatcher()
        );
        diffResult = {
          changes: [
  ...
  test("diff insert 3", async () => {
    await withTestNotebookDiffModel(
      ...
      async (model, disposables2, accessor) => {
        ...
        assert.strictEqual(
          diffViewModel.items[0].type,
          "unchanged"
        );
        assert.strictEqual(diffViewModel.items[1].type, "unchanged");
        assert.strictEqual(
          diffViewModel.items[2].type,
          "unchanged"
        );
        assert.strictEqual(diffViewModel.items[3].type, "unchanged");
        assert.strictEqual(diffViewModel.items[4].type, "insert");
  ...
  test("LCS", async () => {
    await withTestNotebookDiffModel(
      ...
      async (model) => {
        ...
        assert.deepStrictEqual(
          diffResult2.changes.map((change) => ({
            originalStart: change.originalStart,
            originalLength: change.originalLength,
            modifiedStart: change.modifiedStart,
            modifiedLength: change.modifiedLength
  ...
  test("LCS 2", async () => {
    await withTestNotebookDiffModel(
      ...
      async (model) => {
        ...
        const diffResult2 = diff.ComputeDiff(false);
        prettyChanges(model, diffResult2);
        assert.deepStrictEqual(
          diffResult2.changes.map((change) => ({
            originalStart: change.originalStart,
            originalLength: change.originalLength,
  ...
  test("diff output", async () => {
    await withTestNotebookDiffModel(
          ...
          [
            {
              ...
              outputs: [
                {
                  mime: Mimes.text,
                  data: VSBuffer.wrap(new Uint8Array([4]))
                }
              ]
            }
          ],
      ...
      async (model, disposables2, accessor) => {
        ...
        diffViewModel.items[0].showHiddenCells();
        assert.strictEqual(
          diffViewModel.items[0].checkIfOutputsModified(),
          false
        );
        assert.strictEqual(diffViewModel.items[1].type, "modified");
        await verifyChangeEventIsNotFired(diffViewModel);
  ...
  test("diff output fast check", async () => {
    await withTestNotebookDiffModel(
      ...
      async (model, disposables2, accessor) => {
        ...
        diffViewModel.items[0].showHiddenCells();
        assert.strictEqual(
          diffViewModel.items[0].original.textModel.equal(
            diffViewModel.items[0].modified.textModel
          ),
          true
        );
        assert.strictEqual(
          diffViewModel.items[1].original.textModel.equal(
            diffViewModel.items[1].modified.textModel
          ),
          false
        );
        await verifyChangeEventIsNotFired(diffViewModel);
