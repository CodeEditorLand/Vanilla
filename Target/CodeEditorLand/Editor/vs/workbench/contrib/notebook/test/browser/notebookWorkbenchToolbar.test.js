...
} from "../../browser/viewParts/notebookEditorToolbar.js";
suite("Workbench Toolbar calculateActions (strategy always + never)", () => {
  ...
  const defaultSecondaryActions = defaultSecondaryActionModels.map(
  ...
  setup(() => {
    defaultSecondaryActionModels.forEach(
      (action) => disposables.add(action.action)
    );
  });
  ...
  test("should return all primary actions when they fit within the container width", () => {
    ...
    const result = workbenchCalculateActions(
      actions,
  ...
  test("should keep actions with size 0 in primary actions", () => {
    ...
    assert.deepEqual(
      result.secondaryActions,
      [actions[2], separator, ...defaultSecondaryActionModels].map(
        (action) => action.action
      )
    );
  });
...
suite("Workbench Toolbar Dynamic calculateActions (strategy dynamic)", () => {
  ...
  test("should return all primary actions as visiblewhen they fit within the container width", () => {
    ...
    const input = [
      {
        action: actionTemplate[0],
        size: 50,
        visible: true,
        renderLabel: true
      },
      {
        action: actionTemplate[1],
        size: 50,
        visible: true,
        renderLabel: true
      },
      {
        action: actionTemplate[2],
        size: 50,
        visible: true,
        renderLabel: true
      }
    ...
    const result = workbenchDynamicCalculateActions(
      input,
      defaultSecondaryActions,
      constainerSize
    );
  ...
  test("actions all within a group that cannot all fit, will all be icon only", () => {
    ...
    assert.deepEqual(
      result.secondaryActions,
      [...defaultSecondaryActionModels].map((action) => action.action)
    );
  });
  test("should ignore second separator when two separators are in a row", () => {
