...
suite("NotebookProviderInfoStore", () => {
  const disposables = ensureNoDisposablesAreLeakedInTestSuite();
  test("Can't open untitled notebooks in test #119363", () => {
    ...
    let providers = store.getContributedNotebook(
      URI.parse("file:///test/nb.foo")
    );
    assert.strictEqual(providers.length, 1);
    assert.strictEqual(providers[0] === fooInfo, true);
    providers = store.getContributedNotebook(
      URI.parse("file:///test/nb.bar")
    );
    assert.strictEqual(providers.length, 1);
    ...
    providers = store.getContributedNotebook(
      URI.parse("untitled:///test/nb.bar")
    );
    assert.strictEqual(providers.length, 1);
    assert.strictEqual(providers[0] === barInfo, true);
  });
