...
registerAction2(
  class extends Action2 {
    ...
    async run(accessor, item) {
      ...
      const selectedEntry = (await findLocalHistoryEntry(
        workingCopyHistoryService,
  ...
  class extends Action2 {
    ...
    async run(accessor) {
      ...
      const recentEditorResources = new ResourceSet(
        coalesce(
          historyService.getHistory().map(({ resource: resource2 }) => resource2)
        )
      );
      ...
      resourcePicker.items = [...resourcesSortedByRecency].map(
        (resource2) => ({
          ...
          description: labelService.getUriLabel(dirname(resource2), {
            relative: true
          }),
