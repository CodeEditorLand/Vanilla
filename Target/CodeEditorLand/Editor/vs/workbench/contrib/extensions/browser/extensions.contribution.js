...
let ExtensionsContributions = class extends Disposable {
  ...
  registerGlobalActions() {
    ...
    this.registerExtensionAction({
      ...
      run: /* @__PURE__ */ __name(async () => {
        ...
        results.forEach((result) => {
          if (result.error) {
            const extension = outdated.find((extension2) => areSameExtensions(extension2.identifier, result.identifier));
            if (extension) {
    ...
    this.registerExtensionAction({
      ...
      run: /* @__PURE__ */ __name(() => runAction(this.instantiationService.createInstance(ReinstallAction, ReinstallAction.ID, ReinstallAction.LABEL)), "run")
    });
  }
