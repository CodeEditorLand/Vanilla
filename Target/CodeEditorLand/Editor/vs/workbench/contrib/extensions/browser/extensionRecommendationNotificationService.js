...
class RecommendationsNotification extends Disposable {
    ...
    if (!this.notificationHandle) {
      this.updateNotificationHandle(
        this.notificationService.prompt(
          ...
          this.message,
          this.choices,
          { sticky: true, onCancel: /* @__PURE__ */ __name(() => this.cancelled = true, "onCancel") }
        )
      );
    }
  }
      ...
      this.updateNotificationHandle(
        this.notificationService.prompt(
          ...
          this.choices,
          {
            ...
            onCancel: /* @__PURE__ */ __name(() => this.cancelled = true, "onCancel")
          }
        )
      );
    }
  ...
  onDidCloseDisposable = this._register(
    new MutableDisposable()
  );
  onDidChangeVisibilityDisposable = this._register(
    new MutableDisposable()
  );
  updateNotificationHandle(notificationHandle) {
    ...
    this.onDidChangeVisibilityDisposable.value = this.notificationHandle.onDidChangeVisibility(
      (e) => this._onDidChangeVisibility.fire(e)
    );
  }
}
let ExtensionRecommendationNotificationService = class extends Disposable {
  ...
  async promptImportantExtensionsInstallNotification(extensionRecommendations) {
    ...
    return this.promptRecommendationsNotification(
      ...
      {
        onDidInstallRecommendedExtensions: /* @__PURE__ */ __name((extensions2) => extensions2.forEach(
          (extension) => this.telemetryService.publicLog2("extensionRecommendations:popup", {
        ...
        onDidShowRecommendedExtensions: /* @__PURE__ */ __name((extensions2) => extensions2.forEach(
          (extension) => this.telemetryService.publicLog2("extensionRecommendations:popup", {
            userReaction: "show",
            extensionId: extension.identifier.id,
            source: RecommendationSourceToString(
        ...
        onDidCancelRecommendedExtensions: /* @__PURE__ */ __name((extensions2) => extensions2.forEach(
          (extension) => this.telemetryService.publicLog2("extensionRecommendations:popup", {
  ...
  async promptWorkspaceRecommendations(recommendations) {
    ...
    installed = installed.filter(
      (l) => this.extensionEnablementService.getEnablementState(l) !== EnablementState.DisabledByExtensionKind
    );
    ...
    await this.promptRecommendationsNotification(
      ...
      {
        onDidInstallRecommendedExtensions: /* @__PURE__ */ __name(() => this.telemetryService.publicLog2("extensionWorkspaceRecommendations:popup", {
          userReaction: "install"
        }), "onDidInstallRecommendedExtensions"),
        onDidShowRecommendedExtensions: /* @__PURE__ */ __name(() => this.telemetryService.publicLog2("extensionWorkspaceRecommendations:popup", {
          userReaction: "show"
        }), "onDidShowRecommendedExtensions"),
        onDidCancelRecommendedExtensions: /* @__PURE__ */ __name(() => this.telemetryService.publicLog2("extensionWorkspaceRecommendations:popup", {
          userReaction: "cancelled"
        }), "onDidCancelRecommendedExtensions"),
        onDidNeverShowRecommendedExtensionsAgain: /* @__PURE__ */ __name(() => {
          this.telemetryService.publicLog2("extensionWorkspaceRecommendations:popup", {
            userReaction: "neverShowAgain"
          });
  ...
  async promptRecommendationsNotification({
    ...
    if (source === RecommendationSource.EXE && extensionIds.every(
    ...
    if (extensions.length === 1) {
      ...
      const publishers = [
        ...extensions.reduce(
          (result, extension) => result.add(extension.publisherDisplayName),
          /* @__PURE__ */ new Set()
        )
      ];
    ...
    if (!searchValue) {
      searchValue = source === RecommendationSource.WORKSPACE ? "@recommended" : extensions.map(
        (extensionId) => `@id:${extensionId.identifier.id}`
      ).join(" ");
    }
    const donotShowAgainLabel = source === RecommendationSource.WORKSPACE ? localize(
      "donotShowAgain",
      "Don't Show Again for this Repository"
    ) : extensions.length > 1 ? localize(
  ...
  showRecommendationsNotification(extensions, message, searchValue, donotShowAgainLabel, source, {
    onDidInstallRecommendedExtensions,
    onDidShowRecommendedExtensions,
    onDidCancelRecommendedExtensions,
    onDidNeverShowRecommendedExtensionsAgain
      ...
      async (token) => {
        ...
        const installExtensions = /* @__PURE__ */ __name(async (isMachineScoped) => {
          ...
          await Promises.settled([
            Promises.settled(
              extensions.map(
                (extension) => this.extensionsWorkbenchService.open(
                  extension,
                  { pinned: true }
                )
              )
            ),
            galleryExtensions.length ? this.extensionManagementService.installGalleryExtensions(
              galleryExtensions.map((e) => ({
                extension: e,
                options: { isMachineScoped }
              }))
            ) : Promise.resolve(),
            resourceExtensions.length ? Promise.allSettled(
              resourceExtensions.map(
                (r) => this.extensionsWorkbenchService.install(
                  r
                )
              )
            ) : Promise.resolve()
            ...
            this.extensionManagementService.onInstallExtension(
              (e2) => {
                installedExtensions.push(
                  e2.identifier.id.toLowerCase()
                );
                if (extensions.every(
                  (e3) => installedExtensions.includes(
                    e3.identifier.id.toLowerCase()
                  )
                )) {
                  c(
  ...
  async getInstallableExtensions(recommendations) {
    ...
    if (recommendations.length) {
      ...
      if (galleryExtensions.length) {
        ...
        for (const extension of extensions) {
          if (extension.gallery && await this.extensionManagementService.canInstall(
            extension.gallery
          )) {
