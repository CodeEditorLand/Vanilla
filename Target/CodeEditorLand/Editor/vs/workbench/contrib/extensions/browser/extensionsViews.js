...
let ExtensionsListView = class extends ViewPane {
  ...
  filterInstalledExtensions(local, runningExtensions, query, options) {
    ...
    const matchingText = (e) => (e.name.toLowerCase().indexOf(value) > -1 || e.displayName.toLowerCase().indexOf(value) > -1 || e.description.toLowerCase().indexOf(value) > -1) && this.filterExtensionByCategory(
      e,
      includedCategories,
      excludedCategories
    );
