...
} from "../../../../services/extensionManagement/test/browser/extensionEnablementService.test.js";
...
suite("ExtensionsWorkbenchServiceTest", () => {
  ...
  setup(async () => {
    ...
    installEvent = disposableStore.add(
      new Emitter()
    );
    didInstallEvent = disposableStore.add(
      new Emitter()
    );
    uninstallEvent = disposableStore.add(
      new Emitter()
    );
    didUninstallEvent = disposableStore.add(
      new Emitter()
    );
  ...
  test("test gallery extension", async () => {
    const expected = aGalleryExtension(
      "expectedName",
      {
      ...
      },
      {
        ...
        download: {
          uri: "uri:download",
          fallbackUri: "fallback:download"
        },
        icon: { uri: "uri:icon", fallbackUri: "fallback:icon" },
        license: {
          uri: "uri:license",
          fallbackUri: "fallback:license"
        },
        repository: {
          uri: "uri:repository",
  ...
  test("test for installed extensions", async () => {
    const expected1 = aLocalExtension(
      "local1",
      {
    ...
    const expected2 = aLocalExtension(
      "local2",
      {
    ...
    assert.ok(
      actual.iconUrl === "file:///localPath1/localIcon1" || actual.iconUrl === "vscode-file://vscode-app/localPath1/localIcon1"
    );
    assert.ok(
      actual.iconUrlFallback === "file:///localPath1/localIcon1" || actual.iconUrlFallback === "vscode-file://vscode-app/localPath1/localIcon1"
    );
  ...
  test("test installed extensions get syncs with gallery", async () => {
    ...
    const gallery1 = aGalleryExtension(
      local1.manifest.name,
      {
  ...
  test("test extension state computation", async () => {
    ...
    return testObject.queryGallery(CancellationToken.None).then((page) => {
      ...
      assert.strictEqual(
        `${gallery.publisher}.${gallery.name}`,
        actual.identifier.id
      );
      ...
      didInstallEvent.fire([
        {
          ...
          local: aLocalExtension(gallery.name, gallery, {
            identifier
          }),
          profileLocation: null
        }
  ...
  test("test extension doesnot show outdated for system extensions", async () => {
    ...
    instantiationService.stubPromise(
      ...
      "query",
      aPage(
        aGalleryExtension(local.manifest.name, {
  ...
  test("test canInstall returns true for extensions with gallery", async () => {
    ...
    await Event.toPromise(
      Event.filter(testObject.onChange, (e) => !!e?.gallery)
    );
    assert.ok(await testObject.canInstall(target));
  });
  ...
  test("test uninstalled extensions are always enabled", async () => {
    return instantiationService.get(IWorkbenchExtensionEnablementService).setEnablement(
      [aLocalExtension("b")],
      EnablementState.DisabledGlobally
    ).then(
      () => instantiationService.get(IWorkbenchExtensionEnablementService).setEnablement(
        [aLocalExtension("c")],
  ...
  test("test system extensions can be disabled", async () => {
    instantiationService.stubPromise(
      IExtensionManagementService,
      "getInstalled",
      [aLocalExtension("a", {}, { type: ExtensionType.System })]
    );
    testObject = await aWorkbenchService();
    return testObject.setEnablement(
      testObject.local[0],
  ...
  test("test disable extension with dependencies disable only itself", async () => {
    ...
    return instantiationService.get(IWorkbenchExtensionEnablementService).setEnablement([extensionA], EnablementState.EnabledGlobally).then(
      () => instantiationService.get(IWorkbenchExtensionEnablementService).setEnablement(
        [extensionB],
        EnablementState.EnabledGlobally
      )
      ...
      return testObject.setEnablement(
        ...
        assert.strictEqual(
          testObject.local[0].enablementState,
          EnablementState.DisabledGlobally
        );
        assert.strictEqual(
          testObject.local[1].enablementState,
          EnablementState.EnabledGlobally
        );
      });
    });
  ...
  test("test disable both dependency and dependent do not promot and do not fail", async () => {
    ...
    return instantiationService.get(IWorkbenchExtensionEnablementService).setEnablement([extensionA], EnablementState.EnabledGlobally).then(
      ...
      testObject = await aWorkbenchService();
      return testObject.setEnablement(
        [testObject.local[1], testObject.local[0]],
        EnablementState.DisabledGlobally
      ).then(() => {
  ...
  test("test enable both dependency and dependent do not promot and do not fail", async () => {
    ...
    const extensionB = aLocalExtension("b");
    const extensionC = aLocalExtension("c");
    return instantiationService.get(IWorkbenchExtensionEnablementService).setEnablement([extensionA], EnablementState.DisabledGlobally).then(
      () => instantiationService.get(IWorkbenchExtensionEnablementService).setEnablement(
        [extensionB],
        EnablementState.DisabledGlobally
      )
  ...
  test("test disable extension if its dependencys dependency is itself", async () => {
    ...
    return instantiationService.get(IWorkbenchExtensionEnablementService).setEnablement([extensionA], EnablementState.EnabledGlobally).then(
      ...
      return testObject.setEnablement(
        ...
        () => assert.fail(
          "An extension with dependent should not be disabled"
        ),
        () => null
      );
    });
  ...
  test("test enable extension with cyclic dependencies", async () => {
    ...
    return instantiationService.get(IWorkbenchExtensionEnablementService).setEnablement([extensionA], EnablementState.DisabledGlobally).then(
      ...
      return testObject.setEnablement(
        ...
        assert.strictEqual(
          testObject.local[2].enablementState,
          EnablementState.EnabledGlobally
        );
      });
    });
  ...
  test("Test disable autoupdate for extension when auto update is enabled for all", async () => {
    ...
    await testObject.updateAutoUpdateEnablementFor(
      testObject.local[0],
      false
    );
    assert.strictEqual(testObject.local[0].local?.pinned, void 0);
  ...
  test("Test enable autoupdate for extension when auto update is enabled for all", async () => {
    ...
    assert.deepStrictEqual(
      testObject.getDisabledAutoUpdateExtensions(),
      []
    );
  });
  ...
  async function aWorkbenchService() {
    const workbenchService = disposableStore.add(
  ...
  function stubConfiguration(autoUpdateValue, autoCheckUpdatesValue) {
    ...
    const emitter = disposableStore.add(
      new Emitter()
    );
