...
suite("ExtensionsViews Tests", () => {
  ...
  let testableView;
  const localEnabledTheme = aLocalExtension(
    "first-enabled-extension",
    { categories: ["Themes", "random"] },
    { installedTimestamp: 123456 }
  );
  const localEnabledLanguage = aLocalExtension(
    "second-enabled-extension",
    { categories: ["Programming languages"], version: "1.0.0" },
    { installedTimestamp: Date.now(), updated: false }
  );
  const localDisabledTheme = aLocalExtension(
    "first-disabled-extension",
    { categories: ["themes"] },
    { installedTimestamp: 234567 }
  );
  const localDisabledLanguage = aLocalExtension(
    "second-disabled-extension",
    { categories: ["programming languages"] },
    { installedTimestamp: Date.now() - 5e4, updated: true }
  );
  const localRandom = aLocalExtension(
    "random-enabled-extension",
    { categories: ["random"] },
    { installedTimestamp: 345678 }
  );
  const builtInTheme = aLocalExtension(
    "my-theme",
    { categories: ["Themes"], contributes: { themes: ["my-theme"] } },
    { type: ExtensionType.System, installedTimestamp: 222 }
  );
  const builtInBasic = aLocalExtension(
    "my-lang",
    {
      categories: ["Programming Languages"],
      contributes: { grammars: [{ language: "my-language" }] }
    },
    { type: ExtensionType.System, installedTimestamp: 666666 }
  );
  const galleryEnabledLanguage = aGalleryExtension(
  ...
  const workspaceRecommendationA = aGalleryExtension(
    "workspace-recommendation-A"
  );
  const workspaceRecommendationB = aGalleryExtension(
    "workspace-recommendation-B"
  );
  const configBasedRecommendationA = aGalleryExtension(
    "configbased-recommendation-A"
  );
  const configBasedRecommendationB = aGalleryExtension(
    "configbased-recommendation-B"
  );
  const fileBasedRecommendationA = aGalleryExtension(
    "filebased-recommendation-A"
  );
  const fileBasedRecommendationB = aGalleryExtension(
    "filebased-recommendation-B"
  );
  ...
  setup(async () => {
    ...
    instantiationService.stub(
      IFileService,
      disposableStore.add(new FileService(new NullLogService()))
    );
    ...
    const localExtensionManagementServer = {
      extensionManagementService: instantiationService.get(
        IExtensionManagementService
      ),
      label: "local",
      id: "vscode-local"
    ...
    instantiationService.stubPromise(
      IExtensionManagementService,
      "getExtensgetExtensionsControlManifestionsReport",
      {}
    );
    ...
    instantiationService.stubPromise(
      IExtensionGalleryService,
      "query",
      aPage(galleryEnabledLanguage)
    );
    instantiationService.stubPromise(
      IExtensionGalleryService,
      "getCompatibleExtension",
      galleryEnabledLanguage
    );
    instantiationService.stubPromise(
      IExtensionGalleryService,
      "getExtensions",
    ...
    await instantiationService.get(IWorkbenchExtensionEnablementService).setEnablement(
      [localDisabledTheme],
    ...
    testableView = disposableStore.add(
  ...
  test("Test query types", () => {
    assert.strictEqual(
      ExtensionsListView.isBuiltInExtensionsQuery("@builtin"),
      true
    );
    assert.strictEqual(
      ExtensionsListView.isLocalExtensionsQuery("@installed"),
      true
    );
    assert.strictEqual(
      ExtensionsListView.isLocalExtensionsQuery("@enabled"),
      true
    );
    assert.strictEqual(
      ExtensionsListView.isLocalExtensionsQuery("@disabled"),
      true
    );
    assert.strictEqual(
      ExtensionsListView.isLocalExtensionsQuery("@outdated"),
      true
    );
    assert.strictEqual(
      ExtensionsListView.isLocalExtensionsQuery("@updates"),
      true
    );
    assert.strictEqual(
      ExtensionsListView.isLocalExtensionsQuery("@sort:name"),
      true
    );
    assert.strictEqual(
      ExtensionsListView.isLocalExtensionsQuery("@sort:updateDate"),
      true
    );
    assert.strictEqual(
      ExtensionsListView.isLocalExtensionsQuery("@installed searchText"),
      true
    );
    assert.strictEqual(
      ExtensionsListView.isLocalExtensionsQuery("@enabled searchText"),
      true
    );
    assert.strictEqual(
      ExtensionsListView.isLocalExtensionsQuery("@disabled searchText"),
      true
    );
    assert.strictEqual(
      ExtensionsListView.isLocalExtensionsQuery("@outdated searchText"),
      true
    );
    assert.strictEqual(
      ExtensionsListView.isLocalExtensionsQuery("@updates searchText"),
      true
    );
  });
  test("Test empty query equates to sort by install count", () => {
    const target = instantiationService.stubPromise(
      IExtensionGalleryService,
      "query",
      aPage()
    );
  ...
  test("Test default view actions required sorting", async () => {
    const workbenchService = instantiationService.get(
      IExtensionsWorkbenchService
    );
    const extension = (await workbenchService.queryLocal()).find(
      (ex) => ex.identifier === localEnabledLanguage.identifier
    );
    await new Promise((c) => {
    ...
    assert.strictEqual(
      result.length,
      5,
      "Unexpected number of results for @installed query"
    );
    const actual = [
      result.get(0).name,
      result.get(1).name,
    ...
    for (let i = 0; i < result.length; i++) {
      assert.strictEqual(
        actual[i],
        expected[i],
        "Unexpected extension for @installed query with outadted extension."
      );
    }
  });
  test("Test installed query results", async () => {
    await testableView.show("@installed").then((result) => {
      ...
      for (let i = 0; i < result.length; i++) {
        assert.strictEqual(
          actual[i],
          expected[i],
          "Unexpected extension for @installed query."
        );
      }
    });
    await testableView.show("@installed first").then((result) => {
      ...
      assert.strictEqual(
        result.get(0).name,
        localEnabledTheme.manifest.name,
        "Unexpected extension for @installed query with search text."
      );
      assert.strictEqual(
        result.get(1).name,
        localDisabledTheme.manifest.name,
    ...
    await testableView.show("@disabled").then((result) => {
      assert.strictEqual(
        result.length,
        2,
        "Unexpected number of results for @disabled query"
      );
      assert.strictEqual(
        result.get(0).name,
        localDisabledTheme.manifest.name,
        "Unexpected extension for @disabled query."
      );
      assert.strictEqual(
        result.get(1).name,
        localDisabledLanguage.manifest.name,
    ...
    await testableView.show("@enabled").then((result) => {
      assert.strictEqual(
        result.length,
        3,
        "Unexpected number of results for @enabled query"
      );
      assert.strictEqual(
        result.get(0).name,
        localEnabledTheme.manifest.name,
        "Unexpected extension for @enabled query."
      );
      assert.strictEqual(
        result.get(1).name,
        localRandom.manifest.name,
    ...
    await testableView.show("@builtin category:themes").then((result) => {
      assert.strictEqual(
        result.length,
        1,
        "Unexpected number of results for @builtin category:themes query"
      );
      assert.strictEqual(
        result.get(0).name,
        builtInTheme.manifest.name,
        "Unexpected extension for @builtin:themes query."
      );
    });
    await testableView.show('@builtin category:"programming languages"').then((result) => {
      assert.strictEqual(
        result.length,
        1,
        "Unexpected number of results for @builtin:basics query"
      );
      assert.strictEqual(
        result.get(0).name,
        builtInBasic.manifest.name,
        "Unexpected extension for @builtin:basics query."
      );
    });
    await testableView.show("@builtin").then((result) => {
      assert.strictEqual(
        result.length,
        2,
        "Unexpected number of results for @builtin query"
      );
      assert.strictEqual(
        result.get(0).name,
        builtInBasic.manifest.name,
  ...
  test("Test installed query with category", async () => {
    await testableView.show("@installed category:themes").then((result) => {
      assert.strictEqual(
        result.length,
        2,
        "Unexpected number of results for @installed query with category"
      );
      assert.strictEqual(
        result.get(0).name,
        localEnabledTheme.manifest.name,
        "Unexpected extension for @installed query with category."
      );
      assert.strictEqual(
        result.get(1).name,
        localDisabledTheme.manifest.name,
    ...
    await testableView.show('@installed category:"themes"').then((result) => {
      assert.strictEqual(
        result.length,
        2,
        "Unexpected number of results for @installed query with quoted category"
      );
      assert.strictEqual(
        result.get(0).name,
        localEnabledTheme.manifest.name,
        "Unexpected extension for @installed query with quoted category."
      );
      assert.strictEqual(
        result.get(1).name,
        localDisabledTheme.manifest.name,
    ...
    await testableView.show('@installed category:"programming languages"').then((result) => {
      assert.strictEqual(
        result.length,
        2,
        "Unexpected number of results for @installed query with quoted category including space"
      );
      assert.strictEqual(
        result.get(0).name,
        localEnabledLanguage.manifest.name,
        "Unexpected extension for @installed query with quoted category including space."
      );
      assert.strictEqual(
        result.get(1).name,
        localDisabledLanguage.manifest.name,
        "Unexpected extension for @installed query with quoted category inlcuding space."
      );
    });
    await testableView.show("@installed category:themes category:random").then((result) => {
      assert.strictEqual(
        result.length,
        3,
        "Unexpected number of results for @installed query with multiple category"
      );
      assert.strictEqual(
        result.get(0).name,
        localEnabledTheme.manifest.name,
        "Unexpected extension for @installed query with multiple category."
      );
      assert.strictEqual(
        result.get(1).name,
        localRandom.manifest.name,
    ...
    await testableView.show("@enabled category:themes").then((result) => {
      assert.strictEqual(
        result.length,
        1,
        "Unexpected number of results for @enabled query with category"
      );
      assert.strictEqual(
        result.get(0).name,
        localEnabledTheme.manifest.name,
        "Unexpected extension for @enabled query with category."
      );
    });
    await testableView.show('@enabled category:"themes"').then((result) => {
      assert.strictEqual(
        result.length,
        1,
        "Unexpected number of results for @enabled query with quoted category"
      );
      assert.strictEqual(
        result.get(0).name,
        localEnabledTheme.manifest.name,
        "Unexpected extension for @enabled query with quoted category."
      );
    });
    await testableView.show('@enabled category:"programming languages"').then((result) => {
      assert.strictEqual(
        result.length,
        1,
        "Unexpected number of results for @enabled query with quoted category inlcuding space"
      );
      assert.strictEqual(
        result.get(0).name,
        localEnabledLanguage.manifest.name,
        "Unexpected extension for @enabled query with quoted category including space."
      );
    });
    await testableView.show("@disabled category:themes").then((result) => {
      assert.strictEqual(
        result.length,
        1,
        "Unexpected number of results for @disabled query with category"
      );
      assert.strictEqual(
        result.get(0).name,
        localDisabledTheme.manifest.name,
        "Unexpected extension for @disabled query with category."
      );
    });
    await testableView.show('@disabled category:"themes"').then((result) => {
      assert.strictEqual(
        result.length,
        1,
        "Unexpected number of results for @disabled query with quoted category"
      );
      assert.strictEqual(
        result.get(0).name,
        localDisabledTheme.manifest.name,
        "Unexpected extension for @disabled query with quoted category."
      );
    });
    await testableView.show('@disabled category:"programming languages"').then((result) => {
      assert.strictEqual(
        result.length,
        1,
        "Unexpected number of results for @disabled query with quoted category inlcuding space"
      );
      assert.strictEqual(
        result.get(0).name,
        localDisabledLanguage.manifest.name,
  ...
  test("Test local query with sorting order", async () => {
    await testableView.show("@recentlyUpdated").then((result) => {
      assert.strictEqual(
        result.length,
        1,
        "Unexpected number of results for @recentlyUpdated"
      );
      assert.strictEqual(
        result.get(0).name,
        localDisabledLanguage.manifest.name,
        "Unexpected default sort order of extensions for @recentlyUpdate query"
      );
    });
    await testableView.show("@installed @sort:updateDate").then((result) => {
      assert.strictEqual(
        result.length,
        5,
        "Unexpected number of results for @sort:updateDate. Expected all localy installed Extension which are not builtin"
      );
      const actual = [
      ...
      for (let i = 0; i < result.length; i++) {
        assert.strictEqual(
          actual[i],
          expected[i],
          "Unexpected extension sorting for @sort:updateDate query."
        );
      }
    });
  });
  test("Test @recommended:workspace query", () => {
    ...
    return testableView.show("@recommended:workspace").then((result) => {
      ...
      assert.strictEqual(
        ...
        workspaceRecommendedExtensions.length
      );
      assert.strictEqual(
        result.length,
        workspaceRecommendedExtensions.length
      ...
      for (let i = 0; i < workspaceRecommendedExtensions.length; i++) {
        assert.strictEqual(
          extensionInfos[i].id,
          workspaceRecommendedExtensions[i].identifier.id
        );
        assert.strictEqual(
          result.get(i).identifier.id,
          workspaceRecommendedExtensions[i].identifier.id
        );
      }
    });
  ...
  test("Test search", () => {
    ...
    const queryTarget = instantiationService.stubPromise(
      IExtensionGalleryService,
      "query",
      aPage(...results)
    );
  ...
  test("Test preferred search experiment", () => {
    ...
    const experimentTarget = instantiationService.stubPromise(
      IWorkbenchExtensionManagementService,
