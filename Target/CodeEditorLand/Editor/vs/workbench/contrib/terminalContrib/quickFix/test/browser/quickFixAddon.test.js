...
suite("QuickFixAddon", () => {
  ...
  suite("registerCommandFinishedListener & getMatchActions", () => {
    ...
    suite("gitPull", () => {
      ...
      suite("returns undefined when", () => {
        test("output does not match", async () => {
          strictEqual(await getQuickFixesForCommand([], terminal, createCommand(command, `invalid output`, GitPullOutputRegex, exitCode), expectedMap, commandService, openerService, labelService), void 0);
        });
        test("command does not match", async () => {
          strictEqual(await getQuickFixesForCommand([], terminal, createCommand(`gt add`, output, GitPullOutputRegex, exitCode), expectedMap, commandService, openerService, labelService), void 0);
        });
        test("exit code does not match", async () => {
          strictEqual(await getQuickFixesForCommand([], terminal, createCommand(command, output, GitPullOutputRegex, 2), expectedMap, commandService, openerService, labelService), void 0);
        });
      });
      suite("returns actions when", () => {
...
function createCommand(command, output, outputMatcher, exitCode, outputLines) {
  return {
    ...
    isTrusted: true,
    exitCode,
    getOutput: /* @__PURE__ */ __name(() => {
      return output;
    }, "getOutput"),
    getOutputMatch: /* @__PURE__ */ __name((_matcher) => {
      if (outputMatcher) {
    ...
    timestamp: Date.now(),
    hasOutput: /* @__PURE__ */ __name(() => !!output, "hasOutput")
  };
}
...
__name(assertMatchOptions, "assertMatchOptions");
