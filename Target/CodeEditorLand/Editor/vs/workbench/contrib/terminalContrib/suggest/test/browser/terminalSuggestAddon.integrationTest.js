...
} from "../../browser/terminalSuggestAddon.js";
...
const recordedTestCases = [
  {
    name: "macos_bash_echo_simple",
    events: macos_bash_echo_simple
  },
  {
    name: "macos_bash_echo_multiline",
    events: macos_bash_echo_multiline
  },
  {
    name: "windows11_pwsh_getcontent_delete_ghost",
    events: windows11_pwsh_getcontent_delete_ghost
  },
  {
    name: "windows11_pwsh_getcontent_file",
    events: windows11_pwsh_getcontent_file
  },
  {
    name: "windows11_pwsh_input_ls_complete_ls",
    events: windows11_pwsh_input_ls_complete_ls
  },
  {
    name: "windows11_pwsh_namespace_completion",
    events: windows11_pwsh_namespace_completion
  },
  {
    name: "windows11_pwsh_type_before_prompt",
    events: windows11_pwsh_type_before_prompt
  },
  {
    name: "windows11_pwsh_writehost_multiline_nav_up",
    events: windows11_pwsh_writehost_multiline_nav_up
  },
  {
    name: "windows11_pwsh_writehost_multiline",
    events: windows11_pwsh_writehost_multiline
  }
];
suite("Terminal Contrib Suggest Recordings", () => {
  ...
  setup(async () => {
    ...
    suggestWidgetVisibleContextKey = TerminalContextKeys.suggestWidgetVisible.bindTo(
      instantiationService.get(IContextKeyService)
    );
    suggestAddon = store.add(
      instantiationService.createInstance(
        SuggestAddon,
        new Set(parseCompletionsFromShell(testRawPwshCompletions)),
        shellIntegrationAddon.capabilities,
        suggestWidgetVisibleContextKey
      )
    );
  ...
  for (const testCase of recordedTestCases) {
    test(testCase.name, async () => {
      ...
      store.add(
        suggestAddon.onAcceptedCompletion(
          (e) => suggestDataEvents.push(e)
        )
      );
