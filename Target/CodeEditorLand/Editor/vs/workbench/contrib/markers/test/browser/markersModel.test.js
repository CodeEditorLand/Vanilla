...
suite("MarkersModel Test", () => {
  ...
  test("sort palces resources with no errors at the end", () => {
  ...
  test("sort resources by file path", () => {
  ...
  test("sort markers by severity, line and column", () => {
  ...
  test("toString()", () => {
    ...
    assert.strictEqual(
      JSON.stringify(
        { ...marker, resource: marker.resource.path },
        null,
        "	"
      ),
    ...
    testObject.relatedInformation = marker.relatedInformation.map(
  ...
  test("Markers for same-document but different fragment", () => {
    ...
    assert.ok(a === c);
    model.setResourceMarkers([
      [document, [{ ...aMarker(), resource: frag2 }]]
    ]);
    assert.strictEqual(model.total, 2);
  });
  test("Problems are no sorted correctly #99135", () => {
    ...
    model.setResourceMarkers([
      [
        ...
        [
          { ...aMarker(), resource: frag1 },
          {
            ...aMarker(void 0, MarkerSeverity.Warning),
            resource: frag1
          }
        ]
      ]
