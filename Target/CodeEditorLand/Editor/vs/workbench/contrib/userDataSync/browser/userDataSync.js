...
const CONTEXT_TURNING_ON_STATE = new RawContextKey(
...
let UserDataSyncWorkbenchContribution = class extends Disposable {
  ...
  globalActivityBadgeDisposable = this._register(
    new MutableDisposable()
  );
  accountBadgeDisposable = this._register(
    new MutableDisposable()
  );
  get turningOnSync() {
  ...
  onDidChangeConflicts(conflicts) {
    ...
    if (conflicts.length) {
      for (const [
        key,
        disposable
      ] of this.conflictsDisposables.entries()) {
            ...
            [
              {
                ...
                run: /* @__PURE__ */ __name(() => {
                  this.telemetryService.publicLog2("sync/handleConflicts", {
      ...
      this.conflictsDisposables.forEach(
        ...
        this.notificationService.notify({
          ...
          actions: {
            primary: [
              new Action(
                "reset",
                localize("reset", "Clear Data in Cloud..."),
                void 0,
                true,
      ...
      actions: {
        primary: [
          new Action(
            ...
            void 0,
            true,
            () => resource === SyncResource.Settings ? this.preferencesService.openUserSettings({
              jsonEditor: true
  ...
  onSynchronizerErrors(errors) {
    if (errors.length) {
      ...
      this.invalidContentErrorDisposables.forEach(
        (disposable) => disposable.dispose()
      );
      this.invalidContentErrorDisposables.clear();
    }
  }
  handleInvalidContentError({
    profile,
    syncResource: source
  }) {
      ...
      actions: {
        primary: [
          new Action(
            ...
            void 0,
            true,
            () => source === SyncResource.Settings ? this.preferencesService.openUserSettings({
              jsonEditor: true
            }) : this.preferencesService.openGlobalKeybindingSettings(
              true
            )
    ...
    this.invalidContentErrorDisposables.set(
      key,
      toDisposable(() => {
        handle.close();
        this.invalidContentErrorDisposables.delete(key);
  ...
  getConflictsCount() {
    return this.userDataSyncService.conflicts.reduce(
      (result, { conflicts }) => {
  ...
  async askToConfigure() {
    return new Promise((c, e) => {
      ...
      quickPick.selectedItems = items.filter(
        (item) => this.userDataSyncEnablementService.isResourceEnabled(item.id)
      );
      let accepted = false;
      disposables.add(
        Event.any(
  ...
  registerTurnOnSyncAction() {
    ...
    this._register(
      registerAction2(
        class TurningOnSyncAction extends Action2 {
          static {
  ...
  registerSignInAction() {
    ...
    this._register(
      registerAction2(
        class StopSyncAction extends Action2 {
          static {
  ...
  conflictsActionDisposable = this._register(
    new MutableDisposable()
  );
  registerShowConflictsAction() {
    this.conflictsActionDisposable.value = void 0;
    const that = this;
    this.conflictsActionDisposable.value = registerAction2(
      class TurningOnSyncAction extends Action2 {
        static {
          __name(this, "TurningOnSyncAction");
        }
  ...
  registerManageSyncAction() {
    ...
    this._register(
      registerAction2(
        class SyncStatusAction extends Action2 {
          static {
            ...
            return new Promise((c, e) => {
              ...
              if (that.userDataSyncEnablementService.canToggleEnablement()) {
                ...
                items.push({
                  ...
                  description: account ? `${account.accountName} (${that.authenticationService.getProvider(account.authenticationProviderId).label})` : void 0
                });
              }
              quickPick.items = items;
              disposables.add(
  ...
  registerSyncNowAction() {
    ...
    this._register(
      registerAction2(
        class SyncNowAction extends Action2 {
          static {
  ...
  registerConfigureSyncAction() {
    ...
    this._register(
      registerAction2(
        class ConfigureSyncAction extends Action2 {
          static {
  ...
  registerShowLogAction() {
    ...
    this._register(
      registerAction2(
        class ShowSyncActivityAction extends Action2 {
          static {
  ...
  registerShowSettingsAction() {
    this._register(
      registerAction2(
        class ShowSyncSettingsAction extends Action2 {
          static {
  ...
  registerAcceptMergesAction() {
    ...
    this._register(
      registerAction2(
        class AcceptMergesAction extends Action2 {
          static {
          ...
          async run(accessor, previewResource) {
            ...
            await that.userDataSyncService.accept(
          ...
          getSyncResource(previewResource) {
            const conflict = that.userDataSyncService.conflicts.find(
              ({ conflicts }) => conflicts.some(
                (conflict2) => isEqual(
                  conflict2.previewResource,
                  previewResource
                )
              )
            );
...
let UserDataRemoteContentProvider = class {
  ...
  provideTextContent(uri) {
    if (uri.scheme === USER_DATA_SYNC_SCHEME) {
      return this.userDataSyncService.resolveContent(uri).then(
        (content) => this.modelService.createModel(
          content || "",
