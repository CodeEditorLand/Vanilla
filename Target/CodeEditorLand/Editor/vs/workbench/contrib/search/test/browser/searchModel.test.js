...
suite("SearchModel", () => {
  ...
  setup(() => {
    ...
    instantiationService.stub(INotebookService, {
  ...
  test("Search Model: Search adds to results", async () => {
    ...
    instantiationService.stub(
      ISearchService,
      searchServiceWithResults(results, {
        limitHit: false,
        messages: [],
        results
      })
    ...
    instantiationService.stub(
      ...
      notebookSearchServiceWithInfo([], void 0)
    );
  ...
  test("Search Model: Search can return notebook results", async () => {
    ...
    const cellMatchMd = {
      cell: mdInputCell,
      index: 0,
      contentResults: contentMatchesToTextSearchMatches(
        findMatchMds,
        mdInputCell
      ),
      webviewResults: []
    ...
    const notebookSearchService = instantiationService.stub(
      INotebookSearchService,
      notebookSearchServiceWithInfo(
        [aRawMatchWithCells("/1", cellMatchMd, cellMatchCode)],
        void 0
      )
    );
    const notebookSearch = sinon.spy(
    ...
    assert.ok(
      notebookFileMatches[0].range().equalsRange(new Range(1, 3, 1, 7))
    );
    assert.ok(
      notebookFileMatches[1].range().equalsRange(new Range(1, 8, 1, 12))
    );
    assert.ok(
      notebookFileMatches[2].range().equalsRange(new Range(1, 14, 1, 18))
    );
    assert.ok(
      notebookFileMatches[3].range().equalsRange(new Range(1, 2, 1, 6))
    );
    assert.ok(
      notebookFileMatches[4].range().equalsRange(new Range(1, 8, 1, 12))
    );
    notebookFileMatches.forEach(
      (match) => match instanceof MatchInNotebook
    );
    assert(
      notebookFileMatches[0].cell?.id === "mdInputCell"
    );
    assert(
      notebookFileMatches[1].cell?.id === "codeCell"
    );
    assert(
      notebookFileMatches[2].cell?.id === "codeCell"
    );
    assert(
      notebookFileMatches[3].cell?.id === "codeCell"
    );
    assert(
      notebookFileMatches[4].cell?.id === "codeCell"
    );
    ...
    assert(
      mdCellMatchProcessed.contentMatches[0] === notebookFileMatches[0]
    );
    assert(
      codeCellMatchProcessed.contentMatches[0] === notebookFileMatches[1]
    );
    assert(
      codeCellMatchProcessed.contentMatches[1] === notebookFileMatches[2]
    );
    assert(
      codeCellMatchProcessed.webviewMatches[0] === notebookFileMatches[3]
    );
    assert(
      codeCellMatchProcessed.webviewMatches[1] === notebookFileMatches[4]
    );
    assert.strictEqual(
      URI.file(`${getRootName()}/2`).toString(),
      actual[1].resource.toString()
    );
    assert.strictEqual(
      URI.file(`${getRootName()}/3`).toString(),
      actual[2].resource.toString()
    );
  });
    ...
    instantiationService.stub(
      ...
      searchServiceWithResults(
        [
          aRawMatch(
            "/1",
            new TextSearchMatch("some preview", lineOneRange)
          )
        ],
  ...
  test("Search Model: Search reports timed telemetry on search when error is cancelled error", () => {
    ...
    instantiationService.stub(
      ISearchService,
      searchServiceWithDeferredPromise(deferredPromise.p)
    );
    instantiationService.stub(
      INotebookSearchService,
      notebookSearchServiceWithInfo([], void 0)
  ...
  test("Search Model: Previous search is cancelled when new search is called", async () => {
    ...
    instantiationService.stub(
      ISearchService,
      canceleableSearchService(tokenSource)
    );
    instantiationService.stub(
      INotebookSearchService,
      notebookSearchServiceWithInfo([], tokenSource)
    );
    const testObject = instantiationService.createInstance(SearchModel);
    store.add(testObject);
    testObject.search({
