...
suite("SearchResult", () => {
  ...
  setup(() => {
    ...
    instantiationService.stub(
      ...
      stubNotebookEditorService(instantiationService)
    );
  ...
  test("Line Match", () => {
    ...
    const lineMatch = new Match(
      fileMatch,
      ["0 foo bar"],
      new OneLineRange(0, 2, 5),
      new OneLineRange(1, 0, 5),
      false
    );
  ...
  test("File Match: Select an existing match", () => {
    ...
    assert.strictEqual(
      testObject.matches()[0],
      testObject.getSelectedMatch()
    );
  });
  test("File Match: Select non existing match", () => {
    const testObject = aFileMatch(
      "folder/file.txt",
      aSearchResult(),
      new TextSearchMatch("foo", new OneLineRange(1, 0, 3)),
  ...
  test("File Match: isSelected return true for selected match", () => {
    const testObject = aFileMatch(
      "folder/file.txt",
      aSearchResult(),
      new TextSearchMatch("foo", new OneLineRange(1, 0, 3)),
  ...
  test("File Match: isSelected return false for un-selected match", () => {
    const testObject = aFileMatch(
      "folder/file.txt",
      aSearchResult(),
      new TextSearchMatch("foo", new OneLineRange(1, 0, 3)),
  ...
  test("File Match: unselect when not selected", () => {
    const testObject = aFileMatch(
      "folder/file.txt",
      aSearchResult(),
      new TextSearchMatch("foo", new OneLineRange(1, 0, 3)),
  ...
  test("Match -> FileMatch -> SearchResult hierarchy exists", () => {
    const searchModel = instantiationService.createInstance(SearchModel);
    store.add(searchModel);
    const searchResult = instantiationService.createInstance(
      SearchResult,
      searchModel
    );
    ...
    const lineMatch = new Match(
      fileMatch,
      ["foo bar"],
      new OneLineRange(0, 0, 3),
      new OneLineRange(1, 0, 3),
      false
    );
    assert(lineMatch.parent() === fileMatch);
    assert(fileMatch.parent() === searchResult.folderMatches()[0]);
  });
  test("Adding a raw match will add a file match with line matches", () => {
    ...
    assert.strictEqual(
      URI.file(`${getRootName()}/1`).toString(),
      actual[0].resource.toString()
    );
  ...
  test("Adding multiple raw matches", () => {
    const testObject = aSearchResult();
    const target = [
      aRawMatch(
        "/1",
  ...
  test("Test that notebook matches get added correctly", () => {
    const testObject = aSearchResult();
    ...
    assert.deepStrictEqual(
      fileMatch1.cellResults[0].contentResults,
      addFileMatch.getCall(0).args[0][0].cellResults[0].contentResults
    );
    assert.deepStrictEqual(
      fileMatch1.cellResults[0].webviewResults,
      addFileMatch.getCall(0).args[0][0].cellResults[0].webviewResults
    );
    assert.deepStrictEqual(
      fileMatch2.cellResults[0].contentResults,
      addFileMatch.getCall(0).args[0][1].cellResults[0].contentResults
    );
    assert.deepStrictEqual(
      fileMatch2.cellResults[0].webviewResults,
      addFileMatch.getCall(0).args[0][1].cellResults[0].webviewResults
    );
  });
  test("Dispose disposes matches", () => {
  ...
  test("remove triggers change event", () => {
    ...
    assert.deepStrictEqual(
      [{ elements: [objectToRemove], removed: true }],
      target.args[0]
    );
  });
  test("remove array triggers change event", () => {
    const target = sinon.spy();
    const testObject = aSearchResult();
    addToSearchResult(testObject, [
      aRawMatch("/1", new TextSearchMatch("preview 1", lineOneRange)),
    ...
    assert.ok(target.calledOnce);
    assert.deepStrictEqual(
      [{ elements: arrayToRemove, removed: true }],
      target.args[0]
    );
  });
  test("Removing all line matches and adding back will add file back to result", () => {
    const testObject = aSearchResult();
    addToSearchResult(testObject, [
      aRawMatch("/1", new TextSearchMatch("preview 1", lineOneRange))
    ]);
  ...
  test("replace should remove the file match", () => {
  ...
  test("replace should trigger the change event", () => {
    const target = sinon.spy();
    const voidPromise = Promise.resolve(null);
    instantiationService.stub(IReplaceService, "replace", voidPromise);
    const testObject = aSearchResult();
  ...
  test("replaceAll should remove all file matches", () => {
    const voidPromise = Promise.resolve(null);
  ...
  test("batchRemove should trigger the onChange event correctly", () => {
    const target = sinon.spy();
    ...
    assert.ok(target.calledOnce);
    assert.deepStrictEqual(
      [{ elements: expectedArrayResult, removed: true, added: false }],
      target.args[0]
    );
  });
  test("batchReplace should trigger the onChange event correctly", async () => {
  ...
  test("Creating a model with nested folders should create the correct structure", () => {
    ...
    assert.deepStrictEqual(
      getFolderMatchAtIndex(root0, 0).allDownstreamFileMatches(),
      Array.from(getFolderMatchAtIndex(root0, 0).fileMatchesIterator())
    );
    assert.deepStrictEqual(
      getFileMatchAtIndex(getFolderMatchAtIndex(root0, 0), 0).parent(),
      getFolderMatchAtIndex(root0, 0)
    );
    assert.deepStrictEqual(getFolderMatchAtIndex(root0, 0).parent(), root0);
    assert.deepStrictEqual(
      getFolderMatchAtIndex(root0, 0).closestRoot,
      root0
    );
    ...
    assert.deepStrictEqual(
      getFileMatchAtIndex(getFolderMatchAtIndex(root1, 0), 0).parent(),
      getFolderMatchAtIndex(root1, 0)
    );
    ...
    assert.deepStrictEqual(
      root2DownstreamFiles,
      Array.from(root2.fileMatchesIterator())
    );
    assert.deepStrictEqual(getFileMatchAtIndex(root2, 0).parent(), root2);
    assert.deepStrictEqual(
      getFileMatchAtIndex(root2, 0).closestRoot,
      root2
    );
    const root3DownstreamFiles = root3.allDownstreamFileMatches();
    const root3Level3Folder = getFolderMatchAtIndex(
      getFolderMatchAtIndex(root3, 0),
      0
    );
    assert.deepStrictEqual(
      root3DownstreamFiles,
      [
        ...root3.fileMatchesIterator(),
        ...getFolderMatchAtIndex(
          root3Level3Folder,
          0
        ).fileMatchesIterator(),
        ...getFolderMatchAtIndex(
    ...
    assert.deepStrictEqual(
      root3Level3Folder.allDownstreamFileMatches(),
      getFolderMatchAtIndex(root3, 0).allDownstreamFileMatches()
    );
    assert.deepStrictEqual(
      getFileMatchAtIndex(
        getFolderMatchAtIndex(root3Level3Folder, 1),
        0
      ).parent(),
      getFolderMatchAtIndex(root3Level3Folder, 1)
    );
    assert.deepStrictEqual(
      getFolderMatchAtIndex(root3Level3Folder, 1).parent(),
      root3Level3Folder
    );
    assert.deepStrictEqual(
      root3Level3Folder.parent(),
      getFolderMatchAtIndex(root3, 0)
    );
  ...
  test("Removing an intermediate folder should call OnChange() on all downstream file matches", () => {
    const target = sinon.spy();
    const testObject = getPopulatedSearchResultForTreeTesting();
    const folderMatch = getFolderMatchAtIndex(
      getFolderMatchAtIndex(
        getFolderMatchAtIndex(testObject.folderMatches()[3], 0),
        0
      ),
      0
    );
  ...
  test("Replacing an intermediate folder should remove all downstream folders and file matches", async () => {
    const target = sinon.spy();
    const testObject = getPopulatedSearchResultForTreeTesting();
    const folderMatch = getFolderMatchAtIndex(
      testObject.folderMatches()[3],
      0
    );
    const expectedArrayResult = folderMatch.allDownstreamFileMatches();
    store.add(testObject.onChange(target));
  ...
  function stubNotebookEditorService(instantiationService2) {
    ...
    const notebookEditorWidgetService = instantiationService2.createInstance(
