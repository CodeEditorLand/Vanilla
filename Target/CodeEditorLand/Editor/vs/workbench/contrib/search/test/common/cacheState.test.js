...
suite("FileQueryCacheState", () => {
  ...
  test("reuse old cacheKey until new cache is loaded", async () => {
  ...
  test("do not spawn additional load if previous is still loading", async () => {
    const cache = new MockCache();
    const first = createCacheState(cache);
    const firstKey = first.cacheKey;
    first.load();
  ...
  test("do not use previous cacheKey if query changed", async () => {
    const cache = new MockCache();
    const first = createCacheState(cache);
    const firstKey = first.cacheKey;
    first.load();
  ...
  test("keep using old cacheKey when loading fails", async () => {
    const cache = new MockCache();
    const first = createCacheState(cache);
    const firstKey = first.cacheKey;
    first.load();
