...
function buildPicker(accessor, {
  ...
  if (onlyForTest !== void 0) {
    pushItems(
      profileService.getControllerProfiles(onlyForTest.controllerId).filter((p) => canUseProfileWithTest(p, onlyForTest))
    );
  } else {
    ...
    const pick = await new Promise(
      (resolve) => {
        disposables.add(
          quickpick.onDidAccept(
            () => resolve(
              quickpick.selectedItems[0]?.profile
            )
          )
        );
        disposables.add(quickpick.onDidHide(() => resolve(void 0)));
