...
suite("Workbench - Testing Explorer Hierarchal by Location Projection", () => {
  ...
  setup(() => {
    ...
    harness = ds.add(
      new TestTreeTestHarness(
        (l) => new TestHierarchicalByLocationProjection(
          {},
          l,
          resultsService
        )
  ...
  test("expands children", async () => {
    harness.flush();
    harness.tree.expand(
      harness.projection.getElementByTestId(
  ...
  test("applies state changes", async () => {
    ...
    resultsService.getStateById = () => [
      void 0,
      resultInState(TestResultState.Queued)
    ];
    onTestChanged.fire({
    ...
    resultsService.getStateById = () => [
      void 0,
      resultInState(TestResultState.Failed)
    ];
    onTestChanged.fire({
      reason: TestResultItemChangeReason.OwnStateChange,
      result: null,
