...
suite("Code Coverage Decorations", () => {
  ...
  const assertRanges = async (model) => await assertSnapshot(
    model.ranges.map((r) => ({
  ...
  test("CoverageDetailsModel#1", async () => {
    const details = [
      {
        location: new Range(1, 0, 5, 0),
        type: DetailType.Statement,
        count: 1
      },
      {
        location: new Range(2, 0, 3, 0),
        type: DetailType.Statement,
        count: 2
      },
      {
  ...
  test("CoverageDetailsModel#2", async () => {
    const details = [
      ...
      {
        location: new Range(2, 0, 4, 0),
        type: DetailType.Statement,
        count: 2
      },
      {
        location: new Range(3, 0, 3, 5),
        type: DetailType.Statement,
        count: 3
      }
    ];
