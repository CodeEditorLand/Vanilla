...
suite("Workbench - Test Result Storage", () => {
  let storage;
  let ds;
  const makeResult = /* @__PURE__ */ __name((taskName = "t") => {
  ...
  const assertStored = /* @__PURE__ */ __name(async (stored) => assert.deepStrictEqual((await storage.read()).map((r) => r.id), stored.map((s) => s.id)), "assertStored");
  setup(async () => {
