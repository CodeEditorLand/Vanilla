...
suite("Workbench - Test Results Service", () => {
  ...
  const getChangeSummary = () => [...changed].map((c) => ({
    reason: c.reason,
    label: c.item.item.label
  }));
  let r;
  ...
  setup(async () => {
    ...
    const ok = await Promise.race([
      Promise.resolve(
        tests.expand(tests.root.id, Number.POSITIVE_INFINITY)
      ).then(() => true),
    ...
    if (!ok) {
      throw new Error(
        "timed out while expanding, diff: " + JSON.stringify(tests.collectDiff())
      );
    }
  ...
  suite("LiveTestResult", () => {
    test("is empty if no tests are yet present", async () => {
      assert.deepStrictEqual(
        getLabelsIn(
          new TestLiveTestResult("foo", false, defaultOpts(["id-a"])).tests
        ),
        []
      );
    });
    ...
    test("setAllToState", () => {
      changed.clear();
      r.setAllToStatePublic(
        TestResultState.Queued,
        "t",
        (_, t) => t.item.label !== "root"
      );
      const c = makeEmptyCounts();
    ...
    test("updateState", () => {
      ...
      assert.deepStrictEqual(r.counts, c);
      assert.deepStrictEqual(
        r.getStateById(testId)?.ownComputedState,
        TestResultState.Running
      );
      assert.deepStrictEqual(
        r.getStateById(tests.root.id)?.computedState,
        TestResultState.Running
      );
      assert.deepStrictEqual(getChangeSummary(), [
        {
    ...
    test("ignores outside run", () => {
      ...
      assert.deepStrictEqual(r.counts, c);
      assert.deepStrictEqual(
        r.getStateById(new TestId(["ctrlId", "id-b"]).toString()),
        void 0
      );
    });
    test("markComplete", () => {
      ...
      assert.deepStrictEqual(r.counts, c);
      assert.deepStrictEqual(
        r.getStateById(tests.root.id)?.ownComputedState,
  ...
  suite("service", () => {
    ...
    const makeHydrated = async (completedAt = 42, state = TestResultState.Passed) => new HydratedTestResult(
      ...
      },
      {
        ...
        items: [
          {
            ...(await getInitializedMainTestCollection()).getNodeById(
              new TestId(["ctrlId", "id-a"]).toString()
            ),
  ...
  test("resultItemParents", () => {
    ...
    assert.deepStrictEqual(
      [...resultItemParents(r, r.getStateById(tests.root.id))],
  ...
  suite("output controller", () => {
    test("reads live output ranges", async () => {
      ...
      assert.deepStrictEqual(
        ctrl.getRange(0, 5),
        VSBuffer.fromString("12345")
      );
      assert.deepStrictEqual(
        ctrl.getRange(5, 5),
        VSBuffer.fromString("67890")
      );
      assert.deepStrictEqual(
        ctrl.getRange(7, 6),
        VSBuffer.fromString("890123")
      );
      assert.deepStrictEqual(
        ctrl.getRange(15, 5),
        VSBuffer.fromString("67890")
    ...
    test("corrects offsets for marked ranges", async () => {
      ...
      assert.deepStrictEqual(
        ...
        VSBuffer.fromString(
          "\x1B]633;SetMark;Id=s1;Hidden\x0712345\x1B]633;SetMark;Id=e1;Hidden\x07"
        )
      );
      assert.deepStrictEqual(
        ctrl.getRange(a2.offset, a2.length),
        VSBuffer.fromString(
          "\x1B]633;SetMark;Id=s1234;Hidden\x0767890\x1B]633;SetMark;Id=e1234;Hidden\x07"
        )
      );
      assert.deepStrictEqual(
        ctrl.getRange(a3.offset, a3.length),
        VSBuffer.fromString(
          "\x1B]633;SetMark;Id=s4;Hidden\x07with new line\x1B]633;SetMark;Id=e4;Hidden\x07\r\n"
        )
      );
    });
  });
