...
suite("Workbench - Test Results Service", () => {
  const getLabelsIn = /* @__PURE__ */ __name((it) => [...it].map((t) => t.item.label).sort(), "getLabelsIn");
  const getChangeSummary = /* @__PURE__ */ __name(() => [...changed].map((c) => ({ reason: c.reason, label: c.item.item.label })), "getChangeSummary");
  let r;
  let changed = /* @__PURE__ */ new Set();
  let tests;
  const defaultOpts = /* @__PURE__ */ __name((testIds) => ({
  ...
  class TestLiveTestResult extends LiveTestResult {
    static {
      __name(this, "TestLiveTestResult");
    }
  ...
  suite("service", () => {
    ...
    class TestTestResultService extends TestResultService {
      static {
        __name(this, "TestTestResultService");
      }
      persistScheduler = { schedule: /* @__PURE__ */ __name(() => this.persistImmediately(), "schedule") };
    }
    setup(() => {
    ...
    const makeHydrated = /* @__PURE__ */ __name(async (completedAt = 42, state = TestResultState.Passed) => new HydratedTestResult({
      asCanonicalUri(uri) {
        return uri;
      }
    }, {
