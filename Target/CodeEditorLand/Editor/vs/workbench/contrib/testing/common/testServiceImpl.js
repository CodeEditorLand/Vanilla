...
let TestService = class extends Disposable {
   ...
   * @inheritdoc
   */
  collection = new MainThreadTestCollection(this.uriIdentityService, this.expandTest.bind(this));
  /**
   * @inheritdoc
   */
  excluded;
   ...
   * @inheritdoc
   */
  showInlineOutput = this._register(MutableObservableValue.stored(new StoredValue({
  ...
  async runTests(req, token = CancellationToken.None) {
    ...
    for (const test of req.tests) {
      ...
      const allProfiles = this.testProfiles.getControllerProfiles(test.controllerId).filter((p) => (p.group & req.group) !== 0 && canUseProfileWithTest(p, test));
