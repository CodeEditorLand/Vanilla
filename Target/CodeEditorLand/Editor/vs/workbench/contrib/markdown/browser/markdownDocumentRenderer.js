...
async function renderMarkdownDocument(text, extensionService, languageService, options) {
  const m = new marked.Marked(
    ...
    MarkedGfmHeadings.gfmHeadingId(),
    ...
    return {
      ...
      renderer: {
        heading({ tokens, depth }) {
          ...
          const raw = unescape(this.parser.parseInline(tokens, this.parser.textRenderer)).trim().replace(/<[!\/a-z].*?>/gi, "");
          const level = depth;
...
`;
  ...
  MarkedGfmHeadings2.gfmHeadingId = gfmHeadingId;
  __name(gfmHeadingId, "gfmHeadingId");
})(MarkedGfmHeadings || (MarkedGfmHeadings = {}));
export {
