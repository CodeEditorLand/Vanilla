...
suite("Markdown Setting Renderer Test", () => {
  ...
  suiteSetup(() => {
    ...
    settingRenderer = new SimpleSettingRenderer(
  ...
  suiteTeardown(() => {
    Registry.as(
      Extensions.Configuration
    ).deregisterConfigurations([configuration]);
  });
  ...
  test("actions with no value", () => {
    const uri = URI.parse(
      settingRenderer.settingToUriString("example.booleanSetting")
    );
    const actions = settingRenderer.getActions(uri);
    assert.strictEqual(actions?.length, 2);
    assert.strictEqual(
      actions[0].label,
      'View "Example: Boolean Setting" in Settings'
    );
  });
  test("actions with value + updating and restoring", async () => {
    await configurationService.setUserConfiguration("example", {
      stringSetting: "two"
    });
    const uri = URI.parse(
      settingRenderer.settingToUriString(
        "example.stringSetting",
        "three"
      )
    );
    const verifyOriginalState = (actions2) => {
      ...
      assert.strictEqual(
        actions2[0].label,
        'Set "Example: String Setting" to "three"'
      );
      assert.strictEqual(actions2[1].label, "View in Settings");
      assert.strictEqual(
        configurationService.getValue("example.stringSetting"),
        "two"
      );
      return true;
    };
    ...
    if (verifyOriginalState(actions)) {
      ...
      assert.strictEqual(
        actionsUpdated[0].label,
        'Restore value of "Example: String Setting"'
      );
