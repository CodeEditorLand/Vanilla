...
let ProfileElementRenderer = class {
  ...
  renderTemplate(container) {
    ...
    append(description, $(`span${ThemeIcon.asCSSSelector(Codicon.check)}`), $("span", void 0, localize("activeProfile", "In use")));
...
let ContentsProfileRenderer = class extends ProfilePropertyRenderer {
  ...
  renderTemplate(parent) {
    ...
    const updateDescription = /* @__PURE__ */ __name((element) => {
      ...
      const markdown = new MarkdownString().appendMarkdown(localize("contents source description", "Configure source of contents for this profile\n"));
      ...
      if (element.root instanceof NewProfileElement) {
        ...
        markdown.appendMarkdown(defaultHelpInfo).appendMarkdown(localize("none info", "- *None:* Create empty contents\n"));
      } else if (element.root instanceof UserDataProfileElement) {
        markdown.appendMarkdown(defaultHelpInfo).appendMarkdown(localize("current info", "- *{1}:* Use contents from the {0} profile\n", element.root.profile.name, element.root.profile.name));
      }
