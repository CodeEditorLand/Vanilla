...
registerAction2(class extends Action2 {
  ...
  run(accessor, walkthroughID, toSide) {
    ...
    if (walkthroughID) {
      ...
      const result = editorService.findEditors({ typeId: GettingStartedInput.ID, editorId: void 0, resource: GettingStartedInput.RESOURCE });
      ...
      if (activeEditor instanceof ExtensionsInput) {
        ...
        activeGroup.replaceEditors([{
          editor: activeEditor,
          replacement: instantiationService.createInstance(GettingStartedInput, { selectedCategory, selectedStep })
        }]);
      } else {
        const options = { selectedCategory, selectedStep, preserveFocus: toSide ?? false };
        editorService.openEditor({
          resource: GettingStartedInput.RESOURCE,
          options
        }, toSide ? SIDE_GROUP : void 0).then((editor) => {
