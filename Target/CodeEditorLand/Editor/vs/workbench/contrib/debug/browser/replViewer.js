...
class ReplEvaluationInputsRenderer {
  ...
  renderTemplate(container) {
    dom.append(
      container,
      $(
        "span.arrow" + ThemeIcon.asCSSSelector(debugConsoleEvaluationInput)
      )
    );
...
let ReplOutputElementRenderer = class {
  ...
  renderElement({ element }, index, templateData) {
    ...
    const detector = locationReference !== void 0 ? this.linkDetector.makeReferencedLinkDetector(
      locationReference,
      element.session
    ) : this.linkDetector;
    templateData.value.appendChild(
      handleANSIOutput(
        element.value,
        detector,
        this.themeService,
    ...
    templateData.value.classList.add(
      element.severity === severity.Warning ? "warn" : element.severity === severity.Error ? "error" : element.severity === severity.Ignore ? "ignore" : "info"
    );
...
let ReplVariablesRenderer = class extends AbstractExpressionsRenderer {
  ...
  renderElement(node, _index, data) {
    ...
    super.renderExpressionElement(
      element instanceof ReplVariableElement ? element.expression : element,
      node,
      data
    );
  }
  renderExpression(expression, data, highlights) {
    ...
    if (isReplVariable || !expression.name) {
      ...
      renderExpressionValue(
        ...
        isReplVariable ? expression.expression : expression,
        data.value,
        {
          colorize: true,
          linkDetector: this.linkDetector,
...
class ReplDelegate extends CachedListVirtualDelegate {
  ...
  estimateHeight(element, ignoreValueLength = false) {
    ...
    if (hasValue(element) && !isNestedVariable(element)) {
      ...
      const valueRows = countNumberOfLines(value) + (ignoreValueLength ? 0 : Math.floor(value.length / 70)) + // Make an estimate for wrapping
...
let SourceWidget = class extends Disposable {
  ...
  setSource(source) {
    ...
    this.hover.update(
      source ? `${this.labelService.getUriLabel(source.source.uri)}:${source.lineNumber}` : ""
    );
  }
};
