...
let DebugService = class {
    ...
    this.disposables.add(
      this.model.onDidChangeCallStack(() => {
        ...
        if (numberOfSessions > 0) {
          const viewContainer = this.viewDescriptorService.getViewContainerByViewId(
            CALLSTACK_VIEW_ID
          );
          if (viewContainer) {
            this.activity = this.activityService.showViewContainerActivity(
              viewContainer.id,
              {
  ...
  initContextKeys(contextKeyService) {
    queueMicrotask(() => {
      ...
      const setBreakpointsExistContext = /* @__PURE__ */ __name(() => this.breakpointsExist.set(
        !!(this.model.getBreakpoints().length || this.model.getDataBreakpoints().length || this.model.getFunctionBreakpoints().length)
  ...
  async startDebugging(launch, configOrName, options, saveBeforeStart = !options?.parentSession) {
    ...
    try {
          ...
          compound.configurations.map((configData) => {
            ...
            if (typeof configData === "string") {
              ...
              if (launchesContainingName.length === 1) {
                ...
                throw new Error(
                  launchesContainingName.length === 0 ? nls.localize(
              ...
              const launchesMatchingConfigData = this.configurationManager.getLaunches().filter(
                (l) => l.workspace && l.workspace.name === configData.folder && !!l.getConfiguration(
                  configData.name
                )
              );
      ...
      if (configOrName && !config) {
        const message2 = !!launch ? nls.localize(
          "configMissing",
  ...
  async createSession(launch, config, options) {
    ...
    const configByProviders = await this.configurationManager.resolveConfigurationByProviders(
      launch && launch.workspace ? launch.workspace.uri : void 0,
      type,
      config,
    ...
    if (configByProviders && configByProviders.type) {
      try {
        ...
        const cfg = await this.configurationManager.resolveDebugConfigurationWithSubstitutedVariables(
          launch && launch.workspace ? launch.workspace.uri : void 0,
          resolvedConfig.type,
          resolvedConfig,
          initCancellationToken.token
        ...
        if (!dbg || configByProviders.request !== "attach" && configByProviders.request !== "launch") {
          ...
          actionList.push(
            new Action(
              ...
              void 0,
              true,
              async () => this.commandService.executeCommand(
  ...
  async doCreateSession(sessionId, root, configuration, options) {
    ...
    const openDebug = this.configurationService.getValue(
      "debug"
    ).openDebug;
  ...
  async launchOrAttachToSession(session, forceFocus = false) {
    ...
    try {
      ...
      const launchJsonExists = !!session.root && !!this.configurationService.getValue("launch", {
    ...
    listenerDisposables.add(
      session.onDidEndAdapter(async (adapterExitEvent) => {
        ...
        if (focusedSession && focusedSession.getId() === session.getId()) {
          const { session: session2, thread, stackFrame } = getStackFrameThreadAndSessionToFocus(
            this.model,
            void 0,
            void 0,
            void 0,
        ...
        if (this.model.getSessions().length === 0) {
          ...
          if (this.layoutService.isVisible(Parts.SIDEBAR_PART) && this.configurationService.getValue(
            "debug"
          ).openExplorerOnEnd) {
          ...
          dataBreakpoints.forEach(
            (dbp) => this.model.removeDataBreakpoints(dbp.getId())
          );
          if (this.configurationService.getValue(
            "debug"
          ).console.closeOnEnd) {
            const debugConsoleContainer = this.viewDescriptorService.getViewContainerByViewId(
              REPL_VIEW_ID
            );
            if (debugConsoleContainer && this.viewsService.isViewContainerVisible(
  ...
  async restartSession(session, restartData) {
    ...
    if (launch && needsToSubstitute && unresolved) {
      ...
      const resolvedByProviders = await this.configurationManager.resolveConfigurationByProviders(
        launch.workspace ? launch.workspace.uri : void 0,
        unresolved.type,
        unresolved,
        initCancellationToken.token
      );
      if (resolvedByProviders) {
        ...
        if (resolved && !initCancellationToken.token.isCancellationRequested) {
          resolved = await this.configurationManager.resolveDebugConfigurationWithSubstitutedVariables(
            launch && launch.workspace ? launch.workspace.uri : void 0,
            resolved.type,
            resolved,
            initCancellationToken.token
  ...
  async stopSession(session, disconnect = false, suspend = false) {
    ...
    return Promise.all(
      sessions.map(
        (s) => disconnect ? s.disconnect(void 0, suspend) : s.terminate()
      )
    );
  }
  ...
  async showError(message, errorActions = [], promptLaunchJson = true) {
    ...
    const actions = errorActions.filter((action) => action.id.endsWith(".command")).length > 0 ? errorActions : [
      ...errorActions,
  ...
  async focusStackFrame(_stackFrame, _thread, _session, options) {
    const { stackFrame, thread, session } = getStackFrameThreadAndSessionToFocus(
      this.model,
      _stackFrame,
      _thread,
      _session
  ...
  async removeBreakpoints(id) {
    ...
    this.unlinkTriggeredBreakpoints(breakpoints, toRemove).forEach(
      (uri) => urisToClear.add(uri.toString())
    );
    this.debugStorage.storeBreakpoints(this.model);
    await Promise.all(
      ...
      for (const existing of allBreakpoints) {
        if (!removedBreakpoints.includes(existing) && existing.triggeredBy === removed.getId()) {
          this.model.updateBreakpoints(
            /* @__PURE__ */ new Map([
              [existing.getId(), { triggeredBy: void 0 }]
            ])
          );
  ...
  async makeTriggeredBreakpointsMatchEnablement(enable, breakpoint) {
    ...
    await Promise.all(
      this.model.getBreakpoints().filter(
        (bp) => bp.triggeredBy === breakpoint.getId() && bp.enabled !== enable
    ...
    await sendToOneOrAllSessions(this.model, session, async (s) => {
      if (!s.configuration.noDebug) {
        const sessionBps = breakpointsToSend.filter(
          (bp) => !bp.triggeredBy || bp.getSessionDidTrigger(s.getId())
        );
  ...
  async sendFunctionBreakpoints(session) {
    const breakpointsToSend = this.model.getFunctionBreakpoints().filter(
  ...
  async sendDataBreakpoints(session) {
    const breakpointsToSend = this.model.getDataBreakpoints().filter(
      (fbp) => fbp.enabled && this.model.areBreakpointsActivated()
    );
    await sendToOneOrAllSessions(this.model, session, async (s) => {
  ...
  async sendInstructionBreakpoints(session) {
    const breakpointsToSend = this.model.getInstructionBreakpoints().filter(
      (fbp) => fbp.enabled && this.model.areBreakpointsActivated()
    );
    await sendToOneOrAllSessions(this.model, session, async (s) => {
  ...
  onFileChanges(fileChangesEvent) {
    const toRemove = this.model.getBreakpoints().filter(
      (bp) => fileChangesEvent.contains(
        bp.originalUri,
    ...
    for (const sessionId of breakpoint.sessionsThatVerified) {
      ...
      if (bestScore < 2 /* VerifiedAndPausedInFile */) {
        const pausedInThisFile = threads.find((t) => {
          ...
          return top && this.uriIdentityService.extUri.isEqual(
            top.source.uri,
            uri
          );
        });
...
function getStackFrameThreadAndSessionToFocus(model, stackFrame, thread, session, avoidSession) {
  if (!session) {
    if (stackFrame || thread) {
      ...
      session = stoppedSession || sessions.find(
        (s) => s !== avoidSession && s !== avoidSession?.parentSession
      ) || (sessions.length ? sessions[0] : void 0);
    }
  }
  if (!thread) {
    if (stackFrame) {
