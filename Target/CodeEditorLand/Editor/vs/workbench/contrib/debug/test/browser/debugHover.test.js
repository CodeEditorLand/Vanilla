...
suite("Debug - Hover", () => {
  ...
  test("find expression in stack frame", async () => {
    ...
    assert.strictEqual(
      await findExpressionInStackFrame(stackFrame, []),
      void 0
    );
    assert.strictEqual(
      await findExpressionInStackFrame(stackFrame, ["A"]),
      variableA
    );
    assert.strictEqual(
      await findExpressionInStackFrame(stackFrame, [
    ...
    assert.strictEqual(
      await findExpressionInStackFrame(stackFrame, ["B"]),
      void 0
    );
    assert.strictEqual(
      await findExpressionInStackFrame(stackFrame, ["A", "B"]),
      variableB
    );
    assert.strictEqual(
      await findExpressionInStackFrame(stackFrame, ["A", "C"]),
      void 0
    );
