...
suite("Debug - Source", () => {
  ...
  test("from raw internal source", () => {
    ...
    assert.strictEqual(
      source.uri.toString(),
      "debug:internalModule.js?session%3DaDebugSessionId%26ref%3D11"
    );
  });
  test("get encoded debug data", () => {
    ...
    checkData(
      uri.from({
        scheme: "file",
        path: "/my/path/test.js",
        query: "ref=1&session=2"
      }),
      "test.js",
      isWindows ? "\\my\\path\\test.js" : "/my/path/test.js",
      void 0,
      void 0
    );
    checkData(
    ...
    checkData(
      uri.from({
        scheme: "debug",
        path: "a/b/c/d/foo.txt",
        query: "session=100&ref=10"
      }),
      "foo.txt",
      "a/b/c/d/foo.txt",
      10,
