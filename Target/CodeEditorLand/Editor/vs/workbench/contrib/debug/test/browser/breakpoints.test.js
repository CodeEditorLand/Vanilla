...
import {
  ...
  getExpandedBodySize
} from "../../browser/breakpointsView.js";
import {
...
suite("Debug - Breakpoints", () => {
  ...
  test("two files", () => {
    ...
    assert.strictEqual(
      model.getBreakpoints({ enabledOnly: true }).length,
      3
    );
  ...
  test("exception breakpoints", () => {
    ...
    assert.strictEqual(
      model.getExceptionBreakpointsForSession("session-id-3").length,
      1
    );
  ...
  test("exception breakpoints multiple sessions", () => {
    ...
    assert.strictEqual(
      exceptionBreakpointsForSession[0].filter,
      "uncaught"
    );
    ...
    assert.strictEqual(
      exceptionBreakpointsForUndefined[0].filter,
      "uncaught"
    );
    assert.strictEqual(
      exceptionBreakpointsForUndefined[1].filter,
      "caught"
    );
  ...
  test("instruction breakpoints", () => {
    ...
    assert.strictEqual(
      instructionBreakpoints[0].instructionReference,
      "0xCCCCFFFF"
    );
    ...
    assert.strictEqual(instructionBreakpoints[0].offset, 0);
    assert.strictEqual(
      instructionBreakpoints[1].instructionReference,
      "0xCCCCEEEE"
    );
  ...
  test("message and class name", () => {
    ...
    let result = getBreakpointMessageAndIcon(
      State.Stopped,
      true,
      breakpoints[0],
      ls,
    ...
    result = getBreakpointMessageAndIcon(
      State.Stopped,
      true,
      breakpoints[1],
      ls,
    ...
    const functionBreakpoint = model.addFunctionBreakpoint(
      { name: "foo" },
      "1"
    );
    result = getBreakpointMessageAndIcon(
  ...
  test("decorations", () => {
    ...
    let decorations = instantiationService.invokeFunction(
    ...
    assert.strictEqual(
      decorations[0].options.beforeContentClassName,
      void 0
    );
    assert.strictEqual(
    ...
    assert.strictEqual(
      decorations[0].options.overviewRuler?.position,
      OverviewRulerLane.Left
    );
    const expected = new MarkdownString(void 0, {
      isTrusted: true,
      supportThemeIcons: true
    }).appendCodeblock(languageId, "Condition: x > 5");
    assert.deepStrictEqual(
      decorations[0].options.glyphMarginHoverMessage,
      expected
    );
    decorations = instantiationService.invokeFunction(
      (accessor) => createBreakpointDecorations(
        accessor,
        textModel,
        breakpoints,
