...
suite("Files - View Model", () => {
  ...
  test("Properties", function() {
    ...
    assert.strictEqual(
      s.resource.fsPath,
      toResource.call(this, "/path/to/stat").fsPath
    );
  ...
  test("Add and Remove Child, check for hasChild", function() {
    ...
    assert.strictEqual(
      ...
      toResource.call(this, "/path/to/stat/" + child4.name).fsPath
    );
  });
  test("Move", function() {
    const d = (/* @__PURE__ */ new Date()).getTime();
    ...
    assert.strictEqual(
      s4.resource.fsPath,
      toResource.call(this, "/" + s4.name).fsPath
    );
    const leaf = createStat.call(
      this,
      "/leaf",
      "leaf",
      true,
  ...
  test("Rename", function() {
    ...
    const s2renamed = createStat.call(
      this,
      "/otherpath",
      "otherpath",
      true,
    ...
    assert.strictEqual(
      s3.resource.fsPath,
      toResource.call(this, "/otherpath/to").fsPath
    );
    assert.strictEqual(
      s4.resource.fsPath,
      toResource.call(this, "/otherpath/to/stat").fsPath
    );
    const s4renamed = createStat.call(
      this,
  ...
  test("Find with mixed case", function() {
    ...
    if (isLinux) {
      ...
      assert.ok(
        !s1.find(toResource.call(this, "/Path/to/stat/foo/bar.html"))
      );
    } else {
      assert.ok(s1.find(toResource.call(this, "/path/to/stat/Foo")));
      assert.ok(
  ...
  test("Validate File Name (For Create)", function() {
    ...
    assert(validateFileName(pathService, s, "  ", OS) !== null);
    assert(
      validateFileName(pathService, s, "Read Me", OS) === null,
  ...
  test("Validate Multi-Path File Names", function() {
    const d = (/* @__PURE__ */ new Date()).getTime();
    const wsFolder = createStat.call(
      this,
      "/",
      "workspaceFolder",
      true,
    ...
    assert(
      validateFileName(pathService, wsFolder, "foo\\bar", OS) === null
    );
    assert(
      validateFileName(
        pathService,
    ...
    assert(
      validateFileName(
        pathService,
        wsFolder,
        "theres/one/different\\slash",
        OS
      ) === null
    );
    assert(
      validateFileName(pathService, wsFolder, "/slashAtBeginning", OS) !== null
    );
    const s1 = createStat.call(this, "/path", "path", true, false, 8096, d);
    const s2 = createStat.call(
      this,
      "/path/to",
      "to",
      true,
    ...
    s1.addChild(s2);
    s2.addChild(s3);
    const fileDeeplyNested = createStat.call(
      this,
      "/path/to/stat/fileNested",
      "fileNested",
      false,
      false,
      8096,
    ...
    assert(
      validateFileName(
        pathService,
        wsFolder,
        "/path/to/stat/fileNested/aChild",
        OS
      ) !== null
    );
    assert(
    ...
    assert(
      validateFileName(pathService, wsFolder, "/path/to/stat/", OS) !== null
    );
  });
  test("Merge Local with Disk", () => {
    ...
    assert.deepStrictEqual(
      merge1.getChild("foo.html").parent,
      merge1,
      "Check parent"
    );
