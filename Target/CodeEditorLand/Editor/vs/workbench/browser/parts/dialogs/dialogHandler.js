...
let BrowserDialogHandler = class extends AbstractDialogHandler {
  ...
  async prompt(prompt) {
    ...
    const { button, checkboxChecked } = await this.doShow(
      ...
      prompt.checkbox,
      void 0,
      typeof prompt?.custom === "object" ? prompt.custom : void 0
    );
  ...
  async confirm(confirmation) {
    ...
    const { button, checkboxChecked } = await this.doShow(
      ...
      void 0,
      typeof confirmation?.custom === "object" ? confirmation.custom : void 0
    );
  ...
  async input(input) {
    ...
    const { button, checkboxChecked, values } = await this.doShow(
      ...
      input?.checkbox,
      input.inputs,
      typeof input.custom === "object" ? input.custom : void 0
    );
  ...
  async doShow(type, message, buttons, detail, cancelId, checkbox, inputs, customOptions) {
    ...
    const renderBody = customOptions ? (parent) => {
      ...
      customOptions.markdownDetails?.forEach((markdownDetail) => {
        const result2 = this.markdownRenderer.render(
          markdownDetail.markdown
        );
        parent.appendChild(result2.element);
        result2.element.classList.add(
      ...
      buttons,
      {
        ...
        keyEventProcessor: (event) => {
          ...
          if (resolved.kind === ResultKind.KbFound && resolved.commandId) {
            if (BrowserDialogHandler.ALLOWABLE_COMMANDS.indexOf(
              resolved.commandId
            ) === -1) {
