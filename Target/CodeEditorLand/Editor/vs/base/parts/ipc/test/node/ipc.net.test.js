...
suite("IPC, Socket Protocol", () => {
  ...
  test("issue #211462: destroy socket after end timeout", async () => {
    ...
    Object.assign(socket, { destroy: /* @__PURE__ */ __name(() => socket.emit("close"), "destroy") });
...
suite("PersistentProtocol reconnection", () => {
  ...
  test("ack gets sent after a while", async () => {
    await runWithFakedTimers({ useFakeTimers: true, maxTaskCount: 100 }, async () => {
      const loadEstimator = {
        hasHighLoad: /* @__PURE__ */ __name(() => false, "hasHighLoad")
      };
      const ether = new Ether();
...
flakySuite("IPC, create handle", () => {
  ...
  __name(testIPCHandle, "testIPCHandle");
});
suite("WebSocketNodeSocket", () => {
  ...
  __name(fromCharCodeArray, "fromCharCodeArray");
  class FakeNodeSocket extends Disposable {
    static {
      __name(this, "FakeNodeSocket");
    }
    _onData = new Emitter();
  ...
  __name(testReading, "testReading");
  ...
  __name(generateRandomBuffer, "generateRandomBuffer");
  ...
  __name(listenOnRandomPort, "listenOnRandomPort");
});
