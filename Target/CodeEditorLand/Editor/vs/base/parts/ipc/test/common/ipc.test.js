...
class TestService {
  ...
  neverCompleteCT(cancellationToken) {
    ...
    return new Promise(
      (_, e) => this.disposables.add(
        cancellationToken.onCancellationRequested(() => e(canceled()))
      )
    );
  }
  buffersLength(buffers) {
    return Promise.resolve(
      buffers.reduce((r, b) => r + b.buffer.length, 0)
    );
  }
  ping(msg) {
...
suite("Base IPC", () => {
  ...
  suite("one to one", () => {
    ...
    setup(() => {
      ...
      ipcService = new TestChannelClient(
        client.getChannel(TestChannelId)
      );
    });
    ...
    test("buffers in arrays", async () => {
      const r = await ipcService.buffersLength([
        VSBuffer.alloc(2),
        VSBuffer.alloc(3)
      ]);
    ...
    test("round trips numbers", () => {
      ...
      assert.deepStrictEqual(
        deserialize(new BufferReader(writer.buffer)),
        input
      );
    });
  });
  suite("one to one (proxy)", () => {
    ...
    setup(() => {
      ...
      ipcService = ProxyChannel.toService(
        client.getChannel(TestChannelId)
      );
    });
    teardown(() => {
  ...
  suite("one to many", () => {
    test("all clients get pinged", async () => {
      ...
      const ipcService1 = new TestChannelClient(
        client1.getChannel("channel")
      );
      ...
      const ipcService2 = new TestChannelClient(
        client2.getChannel("channel")
      );
