...
flakySuite("Storage Library", () => {
  ...
  test("objects", () => {
    return runWithFakedTimers({}, async () => {
      const storage = new Storage(
        new SQLiteStorageDatabase(join(testDir, "storage.db"))
      );
      await storage.init();
  ...
  test("basics", () => {
    return runWithFakedTimers({}, async () => {
      ...
      deepStrictEqual(storage.getObject("foo", { bar: "baz" }), {
        bar: "baz"
      });
      ...
      await Promise.all([
        ...
        delete3Promise,
        delete4Promise
      ]).then(() => deletePromiseResolved = true);
  ...
  test("external changes", () => {
    return runWithFakedTimers({}, async () => {
      ...
      changes.clear();
      const deleted = /* @__PURE__ */ new Set(["foo"]);
      database.fireDidChangeItemsExternal({ deleted });
      ok(changes.has("foo"));
      strictEqual(storage.get("foo", void 0), void 0);
      changes.clear();
      database.fireDidChangeItemsExternal({ deleted });
      strictEqual(changes.size, 0);
      ...
      strictEqual(isStorageItemsChangeEvent(void 0), false);
      strictEqual(
        isStorageItemsChangeEvent({ changed: "yes", deleted: false }),
        false
      );
      await storage.close();
    });
  ...
  test("close flushes data", async () => {
    ...
    strictEqual(storage.get("bar"), "foo");
    let setPromiseResolved = false;
    Promise.all([set1Promise, set2Promise]).then(
    ...
    ok(!storage.get("bar"));
    let deletePromiseResolved = false;
    Promise.all([delete1Promise, delete2Promise]).then(
      () => deletePromiseResolved = true
    );
    await storage.close();
    strictEqual(deletePromiseResolved, true);
    storage = new Storage(
  ...
  test("conflicting updates", () => {
    return runWithFakedTimers({}, async () => {
      ...
      ok(changes.has("foo"));
      let setPromiseResolved = false;
      await Promise.all([set1Promise, set2Promise, set3Promise]).then(
        () => setPromiseResolved = true
      );
      ...
      await Promise.all([set4Promise, delete1Promise]).then(
        () => setAndDeletePromiseResolved = true
      );
...
flakySuite("SQLite Storage Library", () => {
  ...
  async function testDBBasics(path, logError) {
    ...
    strictEqual(
      storedItems.get(JSON.stringify({ foo: "bar" })),
      JSON.stringify({ bar: "foo" })
    );
    ...
    const itemsChange = /* @__PURE__ */ new Map();
    ...
    await storage.updateItems({
      insert: items,
      delete: toSet(["foo", "some/foo/path", "other"])
    });
    storedItems = await storage.getItems();
    ...
    await storage.updateItems({
      delete: toSet([JSON.stringify({ foo: "bar" })])
    });
    storedItems = await storage.getItems();
    strictEqual(storedItems.size, 0);
  ...
  (isWindows ? test.skip : test)(
    "basics (DB that becomes corrupt during runtime stores all state from cache on close)",
    async () => {
      const storagePath = join(testdir, "storage.db");
      let storage = new SQLiteStorageDatabase(storagePath);
      const items = /* @__PURE__ */ new Map();
      ...
      await Promises.writeFile(storagePath, "This is now a broken DB");
      await storage.checkIntegrity(true).then(
        null,
        (error) => {
        }
      );
  ...
  test("real world example", async () => {
    ...
    const items3 = /* @__PURE__ */ new Map();
    items3.set("nps/iscandidate", "false");
    items3.set(
      "telemetry.instanceid",
      "d52bfcd4-4be6-476b-a38f-d44c717c41d6"
    );
    items3.set(
      "workbench.activity.pinnedviewlets",
      '[{"id":"workbench.view.explorer","pinned":true,"order":0,"visible":true},{"id":"workbench.view.search","pinned":true,"order":1,"visible":true},{"id":"workbench.view.scm","pinned":true,"order":2,"visible":true},{"id":"workbench.view.debug","pinned":true,"order":3,"visible":true},{"id":"workbench.view.extensions","pinned":true,"order":4,"visible":true},{"id":"workbench.view.extension.gitlens","pinned":true,"order":7,"visible":true},{"id":"workbench.view.extension.test","pinned":false,"visible":false}]'
    );
    items3.set("workbench.panel.height", "419");
    items3.set(
      "very.long.key.very.long.key.very.long.key.very.long.key.very.long.key.very.long.key.very.long.key.very.long.key.very.long.key.very.long.key.very.long.key.very.long.key.very.long.key.very.long.key.very.long.key.very.long.key.very.long.key.very.long.key.very.long.key.very.long.key.very.long.key.very.long.key.very.long.key.very.long.key.very.long.key.very.long.key.very.long.key.very.long.key.very.long.key.very.long.key.very.long.key.very.long.key.very.long.key.very.long.key.very.long.key.",
      "is long"
    );
    let storedItems = await storage.getItems();
    strictEqual(storedItems.size, 0);
  ...
  test("very large item value", async () => {
    ...
    let randomData = createLargeRandomData();
    await storage.updateItems({ insert: randomData.items });
    let storedItems = await storage.getItems();
    strictEqual(
      randomData.items.get("colorthemedata"),
      storedItems.get("colorthemedata")
    );
    strictEqual(
      randomData.items.get("commandpalette.mru.cache"),
      storedItems.get("commandpalette.mru.cache")
    );
    strictEqual(
      randomData.items.get("super.large.string"),
      storedItems.get("super.large.string")
    );
  ...
  test("invalid path does not hang", async () => {
    const storage = new SQLiteStorageDatabase(
      join(testdir, "nonexist", "storage.db")
    );
    let error;
    try {
      await storage.getItems();
  ...
  test("optimize", async () => {
    ...
    strictEqual(
      sizeAfterDeleteAndOptimize < sizeBeforeDeleteAndOptimize,
      true
    );
  });
  ...
  function createLargeRandomData() {
    ...
    items.set("super.large.string", value.join());
