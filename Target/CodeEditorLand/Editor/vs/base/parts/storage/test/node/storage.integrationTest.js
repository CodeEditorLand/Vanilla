  ...
  test("external changes", () => {
    return runWithFakedTimers({}, async function() {
      class TestSQLiteStorageDatabase extends SQLiteStorageDatabase {
        static {
          __name(this, "TestSQLiteStorageDatabase");
        }
...
flakySuite("SQLite Storage Library", function() {
  ...
  __name(testDBBasics, "testDBBasics");
  test("basics", async () => {
  ...
  (isWindows ? test.skip : test)("basics (DB that becomes corrupt during runtime stores all state from cache on close)", async () => {
    const storagePath = join(testdir, "storage.db");
    let storage = new SQLiteStorageDatabase(storagePath);
    const items = /* @__PURE__ */ new Map();
    items.set("foo", "bar");
    ...
    await storage.checkIntegrity(true).then(
      ...
      /* error is expected here but we do not want to fail */
    );
  ...
  __name(createManyRandomData, "createManyRandomData");
  ...
  __name(createLargeRandomData, "createLargeRandomData");
});
