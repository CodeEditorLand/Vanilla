...
function format2(template, values) {
  ...
  return template.replace(
    _format2Regexp,
    (match, group) => values[group] ?? match
  );
}
...
__name(stripWildcards, "stripWildcards");
...
function splitLinesIncludeSeparators(str) {
  ...
  for (let i = 0; i < Math.ceil(splitLinesAndSeparators.length / 2); i++) {
    linesWithSeparators.push(
      splitLinesAndSeparators[2 * i] + (splitLinesAndSeparators[2 * i + 1] ?? "")
    );
  }
...
class GraphemeIterator {
  ...
  nextGraphemeLength() {
    ...
    while (!iterator.eol()) {
      ...
      const nextGraphemeBreakType = graphemeBreakTree.getGraphemeBreakType(
        iterator.nextCodePoint()
      );
      if (breakBetweenGraphemeBreakType(
        graphemeBreakType,
  ...
  prevGraphemeLength() {
    ...
    while (iterator.offset > 0) {
      const offset = iterator.offset;
      const prevGraphemeBreakType = graphemeBreakTree.getGraphemeBreakType(
        iterator.prevCodePoint()
      );
      if (breakBetweenGraphemeBreakType(
...
var GraphemeBreakType = /* @__PURE__ */ ((GraphemeBreakType2) => {
