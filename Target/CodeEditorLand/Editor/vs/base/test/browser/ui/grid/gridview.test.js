...
import {
  ...
  Orientation,
  Sizing
} from "../../../../browser/ui/grid/gridview.js";
...
suite("Gridview", () => {
  ...
  test("gridview addView deep nested", () => {
    const gridview = createGridView();
  ...
  test("simple layout", () => {
    ...
    assert.deepStrictEqual(gridview.getViewSize([0]), {
      width: 800,
      height: 600
    });
    ...
    assert.deepStrictEqual(gridview.getViewSize([1]), {
      width: 800,
      height: 400
    });
    ...
    assert.deepStrictEqual(gridview.getViewSize([1, 0]), {
      width: 600,
      height: 400
    });
    assert.deepStrictEqual(view2.size, [800, 200]);
    ...
    assert.deepStrictEqual(gridview.getViewSize([1, 1]), {
      width: 200,
      height: 400
    });
    ...
    assert.deepStrictEqual(gridview.getViewSize([0, 1]), {
      width: 600,
      height: 200
    });
    assert.deepStrictEqual(view3.size, [200, 400]);
    ...
    assert.deepStrictEqual(gridview.getViewSize([0, 0]), {
      width: 200,
      height: 200
    });
    ...
    assert.deepStrictEqual(gridview.getViewSize([1, 0, 0]), {
      width: 600,
      height: 300
    });
    assert.deepStrictEqual(view2.size, [600, 200]);
    ...
    assert.deepStrictEqual(gridview.getViewSize([1, 0, 1]), {
      width: 600,
      height: 100
    });
  });
  test("simple layout with automatic size distribution", () => {
    const gridview = createGridView();
    gridview.layout(800, 600);
    const view1 = store.add(
      new TestView(
  ...
  test("addviews before layout call 1", () => {
    const gridview = createGridView();
    const view1 = store.add(
      new TestView(
        50,
  ...
  test("addviews before layout call 2", () => {
    const gridview = createGridView();
    const view1 = store.add(
      new TestView(
        50,
  ...
  test("flipping orientation should preserve absolute offsets", () => {
    const gridview = createGridView();
    const view1 = store.add(
      new TestView(
        50,
