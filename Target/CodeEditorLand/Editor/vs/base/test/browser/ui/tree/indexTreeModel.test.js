...
__name(toElements, "toElements");
const diffIdentityProvider = { getId: /* @__PURE__ */ __name((n) => String(n), "getId") };
function withSmartSplice(fn) {
  fn({});
  fn({ diffIdentityProvider });
}
__name(withSmartSplice, "withSmartSplice");
suite("IndexTreeModel", () => {
  ...
  test("smart diff consistency", () => {
    ...
    for (let i = 0; i < times; i++) {
      const list = [];
      const options = { diffIdentityProvider: { getId: /* @__PURE__ */ __name((n) => String(n), "getId") } };
      const model = new IndexTreeModel("test", -1);
      const disposable = bindListToModel(list, model);
      const changes = [];
      const expected = [];
