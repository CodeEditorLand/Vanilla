...
} from "../../../../browser/ui/tree/asyncDataTree.js";
...
suite("AsyncDataTree", () => {
  ...
  test("Collapse state should be preserved across refresh calls", async () => {
    ...
    tree.layout(200);
    assert.strictEqual(
      container.querySelectorAll(".monaco-list-row").length,
      0
    );
    ...
    const twistie = container.querySelector(
  ...
  test("issue #68648", async () => {
    ...
    let twistie = container.querySelector(
      ".monaco-list-row:first-child .monaco-tl-twistie"
    );
    assert(!twistie.classList.contains("collapsible"));
    assert(!twistie.classList.contains("collapsed"));
    ...
    twistie = container.querySelector(
      ".monaco-list-row:first-child .monaco-tl-twistie"
    );
    assert(twistie.classList.contains("collapsible"));
    assert(twistie.classList.contains("collapsed"));
    ...
    model.get("a").children = [{ id: "aa" }, { id: "ab" }, { id: "ac" }];
    await tree.updateChildren(model.root);
    assert.deepStrictEqual(getChildrenCalls, [
      "root",
      "root",
      "root",
      "root"
  ...
  test("issue #67722 - once resolved, refreshed collapsed nodes should only get children when expanded", async () => {
    ...
    assert.deepStrictEqual(
      getChildrenCalls,
      ["root", "a", "root"],
      "a should not be refreshed, since it' collapsed"
    );
  });
  ...
  test("issue #192422 - resolved collapsed nodes with changed children don't show old children", async () => {
    ...
    const bChild = container.querySelector(
      ".monaco-list-row:nth-child(2)"
    );
  ...
  test("issue #80098 - concurrent refresh and expand", async () => {
    ...
    const dataSource = new class {
      ...
      getChildren(element) {
        return new Promise(
          (c) => calls.push(() => c(element.children || []))
        );
      }
    }();
    const model = new Model({
    ...
    assert.strictEqual(
      calls.length,
      1,
      "expand(a) still hasn't called getChildren(a)"
    );
    calls.pop()();
    ...
    assert.strictEqual(
      calls.length,
      0,
      "expand(a) should not have forced a second refresh"
    );
  ...
  test("issue #80098 - first expand should call getChildren", async () => {
    ...
    assert.strictEqual(
      calls.length,
      1,
      "expand(a) should've called getChildren(a)"
    );
  ...
  test("issues #84569, #82629 - rerender", async () => {
    ...
    await tree.expand(model.get("a"));
    assert.deepStrictEqual(
      Array.from(container.querySelectorAll(".monaco-list-row")).map(
        (e) => e.textContent
      ),
      ["a", "b1"]
    );
  ...
  test("issue #121567", async () => {
    ...
    assert.strictEqual(
      calls.length,
      1,
      "There should be a single getChildren call for the root"
    );
    ...
    assert.strictEqual(
      calls.length,
      1,
      "There should be no changes to the calls list, since a was collapsed"
    );
    assert(tree.isCollapsible(a), "a is collapsible");
    assert(tree.isCollapsed(a), "a is collapsed");
    ...
    assert.strictEqual(
      calls.length,
      1,
      "There should still be no changes to the calls list, since a was collapsed"
    );
    ...
    assert.strictEqual(
      calls.length,
      2,
      "Finally, there should be a getChildren call for a"
    );
  ...
  test("issue #199441", async () => {
    ...
    const tree = store.add(
      new CompressibleAsyncDataTree(
        ...
        [new Renderer()],
        dataSource,
        { identityProvider: new IdentityProvider(), collapseByDefault }
      )
    );
    tree.layout(200);
    await tree.setInput(model.root);
