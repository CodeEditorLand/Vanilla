...
import { StringSHA1, hash } from "../../common/hash.js";
...
suite("Hash", () => {
  ...
  test("object", () => {
    ...
    assert.strictEqual(hash({ foo: "bar" }), hash({ foo: "bar" }));
    assert.strictEqual(
      hash({ foo: "bar", foo2: void 0 }),
      hash({ foo2: void 0, foo: "bar" })
    );
    assert.notStrictEqual(hash({ foo: "bar" }), hash({ foo: "bar2" }));
    assert.notStrictEqual(hash({}), hash([]));
  });
  test("array - unexpected collision", () => {
  ...
  test("sha1-3", () => {
    return checkSHA1(
      "\uDA02\uA44D",
      "9b483a471f22fe7e09d83f221871a987244bbd3f"
    );
  });
  test("sha1-4", () => {
