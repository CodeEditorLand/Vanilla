...
flakySuite("IndexedDB", () => {
  let indexedDB;
  setup(async () => {
    indexedDB = await IndexedDB.create("vscode-indexeddb-test", 1, [
      "test-store"
    ]);
    await indexedDB.runInTransaction(
      "test-store",
      "readwrite",
      (store) => store.clear()
    );
  ...
  test("runInTransaction", async () => {
    await indexedDB.runInTransaction(
      "test-store",
      "readwrite",
      (store) => store.add("hello1", "key1")
    );
    const value = await indexedDB.runInTransaction(
      "test-store",
      "readonly",
      (store) => store.get("key1")
    );
  ...
  test("hasPendingTransactions", async () => {
    const promise = indexedDB.runInTransaction(
      "test-store",
      "readwrite",
      (store) => store.add("hello2", "key2")
    );
