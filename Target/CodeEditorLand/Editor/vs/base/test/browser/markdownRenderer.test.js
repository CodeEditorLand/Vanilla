...
suite("MarkdownRenderer", () => {
  ...
  suite("Sanitization", () => {
    test("Should not render images with unknown schemes", () => {
      const markdown = {
        value: `![image](no-such://example.com/cat.gif)`
      };
  ...
  suite("Images", () => {
    test("image rendering conforms to default", () => {
      const markdown = {
        value: `![image](http://example.com/cat.gif 'caption')`
      };
      const result = store.add(
        renderMarkdown(markdown)
      ).element;
    ...
    test("image width from title params", () => {
      const result = store.add(
        renderMarkdown({
          value: `![image](http://example.com/cat.gif|width=100px 'caption')`
        })
      ).element;
      assertNodeEquals(
        result,
    ...
    test("image height from title params", () => {
      const result = store.add(
        renderMarkdown({
          value: `![image](http://example.com/cat.gif|height=100 'caption')`
        })
      ).element;
      assertNodeEquals(
        result,
...
`
    ...
    test("render icon in <a> without href (#152170)", () => {
      ...
      assert.strictEqual(
        result.innerHTML,
        `<p><span class="codicon codicon-sync"></span></p>`
      );
    });
  });
  ...
  test("npm Hover Run Script not working #90855", () => {
    const md = JSON.parse(
  ...
  suite("supportHtml", () => {
    ...
    test("Should not include scripts even when supportHtml=true", () => {
      const mds = new MarkdownString(void 0, { supportHtml: true });
      mds.appendMarkdown(
        'a<b onclick="alert(1)">b</b><script>alert(2)</script>c'
      );
      const result = store.add(renderMarkdown(mds)).element;
      assert.strictEqual(result.innerHTML, `<p>a<b>b</b>c</p>`);
    });
  ...
  suite("fillInIncompleteTokens", () => {
    ...
    suite("table", () => {
      ...
      test("incomplete header one column", () => {
        ...
        const tokens = marked.marked.lexer(incompleteTable);
        const completeTableTokens = marked.marked.lexer(
          incompleteTable + "|\n| --- |"
        );
        const newTokens = fillInIncompleteTokens(tokens);
        if (newTokens) {
    ...
    function simpleMarkdownTestSuite(name, delimiter) {
      test(`incomplete ${name}`, () => {
        ...
        const newTokens = fillInIncompleteTokens(tokens);
        const completeTokens = marked.marked.lexer(
...
- item two [link`;
        ...
        const newTokens = fillInIncompleteTokens(tokens);
        const completeTokens = marked.marked.lexer(
          incomplete + "](https://microsoft.com)"
        );
        assert.deepStrictEqual(newTokens, completeTokens);
      });
