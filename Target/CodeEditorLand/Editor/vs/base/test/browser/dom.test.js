...
suite("dom", () => {
  ...
  test("removeClass should consider hyphens", () => {
    const element = document.createElement("div");
  ...
  test("multibyteAwareBtoa", () => {
    ...
    assert.ok(
      multibyteAwareBtoa(new Array(1e5).fill("vs").join("")).length > 0
    );
  });
  suite("$", () => {
    ...
    test("should build nodes with children", () => {
      ...
      assert.strictEqual(
        div.firstChild && div.firstChild.textContent,
        "hello"
      );
    });
  ...
  suite("SafeTriangle", () => {
    ...
    test("works", () => {
      const safeTriangle = new SafeTriangle(
        0,
        0,
        fakeElement(10, 20, 10, 20)
      );
