...
suite("dom", () => {
  ...
  test("multibyteAwareBtoa", () => {
    ...
    assert.ok(multibyteAwareBtoa(new Array(1e5).fill("vs").join("")).length > 0);
  });
  suite("$", () => {
  ...
  suite("SafeTriangle", () => {
    const fakeElement = /* @__PURE__ */ __name((left, right, top, bottom) => {
      return { getBoundingClientRect: /* @__PURE__ */ __name(() => ({ left, right, top, bottom }), "getBoundingClientRect") };
    }, "fakeElement");
    test("works", () => {
