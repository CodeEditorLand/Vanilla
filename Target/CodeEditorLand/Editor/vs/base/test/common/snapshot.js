...
class SnapshotContext {
  constructor(test) {
    ...
    this.snapshotsDir = URI.joinPath(
      src,
      ...[...parts.slice(0, -1), "__snapshots__"]
    );
  }
  nextIndex = 0;
  snapshotsDir;
  ...
  async removeOldSnapshots() {
    ...
    if (toDelete.length) {
      console.info(
        `Deleting ${toDelete.length} old snapshots for ${this.test?.fullTitle()}`
      );
    }
    await Promise.all(
      toDelete.map(
        (f) => __unlinkInTests(URI.joinPath(this.snapshotsDir, f).fsPath)
      )
    );
  }
}
    ...
    case "object": {
      ...
      if (Array.isArray(value)) {
        const children = value.map(
          (v) => formatValue(v, level + 1, [...seen, value])
        );
