...
suite("Async", () => {
  ...
  suite("Throttler", function() {
    ...
    test("async", () => {
      let count = 0;
      const factory = /* @__PURE__ */ __name(() => async.timeout(0).then(() => ++count), "factory");
      const throttler = new async.Throttler();
      return Promise.all([
        throttler.queue(factory).then((result) => {
          assert.strictEqual(result, 1);
    ...
    test("last factory should be the one getting called", function() {
      const factoryFactory = /* @__PURE__ */ __name((n) => () => {
  ...
  suite("Queue", () => {
    test("simple", function() {
      ...
      const f1 = /* @__PURE__ */ __name(() => Promise.resolve(true).then(() => syncPromise = true), "f1");
      let asyncPromise = false;
      const f2 = /* @__PURE__ */ __name(() => async.timeout(10).then(() => asyncPromise = true), "f2");
    ...
    test("order is kept", function() {
      return runWithFakedTimers({}, () => {
        const queue = new async.Queue();
        const res = [];
        const f1 = /* @__PURE__ */ __name(() => Promise.resolve(true).then(() => res.push(1)), "f1");
        const f2 = /* @__PURE__ */ __name(() => async.timeout(10).then(() => res.push(2)), "f2");
        const f3 = /* @__PURE__ */ __name(() => Promise.resolve(true).then(() => res.push(3)), "f3");
        const f4 = /* @__PURE__ */ __name(() => async.timeout(20).then(() => res.push(4)), "f4");
        const f5 = /* @__PURE__ */ __name(() => async.timeout(0).then(() => res.push(5)), "f5");
        queue.queue(f1);
        queue.queue(f2);
        queue.queue(f3);
        queue.queue(f4);
    ...
    test("errors bubble individually but not cause stop", function() {
      ...
      const f1 = /* @__PURE__ */ __name(() => Promise.resolve(true).then(() => res.push(1)), "f1");
      const f2 = /* @__PURE__ */ __name(() => async.timeout(10).then(() => res.push(2)), "f2");
      const f3 = /* @__PURE__ */ __name(() => Promise.resolve(true).then(() => Promise.reject(new Error("error"))), "f3");
      const f4 = /* @__PURE__ */ __name(() => async.timeout(20).then(() => res.push(4)), "f4");
      const f5 = /* @__PURE__ */ __name(() => async.timeout(0).then(() => res.push(5)), "f5");
      queue.queue(f1);
      queue.queue(f2);
    ...
    test("events", async function() {
      ...
      const res = [];
      const f1 = /* @__PURE__ */ __name(() => async.timeout(10).then(() => res.push(2)), "f1");
      const f2 = /* @__PURE__ */ __name(() => async.timeout(20).then(() => res.push(4)), "f2");
      const f3 = /* @__PURE__ */ __name(() => async.timeout(0).then(() => res.push(5)), "f3");
  ...
  suite("ThrottledWorker", () => {
    ...
    __name(assertArrayEquals, "assertArrayEquals");
    test("basics", async () => {
      let handled = [];
      let handledCallback;
      ...
      const handler = /* @__PURE__ */ __name((units) => {
        handled.push(...units);
    ...
    test("do not accept too much work", async () => {
      const handled = [];
      const handler = /* @__PURE__ */ __name((units) => handled.push(...units), "handler");
      const worker = store.add(new async.ThrottledWorker({
        maxWorkChunkSize: 5,
        maxBufferedWork: 5,
        throttleDelay: 1
  ...
  suite("AsyncIterableObject", function() {
    ...
    test("onReturn called on return", async function() {
      ...
      await (/* @__PURE__ */ __name(async function test2() {
        for await (const item of iter) {
          assert.strictEqual(item, 1);
          return;
        }
