...
suite("Cache", () => {
  ...
  test("simple value", () => {
    ...
    return cache.get().promise.then(
      (c) => assert.strictEqual(c, 0),
      () => assert.fail("Unexpected assertion error")
    ).then(() => cache.get().promise).then(
      (c) => assert.strictEqual(c, 0),
      () => assert.fail("Unexpected assertion error")
    );
  });
  test("simple error", () => {
    ...
    const cache = new Cache(
      (_) => Promise.reject(new Error(String(counter++)))
    );
    return cache.get().promise.then(
      () => assert.fail("Unexpected assertion error"),
      (err) => assert.strictEqual(err.message, "0")
    ).then(() => cache.get().promise).then(
      () => assert.fail("Unexpected assertion error"),
      (err) => assert.strictEqual(err.message, "0")
    );
