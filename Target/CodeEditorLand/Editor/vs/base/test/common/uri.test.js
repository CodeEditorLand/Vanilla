...
suite("URI", () => {
  ...
  test("http#toString, encode=FALSE", () => {
    ...
    assert.strictEqual(
      value.toString(true),
      "file://shares/pr\xF6jects/c%23/#l12"
    );
  ...
  test("with, remove components #8465", () => {
    ...
    assert.strictEqual(
      URI.parse("scheme:/path").with({ authority: "authority" }).with({ authority: "" }).toString(),
      "scheme:/path"
    );
    assert.strictEqual(
      URI.parse("scheme:/path").with({ authority: "authority" }).with({ authority: null }).toString(),
      "scheme:/path"
    );
    assert.strictEqual(
      URI.parse("scheme:/path").with({ authority: "authority" }).with({ path: "" }).toString(),
      "scheme://authority"
    );
    assert.strictEqual(
      URI.parse("scheme:/path").with({ authority: "authority" }).with({ path: null }).toString(),
      "scheme://authority"
    );
    assert.strictEqual(
  ...
  test("parse", () => {
    ...
    assert.strictEqual(
      value.path,
      "/c:/Source/Z\xFCrich or Zurich (\u02C8zj\u028A\u0259r\u026Ak,/Code/resources/app/plugins/c#/plugin.json"
    );
    assert.strictEqual(value.fragment, "");
    assert.strictEqual(value.query, "");
  ...
  test("URI#toString, escape all the bits", () => {
    const value = URI.file(
      "/Users/jrieken/Code/_samples/18500/M\xF6del + Other Th\xEEng\xDF/model.js"
    );
    assert.strictEqual(
      value.toString(),
  ...
  test("URI#toString, user information in authority", () => {
    ...
    value = URI.from({
      scheme: "http",
      authority: "f\xF6\xF6:b\xF6r@l\xF6calhost:8080",
      path: "/far",
      query: void 0,
      fragment: void 0
    });
  ...
  test("correctFileUriToFilePath2", () => {
    ...
    test2(
      ...
      isWindows ? "c:\\Source\\Z\xFCrich or Zurich (\u02C8zj\u028A\u0259r\u026Ak,\\Code\\resources\\app\\plugins" : "c:/Source/Z\xFCrich or Zurich (\u02C8zj\u028A\u0259r\u026Ak,/Code/resources/app/plugins"
    );
    test2(
  ...
  test("URI - http, query & toString", () => {
    ...
    uri = URI.parse(
      "https://go.microsoft.com/fwlink/?LinkId=518008&fo\xF6&k\xE9\xA5=\xFC\xFC"
    );
  ...
  test("Bug in URI.isUri() that fails `thing` type comparison #114971", () => {
  ...
  test.skip("Links in markdown are broken if url contains encoded parameters #79474", () => {
  ...
  test.skip("Uri#parse can break path-component #45515", () => {
