...
import { VSBuffer, bufferToReadable } from "../../common/buffer.js";
...
suite("Stream", () => {
  ...
  test("peekReadable", () => {
    for (let i = 0; i < 5; i++) {
      ...
      const consumedOrReadable2 = peekReadable(
        readable2,
      ...
      if (typeof consumedOrReadable2 === "string") {
        ...
        const consumed = consumeReadable(
          consumedOrReadable2,
          (strings) => strings.join()
        );
        assert.strictEqual(consumed, "1,2,3,4,5");
    ...
    let consumedOrReadable = peekReadable(
      readable,
      (strings) => strings.join(),
      5
    );
    ...
    consumedOrReadable = peekReadable(
      readable,
      (strings) => strings.join(),
      6
    );
  ...
  test("consumeStream", async () => {
    const stream = readableToStream(
      arrayToReadable(["1", "2", "3", "4", "5"])
    );
  ...
  test("peekStream", async () => {
    for (let i = 0; i < 5; i++) {
      const stream2 = readableToStream(
        arrayToReadable(["1", "2", "3", "4", "5"])
      );
      ...
      if (result2.ended) {
        assert.fail(
          "Unexpected result, stream should not have ended yet"
        );
      } else {
        assert.strictEqual(
          result2.buffer.length,
          i + 1,
          `maxChunks: ${i}`
        );
        ...
        assert.strictEqual(
          [...result2.buffer, ...additionalResult].join(),
          "1,2,3,4,5"
        );
      }
    }
    let stream = readableToStream(
      arrayToReadable(["1", "2", "3", "4", "5"])
    );
  ...
  test("prefixedReadable", () => {
    let readable = prefixedReadable(
      "1,2",
      arrayToReadable(["3", "4", "5"]),
      (val) => val.join(",")
    );
    assert.strictEqual(
  ...
  test("prefixedStream", async () => {
    ...
    stream.write("5");
    stream.end();
    let prefixStream = prefixedStream(
      "1,2",
      stream,
      (val) => val.join(",")
    );
