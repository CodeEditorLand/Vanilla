...
suite("Buffer", () => {
  ...
  test("bufferedStreamToBuffer", async () => {
    ...
    const stream = await peekStream(
      bufferToStream(VSBuffer.fromString(content)),
      1
    );
    assert.strictEqual(
      (await bufferedStreamToBuffer(stream)).toString(),
      content
    );
  });
  ...
  test("Performance issue with VSBuffer#slice #76076", () => {
  ...
  test("indexOf", () => {
    ...
    assert.strictEqual(
      haystack.indexOf(VSBuffer.fromString("a".repeat(100))),
      -1
    );
  ...
  suite("base64", () => {
    const testCases = [
      ...
      [
        new Uint8Array([164, 16, 88, 88, 224, 173, 144, 114]),
        "pBBYWOCtkHI="
      ],
      [
        new Uint8Array([0, 196, 99, 12, 21, 229, 78, 101, 13]),
        "AMRjDBXlTmUN"
      ],
      [
        new Uint8Array([167, 114, 225, 116, 226, 83, 51, 48, 88, 114]),
        "p3LhdOJTMzBYcg=="
      ],
      [
        new Uint8Array([75, 33, 118, 10, 77, 5, 168, 194, 59, 47, 59]),
        "SyF2Ck0FqMI7Lzs="
      ],
      [
        new Uint8Array([
    ...
    test("encodes", () => {
      for (const [bytes, expected] of testCases) {
        assert.strictEqual(
          encodeBase64(VSBuffer.wrap(bytes)),
          expected
        );
      }
    });
