...
suite("Arrays", () => {
  ...
  test("findFirst", () => {
    const array = [1, 4, 5, 7, 55, 59, 60, 61, 64, 69];
    let idx = arraysFind.findFirstIdxMonotonousOrArrLen(
      array,
      (e) => e >= 0
    );
  ...
  test("distinct", () => {
    ...
    assert.deepStrictEqual(
      arrays.distinct(["32", "4", "5", "4"], compare),
      ["32", "4", "5"]
    );
    assert.deepStrictEqual(
  ...
  async function testTopAsync(cmp, m) {
    ...
    {
      const result = await arrays.topAsync(
        [4, 6, 2, 7, 8, 3, 5, 1],
        cmp,
        3,
        m
      ...
      function testMonotonous(array3, predicate) {
        ...
        const negatedPredicate = (a) => !predicate(a);
        {
          ...
          assert.deepStrictEqual(
            ...
            normalize(array3.filter(predicate))
          );
          assert.deepStrictEqual(
            queue1.length,
            array3.length - array3.filter(predicate).length
          );
          assert.deepStrictEqual(
            queue1.takeWhile(() => true),
            normalize(array3.filter(negatedPredicate))
          );
        }
        {
          ...
          assert.deepStrictEqual(
            queue3.length,
            array3.length - array3.filter(negatedPredicate).length
          );
          assert.deepStrictEqual(
