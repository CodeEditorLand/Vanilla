...
suite("Event utils dispose", () => {
  ...
  function assertDisposablesCount(expected) {
    if (Array.isArray(expected)) {
      ...
      assert.strictEqual(
        tracker.getTrackedDisposables().length,
        expected
      );
    }
  }
...
suite("Event", () => {
  ...
  test("onFirstAdd|onLastRemove", () => {
    ...
    let subscription1 = ds.add(a.event(() => {
    }));
    const subscription2 = ds.add(a.event(() => {
    }));
  ...
  test("onWillRemoveListener", () => {
    ...
    let subscription = ds.add(a.event(() => {
    }));
    assert.strictEqual(count, 0);
    subscription.dispose();
    assert.strictEqual(count, 1);
    subscription = ds.add(a.event(() => {
    }));
    assert.strictEqual(count, 1);
  });
  ...
  test("DebounceEmitter", async () => runWithFakedTimers({}, async () => {
    let callCount = 0;
    let sum = 0;
  ...
  test("Emitter, - In Order Delivery 3x", () => {
    const a = ds.add(new Emitter());
    const listener2Events = [];
    ds.add(
      a.event(function listener1(event) {
...
suite("AsyncEmitter", () => {
  ...
  test("sequential delivery", async () => runWithFakedTimers({}, async () => {
    let globalState = 0;
    const emitter = new AsyncEmitter();
    ds.add(
      emitter.event((e) => {
  ...
  test("sequential, in-order delivery", async () => runWithFakedTimers({}, async () => {
    const events = [];
    let done = false;
    const emitter = new AsyncEmitter();
    ds.add(
...
suite("PausableEmitter", () => {
  ...
  test("empty pause with merge", () => {
    const data = [];
    const emitter = ds.add(
      new PauseableEmitter({ merge: (a) => a[0] })
    );
...
suite("Event utils - ensureNoDisposablesAreLeakedInTestSuite", () => {
...
suite("Event utils", () => {
  ...
  suite("DynamicListEventMultiplexer", () => {
    ...
    class TestItem {
      onTestEventEmitter = ds.add(new Emitter());
      onTestEvent = this.onTestEventEmitter.event;
    }
    let items;
    let m;
    setup(() => {
      ...
      m = new DynamicListEventMultiplexer(
        items,
        addEmitter.event,
        removeEmitter.event,
        (e) => e.onTestEvent
  ...
  suite("fromPromise", () => {
    test("not yet resolved", async () => new Promise((resolve) => {
      ...
      ds.add(
        Event.fromPromise(promise.p)((e) => {
          ...
          promise.error(void 0);
        })
      );
      promise.complete(1);
    }));
    test("already resolved", async () => new Promise((resolve) => {
      let promise = new DeferredPromise();
      promise.complete(1);
      ds.add(
        Event.fromPromise(promise.p)((e) => {
  ...
  suite("accumulate", () => {
    test("should not fire after a listener is disposed with undefined or []", async () => {
      ...
      assert.deepStrictEqual(
        calls2,
        [[1]],
        "should not fire after a listener is disposed with undefined or []"
      );
    });
  ...
  suite("debounce", () => {
    ...
    test("flushOnListenerRemove - should flush events when a listener is disposed", async () => {
      ...
      assert.deepStrictEqual(
        calls,
        [1],
        "should fire with the first event, not the second (after listener dispose)"
      );
    });
