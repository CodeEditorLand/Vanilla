...
suite("Event utils dispose", function() {
  ...
  __name(assertDisposablesCount, "assertDisposablesCount");
  setup(() => {
...
suite("Event", function() {
  ...
  test("Emitter duplicate functions", () => {
    const calls = [];
    const a = /* @__PURE__ */ __name((v) => calls.push(`a${v}`), "a");
    const b = /* @__PURE__ */ __name((v) => calls.push(`b${v}`), "b");
    const emitter = ds.add(new Emitter());
  ...
  test("Emitter - In Order Delivery", function() {
    ...
    ds.add(a.event(/* @__PURE__ */ __name(function listener1(event) {
      if (event === "e1") {
        a.fire("e2");
        assert.deepStrictEqual(listener2Events, ["e1", "e2"]);
      }
    }, "listener1")));
    ds.add(a.event(/* @__PURE__ */ __name(function listener2(event) {
...
suite("PausableEmitter", function() {
  ...
  test("pause/resume - merge", function() {
    const data = [];
    const emitter = ds.add(new PauseableEmitter({ merge: /* @__PURE__ */ __name((a) => a.reduce((p, c) => p + c, 0), "merge") }));
    ds.add(emitter.event((e) => data.push(e)));
    emitter.fire(1);
    emitter.fire(2);
    assert.deepStrictEqual(data, [1, 2]);
  ...
  test("empty pause with merge", function() {
    const data = [];
    const emitter = ds.add(new PauseableEmitter({ merge: /* @__PURE__ */ __name((a) => a[0], "merge") }));
...
suite("Event utils", () => {
  ...
  suite("Relay", () => {
    test("should input work", () => {
      ...
      const result = [];
      const listener = /* @__PURE__ */ __name((num) => result.push(num), "listener");
