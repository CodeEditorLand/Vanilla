...
perfSuite("Performance - fuzzyMatch", async () => {
  const uri = FileAccess.asBrowserUri(
    "vs/base/test/common/filters.perf.data"
  ).toString(true);
  ...
  function perfTest(name, match) {
    test(name, () => {
      ...
      console.log(
        name,
        `${d}ms, ${Math.round(count / d) * 15}/15ms, ${Math.round(count / d)}/1ms`
      );
    });
  }
...
perfSuite("Performance - IFilter", async () => {
  const uri = FileAccess.asBrowserUri(
    "vs/base/test/common/filters.perf.data"
  ).toString(true);
  const { data } = await importAMDNodeModule(uri, "");
