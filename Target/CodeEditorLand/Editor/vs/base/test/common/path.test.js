...
${message}`);
  ...
  test("basename", () => {
    ...
    assert.strictEqual(path.win32.basename("C:\\"), "");
    assert.strictEqual(
      path.win32.basename("C:\\dir\\base.ext"),
      "base.ext"
    );
    assert.strictEqual(
      path.win32.basename("C:\\basename.ext"),
      "basename.ext"
    );
    assert.strictEqual(
      path.win32.basename("C:basename.ext"),
...
${message}`);
  ...
  test("normalize", () => {
    ...
    assert.strictEqual(path.win32.normalize("bar\\foo..\\.."), "bar");
    assert.strictEqual(
      path.win32.normalize("bar\\foo..\\..\\baz"),
      "bar\\baz"
    );
    assert.strictEqual(
      path.win32.normalize("bar\\foo..\\"),
      "bar\\foo..\\"
    );
    ...
    assert.strictEqual(
      path.posix.normalize("foo/bar\\baz"),
      "foo/bar\\baz"
    );
  });
  test("isAbsolute", () => {
    ...
    assert.strictEqual(
      path.win32.isAbsolute("directory\\directory"),
      false
    );
    ...
    ["", "foo", "foo/bar", "./foo", "http://foo.com/bar"].forEach(
      (nonAbsolutePath) => {
        assert.ok(
          !path.win32.isAbsolute(nonAbsolutePath),
          nonAbsolutePath
        );
      }
    );
