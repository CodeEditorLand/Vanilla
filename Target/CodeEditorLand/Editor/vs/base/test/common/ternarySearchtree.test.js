...
suite("Ternary Search Tree", () => {
  ...
  test("URIIterator - ignore query/fragment", () => {
    const iter = new UriIterator(
      () => false,
      () => true
    );
  ...
  test("TernarySearchTree - (AVL) set", () => {
    {
      const trie = new TernarySearchTree(
        new PathIterator()
      );
  ...
  test("TernarySearchTree - (BST) delete", () => {
    const trie = new TernarySearchTree(
      new StringIterator()
    );
    trie.set("d", 1);
  ...
  test("TernarySearchTree - (AVL) delete", () => {
    const trie = new TernarySearchTree(
      new StringIterator()
    );
    trie.clear();
  ...
  test("TernarySearchTree: Cannot read property '1' of undefined #138284", () => {
    const keys = [
      URI.parse("fake-fs:/C"),
      URI.parse("fake-fs:/A"),
      URI.parse("fake-fs:/D"),
  ...
  test("TernarySearchTree: Cannot read property '1' of undefined #138284 (simple)", () => {
    const keys = ["C", "A", "D", "B"];
  ...
  test("TernarySearchTree: Cannot read property '1' of undefined #138284 (random)", () => {
    for (let round = 10; round >= 0; round--) {
      ...
      for (let i = 0; i < 100; i++) {
        keys.push(
          URI.from({
            scheme: "fake-fs",
            path: randomPath(void 0, void 0, 10)
          })
        );
      }
      const tst = TernarySearchTree.forUris();
      ...
      for (const item of keys) {
        ...
        assert.ok(
          tst._isBalanced(),
          `DEL${item}|${keys.map(String).join()}`
        );
      }
    }
  });
  test("TernarySearchTree: Cannot read properties of undefined (reading 'length'): #161618 (simple)", () => {
  ...
  test("TernarySearchTree: Cannot read properties of undefined (reading 'length'): #161618 (random)", () => {
    const raw = "config.debug.toolBarLocation,floating,config.editor.renderControlCharacters,true,config.editor.renderWhitespace,selection,config.files.autoSave,off,config.git.enabled,true,config.notebook.globalToolbar,true,config.terminal.integrated.tabs.enabled,true,config.terminal.integrated.tabs.showActions,singleTerminalOrNarrow,config.terminal.integrated.tabs.showActiveTerminal,singleTerminalOrNarrow,config.workbench.activityBar.visible,true,config.workbench.experimental.settingsProfiles.enabled,true,config.workbench.layoutControl.type,both,config.workbench.sideBar.location,left,config.workbench.statusBar.visible,true";
    const array = raw.split(",");
    const tuples = [];
    for (let i = 0; i < array.length; i += 2) {
  ...
  test("TernarySearchTree (PathSegments) - delete_superstr", () => {
    const map = new TernarySearchTree(new PathIterator());
    map.set("/user/foo/bar", 1);
    map.set("/user/foo", 2);
    map.set("/user/foo/flip/flop", 3);
  ...
  test("TernarySearchTree (URI) - basics", () => {
    const trie = new TernarySearchTree(
      new UriIterator(
        () => false,
        () => false
    ...
    assert.strictEqual(
      trie.findSubstr(URI.file("/user/foo/bar/far/boo")),
      1
    );
  });
  test("TernarySearchTree (URI) - query parameters", () => {
    const trie = new TernarySearchTree(
      new UriIterator(
        () => false,
        () => true
  ...
  test("TernarySearchTree (URI) - lookup", () => {
    const map = new TernarySearchTree(
      new UriIterator(
        () => false,
        () => false
      )
    ...
    assert.strictEqual(
      map.get(URI.parse("http://foo.bar/user")),
      void 0
    );
    assert.strictEqual(
      map.get(URI.parse("http://foo.bar/user/foo/bar")),
      1
    );
    assert.strictEqual(
      map.get(URI.parse("http://foo.bar/user/foo?query")),
      2
    );
    assert.strictEqual(
      map.get(URI.parse("http://foo.bar/user/foo?Query")),
    ...
    assert.strictEqual(
      map.get(URI.parse("http://foo.bar/user/foo/bar/boo")),
      void 0
    );
  });
  test("TernarySearchTree (URI) - lookup, casing", () => {
    const map = new TernarySearchTree(
      new UriIterator(
    ...
    assert.strictEqual(
      map.get(URI.parse("foo://foo.bar/USER/foo/bar")),
      void 0
    );
  });
  test("TernarySearchTree (URI) - superstr", () => {
    const map = new TernarySearchTree(
      new UriIterator(
        () => false,
        () => false
  ...
  test("TernarySearchTree (ConfigKeySegments) - basics", () => {
    const trie = new TernarySearchTree(
      new ConfigKeysIterator()
    );
  ...
  test("TernarySearchTree (ConfigKeySegments) - lookup", () => {
    const map = new TernarySearchTree(
      new ConfigKeysIterator()
    );
  ...
  test("TernarySearchTree (ConfigKeySegments) - superstr", () => {
    const map = new TernarySearchTree(
      new ConfigKeysIterator()
    );
    map.set("config.foo.bar", 1);
  ...
  test("TernarySearchTree (ConfigKeySegments) - delete_superstr", () => {
    const map = new TernarySearchTree(
      new ConfigKeysIterator()
    );
    map.set("config.foo.bar", 1);
...
suite.skip("TST, perf", () => {
  ...
  suiteSetup(() => {
    ...
    candidates = [
      ...sampleUris.slice(0, len / 2),
      ...createRandomUris(len / 2)
    ];
    shuffle(candidates);
  });
  setup(() => {
