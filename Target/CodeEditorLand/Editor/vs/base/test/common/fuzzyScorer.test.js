...
suite("Fuzzy Scorer", () => {
  ...
  test("score (non fuzzy)", () => {
    const target = "HelLo-World";
    assert.ok(_doScore(target, "HelLo-World", false)[0] > 0);
    assert.strictEqual(
      _doScore(target, "HelLo-World", false)[1].length,
      "HelLo-World".length
    );
    assert.ok(_doScore(target, "hello-world", false)[0] > 0);
  ...
  test("scoreItem - matches are proper", () => {
    ...
    const basenamePrefixRes = scoreItem(
      resource,
      "som",
      true,
      ResourceAccessor
    ...
    const basenameCamelcaseRes = scoreItem(
      resource,
      "sF",
      true,
      ResourceAccessor
  ...
  test("scoreItem - ensure upper case bonus only applies on non-consecutive matches (bug #134723)", () => {
    ...
    assert.ok(
      scoreItem(resourceAllLower, "asdf", true, ResourceAccessor).score > scoreItem(resourceWithUpper, "asdf", true, ResourceAccessor).score
    );
  });
  test("compareItemsByScore - identity", () => {
    ...
    let res = [resourceA, resourceB, resourceC].sort(
      (r1, r2) => compareItemsByScore(r1, r2, query, true, ResourceAccessor)
    );
    assert.strictEqual(res[0], resourceA);
    assert.strictEqual(res[1], resourceB);
    assert.strictEqual(res[2], resourceC);
    res = [resourceC, resourceB, resourceA].sort(
      (r1, r2) => compareItemsByScore(r1, r2, query, true, ResourceAccessor)
    );
    assert.strictEqual(res[0], resourceA);
    assert.strictEqual(res[1], resourceB);
    ...
    res = [resourceA, resourceB, resourceC].sort(
      (r1, r2) => compareItemsByScore(r1, r2, query, true, ResourceAccessor)
    );
  ...
  test("compareFilesByScore - prefer shorter paths (bug #17443)", () => {
    ...
    const query = "co/te";
    const res = [resourceA, resourceB, resourceC].sort(
      (r1, r2) => compareItemsByScore(r1, r2, query, true, ResourceAccessor)
    );
    assert.strictEqual(res[0], resourceB);
    assert.strictEqual(res[1], resourceA);
  ...
  test("compareFilesByScore - prefer matches in label over description if scores are otherwise equal", () => {
    ...
    const res = [resourceA, resourceB].sort(
      (r1, r2) => compareItemsByScore(r1, r2, query, true, ResourceAccessor)
    );
    assert.strictEqual(res[0], resourceB);
    assert.strictEqual(res[1], resourceA);
  ...
  test("compareFilesByScore - prefer camel case matches", () => {
    ...
    for (const query of ["npe", "NPE"]) {
      let res = [resourceA, resourceB].sort(
        (r1, r2) => compareItemsByScore(r1, r2, query, true, ResourceAccessor)
      );
      assert.strictEqual(res[0], resourceA);
      assert.strictEqual(res[1], resourceB);
      res = [resourceB, resourceA].sort(
        (r1, r2) => compareItemsByScore(r1, r2, query, true, ResourceAccessor)
      );
      assert.strictEqual(res[0], resourceA);
      assert.strictEqual(res[1], resourceB);
  ...
  test("compareFilesByScore - avoid match scattering (bug #34210)", () => {
    const resourceA = URI.file(
      "node_modules1/bundle/lib/model/modules/ot1/index.js"
    );
    const resourceB = URI.file(
      "node_modules1/bundle/lib/model/modules/un1/index.js"
    );
    const resourceC = URI.file(
      "node_modules1/bundle/lib/model/modules/modu1/index.js"
    );
    const resourceD = URI.file(
      "node_modules1/bundle/lib/model/modules/oddl1/index.js"
    );
    let query = isWindows ? "modu1\\index.js" : "modu1/index.js";
    let res = [resourceA, resourceB, resourceC, resourceD].sort(
      (r1, r2) => compareItemsByScore(r1, r2, query, true, ResourceAccessor)
    );
    assert.strictEqual(res[0], resourceC);
    res = [resourceC, resourceB, resourceA, resourceD].sort(
      (r1, r2) => compareItemsByScore(r1, r2, query, true, ResourceAccessor)
    );
    assert.strictEqual(res[0], resourceC);
    query = isWindows ? "un1\\index.js" : "un1/index.js";
    res = [resourceA, resourceB, resourceC, resourceD].sort(
      (r1, r2) => compareItemsByScore(r1, r2, query, true, ResourceAccessor)
    );
    assert.strictEqual(res[0], resourceB);
    res = [resourceC, resourceB, resourceA, resourceD].sort(
  ...
  test("compareFilesByScore - avoid match scattering (bug #26649)", () => {
    const resourceA = URI.file(
      "photobook/src/components/AddPagesButton/index.js"
    );
    const resourceB = URI.file(
      "photobook/src/components/ApprovalPageHeader/index.js"
    );
    const resourceC = URI.file(
  ...
  test("compareFilesByScore - avoid match scattering (bug #14879)", () => {
    const resourceA = URI.file(
      "pkg/search/gradient/testdata/constraint_attrMatchString.yml"
    );
  ...
  test("compareFilesByScore - avoid match scattering (bug #36810)", () => {
    const resourceA = URI.file(
      "Trilby.TrilbyTV.Web.Portal/Views/Systems/Index.cshtml"
    );
    const resourceB = URI.file(
      "Trilby.TrilbyTV.Web.Portal/Areas/Admins/Views/Tips/Index.cshtml"
    );
  ...
  test("compareFilesByScore - avoid match scattering (bug #12095)", () => {
    ...
    assert.strictEqual(res[0], resourceB);
    res = [resourceA, resourceC, resourceB].sort(
      (r1, r2) => compareItemsByScore(r1, r2, query, true, ResourceAccessor)
    );
    assert.strictEqual(res[0], resourceB);
  });
  ...
  test("compareFilesByScore - boost shorter prefix match if multiple queries are used", () => {
    const resourceA = URI.file(
      "src/vs/workbench/node/actions/windowActions.ts"
    );
  ...
  test("prepareQuery", () => {
    ...
    assert.strictEqual(
      prepareQuery("model Tester.ts").original,
      "model Tester.ts"
    );
    assert.strictEqual(
    ...
    assert.strictEqual(
      prepareQuery("model Tester.ts").normalized,
      "modelTester.ts"
    );
    assert.strictEqual(
      prepareQuery("model Tester.ts").expectContiguousMatch,
      false
    );
    assert.strictEqual(
      prepareQuery("Model Tester.ts").normalizedLowercase,
      "modeltester.ts"
    );
    assert.strictEqual(
      prepareQuery("ModelTester.ts").containsPathSeparator,
      false
    );
    assert.strictEqual(
      prepareQuery("Model" + sep + "Tester.ts").containsPathSeparator,
      true
    );
    ...
    assert.strictEqual(
      restoredQuery.containsPathSeparator,
      query.containsPathSeparator
    );
    ...
    assert.strictEqual(
      query.values?.[0].originalLowercase,
      "Hello".toLowerCase()
    );
    assert.strictEqual(query.values?.[0].normalized, "Hello");
    assert.strictEqual(
    ...
    assert.strictEqual(query.values?.[1].original, "World");
    assert.strictEqual(
      query.values?.[1].originalLowercase,
      "World".toLowerCase()
    );
    assert.strictEqual(query.values?.[1].normalized, "World");
    assert.strictEqual(
    ...
    if (isWindows) {
      assert.strictEqual(
        prepareQuery("C:\\some\\path").pathNormalized,
        "C:\\some\\path"
      );
      assert.strictEqual(
        prepareQuery("C:\\some\\path").normalized,
        "C:\\some\\path"
      );
      assert.strictEqual(
        prepareQuery("C:\\some\\path").containsPathSeparator,
        true
      );
      assert.strictEqual(
        prepareQuery("C:/some/path").pathNormalized,
      ...
      assert.strictEqual(
        prepareQuery("/some/path").pathNormalized,
        "/some/path"
      );
      assert.strictEqual(
        prepareQuery("/some/path").normalized,
        "/some/path"
      );
      assert.strictEqual(
        prepareQuery("/some/path").containsPathSeparator,
        true
      );
      assert.strictEqual(
        prepareQuery("\\some\\path").pathNormalized,
  ...
  test("fuzzyScore2 (multiple queries)", () => {
    const target = "HelLo-World";
    const [firstSingleScore, firstSingleMatches] = _doScore2(
      target,
      "HelLo"
    );
    const [secondSingleScore, secondSingleMatches] = _doScore2(
      target,
      "World"
    );
    ...
    function assertScore() {
      assert.ok(
        multiScore ?? (firstSingleScore ?? 0) + (secondSingleScore ?? 0) <= 0
      );
      for (let i = 0; multiMatches && i < multiMatches.length; i++) {
        ...
        if (multiMatch && firstAndSecondSingleMatch) {
          assert.strictEqual(
            multiMatch.start,
            firstAndSecondSingleMatch.start
          );
          assert.strictEqual(
            multiMatch.end,
            firstAndSecondSingleMatch.end
          );
        } else {
          assert.fail();
        }
