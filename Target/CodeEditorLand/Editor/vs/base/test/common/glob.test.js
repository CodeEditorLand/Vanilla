...
suite("Glob", () => {
  function assertGlobMatch(pattern, input) {
    assert(
      glob.match(pattern, input),
      `${JSON.stringify(pattern)} should match ${input}`
    );
    assert(
      glob.match(pattern, nativeSep(input)),
      `${pattern} should match ${nativeSep(input)}`
    );
  }
  function assertNoGlobMatch(pattern, input) {
    ...
    assert(
      !glob.match(pattern, nativeSep(input)),
      `${pattern} should not match ${nativeSep(input)}`
    );
  }
  test("simple", () => {
    let p = "node_modules";
  ...
  test("expression support (single)", () => {
    ...
    assert.strictEqual(
      "**/*.js",
      glob.match(expression, "test.js", hasSibling)
    );
    assert.strictEqual(
      glob.match(expression, "test.js", () => false),
      null
    );
    assert.strictEqual(
      glob.match(expression, "test.js", (name) => name === "te.ts"),
      null
    );
  ...
  test("expression support (multiple)", () => {
    ...
    assert.strictEqual(
      "**/*.as",
      glob.match(expression, "test.as", hasSibling)
    );
    assert.strictEqual(
      "**/*.bananas",
      glob.match(expression, "test.bananas", hasSibling)
    );
    assert.strictEqual(
      "**/*.bananas",
      glob.match(expression, "test.bananas")
    );
    assert.strictEqual(
      glob.match(expression, "test.foo", hasSibling),
      null
    );
  });
  test("brackets", () => {
  ...
  test("split glob aware", () => {
    assert.deepStrictEqual(glob.splitGlobAware("foo,bar", ","), [
      "foo",
      "bar"
    ]);
    ...
    assert.deepStrictEqual(
      glob.splitGlobAware("{foo,bar},foo,bar,{foo,bar}", ","),
  ...
  test("expression with two basename globs and a siblings expression", () => {
    ...
    if (isWindows) {
      assert.strictEqual(
        glob.match(expr, "foo\\bar", hasSibling),
        "**/bar"
      );
    }
  ...
  test("expression/pattern basename", () => {
    ...
    assert.strictEqual(
      glob.parse("{**/baz,**/foo}")("baz/bar", "bar"),
      false
    );
    assert.strictEqual(
      glob.parse("{**/baz,**/foo}")("baz/foo", "foo"),
      true
    );
    ...
    const hasSibling = (name) => siblings.indexOf(name) !== -1;
    assert.strictEqual(
      glob.parse(expr)("bar/baz.js", "baz.js", hasSibling),
      null
    );
    assert.strictEqual(
      glob.parse(expr)("bar/foo.js", "foo.js", hasSibling),
      "**/*.js"
    );
  });
  test("expression/pattern basename terms", () => {
    assert.deepStrictEqual(
      glob.getBasenameTerms(glob.parse("**/*.foo")),
      []
    );
    ...
    assert.deepStrictEqual(
      glob.getBasenameTerms(glob.parse("{**/baz,**/foo}")),
      ["baz", "foo"]
    );
    assert.deepStrictEqual(
      glob.getBasenameTerms(glob.parse("{**/baz/,**/foo/}")),
  ...
  test("expression/pattern optimization for basenames", () => {
    ...
    assert.deepStrictEqual(
      glob.getBasenameTerms(
        glob.parse("**/foo/**", { trimForExclusions: true })
      ),
      ["foo"]
    );
  ...
  function testOptimizationForBasenames(pattern, basenameTerms, matches, siblingsFns = []) {
    const parsed = glob.parse(pattern, {
  ...
  test("trailing slash", () => {
    ...
    assert.strictEqual(glob.parse("**/foo/")("bar/foo", "foo"), true);
    assert.strictEqual(
      glob.parse("**/*.foo/")("bar/file.baz", "file.baz"),
      false
    );
    assert.strictEqual(
      glob.parse("**/*.foo/")("bar/file.foo", "file.foo"),
      true
    );
    assert.strictEqual(
      glob.parse("{**/foo/,**/abc/}")("bar/baz", "baz"),
      false
    );
    assert.strictEqual(
      glob.parse("{**/foo/,**/abc/}")("bar/foo", "foo"),
      true
    );
    assert.strictEqual(
      glob.parse("{**/foo/,**/abc/}")("bar/abc", "abc"),
      true
    );
    assert.strictEqual(
      glob.parse("{**/foo/,**/abc/}", { trimForExclusions: true })(
        "bar/baz",
        "baz"
      ),
      false
  ...
  test("expression/pattern path", () => {
    assert.strictEqual(
      glob.parse("**/foo/bar")(nativeSep("foo/baz"), "baz"),
      false
    );
    assert.strictEqual(
      glob.parse("**/foo/bar")(nativeSep("foo/bar"), "bar"),
      true
    );
    assert.strictEqual(
    ...
    assert.strictEqual(
      glob.parse("**/foo/bar/**", { trimForExclusions: true })(
        nativeSep("bar/foo/bar"),
        "bar"
      ),
      true
    );
    ...
    assert.strictEqual(
      glob.parse("foo/bar/**")(nativeSep("foo/bar/baz"), "baz"),
      true
    );
    assert.strictEqual(
      glob.parse("foo/bar/**", { trimForExclusions: true })(
  ...
  test("expression/pattern paths", () => {
    ...
    assert.deepStrictEqual(glob.getPathTerms(parsed), [
      "*/foo/bar",
      "*/foo2/bar2"
    ]);
    assert.deepStrictEqual(glob.getBasenameTerms(parsed), [
      "bulb",
      "bulb2"
    ]);
    assert.deepStrictEqual(
  ...
  test("expression/pattern optimization for paths", () => {
    assert.deepStrictEqual(
      glob.getPathTerms(glob.parse("**/foo/bar/**")),
      []
    );
    assert.deepStrictEqual(
      glob.getPathTerms(
  ...
  test("URI match", () => {
    ...
    assertGlobMatch(
      p,
      URI.file("super/duper/long/some/file.md").with({ scheme: "scheme" }).toString()
    );
  });
  test("expression fails when siblings use promises (https://github.com/microsoft/vscode/issues/146294)", async () => {
    ...
    assert.strictEqual(
      "**/*.js",
      await parsedExpression("test.js", void 0, hasSibling)
    );
  });
  test("patternsEquals", () => {
