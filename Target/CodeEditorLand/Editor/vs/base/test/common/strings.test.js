...
suite("Strings", () => {
  ...
  test("compareIgnoreCase (substring)", () => {
    function assertCompareIgnoreCase(a, b, aStart, aEnd, bStart, bEnd, recurse = true) {
      let actual = strings.compareSubstringIgnoreCase(
        a,
        b,
        aStart,
        aEnd,
  ...
  test("format", () => {
    ...
    assert.strictEqual(
      strings.format("Foo {0} Bar {0}", "yes"),
      "Foo yes Bar yes"
    );
    assert.strictEqual(
      strings.format("Foo {0} Bar {1}{2}", "yes"),
      "Foo yes Bar {1}{2}"
    );
    assert.strictEqual(
      strings.format("Foo {0} Bar {1}{2}", "yes", void 0),
      "Foo yes Bar undefined{2}"
    );
    assert.strictEqual(
      strings.format("Foo {0} Bar {1}{2}", "yes", 5, false),
      "Foo yes Bar 5false"
    );
    assert.strictEqual(
  ...
  test("format2", () => {
    ...
    assert.strictEqual(
      strings.format2("Foo {oops} Bar", {}),
      "Foo {oops} Bar"
    );
    assert.strictEqual(
      strings.format2("Foo {foo} Bar", { foo: "bar" }),
      "Foo bar Bar"
    );
    assert.strictEqual(
      strings.format2("Foo {foo} Bar {foo}", { foo: "bar" }),
      "Foo bar Bar bar"
    );
    assert.strictEqual(
    ...
    assert.strictEqual(
      strings.format2("Foo {foo} Bar {bar}{boo}", {
        foo: "bar",
        bar: "undefined"
      }),
    ...
    assert.strictEqual(
      strings.format2("Foo {foo} Bar. {bar}", {
        foo: "(foo)",
        bar: ".test"
      }),
      "Foo (foo) Bar. .test"
  ...
  test("ltrim", () => {
    ...
    assert.strictEqual(
      strings.ltrim("http://www.test.de", "http://"),
      "www.test.de"
    );
  ...
  test("rtrim", () => {
    ...
    assert.strictEqual(
      strings.rtrim("http://www.test.de", ".de"),
      "http://www.test"
    );
  ...
  test("lastNonWhitespaceIndex", () => {
    ...
    assert.strictEqual(strings.lastNonWhitespaceIndex("abc  	 	 "), 2);
    assert.strictEqual(
      strings.lastNonWhitespaceIndex("abc  	 	 abc 	 	 "),
      11
    );
    assert.strictEqual(
      strings.lastNonWhitespaceIndex("abc  	 	 abc 	 	 ", 8),
      2
    );
  ...
  test("containsRTL", () => {
    ...
    assert.strictEqual(
      strings.containsRTL(strings.UTF8_BOM_CHARACTER + "a"),
      false
    );
    assert.strictEqual(strings.containsRTL("hello world!"), false);
    assert.strictEqual(strings.containsRTL("a\u{1F4DA}\u{1F4DA}b"), false);
    assert.strictEqual(
      strings.containsRTL("\u0647\u0646\u0627\u0643 \u062D\u0642\u064A\u0642\u0629 \u0645\u062B\u0628\u062A\u0629 \u0645\u0646\u0630 \u0632\u0645\u0646 \u0637\u0648\u064A\u0644"),
      true
    );
    assert.strictEqual(
      strings.containsRTL("\u05D6\u05D5\u05D4\u05D9 \u05E2\u05D5\u05D1\u05D3\u05D4 \u05DE\u05D1\u05D5\u05E1\u05E1\u05EA \u05E9\u05D3\u05E2\u05EA\u05D5"),
      true
    );
  });
  ...
  test("createRegExp", () => {
    ...
    assert.strictEqual(
      strings.createRegExp("([^ ,.]*)", false).source,
      "\\(\\[\\^ ,\\.\\]\\*\\)"
    );
    assert.strictEqual(
      strings.createRegExp("([^ ,.]*)", true).source,
      "([^ ,.]*)"
    );
    assert.strictEqual(
      strings.createRegExp("abc", false, { wholeWord: true }).source,
      "\\babc\\b"
    );
    assert.strictEqual(
      strings.createRegExp("abc", true, { wholeWord: true }).source,
      "\\babc\\b"
    );
    assert.strictEqual(
      strings.createRegExp(" abc", true, { wholeWord: true }).source,
  ...
  test("getLeadingWhitespace", () => {
    ...
    assert.strictEqual(
      strings.getLeadingWhitespace("		function foo(){", 0, 1),
      "	"
    );
    assert.strictEqual(
      strings.getLeadingWhitespace("		function foo(){", 0, 2),
      "		"
    );
  });
  test("fuzzyContains", () => {
  ...
  test("startsWithUTF8BOM", () => {
    ...
    assert(
      strings.startsWithUTF8BOM(
        strings.UTF8_BOM_CHARACTER + "aaaaaaaaaa"
      )
    );
    assert(!strings.startsWithUTF8BOM(" " + strings.UTF8_BOM_CHARACTER));
  ...
  test("stripUTF8BOM", () => {
    assert.strictEqual(
      strings.stripUTF8BOM(strings.UTF8_BOM_CHARACTER),
      ""
    );
    assert.strictEqual(
      strings.stripUTF8BOM(strings.UTF8_BOM_CHARACTER + "foobar"),
      "foobar"
    );
    assert.strictEqual(
  ...
  test("containsUppercaseCharacter", () => {
    [
      ...
      assert.strictEqual(
        strings.containsUppercaseCharacter(str),
        result,
  ...
  test("containsUppercaseCharacter (ignoreEscapedChars)", () => {
    [
      ...
      assert.strictEqual(
        strings.containsUppercaseCharacter(str, true),
        result,
        `Wrong result for ${str}`
      );
    });
  ...
  test("uppercaseFirstLetter", () => {
    [
      ...
      assert.strictEqual(
        strings.uppercaseFirstLetter(inStr),
        result,
        `Wrong result for ${inStr}`
      );
    });
  });
  test("getNLines", () => {
  ...
  test("getGraphemeBreakType", () => {
    assert.strictEqual(
      strings.getGraphemeBreakType(3009),
      strings.GraphemeBreakType.SpacingMark
    );
  });
  test("truncate", () => {
  ...
  test("truncateMiddle", () => {
    assert.strictEqual(
      "hello world",
      strings.truncateMiddle("hello world", 100)
    );
  ...
  test("removeAnsiEscapeCodesFromPrompt", () => {
    assert.strictEqual(
      strings.removeAnsiEscapeCodesFromPrompt("\x1B[31m$ \x1B[0m"),
      "$ "
    );
    assert.strictEqual(
  ...
  test("containsAmbiguousCharacter", () => {
    assert.strictEqual(
      strings.AmbiguousCharacters.getInstance(
        /* @__PURE__ */ new Set()
      ).containsAmbiguousCharacter("abcd"),
      false
    );
    assert.strictEqual(
      strings.AmbiguousCharacters.getInstance(
        /* @__PURE__ */ new Set()
      ).containsAmbiguousCharacter("\xFC\xE5"),
      false
    );
    assert.strictEqual(
      strings.AmbiguousCharacters.getInstance(
    ...
    assert.strictEqual(
      strings.AmbiguousCharacters.getInstance(
        /* @__PURE__ */ new Set()
      ).containsAmbiguousCharacter("\u03BF"),
      true
    );
    assert.strictEqual(
      strings.AmbiguousCharacters.getInstance(
        /* @__PURE__ */ new Set()
      ).containsAmbiguousCharacter("ab\u0261c"),
      true
    );
  });
  test("containsInvisibleCharacter", () => {
    assert.strictEqual(
      strings.InvisibleCharacters.containsInvisibleCharacter("abcd"),
      false
    );
    assert.strictEqual(
...
test("htmlAttributeEncodeValue", () => {
  ...
  assert.strictEqual(
    strings.htmlAttributeEncodeValue("Hello & World"),
    "Hello &amp; World"
  );
  assert.strictEqual(
    strings.htmlAttributeEncodeValue('"Hello"'),
    "&quot;Hello&quot;"
  );
  assert.strictEqual(
    strings.htmlAttributeEncodeValue("'Hello'"),
  ...
  assert.strictEqual(
    strings.htmlAttributeEncodeValue(`<>&'"`),
    "&lt;&gt;&amp;&apos;&quot;"
  );
});
