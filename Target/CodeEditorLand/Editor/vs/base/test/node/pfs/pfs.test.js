...
flakySuite("PFS", () => {
  ...
  test("writeFile", async () => {
    ...
    assert.strictEqual(
      (await fs.promises.readFile(testFile)).toString(),
      "Hello World"
    );
  });
  test("writeFile - parallel write on different files works", async () => {
    ...
    assert.strictEqual(
      fs.readFileSync(testFile1).toString(),
      "Hello World 1"
    );
    assert.strictEqual(
      fs.readFileSync(testFile2).toString(),
      "Hello World 2"
    );
    assert.strictEqual(
      fs.readFileSync(testFile3).toString(),
      "Hello World 3"
    );
    assert.strictEqual(
      fs.readFileSync(testFile4).toString(),
      "Hello World 4"
    );
    assert.strictEqual(
      fs.readFileSync(testFile5).toString(),
      "Hello World 5"
    );
  });
  test("writeFile - parallel write on same files works and is sequentalized", async () => {
    ...
    await Promise.all([
      ...
      timeout(10).then(
        () => Promises.writeFile(testFile, "Hello World 3", void 0)
      ),
      Promises.writeFile(testFile, "Hello World 4", void 0),
      timeout(10).then(
        () => Promises.writeFile(testFile, "Hello World 5", void 0)
      )
    ]);
    assert.strictEqual(
  ...
  test("rimraf - recursive folder structure - unlink", async () => {
    ...
    fs.writeFileSync(
      join(testDir, "somefolder", "somefile.txt"),
      "Contents"
    );
    await Promises.rm(testDir);
    assert.ok(!fs.existsSync(testDir));
  ...
  test("rimrafSync - swallows file not found error", () => {
  ...
  test("copy handles symbolic links", async () => {
    ...
    if (!isWindows) {
      await Promises.copy(symLink, copyTarget, {
        preserveSymlinks: true
      });
  ...
  test("copy handles symbolic links when the reference is inside source", async () => {
    ...
    const sourceLinkMD5JSFolderLinked = join(
      sourceLinkTestFolder,
      "md5-linked"
    );
    fs.symlinkSync(
    ...
    const targetLinkMD5JSFolderLinked = join(
      targetLinkTestFolder,
      "md5-linked"
    );
    if (!isWindows) {
      await Promises.copy(sourceLinkTestFolder, targetLinkTestFolder, {
        preserveSymlinks: true
      });
      ...
      assert.ok(
        fs.lstatSync(targetLinkMD5JSFolderLinked).isSymbolicLink()
      );
      const linkTarget = await fs.promises.readlink(
        targetLinkMD5JSFolderLinked
      );
  ...
  test("readdir (with file types)", async () => {
    if (typeof process.versions["electron"] !== "undefined") {
      ...
      assert.ok(fs.existsSync(newDir));
      const children = await Promises.readdir(testDir, {
        withFileTypes: true
      });
      assert.strictEqual(
        children.some((n) => n.name === "\xF6\xE4\xFC"),
        true
      );
      assert.strictEqual(
