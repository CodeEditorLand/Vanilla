...
class AppInsightsCoreMock {
  pluginVersionString = "Test Runner";
  events = [];
...
suite("AIAdapter", () => {
  ...
  test("Different data types", () => {
    ...
    assert.strictEqual(
      appInsightsMock.events[0].properties["favoriteColor"],
      "blue"
    );
    assert.strictEqual(
      appInsightsMock.events[0].measurements["likeRed"],
      0
    );
    assert.strictEqual(
      appInsightsMock.events[0].measurements["likeBlue"],
      1
    );
    assert.strictEqual(
      appInsightsMock.events[0].properties["favoriteDate"],
      date.toISOString()
    );
    assert.strictEqual(
    ...
    assert.strictEqual(
      appInsightsMock.events[0].measurements["favoriteNumber"],
      1
    );
  });
  test("Nested data", () => {
    ...
    assert.strictEqual(
      appInsightsMock.events[0].properties["window.title"],
      "some title"
    );
    assert.strictEqual(
      appInsightsMock.events[0].measurements["window.measurements.width"],
      100
    );
    assert.strictEqual(
      appInsightsMock.events[0].measurements["window.measurements.height"],
      200
    );
    assert.strictEqual(
    ...
    assert.strictEqual(
      appInsightsMock.events[0].measurements["nestedObj.testMeasurement"],
      1
    );
  });
});
