...
import {
  TelemetryService
} from "../../common/telemetryService.js";
import {
  NullAppender
} from "../../common/telemetryUtils.js";
...
suite("TelemetryService", () => {
  ...
  test(
    ...
    sinonTestFn(() => {
      ...
      assert.notStrictEqual(testAppender.events[0].data, null);
      assert.strictEqual(
        testAppender.events[0].data["stringProp"],
        "property"
      );
      assert.strictEqual(testAppender.events[0].data["numberProp"], 1);
      assert.strictEqual(
        testAppender.events[0].data["booleanProp"],
        true
      );
      assert.strictEqual(
        testAppender.events[0].data["complexProp"].value,
        0
      );
      service.dispose();
    })
  );
  test("common properties added to *all* events, simple event", () => {
    const testAppender = new TestTelemetryAppender();
    const service = new TelemetryService(
      {
        appenders: [testAppender],
  ...
  test("common properties added to *all* events, event with data", () => {
    const testAppender = new TestTelemetryAppender();
    const service = new TelemetryService(
      {
        appenders: [testAppender],
  ...
  test("TelemetryInfo comes from properties", () => {
    const service = new TelemetryService(
      {
  ...
  test(
    ...
    sinonTestFn(function() {
      ...
      try {
        ...
        assert.strictEqual(testAppender.getEventsCount(), 1);
        assert.strictEqual(
          testAppender.events[0].eventName,
          "UnhandledError"
        );
        assert.strictEqual(
          testAppender.events[0].data.msg,
          "This is a test."
        );
  ...
  test(
    ...
    sinonTestFn(function() {
      ...
      assert.strictEqual(
        testAppender.events[0].data.uncaught_error_msg,
        "test"
      );
      errorTelemetry.dispose();
      service.dispose();
  ...
  test(
    ...
    sinonTestFn(function() {
      ...
      try {
        ...
        const errorTelemetry = new ErrorTelemetry(service);
        const dangerousPathWithoutImportantInfoError = new Error(
        ...
        this.clock.tick(ErrorTelemetry.ERROR_FLUSH_TIMEOUT);
        assert.strictEqual(
          testAppender.events[0].data.msg.indexOf(
            settings.personalInfo
          ),
          -1
        );
  ...
  test(
    ...
    sinonTestFn(function() {
      ...
      try {
        ...
        const errorTelemetry = new ErrorTelemetry(service);
        const dangerousPathWithImportantInfoError = new Error(
          settings.dangerousPathWithImportantInfo
        );
  ...
  test(
    "Telemetry Service sends events when telemetry is on",
    sinonTestFn(() => {
      const testAppender = new TestTelemetryAppender();
      const service = new TelemetryService(
        { appenders: [testAppender] },
  ...
  test("Telemetry Service checks with config service", () => {
