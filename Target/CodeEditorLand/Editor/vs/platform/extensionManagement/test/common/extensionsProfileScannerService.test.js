...
suite("ExtensionsProfileScannerService", () => {
  ...
  setup(async () => {
    ...
    const uriIdentityService = instantiationService.stub(
      IUriIdentityService,
      disposables.add(new UriIdentityService(fileService))
    );
    const environmentService = instantiationService.stub(
      IEnvironmentService,
      { userRoamingDataHome: ROOT, cacheHome: joinPath(ROOT, "cache") }
    );
  ...
  test("write extensions located in the same extensions folder", async () => {
    const testObject = disposables.add(
      instantiationService.createInstance(TestObject, extensionsLocation)
    );
    const extensionsManifest = joinPath(
      extensionsLocation,
      "extensions.json"
    );
    const extension = aExtension(
      "pub.a",
      joinPath(extensionsLocation, "pub.a-1.0.0")
    );
  ...
  test("write extensions located in the same extensions folder has relative location ", async () => {
    ...
    const actual = JSON.parse(
      (await instantiationService.get(IFileService).readFile(extensionsManifest)).value.toString()
    );
  ...
  test("extension in old format is read and migrated during write", async () => {
    ...
    const extension2 = aExtension(
      "pub.b",
      joinPath(extensionsLocation, "pub.b-1.0.0")
    );
    await testObject.addExtensionsToProfile(
      [[extension2, void 0]],
      extensionsManifest
  ...
  test("add extension trigger events", async () => {
    ...
    assert.deepStrictEqual(
      target1.args[0][0].profileLocation.toString(),
      extensionsManifest.toString()
    );
    assert.deepStrictEqual(
      target1.args[0][0].extensions.length,
      1
    );
    assert.deepStrictEqual(
      target1.args[0][0].extensions[0].identifier,
      extension.identifier
    );
    assert.deepStrictEqual(
      target1.args[0][0].extensions[0].version,
      extension.manifest.version
    );
    assert.deepStrictEqual(
      target1.args[0][0].extensions[0].location.toString(),
      extension.location.toString()
    );
    assert.ok(target2.calledOnce);
    assert.deepStrictEqual(
      target2.args[0][0].profileLocation.toString(),
      extensionsManifest.toString()
    );
    assert.deepStrictEqual(
      target2.args[0][0].extensions.length,
      1
    );
    assert.deepStrictEqual(
      target2.args[0][0].extensions[0].identifier,
      extension.identifier
    );
    assert.deepStrictEqual(
      target2.args[0][0].extensions[0].version,
      extension.manifest.version
    );
    assert.deepStrictEqual(
      target2.args[0][0].extensions[0].location.toString(),
      extension.location.toString()
    );
  });
  test("remove extension trigger events", async () => {
    ...
    await testObject.removeExtensionFromProfile(
      extension,
      extensionsManifest
    );
    const actual = await testObject.scanProfileExtensions(extensionsManifest);
  ...
  test("add extension with same id but different version", async () => {
    ...
    assert.deepStrictEqual(
      target1.args[0][0].extensions[0].version,
      extension2.manifest.version
    );
    assert.deepStrictEqual(
      target1.args[0][0].extensions[0].location.toString(),
      extension2.location.toString()
    );
    assert.ok(target2.calledOnce);
    assert.deepStrictEqual(
      target2.args[0][0].profileLocation.toString(),
    ...
    assert.deepStrictEqual(
      target2.args[0][0].extensions[0].identifier,
      extension1.identifier
    );
    assert.deepStrictEqual(
      target2.args[0][0].extensions[0].version,
      extension1.manifest.version
    );
    assert.deepStrictEqual(
      target2.args[0][0].extensions[0].location.toString(),
      extension1.location.toString()
    );
  ...
  test("add same extension with different metadata", async () => {
    ...
    disposables.add(testObject.onDidRemoveExtensions(target4));
    await testObject.addExtensionsToProfile(
      [[extension, { isApplicationScoped: true }]],
      extensionsManifest
    );
    const actual = await testObject.scanProfileExtensions(extensionsManifest);
    assert.deepStrictEqual(
  ...
  test("add extension with different version and metadata", async () => {
    ...
    disposables.add(testObject.onDidRemoveExtensions(target4));
    await testObject.addExtensionsToProfile(
      [[extension2, { isApplicationScoped: true }]],
      extensionsManifest
    );
    const actual = await testObject.scanProfileExtensions(extensionsManifest);
    assert.deepStrictEqual(
