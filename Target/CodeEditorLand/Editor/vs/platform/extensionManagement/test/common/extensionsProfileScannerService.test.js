...
suite("ExtensionsProfileScannerService", () => {
  ...
  test("write extensions located in the same extensions folder", async () => {
    ...
    await testObject.addExtensionsToProfile([[extension, void 0]], extensionsManifest);
    const actual = await testObject.scanProfileExtensions(extensionsManifest);
    assert.deepStrictEqual(actual.map((a) => ({ ...a, location: a.location.toJSON() })), [{ identifier: extension.identifier, location: extension.location.toJSON(), version: extension.manifest.version, metadata: void 0 }]);
  });
  ...
  test("extension in old format is read and migrated during write", async () => {
    ...
    await testObject.addExtensionsToProfile([[extension2, void 0]], extensionsManifest);
    const actual = await testObject.scanProfileExtensions(extensionsManifest);
    assert.deepStrictEqual(actual.map((a) => ({ ...a, location: a.location.toJSON() })), [
      { identifier: extension.identifier, location: extension.location.toJSON(), version: extension.manifest.version, metadata: void 0 },
      { identifier: extension2.identifier, location: extension2.location.toJSON(), version: extension2.manifest.version, metadata: void 0 }
    ]);
    const manifestContent = JSON.parse((await instantiationService.get(IFileService).readFile(extensionsManifest)).value.toString());
  ...
  test("add extension with same id but different version", async () => {
    ...
    await testObject.addExtensionsToProfile([[extension1, void 0]], extensionsManifest);
    const target1 = sinon.stub();
    const target2 = sinon.stub();
    ...
    const extension2 = aExtension("pub.a", joinPath(ROOT, "pub.a-2.0.0"), void 0, { version: "2.0.0" });
    await testObject.addExtensionsToProfile([[extension2, void 0]], extensionsManifest);
    const actual = await testObject.scanProfileExtensions(extensionsManifest);
    assert.deepStrictEqual(actual.map((a) => ({ ...a, location: a.location.toJSON() })), [{ identifier: extension2.identifier, location: extension2.location.toJSON(), version: extension2.manifest.version, metadata: void 0 }]);
    assert.ok(target1.calledOnce);
    assert.deepStrictEqual(target1.args[0][0].profileLocation.toString(), extensionsManifest.toString());
    assert.deepStrictEqual(target1.args[0][0].extensions.length, 1);
