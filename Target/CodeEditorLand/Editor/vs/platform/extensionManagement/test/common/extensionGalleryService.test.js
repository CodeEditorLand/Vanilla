...
suite("Extension Gallery Service", () => {
  ...
  setup(() => {
    ...
    environmentService = new EnvironmentServiceMock(
      serviceMachineIdResource
    );
    ...
    disposables.add(
      fileService.registerProvider(
        serviceMachineIdResource.scheme,
        fileSystemProvider
      )
    );
  ...
  test("marketplace machine id", async () => {
    ...
    assert.strictEqual(
      headers["X-Market-User-Id"],
      headers2["X-Market-User-Id"]
    );
  });
  ...
  test("sorting single extension version with not compatible target platform", async () => {
    const actual = [
      aExtensionVersion("1.1.2", TargetPlatform.DARWIN_ARM64)
    ];
    const expected = [...actual];
