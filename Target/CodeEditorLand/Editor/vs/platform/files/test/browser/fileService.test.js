...
suite("File Service", () => {
    ...
    disposables.add(
      service.onWillActivateFileSystemProvider((e) => {
        ...
        if (e.scheme === "test" && callCount === 1) {
          e.join(
            new Promise((resolve) => {
              registrationDisposable = service.registerProvider(
                "test",
                provider
              );
              resolve();
  ...
  test("watch", async () => {
    ...
    const watcher3Disposable2 = service.watch(resource3, {
      recursive: true,
      excludes: []
    });
  ...
  test("watch - with corelation", async () => {
    ...
    provider.fireFileChange([
      {
        resource: URI.parse("test://watch/folder1"),
        type: FileChangeType.ADDED
      }
    ]);
    provider.fireFileChange([
      {
        resource: URI.parse("test://watch/folder2"),
        type: FileChangeType.ADDED,
        cId: 100
      }
    ]);
    ...
    provider.fireFileChange([
      {
        resource: URI.parse("test://watch/folder3/file"),
    ...
    provider.fireFileChange([
      {
        resource: URI.parse("test://watch/folder3"),
        type: FileChangeType.UPDATED,
        cId: 200
      }
    ]);
    provider.fireFileChange([
      {
        resource: URI.parse("test://watch/folder4"),
        type: FileChangeType.ADDED,
        cId: 50
      }
    ]);
    ...
    const provider = new class extends NullFileSystemProvider {
      ...
      readFileStream(resource, opts, token) {
        if (async) {
          ...
          timeout(5, CancellationToken.None).then(
  ...
  test("readFile/readFileStream supports cancellation (https://github.com/microsoft/vscode/issues/138805)", async () => {
    ...
    try {
      ...
      const promise = service.readFile(
        URI.parse("test://foo/bar"),
        void 0,
        cts.token
      );
  ...
  test("enforced atomic read/write/delete", async () => {
    ...
    await service.writeFile(
      atomicResource,
      bufferToStream(VSBuffer.fromString(""))
    );
    await service.writeFile(
      nonAtomicResource,
      bufferToStream(VSBuffer.fromString(""))
    ...
    await service.writeFile(
      atomicResource,
      bufferToReadable(VSBuffer.fromString(""))
    );
    await service.writeFile(
      nonAtomicResource,
      bufferToReadable(VSBuffer.fromString(""))
