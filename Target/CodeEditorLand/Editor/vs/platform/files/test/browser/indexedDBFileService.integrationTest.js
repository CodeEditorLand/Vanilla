...
flakySuite("IndexedDBFileSystemProvider", () => {
  let service;
  let userdataFileProvider;
  const testDir = "/";
  const userdataURIFromPaths = (paths) => joinPath(
    URI.from({ scheme: Schemas.vscodeUserData, path: testDir }),
    ...paths
  );
  ...
  const initFixtures = async () => {
    ...
    await Promise.all(
      [
        [
          ["fixtures", "resolver", "examples", "company.js"],
          "class company {}"
        ],
        [
          ["fixtures", "resolver", "examples", "conway.js"],
          "export function conway() {}"
        ],
        [
          ["fixtures", "resolver", "examples", "employee.js"],
          'export const employee = "jax"'
        ],
        [["fixtures", "resolver", "examples", "small.js"], ""],
        [
          ["fixtures", "resolver", "other", "deep", "company.js"],
          "class company {}"
        ],
        [
          ["fixtures", "resolver", "other", "deep", "conway.js"],
          "export function conway() {}"
        ],
        [
          [
        ...
        [
          ["fixtures", "service", "deep", "company.js"],
          "class company {}"
        ],
        [
          ["fixtures", "service", "deep", "conway.js"],
          "export function conway() {}"
        ],
        [
          ["fixtures", "service", "deep", "employee.js"],
          'export const employee = "jax"'
        ],
        [["fixtures", "service", "deep", "small.js"], ""],
        [["fixtures", "service", "binary.txt"], "<p>p</p>"]
      ].map(
        ([path, contents]) => [userdataURIFromPaths(path), contents]
      ).map(
        ([uri, contents]) => service.createFile(uri, VSBuffer.fromString(contents))
      )
    );
  };
  const reload = async () => {
  ...
  test("createFolder", async () => {
    ...
    assert.strictEqual(
      (await userdataFileProvider.readdir(parent.resource)).length,
      0
    );
  ...
  test("createFolder: creating multiple folders at once", async () => {
    ...
    const parent = await service.resolve(userdataURIFromPaths([]));
    const newFolderResource = joinPath(
      parent.resource,
      ...multiFolderPaths
    );
    const newFolder = await service.createFolder(newFolderResource);
    ...
    assert.strictEqual(event.operation, FileOperation.CREATE);
    assert.strictEqual(
      event.target.resource.path,
      newFolderResource.path
    );
    assert.strictEqual(event.target.isDirectory, true);
  });
  test("exists", async () => {
  ...
  test("resolve - file", async () => {
    await initFixtures();
    const resource = userdataURIFromPaths([
      "fixtures",
      "resolver",
      "index.html"
    ]);
  ...
  async function assertCreateFile(converter) {
    ...
    assert.strictEqual(
      (await userdataFileProvider.stat(fileStat.resource)).type,
      FileType.File
    );
    assert.strictEqual(
      new TextDecoder().decode(
        await userdataFileProvider.readFile(fileStat.resource)
      ),
      contents
    );
    assert.ok(event);
    ...
    return {
      async create() {
        return creationPromises = Promise.all(
          batch.map(
            (entry) => userdataFileProvider.writeFile(
              entry.resource,
      ...
      async assertContentsCorrect() {
        ...
        await Promise.all(
          batch.map(async (entry, i) => {
            ...
            assert.strictEqual(
              new TextDecoder().decode(
                await userdataFileProvider.readFile(
                  entry.resource
                )
              ),
              entry.contents
  ...
  test("createFile - batch (mixed parallel/sequential)", async () => {
    ...
    batch2.create();
    await Promise.all([
      batch1.assertContentsCorrect(),
      batch2.assertContentsCorrect()
    ]);
    batch3.create();
    batch4.create();
    await Promise.all([
      batch3.assertContentsCorrect(),
      batch4.assertContentsCorrect()
    ]);
    await Promise.all([
      batch1.assertContentsCorrect(),
      batch2.assertContentsCorrect()
  ...
  test("rename to an existing file without overwrite", async () => {
    ...
    await service.writeFile(
      sourceFile,
      VSBuffer.fromString("This is source file")
    );
    const targetFile = joinPath(parent.resource, "targetFile");
    await service.writeFile(
      targetFile,
      VSBuffer.fromString("This is target file")
    );
    try {
      await service.move(sourceFile, targetFile, false);
    } catch (error) {
  ...
  test("rename a folder that has multiple files and folders", async () => {
    ...
    await Promise.all([
      service.writeFile(
        sourceFile1,
        VSBuffer.fromString("Source File 1")
      ),
      service.writeFile(
        sourceFile2,
        VSBuffer.fromString("Source File 2")
      ),
    ...
    assert.deepStrictEqual(await service.exists(targetFolder), true);
    assert.strictEqual(
      (await service.readFile(targetFile1)).value.toString(),
      "Source File 1"
    );
    assert.strictEqual(
      (await service.readFile(targetFile2)).value.toString(),
      "Source File 2"
    );
  ...
  test("rename a folder to another folder that has some files", async () => {
    ...
    await Promise.all([
      ...
      service.writeFile(
        targetFile2,
        VSBuffer.fromString("Target File 2")
      ),
      service.writeFile(
        targetFile3,
        VSBuffer.fromString("Target File 3")
      )
    ]);
    await service.move(sourceFolder, targetFolder, true);
    assert.deepStrictEqual(await service.exists(sourceFolder), false);
  ...
  test("deleteFile", async () => {
    ...
    let event;
    disposables.add(service.onDidRunOperation((e) => event = e));
    const anotherResource = userdataURIFromPaths([
      "fixtures",
      "service",
      "deep",
      "company.js"
    ...
    assert.strictEqual(
      await service.canDelete(source.resource, { useTrash: false }),
      true
    );
  ...
  test("deleteFolder (recursive)", async () => {
    ...
    const subResource1 = userdataURIFromPaths([
      "fixtures",
      "service",
      "deep",
      "company.js"
    ]);
    const subResource2 = userdataURIFromPaths([
      "fixtures",
      "service",
      "deep",
      "conway.js"
  ...
  test("deleteFolder with folders and files (recursive)", async () => {
    ...
    assert.deepStrictEqual(
      await service.exists(joinPath(targetFolder, "folder1")),
      false
    );
    assert.deepStrictEqual(
      await service.exists(joinPath(targetFolder, "folder2")),
      false
    );
