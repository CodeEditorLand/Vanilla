...
flakySuite("IndexedDBFileSystemProvider", function() {
  ...
  const testDir = "/";
  const userdataURIFromPaths = /* @__PURE__ */ __name((paths) => joinPath(URI.from({ scheme: Schemas.vscodeUserData, path: testDir }), ...paths), "userdataURIFromPaths");
  const disposables = new DisposableStore();
  const initFixtures = /* @__PURE__ */ __name(async () => {
    await Promise.all(
      [
  ...
  const fileCreateBatchTester = /* @__PURE__ */ __name((size, name) => {
