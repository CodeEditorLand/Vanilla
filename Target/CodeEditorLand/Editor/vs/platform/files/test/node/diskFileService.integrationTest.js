  ...
  () => {
    ...
    setup(async () => {
      ...
      fileProvider = disposables.add(
        new TestDiskFileSystemProvider(logService)
      );
      disposables.add(
        service.registerProvider(Schemas.file, fileProvider)
      );
      testProvider = disposables.add(
        new TestDiskFileSystemProvider(logService)
      );
      ...
      await Promises.copy(sourceDir, testDir, {
    ...
    test("createFolder", async () => {
      ...
      const newFolderResource = URI.file(
        join(parent.resource.fsPath, "newFolder")
      );
      ...
      assert.strictEqual(
        event.target.resource.fsPath,
        newFolderResource.fsPath
      );
    ...
    test("createFolder: creating multiple folders at once", async () => {
      ...
      const parent = await service.resolve(URI.file(testDir));
      const newFolderResource = URI.file(
        join(parent.resource.fsPath, ...multiFolderPaths)
      );
      const newFolder = await service.createFolder(newFolderResource);
    ...
    test("resolve - file", async () => {
      const resource = FileAccess.asFileUri(
        "vs/platform/files/test/node/fixtures/resolver/index.html"
      );
    ...
    test("resolve - directory", async () => {
      ...
      const resource = FileAccess.asFileUri(
        "vs/platform/files/test/node/fixtures/resolver"
      );
      ...
      result.children.forEach((value) => {
        ...
        if (["examples", "other"].indexOf(
          basename(value.resource.fsPath)
        ) >= 0) {
          ...
          assert.fail(
            "Unexpected value " + basename(value.resource.fsPath)
          );
        }
      });
    });
    ...
    test("resolve - directory - resolveTo single directory", async () => {
      const resolverFixturesPath = FileAccess.asFileUri(
        "vs/platform/files/test/node/fixtures/resolver"
      ).fsPath;
      const result = await service.resolve(
        URI.file(resolverFixturesPath),
        {
          resolveTo: [
    ...
    async function testResolveDirectoryWithTarget(withQueryParam) {
      ...
      const result = await service.resolve(
        URI.file(resolverFixturesPath).with({
          query: withQueryParam ? "test" : void 0
        }),
        {
          resolveTo: [
    ...
    test("resolve directory - resolveSingleChildFolders", async () => {
      const resolverFixturesPath = FileAccess.asFileUri(
        "vs/platform/files/test/node/fixtures/resolver/other"
      ).fsPath;
      const result = await service.resolve(
        URI.file(resolverFixturesPath),
    ...
    test("resolves", async () => {
      const res = await service.resolveAll([
        {
          resource: URI.file(testDir),
          options: { resolveTo: [URI.file(join(testDir, "deep"))] }
        },
    ...
    test("resolve - symbolic link pointing to nonexistent file does not break", async () => {
      ...
      const resolvedLink = resolved.children?.find(
        (child) => child.name === "bar" && child.isSymbolicLink
      );
    ...
    async function testDeleteFile(useTrash, recursive) {
      ...
      assert.strictEqual(
        await service.canDelete(source.resource, {
          useTrash,
          recursive
        }),
        true
    ...
    (isWindows ? test.skip : test)(
      "deleteFile - symbolic link (pointing to nonexistent file)",
      async () => {
    ...
    test("deleteFolder (non recursive)", async () => {
      ...
      const source = await service.resolve(resource);
      assert.ok(
        await service.canDelete(source.resource) instanceof Error
      );
      let error;
      try {
        await service.del(source.resource);
    ...
    async function testDeleteEmptyFolder(recursive) {
      const { resource } = await service.createFolder(
        URI.file(join(testDir, "deep", "empty"))
      );
    ...
    test("move", async () => {
      ...
      assert.strictEqual(
        event.target.resource.fsPath,
        renamed.resource.fsPath
      );
      const targetContents = readFileSync(target.fsPath);
      assert.strictEqual(
        sourceContents.byteLength,
        targetContents.byteLength
      );
      assert.strictEqual(
        sourceContents.toString(),
        targetContents.toString()
      );
    });
    ...
    async function testMoveAcrossProviders(sourceFile = "index.html") {
      ...
      const target = URI.file(
        join(dirname(source.fsPath), "other.html")
      ).with({ scheme: testSchema });
      assert.strictEqual(await service.canMove(source, target), true);
      const renamed = await service.move(source, target);
      assert.strictEqual(existsSync(renamed.resource.fsPath), true);
      assert.strictEqual(existsSync(source.fsPath), false);
    ...
    test("move - multi folder", async () => {
      ...
      const source = URI.file(join(testDir, "index.html"));
      assert.strictEqual(
        await service.canMove(
          source,
          URI.file(join(dirname(source.fsPath), renameToPath))
        ),
        true
      );
      const renamed = await service.move(
        source,
        URI.file(join(dirname(source.fsPath), renameToPath))
      );
      assert.strictEqual(existsSync(renamed.resource.fsPath), true);
    ...
    test("move - directory", async () => {
      ...
      assert.strictEqual(
        await service.canMove(
          source,
          URI.file(join(dirname(source.fsPath), "deeper"))
        ),
        true
      );
      const renamed = await service.move(
    ...
    async function testMoveFolderAcrossProviders() {
      ...
      const target = URI.file(
        join(dirname(source.fsPath), "deeper")
      ).with({ scheme: testSchema });
      assert.strictEqual(await service.canMove(source, target), true);
      const renamed = await service.move(source, target);
      assert.strictEqual(existsSync(renamed.resource.fsPath), true);
    ...
    test("move - MIX CASE", async () => {
      ...
      const source = await service.resolve(
        URI.file(join(testDir, "index.html")),
        { resolveMetadata: true }
      );
      assert.ok(source.size > 0);
      const renamedResource = URI.file(
        join(dirname(source.resource.fsPath), "INDEX.html")
      );
      assert.strictEqual(
        await service.canMove(source.resource, renamedResource),
        true
      );
      ...
      assert.strictEqual(
        event.target.resource.fsPath,
        renamedResource.fsPath
      );
      renamed = await service.resolve(renamedResource, {
        resolveMetadata: true
      });
    ...
    test("move - same file", async () => {
      ...
      assert.strictEqual(
        await service.canMove(
          source.resource,
          URI.file(source.resource.fsPath)
        ),
        true
      );
      let renamed = await service.move(
        source.resource,
        URI.file(source.resource.fsPath)
      );
      assert.strictEqual(existsSync(renamed.resource.fsPath), true);
      ...
      renamed = await service.resolve(renamed.resource, {
        resolveMetadata: true
      });
      assert.strictEqual(source.size, renamed.size);
    });
    test("move - same file #2", async () => {
      ...
      const target = targetParent.with({
        path: posix.join(
          targetParent.path,
          posix.basename(source.resource.path)
        )
      });
      assert.strictEqual(
        await service.canMove(source.resource, target),
        true
      );
    ...
    test("move - source parent of target", async () => {
      let event;
      disposables.add(service.onDidRunOperation((e) => event = e));
      let source = await service.resolve(
        URI.file(join(testDir, "index.html")),
        { resolveMetadata: true }
      );
      ...
      assert.ok(
        await service.canMove(
          URI.file(testDir),
          URI.file(join(testDir, "binary.txt"))
        ) instanceof Error
      );
      let error;
      try {
      ...
      assert.ok(error);
      assert.ok(!event);
      source = await service.resolve(source.resource, {
        resolveMetadata: true
      });
    ...
    test("move - overwrite folder with file", async () => {
      ...
      const parent = await service.resolve(URI.file(testDir));
      const folderResource = URI.file(
        join(parent.resource.fsPath, "conway.js")
      );
      const f = await service.createFolder(folderResource);
      const source = URI.file(join(testDir, "deep", "conway.js"));
      assert.strictEqual(
        await service.canMove(source, f.resource, true),
        true
      );
      ...
      assert.strictEqual(
        moveEvent.target.resource.fsPath,
        moved.resource.fsPath
      );
      assert.strictEqual(
        deleteEvent.resource.fsPath,
        folderResource.fsPath
      );
    });
    test("copy", async () => {
      await doTestCopy();
    ...
    async function doTestCopy(sourceName = "index.html") {
      ...
      disposables.add(service.onDidRunOperation((e) => event = e));
      const source = await service.resolve(
        URI.file(join(testDir, sourceName))
      );
      const target = URI.file(join(testDir, "other.html"));
      assert.strictEqual(
        await service.canCopy(source.resource, target),
        true
      );
    ...
    test("copy - overwrite folder with file", async () => {
      ...
      const source = URI.file(join(testDir, "deep", "conway.js"));
      assert.strictEqual(
        await service.canCopy(source, f.resource, true),
        true
      );
      ...
      assert.strictEqual(
        copyEvent.target.resource.fsPath,
        copied.resource.fsPath
      );
      assert.strictEqual(
        deleteEvent.resource.fsPath,
    ...
    test("copy - MIX CASE same target - overwrite", async () => {
      ...
      const canCopy = await service.canCopy(
        source.resource,
        target,
        true
      );
    ...
    test("copy - MIX CASE different target - overwrite", async () => {
      const source1 = await service.resolve(
        URI.file(join(testDir, "index.html")),
        { resolveMetadata: true }
      );
      ...
      const renamed = await service.move(
        source1.resource,
        URI.file(join(dirname(source1.resource.fsPath), "CONWAY.js"))
      );
      assert.strictEqual(existsSync(renamed.resource.fsPath), true);
      assert.ok(readdirSync(testDir).some((f) => f === "CONWAY.js"));
      assert.strictEqual(source1.size, renamed.size);
      const source2 = await service.resolve(
        URI.file(join(testDir, "deep", "conway.js")),
        { resolveMetadata: true }
      );
      const target = URI.file(
        join(testDir, basename(source2.resource.path))
      );
      assert.strictEqual(
        await service.canCopy(source2.resource, target, true),
        true
      );
    ...
    test("copy - same file", async () => {
      ...
      assert.ok(source.size > 0);
      assert.strictEqual(
        await service.canCopy(
          source.resource,
          URI.file(source.resource.fsPath)
        ),
        true
      );
      let copied = await service.copy(
        source.resource,
        URI.file(source.resource.fsPath)
      );
      assert.strictEqual(existsSync(copied.resource.fsPath), true);
      ...
      copied = await service.resolve(source.resource, {
        resolveMetadata: true
      });
      assert.strictEqual(source.size, copied.size);
    });
    test("copy - same file #2", async () => {
      ...
      assert.strictEqual(
        await service.canCopy(source.resource, URI.file(target.fsPath)),
        true
      );
      let copied = await service.copy(
        source.resource,
        URI.file(target.fsPath)
      );
      assert.strictEqual(existsSync(copied.resource.fsPath), true);
      assert.strictEqual(basename(copied.resource.fsPath), "index.html");
      assert.ok(event);
    ...
    test("cloneFile - via copy capability", () => {
      setCapabilities(
        fileProvider,
        FileSystemProviderCapabilities.FileOpenReadWriteClose | FileSystemProviderCapabilities.FileFolderCopy
      );
      return testCloneFile();
    });
    ...
    async function testCloneFile() {
      ...
      const target1 = targetParent.with({
        path: posix.join(
          targetParent.path,
          `${posix.basename(source1.path)}-clone`
        )
      });
    ...
    test("readFile - small file - streamed / readonly", async () => {
      setCapabilities(
        fileProvider,
        FileSystemProviderCapabilities.FileReadStream | FileSystemProviderCapabilities.Readonly
      );
      return testReadFile(URI.file(join(testDir, "small.txt")));
    });
    ...
    test("readFile - atomic (natively supported)", async () => {
      setCapabilities(
        fileProvider,
        FileSystemProviderCapabilities.FileReadWrite & FileSystemProviderCapabilities.FileAtomicRead
      );
      return testReadFile(URI.file(join(testDir, "lorem.txt")), {
        atomic: true
      });
    ...
    async function testReadFileStream(resource) {
      ...
      assert.strictEqual(
        (await streamToBuffer(content.value)).toString(),
        readFileSync(resource.fsPath).toString()
      );
    }
    ...
    async function testReadFileFromPositionUmlaut() {
      ...
      const contents = await service.readFile(resource, {
        position: Buffer.from("Small File with \xDC").length
      });
    ...
    async function testNotModifiedSince() {
      ...
      assert.ok(
        error instanceof NotModifiedSinceFileOperationError && error.stat
      );
    ...
    (isWindows ? test.skip : test)(
      "readFile - dangling symbolic link - https://github.com/microsoft/vscode/issues/116049",
      async () => {
    ...
    test("createFile", async () => {
      return assertCreateFile(
        (contents) => VSBuffer.fromString(contents)
      );
    });
    test("createFile (readable)", async () => {
      return assertCreateFile(
        (contents) => bufferToReadable(VSBuffer.fromString(contents))
      );
    });
    test("createFile (stream)", async () => {
      return assertCreateFile(
        (contents) => bufferToStream(VSBuffer.fromString(contents))
      );
    });
    async function assertCreateFile(converter) {
      ...
      const fileStat = await service.createFile(
        resource,
        converter(contents)
      );
      ...
      assert.strictEqual(
        readFileSync(fileStat.resource.fsPath).toString(),
        contents
      );
      assert.ok(event);
      assert.strictEqual(event.resource.fsPath, resource.fsPath);
    ...
    test("createFile (allows to overwrite existing)", async () => {
      ...
      writeFileSync(resource.fsPath, "");
      assert.strictEqual(
        await service.canCreateFile(resource, { overwrite: true }),
        true
      );
      const fileStat = await service.createFile(
        resource,
    ...
    test("writeFile - buffered (atomic)", async () => {
      setCapabilities(
        fileProvider,
        FileSystemProviderCapabilities.FileOpenReadWriteClose | FileSystemProviderCapabilities.FileAtomicWrite
      );
      let e;
      try {
    ...
    test("writeFile - unbuffered (atomic)", async () => {
      setCapabilities(
        fileProvider,
        FileSystemProviderCapabilities.FileReadWrite | FileSystemProviderCapabilities.FileAtomicWrite
      );
      return testWriteFile(true);
    });
    (isWindows ? test.skip : test)(
      "writeFile - atomic writing does not break symlinks",
      async () => {
        const link = URI.file(join(testDir, "lorem.txt-linked"));
        await promises.symlink(join(testDir, "lorem.txt"), link.fsPath);
        const content = "Updates to the lorem file";
        await service.writeFile(link, VSBuffer.fromString(content), {
          atomic: { postfix: ".vsctmp" }
        });
        assert.strictEqual(
          readFileSync(link.fsPath).toString(),
          content
        );
        const resolved = await service.resolve(link);
        assert.strictEqual(resolved.isSymbolicLink, true);
    ...
    async function testWriteFileLarge(atomic) {
      ...
      const fileStat = await service.writeFile(
        resource,
    ...
    test("writeFile (large file) - unbuffered (atomic) - concurrent writes with multiple services", async () => {
      ...
      let suffix = 0;
      for (let i = 0; i < 10; i++) {
        const service2 = disposables.add(
          new FileService(new NullLogService())
        );
        disposables.add(
          service2.registerProvider(Schemas.file, fileProvider)
        );
        promises2.push(
          service2.writeFile(
            resource,
            VSBuffer.fromString(`${newContent}${++suffix}`),
            { atomic: { postfix: ".vsctmp" } }
          )
        );
        await timeout(0);
    ...
    test("writeFile (large file) - multiple parallel writes queue up and atomic read support (via file service)", async () => {
      ...
      const writePromises = Promise.all(
        ["0", "00", "000", "0000", "00000"].map(async (offset) => {
          const fileStat = await service.writeFile(
            resource,
            VSBuffer.fromString(offset + newContent)
          );
          assert.strictEqual(fileStat.name, "lorem.txt");
        })
      );
      const readPromises = Promise.all(
        ["0", "00", "000", "0000", "00000"].map(async () => {
          const fileContent = await service.readFile(resource, {
            atomic: true
          });
    ...
    test("provider - write barrier prevents dirty writes", async () => {
      ...
      const writePromises = Promise.all(
        ["0", "00", "000", "0000", "00000"].map(async (offset) => {
          ...
          const fd = await provider.open(resource, {
            create: true,
            unlock: false
          });
          try {
            await provider.write(
              fd,
              0,
              VSBuffer.fromString(content2).buffer,
              0,
              contentBuffer.byteLength
            );
            assert.strictEqual(
              (await promises.readFile(resource.fsPath)).toString(),
              content2
            );
          } finally {
    ...
    test("provider - write barrier is partitioned per resource", async () => {
      ...
      assert.ok(provider);
      assert.ok(hasOpenReadWriteCloseCapability(provider));
      const fd1 = await provider.open(resource1, {
        create: true,
        unlock: false
      });
      const fd2 = await provider.open(resource2, {
        create: true,
        unlock: false
      });
      ...
      try {
        await provider.write(
          fd1,
          0,
          VSBuffer.fromString(newContent).buffer,
          0,
          VSBuffer.fromString(newContent).buffer.byteLength
        );
        assert.strictEqual(
          (await promises.readFile(resource1.fsPath)).toString(),
          newContent
        );
        await provider.write(
          fd2,
        ...
        assert.strictEqual(
          (await promises.readFile(resource2.fsPath)).toString(),
          newContent
        );
      } finally {
    ...
    test("provider - write barrier not becoming stale", async () => {
      ...
      let error;
      try {
        await provider.open(newResource, {
          create: true,
          unlock: false
        });
      } catch (e) {
      ...
      const content = readFileSync(
        URI.file(join(testDir, "lorem.txt")).fsPath
      );
      const newContent = content.toString() + content.toString();
      const newContentBuffer = VSBuffer.fromString(newContent).buffer;
      const fd = await provider.open(newResource, {
        create: true,
        unlock: false
      });
      try {
        await provider.write(
          ...
          newContentBuffer,
          0,
          newContentBuffer.byteLength
        );
        assert.strictEqual(
          (await promises.readFile(newResource.fsPath)).toString(),
          newContent
        );
      } finally {
        await provider.close(fd);
    ...
    test("provider - atomic reads (write pending when read starts)", async () => {
      ...
      let atomicReadPromise;
      const fd = await provider.open(resource, {
        create: true,
        unlock: false
      });
      try {
        atomicReadPromise = provider.readFile(resource, {
          atomic: true
        });
        await timeout(20);
        await provider.write(
    ...
    test("provider - atomic reads (read pending when write starts)", async () => {
      ...
      assert.ok(hasOpenReadWriteCloseCapability(provider));
      assert.ok(hasFileAtomicReadCapability(provider));
      let atomicReadPromise = provider.readFile(resource, {
        atomic: true
      });
    ...
    async function testWriteFileLargeReadable() {
      ...
      const fileStat = await service.writeFile(
        resource,
        toLineByLineReadable(newContent)
      );
      assert.strictEqual(fileStat.name, "lorem.txt");
      assert.strictEqual(
        readFileSync(resource.fsPath).toString(),
    ...
    test("writeFile - locked files and unlocking", async () => {
      setCapabilities(
        fileProvider,
        FileSystemProviderCapabilities.FileReadWrite | FileSystemProviderCapabilities.FileWriteUnlock
      );
    ...
    test("writeFile (stream) - locked files and unlocking", async () => {
      setCapabilities(
        fileProvider,
        FileSystemProviderCapabilities.FileOpenReadWriteClose | FileSystemProviderCapabilities.FileWriteUnlock
      );
      return testLockedFiles(false);
    });
    ...
    async function testLockedFiles(expectError) {
      ...
      const content = await service.writeFile(
        lockedFile,
      ...
      if (expectError) {
        ...
        assert.strictEqual(
          readFileSync(lockedFile.fsPath).toString(),
          newContent
        );
    ...
    test("writeFile (error when folder is encountered)", async () => {
      ...
      try {
        await service.writeFile(
          resource,
          VSBuffer.fromString("File is created including parent")
        );
      } catch (err) {
        error = err;
      }
    ...
    test("writeFile - error when writing to file that has been updated meanwhile", async () => {
      ...
      try {
        await service.writeFile(
          resource,
          VSBuffer.fromString(newContentLeadingToError),
          {
    ...
    test("writeFile - no error when writing to file where size is the same", async () => {
      ...
      try {
        await service.writeFile(
          resource,
          VSBuffer.fromString(newContentLeadingToNoError),
          {
    ...
    test("write - mixed positions", async () => {
      ...
      const fdWrite = await fileProvider.open(resource, {
        create: true,
        unlock: false
      });
      ...
      let posInFileRead = 0;
      const initialContents = VSBuffer.fromString(
        "Lorem ipsum dolor sit amet"
      );
      await fileProvider.write(
        fdWrite,
        posInFileWrite,
        initialContents.buffer,
        0,
        initialContents.byteLength
      );
      ...
      assert.strictEqual(
        buffer.slice(0, contents.byteLength).toString(),
        "Hello World"
      );
    ...
    test("readonly - is handled properly for a single resource", async () => {
      ...
      try {
        await service.writeFile(
          resource,
          VSBuffer.fromString("Hello Test")
        );
      } catch (error) {
