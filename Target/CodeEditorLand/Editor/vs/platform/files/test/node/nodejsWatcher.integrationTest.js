...
suite.skip("File Watcher (node.js)", () => {
  ...
  (isMacintosh ? test.skip : test)("correlated watch requests support suspend/resume (folder, exists in beginning)", async function() {
    const folderPath = join(testDir, "deep");
    await watcher.watch([{ path: folderPath, excludes: [], recursive: false, correlationId: 1 }]);
    const filePath = join(folderPath, "newFile.txt");
    await basicCrudTest(filePath, void 0, 1);
  ...
  __name(createParcelWatcher, "createParcelWatcher");
  ...
  __name(testParcelWatcherReused, "testParcelWatcherReused");
