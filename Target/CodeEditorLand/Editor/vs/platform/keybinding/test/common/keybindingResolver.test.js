...
suite("KeybindingResolver", () => {
  ...
  function kbItem(keybinding, command, commandArgs, when, isDefault) {
    const resolvedKeybinding = createUSLayoutResolvedKeybinding(
      keybinding,
      OS
    );
  ...
  test("resolve key", () => {
    ...
    assert.strictEqual(
      contextRules.evaluate(createContext({ bar: "baz" })),
      true
    );
    assert.strictEqual(
      contextRules.evaluate(createContext({ bar: "bz" })),
      false
    );
    ...
    const r1 = resolver.resolve(
      ...
      [],
      getDispatchStr(runtimeKeybinding)
    );
  ...
  suite("handle keybinding removals", () => {
    test("simple 1", () => {
      ...
      const overrides = [
        kbItem(
          ...
          "yes2",
          null,
          ContextKeyExpr.equals("2", "b"),
          false
        )
      ];
      const actual = KeybindingResolver.handleRemovals([
        ...defaults,
        ...overrides
      ]);
    ...
    test("removal with unspecified when and unspecified keybinding", () => {
      ...
      const overrides = [kbItem(0, "-yes1", null, void 0, false)];
      const actual = KeybindingResolver.handleRemovals([
        ...defaults,
        ...overrides
      ]);
      ...
      const defaults = [
        kbItem(
          ...
          ContextKeyExpr.deserialize(
            `editorTextFocus && activeEditor != workbench.editor.notebook && editorLangId in julia.supportedLanguageIds`
          ),
          true
        )
      ];
      ...
      const assertIsIncluded = (a, b) => {
        assert.strictEqual(
          KeybindingResolver.whenIsEntirelyIncluded(
            toContextKeyExpression(a),
            toContextKeyExpression(b)
          ),
          true
        );
      };
  ...
  suite("resolve command", () => {
    ...
    const items = [
      ...
      _kbItem(KeyCode.KeyZ, "second", void 0),
      // This one sometimes overwrites first
      _kbItem(KeyCode.KeyX, "third", ContextKeyExpr.equals("key3", true)),
    ...
    const testKbLookupByCommand = (commandId, expectedKeys) => {
      ...
      assert.strictEqual(
        lookupResult.length,
        expectedKeys.length,
        "Length mismatch @ commandId " + commandId
      );
      for (let i = 0, len = lookupResult.length; i < len; i++) {
        const expected = createUSLayoutResolvedKeybinding(
          expectedKeys[i],
          OS
        );
        assert.strictEqual(
          lookupResult[i].resolvedKeybinding.getUserSettingsLabel(),
          expected.getUserSettingsLabel(),
          "value mismatch @ commandId " + commandId
        );
      }
    };
    const testResolve = (ctx, _expectedKey, commandId) => {
      ...
      for (let i = 0, len = expectedKeybinding.chords.length; i < len; i++) {
        const chord = getDispatchStr(
          expectedKeybinding.chords[i]
        );
        const result = resolver.resolve(ctx, previousChord, chord);
        if (i === len - 1) {
          assert.ok(
            result.kind === ResultKind.KbFound,
            `Enters multi chord for ${commandId} at chord ${i}`
          );
          assert.strictEqual(
            result.commandId,
            commandId,
          ...
          assert.ok(
            ...
            `Continues multi chord for ${commandId} at chord ${i}`
          );
        } else {
          assert.ok(
            result.kind === ResultKind.MoreChordsNeeded,
