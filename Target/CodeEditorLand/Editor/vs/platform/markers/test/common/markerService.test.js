...
suite("Marker Service", () => {
  ...
  test("query", () => {
    ...
    assert.strictEqual(
      service.read({ severities: MarkerSeverity.Error }).length,
      1
    );
    assert.strictEqual(
      service.read({ severities: MarkerSeverity.Warning }).length,
      1
    );
    assert.strictEqual(
      service.read({ severities: MarkerSeverity.Hint }).length,
      0
    );
    assert.strictEqual(
      service.read({
  ...
  test("changeOne override", () => {
    ...
    assert.strictEqual(service.read().length, 1);
    assert.strictEqual(service.read({ owner: "far" }).length, 1);
    service.changeOne("boo", URI.parse("file:///path/only.cs"), [
      randomMarkerData()
    ]);
    assert.strictEqual(service.read().length, 2);
    assert.strictEqual(service.read({ owner: "far" }).length, 1);
  ...
  test("changeAll sends event for cleared", () => {
    ...
    const d = service.onMarkerChanged((changedResources) => {
      ...
      changedResources.forEach(
        (u) => assert.strictEqual(u.toString(), "file:///d/path")
      );
      assert.strictEqual(service.read({ owner: "far" }).length, 0);
    });
    service.changeAll("far", []);
