...
import {
  IListService,
  ListService
} from "../../../list/browser/listService.js";
...
suite("QuickInput", () => {
  ...
  setup(() => {
    ...
    instantiationService.stub(
      IContextViewService,
      store.add(instantiationService.createInstance(ContextViewService))
    );
    instantiationService.stub(
      IContextKeyService,
      store.add(instantiationService.createInstance(ContextKeyService))
    );
  ...
  test("pick - activeItem is honored", async () => {
    const item = { label: "foo" };
    const wait = setupWaitTilShownListener(controller);
    const pickPromise = controller.pick([{ label: "bar" }, item], {
      activeItem: item
    });
    await wait;
    controller.accept();
  ...
  test("activeItems - verify onDidChangeActive is triggered after setting items", async () => {
    ...
    store.add(
      quickpick.onDidChangeActive(
        (items) => activeItemsFromEvent.push(...items)
      )
    );
    quickpick.show();
