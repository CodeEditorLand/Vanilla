...
suite("DefaultConfiguration", () => {
  ...
  test("Test registering a property after initialize", async () => {
    ...
    assert.strictEqual(
      actual.getValue("defaultConfiguration.testSetting1"),
      false
    );
  ...
  test("Test registering an override identifier", async () => {
    ...
    assert.ok(
      equals(actual.overrides, [
        { contents: { b: true }, identifiers: ["a"], keys: ["b"] }
      ])
    );
  ...
  test("Test deregistering an override identifier", async () => {
    ...
    assert.strictEqual(
      testObject.configurationModel.getOverrideValue("b", "a"),
      void 0
    );
  });
  test("Test deregistering a merged language object setting", async () => {
    ...
    assert.ok(
      equals(testObject.configurationModel.getValue("[a]"), {
        b: { bb: "20", cc: "30" }
      })
    );
    assert.ok(
    ...
    assert.ok(
      equals(testObject.configurationModel.overrides, [
        {
          contents: { b: { bb: "20", cc: "30" } },
          identifiers: ["a"],
          keys: ["b"]
        }
      ])
    );
    assert.deepStrictEqual(testObject.configurationModel.keys.sort(), [
      "[a]",
      "b"
    ]);
    assert.ok(
      equals(testObject.configurationModel.getOverrideValue("b", "a"), {
        bb: "20",
        cc: "30"
      })
    );
