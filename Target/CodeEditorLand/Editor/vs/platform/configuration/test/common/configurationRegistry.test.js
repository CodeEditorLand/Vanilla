...
suite("ConfigurationRegistry", () => {
  ...
  test("configuration override", async () => {
    ...
    configurationRegistry.registerDefaultConfigurations([
      { overrides: { config: { a: 1, b: 2 } } }
    ]);
    configurationRegistry.registerDefaultConfigurations([
      { overrides: { "[lang]": { a: 2, c: 3 } } }
    ]);
    assert.deepStrictEqual(
      configurationRegistry.getConfigurationProperties()["config"].default,
      { a: 1, b: 2 }
    );
    assert.deepStrictEqual(
      configurationRegistry.getConfigurationProperties()["[lang]"].default,
      { a: 2, c: 3 }
    );
  });
  test("configuration override defaults - prevent overriding default value", async () => {
    ...
    configurationRegistry.registerDefaultConfigurations([
      {
        overrides: {
          "config.preventDefaultValueOverride": { a: 1, b: 2 }
        }
      }
    ]);
    assert.deepStrictEqual(
      configurationRegistry.getConfigurationProperties()["config.preventDefaultValueOverride"].default,
      { a: 0 }
    );
  });
  ...
  test("configuration defaults - deregister merged object default language overrides", async () => {
    ...
    configurationRegistry.deregisterDefaultConfigurations(overrides1);
    assert.deepStrictEqual(
      configurationRegistry.getConfigurationProperties()["[lang]"],
      void 0
    );
  });
});
