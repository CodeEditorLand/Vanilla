...
suite("extractSelection", () => {
  ...
  test("withSelection with startLineNumber and startColumn", async () => {
    assert.deepStrictEqual(
      withSelection(URI.parse("file:///some/file.js"), {
        startLineNumber: 73,
        startColumn: 84
      }).toString(),
      "file:///some/file.js#73%2C84"
    );
  });
  ...
  test("extractSelection returns original withSelection URI", async () => {
    ...
    assert.strictEqual(
      uri.toString(),
      extractSelection(uriWithSelection).uri.toString()
    );
    uri = URI.parse("file:///some/file.js");
    assert.strictEqual(
      uri.toString(),
      extractSelection(uri).uri.toString()
    );
  });
});
