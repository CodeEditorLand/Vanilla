...
const IWorkspacesManagementMainService = createDecorator(
...
let WorkspacesManagementMainService = class extends Disposable {
  ...
  _onDidDeleteUntitledWorkspace = this._register(
    new Emitter()
  );
  onDidDeleteUntitledWorkspace = this._onDidDeleteUntitledWorkspace.event;
  _onDidEnterWorkspace = this._register(
    new Emitter()
  );
  ...
  async initialize() {
    ...
    try {
      const untitledWorkspacePaths = (await Promises.readdir(
  ...
  resolveLocalWorkspace(uri) {
    return this.doResolveLocalWorkspace(
      uri,
      (path) => fs.promises.readFile(path, "utf8")
    );
  }
