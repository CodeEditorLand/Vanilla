...
flakySuite("WorkspacesManagementMainService", () => {
  ...
  setup(async () => {
    ...
    service = new WorkspacesManagementMainService(
      ...
      new TestBackupMainService(),
      new TestDialogMainService()
    );
    return fs.promises.mkdir(untitledWorkspacesHomePath, {
      recursive: true
    });
  });
  teardown(() => {
  ...
  test("createWorkspace (folders)", async () => {
    ...
    const ws = JSON.parse(
      fs.readFileSync(workspace.configPath.fsPath).toString()
    );
  ...
  test("createUntitledWorkspace (folders as other resource URIs)", async () => {
    ...
    const workspace = await service.createUntitledWorkspace(
      [{ uri: folder1URI }, { uri: folder2URI }],
      "server"
    );
    assert.ok(workspace);
    assert.ok(fs.existsSync(workspace.configPath.fsPath));
    ...
    assert.strictEqual(
      ws.folders[0].uri,
      folder1URI.toString(true)
    );
    assert.strictEqual(
      ws.folders[1].uri,
      folder2URI.toString(true)
    );
    assert.ok(!ws.folders[0].name);
    assert.ok(!ws.folders[1].name);
  ...
  test("resolveWorkspace", async () => {
    ...
    const newPath = path.join(
      path.dirname(workspace.configPath.fsPath),
      `workspace.${WORKSPACE_EXTENSION}`
    );
    fs.renameSync(workspace.configPath.fsPath, newPath);
    workspace.configPath = URI.file(newPath);
    const resolved = await service.resolveLocalWorkspace(
    ...
    const resolvedInvalid = await service.resolveLocalWorkspace(
      workspace.configPath
    );
    assert.ok(!resolvedInvalid);
    fs.writeFileSync(
      workspace.configPath.fsPath,
      JSON.stringify({ transient: true, folders: [] })
    );
    const resolvedTransient = await service.resolveLocalWorkspace(
      workspace.configPath
    );
  ...
  test("resolveWorkspace (support relative paths)", async () => {
    ...
    fs.writeFileSync(
      workspace.configPath.fsPath,
      JSON.stringify({ folders: [{ path: "./ticino-playground/lib" }] })
    );
    const resolved = await service.resolveLocalWorkspace(
      workspace.configPath
    );
    assertEqualURI(
      resolved.folders[0].uri,
      URI.file(
        path.join(
          path.dirname(workspace.configPath.fsPath),
          "ticino-playground",
  ...
  test("resolveWorkspace (support relative paths #2)", async () => {
    ...
    fs.writeFileSync(
      workspace.configPath.fsPath,
      JSON.stringify({
        folders: [{ path: "./ticino-playground/lib/../other" }]
      })
    );
    const resolved = await service.resolveLocalWorkspace(
      workspace.configPath
  ...
  test("rewriteWorkspaceFileForNewLocation", async () => {
    ...
    const firstConfigPath = path.join(
      tmpDir,
      "myworkspace0.code-workspace"
    );
    createWorkspace(firstConfigPath, [
      folder1,
      "inside",
      path.join("inside", "somefolder")
    ]);
    const origContent = fs.readFileSync(firstConfigPath).toString();
    let origConfigPath = URI.file(firstConfigPath);
    let workspaceConfigPath = URI.file(
      path.join(tmpDir, "inside", "myworkspace1.code-workspace")
    );
    ...
    workspaceConfigPath = URI.file(
      path.join(tmpDir, "myworkspace2.code-workspace")
    );
    ...
    origConfigPath = workspaceConfigPath;
    workspaceConfigPath = URI.file(
      path.join(tmpDir, "other", "myworkspace2.code-workspace")
    );
    newContent = rewriteWorkspaceFileForNewLocation(
      newContent,
      origConfigPath,
    ...
    origConfigPath = workspaceConfigPath;
    workspaceConfigPath = URI.parse(
      "foo://foo/bar/myworkspace2.code-workspace"
    );
    newContent = rewriteWorkspaceFileForNewLocation(
      newContent,
      origConfigPath,
    ...
    assert.strictEqual(
      ws.folders[0].uri,
      URI.file(folder1).toString(true)
    );
    assert.strictEqual(
      ws.folders[1].uri,
      URI.file(tmpInsideDir).toString(true)
    );
    assert.strictEqual(
      ws.folders[2].uri,
      URI.file(path.join(tmpInsideDir, "somefolder")).toString(true)
    );
  ...
  test("rewriteWorkspaceFileForNewLocation (preserves comments)", async () => {
    const workspace = await createUntitledWorkspace([
      cwd,
      tmpDir,
      path.join(tmpDir, "somefolder")
    ]);
    const workspaceConfigPath = URI.file(
      path.join(
        tmpDir,
        `myworkspace.${Date.now()}.${WORKSPACE_EXTENSION}`
      )
    );
...
${origContent}`;
  ...
  (isWindows ? test : test.skip)(
    ...
    async () => {
      ...
      const folder3Location = path.join(
        workspaceLocation,
        "inner",
        "more"
      );
  ...
  test("getUntitledWorkspace", async () => {
