...
suite("Command Tests", () => {
  ...
  test("register command - no handler", () => {
  ...
  test("command with description", () => {
    ...
    assert.throws(
      () => CommandsRegistry.getCommands().get("test3").handler.apply(void 0, [void 0, "string"])
    );
    assert.strictEqual(
      CommandsRegistry.getCommands().get("test3").handler.apply(void 0, [void 0, 1]),
      true
    );
