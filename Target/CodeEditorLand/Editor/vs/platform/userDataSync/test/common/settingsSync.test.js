...
suite("SettingsSync - Auto", () => {
  ...
  test("when settings file does not exist", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    const settingResource = client.instantiationService.get(
      IUserDataProfilesService
  ...
  test("when settings file is empty and remote has no changes", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    const fileService = client.instantiationService.get(IFileService);
    const settingsResource = client.instantiationService.get(
      IUserDataProfilesService
    ).defaultProfile.settingsResource;
    ...
    const remoteUserData = await testObject.getRemoteUserData(null);
    assert.strictEqual(
      parseSettingsSyncContent(lastSyncUserData.syncData.content)?.settings,
      "{}"
    );
    assert.strictEqual(
      parseSettingsSyncContent(remoteUserData.syncData.content)?.settings,
      "{}"
    );
    assert.strictEqual(
      (await fileService.readFile(settingsResource)).value.toString(),
      ""
    );
  }));
  test("when settings file is empty and remote has changes", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    await client2.instantiationService.get(IFileService).writeFile(
      client2.instantiationService.get(IUserDataProfilesService).defaultProfile.settingsResource,
  ...
  test("sync when there are conflicts", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    assert.strictEqual(
      testObject.conflicts.conflicts[0].localResource.toString(),
      testObject.localResource.toString()
    );
    const fileService = client.instantiationService.get(IFileService);
    const mergeContent = (await fileService.readFile(
...
function parseSettings(content) {
  ...
  const settingsSyncContent = JSON.parse(
    syncData.content
  );
  return settingsSyncContent.settings;
}
async function updateSettings(content, client, profile) {
  await client.instantiationService.get(IFileService).writeFile(
    (profile ?? client.instantiationService.get(IUserDataProfilesService).defaultProfile).settingsResource,
    VSBuffer.fromString(content)
  );
