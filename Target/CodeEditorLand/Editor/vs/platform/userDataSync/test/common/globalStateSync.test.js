...
suite("GlobalStateSync", () => {
  ...
  test("when global state is created after first sync", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    assert.deepStrictEqual(
      JSON.parse(lastSyncUserData.syncData.content).storage,
  ...
  test("sync profile state", () => runWithFakedTimers({ useFakeTimers: true }, async () => {
    ...
    await updateUserStorageForProfile(
      "a",
      "value1",
      profile,
      testClient
  ...
  async function updateUserStorageForProfile(key, value, profile, client) {
    const storageService = client.instantiationService.get(
      IUserDataProfileStorageService
    );
    const data = /* @__PURE__ */ new Map();
    data.set(key, value);
    await storageService.updateStorageData(
      profile,
      data,
      StorageTarget.USER
    );
