...
} from "../../common/keybindingsSync.js";
import {
...
suite("KeybindingsSync", () => {
  ...
  test("when keybindings file does not exist", async () => {
    ...
    const keybindingsResource = client.instantiationService.get(
  ...
  test("when keybindings file is empty and remote has no changes", async () => {
    ...
    assert.strictEqual(
      (await fileService.readFile(keybindingsResource)).value.toString(),
      ""
    );
  });
  test("when keybindings file is empty and remote has changes", async () => {
    ...
    await client2.instantiationService.get(IFileService).writeFile(
      client2.instantiationService.get(IUserDataProfilesService).defaultProfile.keybindingsResource,
  ...
  test("when keybindings file is empty with comment and remote has no changes", async () => {
    ...
    await fileService.writeFile(
      keybindingsResource,
      VSBuffer.fromString(expectedContent)
    );
    await testObject.sync(await client.getResourceManifest());
    const lastSyncUserData = await testObject.getLastSyncUserData();
    const remoteUserData = await testObject.getRemoteUserData(null);
  ...
  test("when keybindings file is empty and remote has keybindings", async () => {
    ...
    await fileService.writeFile(
      keybindingsResource,
      VSBuffer.fromString("// Empty Keybindings")
    );
    await testObject.sync(await client.getResourceManifest());
    const lastSyncUserData = await testObject.getLastSyncUserData();
    const remoteUserData = await testObject.getRemoteUserData(null);
...
[
    ...
    await fileService.writeFile(
      keybindingsResource,
      VSBuffer.fromString(expectedLocalContent)
    );
    await testObject.sync(await client.getResourceManifest());
    const lastSyncUserData = await testObject.getLastSyncUserData();
    const remoteUserData = await testObject.getRemoteUserData(null);
  ...
  test("sync throws invalid content error - content is an object", async () => {
    await client.instantiationService.get(IFileService).writeFile(
      client.instantiationService.get(IUserDataProfilesService).defaultProfile.keybindingsResource,
      VSBuffer.fromString("{}")
    );
    try {
      await testObject.sync(await client.getResourceManifest());
