...
suite("UserDataSyncService", () => {
  ...
  test("test first time sync from the client with changes - merge", async () => {
    ...
    const userDataProfilesService = testClient.instantiationService.get(
  ...
  test("test first time sync from the client with changes - merge with profile", async () => {
    ...
    assert.deepStrictEqual(target.requests, [
      ...
      {
        type: "POST",
        url: `${target.url}/v1/resource/profiles`,
        headers: { "If-Match": "0" }
      },
      {
        type: "GET",
        url: `${target.url}/v1/collection/1/resource/settings/latest`,
        headers: {}
      },
      {
        type: "GET",
      ...
      {
        type: "GET",
        url: `${target.url}/v1/collection/1/resource/snippets/latest`,
        headers: {}
      },
      {
        type: "GET",
        url: `${target.url}/v1/collection/1/resource/tasks/latest`,
        headers: {}
      },
      {
        type: "GET",
      ...
      {
        type: "GET",
        url: `${target.url}/v1/collection/1/resource/extensions/latest`,
        headers: {}
      }
    ]);
  });
  ...
  test("test sync status", async () => {
    ...
    const disposable = testObject.onDidChangeStatus(
      (status) => actualStatuses.push(status)
    );
    await (await testObject.createSyncTask(null)).run();
    disposable.dispose();
  ...
  test("test sync conflicts status", async () => {
    ...
    let userDataProfilesService = client.instantiationService.get(
      IUserDataProfilesService
    );
    await fileService.writeFile(
      userDataProfilesService.defaultProfile.settingsResource,
    ...
    userDataProfilesService = testClient.instantiationService.get(
      IUserDataProfilesService
    );
    await fileService.writeFile(
      userDataProfilesService.defaultProfile.settingsResource,
