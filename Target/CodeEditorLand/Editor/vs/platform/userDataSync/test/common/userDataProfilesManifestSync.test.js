...
suite("UserDataProfilesManifestSync", () => {
  ...
  setup(async () => {
    ...
    testObject = testClient.getSynchronizer(SyncResource.Profiles);
  ...
  test("first time sync - incoming from server (no state)", async () => {
    await runWithFakedTimers({}, async () => {
      ...
      assert.deepStrictEqual(profiles, [{ id: "1", name: "name 1", shortName: void 0, useDefaultFlags: void 0 }]);
    });
  });
  test("first time sync when profiles exists", async () => {
    await runWithFakedTimers({}, async () => {
      ...
      assert.deepStrictEqual(testObject.conflicts.conflicts, []);
      const profiles = getLocalProfiles(testClient);
      assert.deepStrictEqual(profiles, [{ id: "1", name: "name 1", shortName: void 0, useDefaultFlags: void 0 }, { id: "2", name: "name 2", shortName: void 0, useDefaultFlags: void 0 }]);
      const { content } = await testClient.read(testObject.resource);
      assert.ok(content !== null);
  ...
  test("sync adding a profile", async () => {
    await runWithFakedTimers({}, async () => {
      ...
      assert.strictEqual(testObject.status, SyncStatus.Idle);
      assert.deepStrictEqual(testObject.conflicts.conflicts, []);
      assert.deepStrictEqual(getLocalProfiles(testClient), [{ id: "1", name: "name 1", shortName: "short 1", useDefaultFlags: void 0 }, { id: "2", name: "name 2", shortName: void 0, useDefaultFlags: void 0 }]);
      await client2.sync();
      assert.deepStrictEqual(getLocalProfiles(client2), [{ id: "1", name: "name 1", shortName: "short 1", useDefaultFlags: void 0 }, { id: "2", name: "name 2", shortName: void 0, useDefaultFlags: void 0 }]);
      const { content } = await testClient.read(testObject.resource);
      assert.ok(content !== null);
      const actual = parseRemoteProfiles(content);
  ...
  test("sync updating a profile", async () => {
    await runWithFakedTimers({}, async () => {
      ...
      assert.strictEqual(testObject.status, SyncStatus.Idle);
      assert.deepStrictEqual(testObject.conflicts.conflicts, []);
      assert.deepStrictEqual(getLocalProfiles(testClient), [{ id: "1", name: "name 2", shortName: "2", useDefaultFlags: void 0 }]);
      await client2.sync();
      assert.deepStrictEqual(getLocalProfiles(client2), [{ id: "1", name: "name 2", shortName: "2", useDefaultFlags: void 0 }]);
      const { content } = await testClient.read(testObject.resource);
      assert.ok(content !== null);
      const actual = parseRemoteProfiles(content);
  ...
  test("sync removing a profile", async () => {
    await runWithFakedTimers({}, async () => {
      ...
      assert.strictEqual(testObject.status, SyncStatus.Idle);
      assert.deepStrictEqual(testObject.conflicts.conflicts, []);
      assert.deepStrictEqual(getLocalProfiles(testClient), [{ id: "2", name: "name 2", shortName: void 0, useDefaultFlags: void 0 }]);
      await client2.sync();
      assert.deepStrictEqual(getLocalProfiles(client2), [{ id: "2", name: "name 2", shortName: void 0, useDefaultFlags: void 0 }]);
      const { content } = await testClient.read(testObject.resource);
      assert.ok(content !== null);
      const actual = parseRemoteProfiles(content);
  ...
  test("sync profile that uses default profile", async () => {
    await runWithFakedTimers({}, async () => {
      ...
      assert.deepStrictEqual(getLocalProfiles(testClient), [{ id: "1", name: "name 1", shortName: void 0, useDefaultFlags: { keybindings: true } }]);
    });
  });
  ...
  __name(parseRemoteProfiles, "parseRemoteProfiles");
  function getLocalProfiles(client) {
    return client.instantiationService.get(IUserDataProfilesService).profiles.slice(1).sort((a, b) => a.name.localeCompare(b.name)).map((profile) => ({ id: profile.id, name: profile.name, shortName: profile.shortName, useDefaultFlags: profile.useDefaultFlags }));
  }
  __name(getLocalProfiles, "getLocalProfiles");
});
