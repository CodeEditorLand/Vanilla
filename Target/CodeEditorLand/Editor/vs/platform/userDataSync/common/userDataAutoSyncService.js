...
let UserDataAutoSyncService = class extends Disposable {
  ...
  async triggerSync(sources, skipIfSyncedRecently, disableCache) {
    ...
    if (skipIfSyncedRecently && this.lastSyncTriggerTime && Math.round(((/* @__PURE__ */ new Date()).getTime() - this.lastSyncTriggerTime) / 1e3) < 10) {
    ...
    }, this.successiveFailures ? this.getSyncTriggerDelayTime() * 1 * Math.min(Math.pow(2, this.successiveFailures), 60) : this.getSyncTriggerDelayTime());
  }
