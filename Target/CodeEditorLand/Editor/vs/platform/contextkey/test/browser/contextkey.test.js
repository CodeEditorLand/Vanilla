...
suite("ContextKeyService", () => {
  ...
  test("updateParent", () => {
    ...
    testDisposables.add(
      child.onDidChangeContext((e) => {
        try {
          assert.ok(
            e.affectsSome(/* @__PURE__ */ new Set(["testA"])),
            "testA changed"
          );
          assert.ok(
            e.affectsSome(/* @__PURE__ */ new Set(["testB"])),
            "testB changed"
          );
          assert.ok(
            e.affectsSome(/* @__PURE__ */ new Set(["testC"])),
            "testC changed"
          );
          assert.ok(
            !e.affectsSome(/* @__PURE__ */ new Set(["testD"])),
            "testD did not change"
          );
          assert.strictEqual(child.getContextKeyValue("testA"), 3);
          assert.strictEqual(
            child.getContextKeyValue("testB"),
            void 0
          );
  ...
  test("issue #147732: URIs as context values", () => {
    ...
    const contextKeyService = testDisposables.add(
    ...
    assert.deepStrictEqual(
      contextKeyService.contextMatchesRules(expr),
      true
    );
  });
