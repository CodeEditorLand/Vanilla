...
suite("ContextKeyExpr", () => {
  ...
  test("ContextKeyInExpr", () => {
    ...
    assert.strictEqual(
      ainb.evaluate(createContext({ a: 3, b: [3, 2, 1] })),
      true
    );
    assert.strictEqual(
      ainb.evaluate(createContext({ a: 3, b: [1, 2, 3] })),
      true
    );
    assert.strictEqual(
      ainb.evaluate(createContext({ a: 3, b: [1, 2] })),
      false
    );
    assert.strictEqual(ainb.evaluate(createContext({ a: 3 })), false);
    assert.strictEqual(
      ainb.evaluate(createContext({ a: 3, b: null })),
      false
    );
    assert.strictEqual(
      ainb.evaluate(createContext({ a: "x", b: ["x"] })),
      true
    );
    assert.strictEqual(
      ainb.evaluate(createContext({ a: "x", b: ["y"] })),
      false
    );
    assert.strictEqual(
      ainb.evaluate(createContext({ a: "x", b: {} })),
      false
    );
    assert.strictEqual(
      ainb.evaluate(createContext({ a: "x", b: { x: false } })),
      true
    );
    assert.strictEqual(
      ainb.evaluate(createContext({ a: "x", b: { x: true } })),
      true
    );
    assert.strictEqual(
      ainb.evaluate(createContext({ a: "prototype", b: {} })),
      false
    );
  });
  test("ContextKeyNotInExpr", () => {
    ...
    assert.strictEqual(
      aNotInB.evaluate(createContext({ a: 3, b: [3, 2, 1] })),
      false
    );
    assert.strictEqual(
      aNotInB.evaluate(createContext({ a: 3, b: [1, 2, 3] })),
      false
    );
    assert.strictEqual(
      aNotInB.evaluate(createContext({ a: 3, b: [1, 2] })),
      true
    );
    assert.strictEqual(aNotInB.evaluate(createContext({ a: 3 })), true);
    assert.strictEqual(
      aNotInB.evaluate(createContext({ a: 3, b: null })),
      true
    );
    assert.strictEqual(
      aNotInB.evaluate(createContext({ a: "x", b: ["x"] })),
      false
    );
    assert.strictEqual(
      aNotInB.evaluate(createContext({ a: "x", b: ["y"] })),
      true
    );
    assert.strictEqual(
      aNotInB.evaluate(createContext({ a: "x", b: {} })),
      true
    );
    assert.strictEqual(
      aNotInB.evaluate(createContext({ a: "x", b: { x: false } })),
      false
    );
    assert.strictEqual(
      aNotInB.evaluate(createContext({ a: "x", b: { x: true } })),
      false
    );
    assert.strictEqual(
      aNotInB.evaluate(createContext({ a: "prototype", b: {} })),
      true
    );
  });
  ...
  test("issue #111899: context keys can use `<` or `>` ", () => {
    const actual = ContextKeyExpr.deserialize(
