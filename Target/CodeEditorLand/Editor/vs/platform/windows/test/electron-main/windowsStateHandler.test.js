...
suite("Windows State Storing", () => {
  ...
  function assertEqualWindowsState(expected, actual, message) {
    ...
    assertEqualWindowState(
      expected.lastActiveWindow,
      actual.lastActiveWindow,
      message
    );
    assert.strictEqual(
      expected.openedWindows.length,
      actual.openedWindows.length,
      message
    );
    for (let i = 0; i < expected.openedWindows.length; i++) {
      assertEqualWindowState(
        expected.openedWindows[i],
        actual.openedWindows[i],
        message
      );
    }
  }
  ...
  test("storing and restoring", () => {
    ...
    windowState = {
      openedWindows: [
        { backupPath: testBackupPath1, uiState: getUIState() }
      ]
    };
