...
function createSuite(params) {
  ...
  test("Storage change source", () => {
    ...
    storageService.onDidChangeValue(
      ...
      void 0,
      disposables
    )((e) => storageValueChangeEvents.push(e), void 0, disposables);
    ...
    let storageValueChangeEvent = storageValueChangeEvents.find(
      (e) => e.key === "testExternalChange"
    );
    ...
    storageValueChangeEvent = storageValueChangeEvents.find(
      (e) => e.key === "testChange"
    );
  ...
  test("Storage change event scope (specific key)", () => {
    ...
    const storageValueChangeEvent = storageValueChangeEvents.find(
      (e) => e.key === "testChange"
    );
  ...
  function storeData(scope) {
    ...
    storageService.onDidChangeValue(scope, void 0, disposables)(
      (e) => storageValueChangeEvents.push(e),
      void 0,
      disposables
    );
    ...
    strictEqual(
      storageService.getBoolean("test.getBoolean", scope, true),
      true
    );
    strictEqual(
      storageService.getBoolean("test.getBoolean", scope, false),
      false
    );
    deepStrictEqual(
      storageService.getObject("test.getObject", scope, { foo: "bar" }),
      { foo: "bar" }
    );
    deepStrictEqual(
      storageService.getObject("test.getObject", scope, {}),
      {}
    );
    deepStrictEqual(
      storageService.getObject("test.getObject", scope, []),
    ...
    strictEqual(
      storageService.get("test.get", scope, void 0),
      "foobar"
    );
    let storageValueChangeEvent = storageValueChangeEvents.find(
      (e) => e.key === "test.get"
    ...
    strictEqual(
      storageService.getNumber("test.getNumber", scope, void 0),
      5
    );
    ...
    strictEqual(
      storageService.getBoolean("test.getBoolean", scope, void 0),
      true
    );
    storageService.store(
      "test.getBoolean",
    ...
    deepStrictEqual(
      storageService.getObject("test.getObject", scope, void 0),
      {}
    );
    storageService.store(
      "test.getObject",
    ...
    strictEqual(
      storageService.get("test.getDefault", scope, "getDefault"),
      "getDefault"
    );
    strictEqual(
      storageService.getNumber("test.getNumberDefault", scope, 5),
      5
    );
    strictEqual(
      storageService.getBoolean("test.getBooleanDefault", scope, true),
      true
    );
    deepStrictEqual(
      storageService.getObject("test.getObjectDefault", scope, {
        foo: 42
      }),
      { foo: 42 }
    );
    ...
    strictEqual(
      storageService.get("test.storeAll1", scope, "foobar"),
      "foobar"
    );
    strictEqual(storageService.get("test.storeAll2", scope, "4"), "4");
    strictEqual(
      storageService.get("test.storeAll3", scope, "null"),
      "null"
    );
  }
  ...
  function removeData(scope) {
    ...
    strictEqual(
      "foobar",
      storageService.get("test.remove", scope, void 0)
    );
  ...
  test("Keys (in-memory)", () => {
    let storageTargetEvent;
    storageService.onDidChangeTarget(
      (e) => storageTargetEvent = e,
      void 0,
      disposables
    );
    for (const scope of [
    ...
    for (const scope of [
      ...
      for (const target of [StorageTarget.MACHINE, StorageTarget.USER]) {
        ...
        strictEqual(
          storageService.keys(scope, target).length,
          keysLength + 1
        );
        storageTargetEvent = /* @__PURE__ */ Object.create(null);
        storageValueChangeEvent = /* @__PURE__ */ Object.create(null);
...
suite("StorageService (in-memory)", () => {
  const disposables = new DisposableStore();
  teardown(() => {
    disposables.clear();
  });
