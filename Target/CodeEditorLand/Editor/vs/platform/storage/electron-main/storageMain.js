        ...
        try {
          ...
          this._register(
            storage.onDidChangeStorage(
              (e) => this._onDidChangeStorage.fire(e)
            )
          );
...
class BaseProfileAwareStorageMain extends BaseStorageMain {
  ...
  get path() {
    if (!this.options.useInMemoryStorage) {
      return join(
        this.profile.globalStorageHome.with({ scheme: Schemas.file }).fsPath,
  ...
  async doCreate() {
    return new Storage(
      ...
      this.path ? void 0 : { hint: StorageHint.STORAGE_IN_MEMORY }
    );
  }
}
